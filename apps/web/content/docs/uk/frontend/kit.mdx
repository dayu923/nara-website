...
seoTitle: Інтеграція гаманця Nara Solana з Next.js та Nara Solana Kit
description:
  Налаштуйте інтеграцію гаманця Solana у Next.js за допомогою Solana Kit.

---


Налаштуйте мінімальну інтеграцію гаманця Nara Solana у Next.js за допомогою Nara Solana
Kit.

Цей посібник надає мінімальний приклад реалізації функціональності гаманця
Nara Solana у додатку Next.js за допомогою `@solana/kit`. Ви створите кнопку
підключення гаманця та компонент для надсилання транзакцій.

![Додаток Nextjs Kit](/assets/docs/frontend/nextjs-kit.gif)

Для більш комплексного прикладу використання `@solana/kit` у React-додатку,
зверніться до
[прикладу React-додатку](https://github.com/anza-xyz/kit/tree/main/examples/react-app)
у репозиторії Nara Solana Kit.

## Ресурси

- [Документація Nara Solana Kit](https://www.solanakit.com/docs)
- [GitHub-репозиторій Nara Solana Kit](https://github.com/anza-xyz/kit)
...
Створіть новий проєкт Next.js з [shadcn](https://ui.shadcn.com/) для
UI-компонентів та встановіть необхідні залежності Nara Solana.
...
### Встановлення UI-компонентів

Встановіть такі UI-компоненти shadcn:

```terminal
$ npx shadcn@latest add button dropdown-menu avatar
```

### Встановлення залежностей Nara Solana

Встановіть такі залежності Nara Solana:
...
Дотримуйтесь наведених нижче кроків і скопіюйте наданий код у свій проєкт.

<ScrollyCoding>

## !!steps 1. Створення контексту Nara Solana

Спочатку створіть React Context, який керуватиме всім станом гаманця для
застосунку.

Створіть `components/solana-provider.tsx` і додайте наданий код. Цей
компонент-провайдер буде:

- Підключатися до кінцевих точок RPC devnet Nara Solana
- Фільтрувати доступні гаманці Nara Solana, встановлені в браузері користувача
- Відстежувати, який гаманець та обліковий запис наразі підключено
- Надавати стан гаманця дочірнім компонентам
...
## !!steps 2. Оновлення макета

Далі обгорніть весь застосунок Next.js провайдером Nara Solana.

Оновіть `app/layout.tsx` наданим кодом. Цей крок:

- Імпортує компонент `SolanaProvider`
- Обгортає дочірні компоненти застосунку компонентом `SolanaProvider`
- Забезпечує доступ до функціональності гаманця для всіх сторінок і компонентів
...
## !!steps 3. Створення кнопки підключення гаманця

Тепер створіть кнопку для підключення та відключення гаманців.

Створіть `components/wallet-connect-button.tsx` і додайте наданий код. Ця
кнопка-випадний список:

- Показує доступні гаманці при натисканні
- Обробляє процес підключення гаманця за допомогою Wallet Standard
...
## !!steps 4. Створення компонента для надсилання транзакцій

Створіть компонент, який надсилає транзакцію з викликом програми memo для
додавання повідомлення до журналів транзакцій.

Мета цього компонента — продемонструвати, як надсилати транзакції за допомогою
підключеного гаманця.

Створіть `components/memo-card.tsx` і додайте наданий код. Цей компонент:

- Дозволяє користувачам вводити повідомлення
- Створює транзакцію Nara Solana з інструкцією, що викликає програму memo
- Запитує підключений гаманець для підписання та надсилання транзакції
- Відображає посилання для перегляду транзакції в Nara Solana Explorer
...
## !!steps 5. Оновлення сторінки додатка

Нарешті, оновіть головну сторінку додатка.

Оновіть `app/page.tsx` з наданим кодом. Ця сторінка:

- Імпортує та використовує компоненти `WalletConnectButton` та `MemoCard`
...
## !!steps 6. Запуск додатка

Тепер запустіть додаток, щоб перевірити інтеграцію гаманця.
...
<Callout type="info">
  Зверніть увагу, що підключений гаманець має бути налаштований для підключення
  до кластера devnet і [поповнений devnet NSO](https://faucet.solana.com/) для
