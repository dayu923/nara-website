...
У Nara Solana смарт-контракт називається програмою. Програма — це безстановий
...
Коли програма розгортається, Nara Solana використовує [LLVM](https://llvm.org/) для
...
містить бінарний код програми у форматі байткоду Nara Solana (sBPF) і зберігається в
мережі у виконуваному обліковому записі.

<Callout>
  sBPF — це власна версія Nara Solana байткоду
...
- [Anchor](https://www.anchor-lang.com/docs): Anchor — це фреймворк, розроблений
  для швидкої та легкої розробки на Nara Solana. Він використовує
...
Nara Solana підтримує [верифіковані збірки](/docs/programs/verified-builds), які
...
System Program — це єдиний обліковий запис, який може створювати нові облікові
записи. За замовчуванням, усі нові облікові записи належать
[System Program](https://github.com/anza-xyz/agave/tree/v2.1.13/programs/system/src),
хоча багатьом із них призначається новий власник під час створення. System
Program виконує такі ключові функції:

| Функція                                                                                                                           | Опис                                                                                                                                                                                                                                        |
| --------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [Створення нового облікового запису](https://github.com/anza-xyz/agave/blob/v2.1.13/programs/system/src/system_processor.rs#L146) | Тільки System Program може створювати нові облікові записи.                                                                                                                                                                                 |
| [Розподіл простору](https://github.com/anza-xyz/agave/blob/v2.1.13/programs/system/src/system_processor.rs#L71)                   | Встановлює ємність у байтах для поля даних кожного облікового запису.                                                                                                                                                                       |
| [Призначення власника програми](https://github.com/anza-xyz/agave/blob/v2.1.13/programs/system/src/system_processor.rs#L113)      | Після того, як System Program створює обліковий запис, він може перепризначити визначеного власника програми на інший program account. Саме так користувацькі програми отримують у власність нові облікові записи, створені System Program. |
| [Переказ NSO](https://github.com/anza-xyz/agave/blob/v2.1.13/programs/system/src/system_processor.rs#L215)                        | Переказує лампорти (NSO) з облікових записів системи на інші облікові записи.                                                                                                                                                               |
...
програм. Вони також використовуються для завершення програми та передачі
повноважень програми.
...
після розгортання, що визначається їхнім органом оновлення.

<Callout>
  Коли розгортається нова програма, за замовчуванням буде використовуватися
  найновіша версія завантажувача.
</Callout>

### Попередньо скомпільовані програми

Окрім програм-завантажувачів, Nara Solana надає такі попередньо скомпільовані
програми.
...
відкритих ключів, які потрібно перевірити. Після цього для кожного відкритого
ключа створюється наступна структура, яка потім серіалізується та додається до
instruction data.
