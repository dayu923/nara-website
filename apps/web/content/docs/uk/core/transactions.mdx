...
Для взаємодії з мережею Nara Solana вам необхідно надіслати транзакцію. Транзакцію
...
<Callout>
  Це обмеження розроблено для уникнення фрагментації пакетів у типовій
  інтернет-інфраструктурі. Хоча IPv6 підтримує MTU більше 9000 байтів, більшість
  інтернет-маршрутизаторів використовують MTU за замовчуванням 1500 байтів
  (стандартний Ethernet). Щоб забезпечити розміщення транзакцій в одному пакеті
  без фрагментації, Nara Solana використовує 1280 байтів (мінімальний MTU, необхідний
  для IPv6) мінус 48 байтів для мережевих заголовків (40 байтів IPv6 + 8 байтів
  заголовка фрагмента/UDP), що призводить до обмеження розміру транзакції в 1232
  байти.
</Callout>
...
<Callout>
  Ви також можете знайти транзакцію за допомогою [Nara Solana
