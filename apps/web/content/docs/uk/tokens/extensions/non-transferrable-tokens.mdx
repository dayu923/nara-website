...
title: Непередавані токени
description: Дізнайтеся, як увімкнути InitializeNonTransferableMint.

---


## Як увімкнути розширення InitializeNonTransferableMint
...
запобігає передачі токенів між token account після їх створення (мінтингу).
Після створення токени залишаються постійно в token account. Однак власник token
account все ще може спалювати токени та закрити token account, коли баланс
досягне нуля.
