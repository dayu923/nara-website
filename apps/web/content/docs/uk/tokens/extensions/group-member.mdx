...
description:
  Дізнайтеся, як увімкнути GroupPointerExtension та GroupMemberPointerExtension.
...
позначає mint як груповий mint (наприклад, Collection NFT), вказуючи на
обліковий запис, що містить
[конфігурації групи](https://github.com/solana-program/token-group/blob/3b4ff305452fa35754206fc1ad4addb2e2bff511/interface/src/state.rs#L16),
такі як authority для оновлення та максимальний розмір.
...
позначає mint як mint учасника (наприклад, NFT у колекції), вказуючи на
обліковий запис, що містить
[дані про членство](https://github.com/solana-program/token-group/blob/3b4ff305452fa35754206fc1ad4addb2e2bff511/interface/src/state.rs#L67),
такі як адреса групи та номер учасника.

Token Extensions Program безпосередньо реалізує
[Token Group Interface](https://github.com/solana-program/token-group),
дозволяючи зберігати дані як групи, так і учасників безпосередньо в самому mint
account. Коли вказівники розширень вказують на сам mint, розширення
...
можуть використовуватися для зберігання всіх даних колекції та членства в mint
account.
