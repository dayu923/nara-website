...
description: Дізнайтеся, як виводити токени з конфіденційного стану.

---


## Як виводити токени з конфіденційного доступного балансу

Щоб вивести токени з конфіденційного доступного балансу на публічний баланс:

1. Створіть
   [два докази](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/confidential-transfer/proof-generation/src/withdraw.rs#L22)
   на стороні клієнта:
   ...
   **Доказ рівності
   ...
   Перевіряє, що шифротекст залишкового доступного балансу після виведення
   відповідає його
   [зобов'язанню Педерсена](https://en.wikipedia.org/wiki/Commitment_scheme),
   забезпечуючи правильне обчислення нового доступного балансу рахунку як
   ...
   **Доказ діапазону
   ...
   Перевіряє, що залишковий доступний баланс після виведення є невід'ємним і
   знаходиться в межах визначеного діапазону.

2. Для кожного доказу:
   - Викличте програму доказу ZK ElGamal для перевірки даних доказу.
   - Збережіть специфічні для доказу метадані в рахунку "контекстного стану"
     доказу для використання в інших інструкціях.
...
3. Викличте інструкцію
   ...
   надаючи два рахунки доказів.

4. Закрийте рахунки доказів, щоб повернути NSO, використаний для їх створення.

Наступна діаграма показує кроки, необхідні для виведення токенів з
конфіденційного доступного балансу на публічний баланс:
...
Щоб вивести токени з конфіденційного доступного балансу на публічний баланс, ви
повинні:

- Згенерувати доказ рівності та доказ діапазону на стороні клієнта
- Викликати програму доказу Zk ElGamal для перевірки доказів та ініціалізації
  рахунків "контекстного стану"
- Викликати інструкцію
  [ConfidentialTransferInstruction::Withdraw](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L493),
  надаючи два рахунки доказів.
- Закрити два рахунки доказів, щоб повернути rent.

Крейт `spl_token_client` надає такі методи:

- Метод `confidential_transfer_create_context_state_account`, який створює
  рахунок доказу.
- Метод `confidential_transfer_withdraw`, який викликає інструкцію `Withdraw`.
- Метод `confidential_transfer_close_context_state_account`, який закриває
  рахунок доказу.

### Приклад коду

Наступний приклад демонструє, як виводити токени з конфіденційного доступного
балансу на публічний баланс.

Щоб запустити приклад, запустіть локальний validator з програмою Token Extension
Program, клонованою з основної мережі, використовуючи наступну команду. Для
запуску локального validator у вас має бути встановлений Nara Solana CLI.
...
<Callout type="info">
  ...
  локальному validator. Ви повинні клонувати Token Extension Program з основної
  мережі, щоб запустити приклад коду.
</Callout>
