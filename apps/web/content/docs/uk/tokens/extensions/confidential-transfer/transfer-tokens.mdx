...
description:
  Дізнайтеся, як приватно переказувати токени з одного token account на інший.

---


<Callout type="warn">
  Програма ZK ElGamal тимчасово вимкнена на мережах mainnet та devnet, оскільки
  проходить аудит безпеки. Це означає, що розширення конфіденційних переказів
  наразі недоступне. Хоча концепції залишаються дійсними, приклади коду не
  працюватимуть.
</Callout>

## Як конфіденційно переказувати токени з одного token account на інший

Щоб конфіденційно переказувати токени з одного token account на інший, і
відправник, і отримувач повинні мати token account, налаштовані зі станом
_rs`ConfidentialTransferAccount`_ та схвалені для конфіденційних переказів.
Token account відправника також повинен мати доступний конфіденційний баланс для
переказу.

Щоб переказати токени конфіденційно:

1. Створіть
   [три докази](https://github.com/solana-program/token-2022/blob/d073cb89dbcd430387b5c6fb4b7157911351e4a3/confidential-transfer/proof-generation/src/transfer.rs#L35)
   на стороні клієнта:
   ...
   **Доказ рівності
   ...
   Перевіряє, що новий шифротекст доступного балансу після переказу відповідає
   його відповідному
   [зобов'язанню Педерсена](https://en.wikipedia.org/wiki/Commitment_scheme),
   забезпечуючи правильне обчислення нового доступного балансу вихідного рахунку
   як `new_balance = current_balance - transfer_amount`.

   **Доказ дійсності шифротексту
   ...
   Перевіряє, що шифротексти суми переказу правильно згенеровані для всіх трьох
   сторін (джерело, призначення та аудитор), забезпечуючи правильне шифрування
   суми переказу під відкритим ключем кожної сторони.
   ...
   **Доказ діапазону
   ...
   Перевіряє, що новий доступний баланс і сума переказу (розділена на
   низькі/високі біти) є невід'ємними та в межах визначеного діапазону.

2. Для кожного доказу:
   - Викличте програму доказів ZK ElGamal для перевірки даних доказу.
   - Збережіть специфічні для доказу метадані в обліковому записі "контекстного
     стану" доказу для використання в інших інструкціях.

3. Викличте інструкцію
   ...
   надаючи облікові записи контекстного стану доказу.

4. Закрийте облікові записи стану контексту доказу, щоб повернути NSO,
   використаний для їх створення.

Наступна діаграма показує кроки, пов'язані з переказом токенів з токен-акаунта
відправника на токен-акаунт отримувача.
...
Щоб конфіденційно переказати токени з одного токен-акаунта на інший, ви повинні:

- Згенерувати доказ рівності, доказ дійсності шифротексту та доказ діапазону на
  стороні клієнта
- Викликати програму доказу Zk ElGamal для перевірки доказів та ініціалізації
  облікових записів "стану контексту"
- Викликати інструкцію
  [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604),
  надаючи три облікові записи доказу
- Закрити три облікові записи доказу, щоб повернути rent.

Крейт `spl_token_client` надає такі методи:

- Метод `confidential_transfer_create_context_state_account`, який створює
  обліковий запис доказу.
- Метод `confidential_transfer_transfer`, який викликає інструкцію `Transfer`.
- Метод `confidential_transfer_close_context_state_account`, який закриває
  обліковий запис доказу.

### Приклад коду

Наступний приклад демонструє, як конфіденційно переказувати токени між
токен-акаунтами.

Щоб запустити приклад, запустіть локальний валідатор з програмою Token
Extensions Program, клонованою з мейннету, використовуючи наступну команду. Для
запуску локального валідатора у вас має бути встановлений Nara Solana CLI.
...
<Callout type="info">
  На момент написання, Confidential Transfers не увімкнено на валідаторі за
  замовчуванням. Ви повинні клонувати програму Token Extensions Program з
  мейннету, щоб запустити приклад коду.
</Callout>
