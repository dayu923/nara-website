...
title: Конфіденційний переказ
description:
  Дізнайтеся про розширення Confidential Transfer і як використовувати його для
  додавання додаткових функцій до токен-мінтів та рахунків.
...
Конфіденційні перекази дозволяють переказувати токени між токен-рахунками без
розкриття суми переказу. Це корисно для транзакцій, що зберігають приватність.
Приватними є лише суми переказів та баланси токенів. Адреси токен-рахунків
залишаються публічними.

- [Огляд протоколу](https://www.solana-program.com/docs/confidential-balances/overview) -
  Деталі базового криптографічного протоколу
- [Посібник для швидкого старту](https://www.solana-program.com/docs/confidential-balances#setup) -
  Налаштування та основні команди CLI
- [Збірник рецептів з конфіденційних балансів](https://github.com/solana-developers/Confidential-Balances-Sample) -
  Фрагменти коду про те, як використовувати розширення Confidential Transfer
...
до програми Token Extensions, що дозволяє переказувати токени між рахунками без
...
Основний процес конфіденційних переказів токенів виглядає так:

1. Створіть mint account з розширенням конфіденційного переказу.
2. Створіть token account з розширенням конфіденційного переказу для відправника
   та отримувача.
3. Емітуйте токени на рахунок відправника.
4. **Внесіть** публічний баланс відправника на **конфіденційний очікуваний
   баланс**.
5. **Застосуйте** очікуваний баланс відправника до **конфіденційного доступного
   балансу**.
6. Конфіденційно **перекажіть** токени з token account відправника на token
   account отримувача.
7. **Застосуйте** очікуваний баланс отримувача до **конфіденційного доступного
   балансу**.
8. **Виведіть** конфіденційний доступний баланс отримувача на **публічний
   баланс**.

Для отримання детальнішої інформації про кроки в процесі конфіденційного
переказу, дивіться відповідні сторінки:
...
<Cards>
  ...
  >
    Як внести токени на конфіденційний очікуваний баланс
  </Card>
  <Card
  ...
  >
    Як конфіденційно переказувати токени між token account
  </Card>
</Cards>

Діаграма нижче показує детальну послідовність базового процесу для
...
Приклад на Rust нижче показує кроки для конфіденційних переказів токенів
відповідно до попередньої діаграми. Кроки розділені на допоміжні функції для
більшої зрозумілості.

Щоб запустити приклад, запустіть локальний validator з програмою Token Extension
Program, клонованою з мережі mainnet, використовуючи наступну команду. У вас має
бути встановлений Nara Solana CLI для використання локального validator.
...
## Інструкції Confidential Transfer
...
розширення Confidential Transfer наведено нижче:
...
| _rs`ApproveAccount`_                | Схвалює token account для конфіденційних переказів, якщо mint вимагає схвалення для нових token account.                                                                |
| _rs`EmptyAccount`_                  | Спустошує очікуваний та доступний конфіденційний баланс, щоб дозволити закриття token account.                                                                          |
...
| _rs`Transfer`_                      | Конфіденційно переказує токени між token account.                                                                                                                       |
...
| _rs`DisableConfidentialCredits`_    | Блокує вхідні конфіденційні перекази, при цьому дозволяючи публічні перекази.                                                                                           |
| _rs`EnableNonConfidentialCredits`_  | Дозволяє token account отримувати публічні перекази токенів.                                                                                                            |
| _rs`DisableNonConfidentialCredits`_ | Блокує звичайні перекази, щоб рахунок отримував лише конфіденційні перекази.                                                                                            |
| _rs`TransferWithFee`_               | Конфіденційно переказує токени між token account з комісією.                                                                                                            |
| _rs`ConfigureAccountWithRegistry`_  | Альтернативний спосіб налаштування token account для конфіденційних переказів з використанням рахунку _rs`ElGamalRegistry`_ замість доказу _rs`VerifyPubkeyValidity`_.  |
