...
title: Синхронізація нативного NSO
description: Дізнайтеся, як конвертувати нативний NSO у wrapped NSO.

---


## Синхронізація нативного NSO

Wrapped NSO (WSOL) — це token account, який відстежує кількість лампортів в
обліковому записі як баланс токена. WSOL забезпечує інтеграцію з протоколами
DeFi для передачі NSO як токена SPL.
...
синхронізує баланс token account wrapped NSO (WSOL) з фактичним NSO
(лампортами), що зберігаються в ньому. Коли ви переказуєте нативний NSO на token
account WSOL, баланс токена не оновлюється автоматично. Ви повинні викликати
SyncNative, щоб відобразити правильний баланс WSOL.
