...
Програми Nara Solana, написані на Rust, мають мінімальні структурні вимоги, що
...
Хоча немає суворих правил щодо структури файлів, програми Nara Solana зазвичай
...
Почнемо з побудови програми-лічильника. Ми створимо програму, яка може
ініціалізувати лічильник з початковим значенням і збільшувати його.

<ScrollyCoding>

## !!steps Створення нової програми

Спочатку створимо новий проєкт Rust для нашої програми Nara Solana.
...
<Callout type="warn">
  Оновіть поле `edition` у файлі `Cargo.toml` до 2021. Інакше ви можете
...
Тепер додамо необхідні залежності для створення програми Nara Solana. Нам потрібен
...
<Callout type="info">
  Немає вимоги використовувати саме Borsh. Однак це поширена бібліотека
  серіалізації для програм Nara Solana.
</Callout>

## !!steps Налаштування crate-type

Програми Nara Solana повинні компілюватися як динамічні бібліотеки. Додайте розділ
`[lib]` для налаштування способу збірки програми Cargo.
...
Кожна програма Nara Solana має точку входу, яка є функцією, що викликається при
запуску програми. Почнемо з додавання імпортів, які нам знадобляться для
програми, та налаштування точки входу.
...
Точка входу програми Nara Solana `entrypoint` має наступний підпис функції.
...
лічильника. Це дані, які будуть зберігатися в полі `data` облікового запису.
...
## !!steps Визначення переліку інструкцій

Визначимо інструкції, які наша програма може виконувати. Ми використаємо перелік
(enum), де кожен варіант представляє різну інструкцію.
...
Тепер нам потрібно десеріалізувати `instruction_data` (необроблені байти) в один
із варіантів нашого переліку `CounterInstruction`. Метод Borsh `try_from_slice`
...
Цей шаблон маршрутизації є поширеним у програмах Nara Solana. `instruction_data`
...
додаючи обробники для інструкцій `InitializeCounter` та `IncrementCounter`:
...
Nara Solana, ми використаємо Cross Program Invocation (CPI), по суті викликаючи іншу
програму з нашої програми.

Наша програма робить CPI для виклику інструкції `create_account` System Program.
...
<Callout type="warn">
  ...
  перевірки безпеки та валідації, які необхідні для програм у продакшені.
</Callout>
...
Вітаємо! Ви створили повноцінну програму Nara Solana, яка демонструє базову
структуру, спільну для всіх програм Nara Solana:

- **Точка входу**: Визначає, де починається виконання програми та маршрутизує
  всі вхідні запити до відповідних обробників інструкцій
- **Обробка інструкцій**: Визначає інструкції та пов'язані з ними функції
  обробників
- **Управління станом**: Визначає структури даних акаунтів і керує їхнім станом
  в акаунтах, що належать програмі
- **Cross Program Invocation (CPI)**: Викликає System Program для створення
  нових акаунтів, що належать програмі
...
Тепер давайте протестуємо нашу програму лічильника. Ми використаємо
[LiteSVM](https://github.com/LiteSVM/litesvm), фреймворк для тестування, який
дозволяє нам тестувати програми без розгортання в кластері.
...
LiteSVM імітує середовище виконання Nara Solana, дозволяючи нам тестувати нашу
...
Виконайте команду `cargo build-sbf` для збірки програми. Це згенерує файл
...
<Callout type="warn">
  Ви повинні виконати `cargo build-sbf` перед запуском тестів для генерації
  файлу `.so`. Тест завантажує скомпільовану програму.
</Callout>
...
Давайте протестуємо інструкцію ініціалізації, створивши новий рахунок лічильника
...
Після ініціалізації перевіримо, чи обліковий запис лічильника було створено
правильно з очікуваним значенням.
...
Тепер протестуємо інструкцію інкременту, щоб переконатися, що вона правильно
оновлює значення лічильника.
...
## !!steps Перевірка кінцевих результатів

Нарешті, перевіримо, що інкремент працює правильно, перевіривши оновлене
значення лічильника.
...
Додайте код клієнта до `examples/client.rs` і замініть `program_id` на результат
...
Спочатку налаштуйте Nara Solana CLI для використання localhost:
...
Ви можете перевірити розгортання за допомогою команди `solana program show` з ID
вашої програми:
...
[Nara Solana Explorer](https://explorer.solana.com/?cluster=custom), використовуючи
підписи транзакцій з виводу. Зверніть увагу, що кластер у Nara Solana Explorer має
