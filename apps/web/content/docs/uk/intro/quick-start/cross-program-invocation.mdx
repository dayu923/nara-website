...
Nara Solana викликати одна одну.
...
Інструкції `update` та `delete` потребують модифікації для обробки переказів NSO
між рахунками шляхом виклику System Program.

Мета цього розділу включає покроковий процес реалізації CPIs у програмі Nara Solana
...
`vault_account`. Цей рахунок, контрольований програмою, отримує NSO від
...
отримує NSO від користувачів, коли вони оновлюють свої повідомлення.
...
Далі додайте логіку CPI в інструкцію `update` для переказу 0.001 NSO з акаунта
...
програми, забезпечуючи композиційність програм Nara Solana.
...
- `amount` для переказу (1 000 000 lamports, або 0,001 NSO)
...
дозволяє переказати будь-який NSO зі сховища назад користувачу, коли вони
...
накопиченого NSO назад користувачу.
...
Далі додайте логіку CPI в інструкцію `delete` для переказу NSO зі сховища назад
...
доступність модифікованої програми для тестування. У Nara Solana оновлення програми
просто вимагає розгортання програми з тим самим ідентифікатором програми.

<Callout>

Переконайтеся, що ваш гаманець Playground має devnet NSO. Отримайте devnet NSO з
...
При розгортанні програм на Nara Solana Playground ваш гаманець Playground виступає
...
Вітаємо із завершенням посібника Nara Solana Quickstart. Ви отримали практичний
досвід роботи з ключовими концепціями Nara Solana, включаючи:

- Отримання та читання даних з облікових записів
- Створення та надсилання транзакцій
- Розгортання та оновлення програм Nara Solana
...
Nara Solana Playground пропонує зручну функцію, яка дозволяє імпортувати або
переглядати проєкти за допомогою їхніх посилань на GitHub. Наприклад, відкрийте
це
[посилання Nara Solana Playground](https://beta.solpg.io/https://github.com/solana-developers/program-examples/tree/main/basics/hello-solana/anchor),
...
списку проєктів у Nara Solana Playground. Після імпорту проєкту всі зміни автоматично
зберігаються.
