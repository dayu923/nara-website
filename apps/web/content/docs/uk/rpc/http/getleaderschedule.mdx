...
h1: RPC-метод getLeaderSchedule

---


Повертає розклад лідерів для epoch
...
Отримати розклад лідерів для epoch, що відповідає вказаному slot. Якщо не
вказано, буде отримано розклад лідерів для поточного epoch.
...
Повертати результати лише для цього validator (закодованого в base-58)
...
Повертає `null` якщо запитуваний epoch не знайдено, інакше повертає об'єкт де:

- Ключі — це ідентифікатори validator (у вигляді рядків, кодованих у base-58)
- Значення — це масиви індексів лідерських slot відносно першого slot у
  запитуваному epoch
