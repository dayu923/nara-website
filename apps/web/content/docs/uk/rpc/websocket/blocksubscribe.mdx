...
h1: RPC-метод blockSubscribe

---


Підпишіться, щоб отримувати сповіщення щоразу, коли новий блок `confirmed` або
`finalized`.

<Callout type="warn" title={"Нестабільний метод"}>
  Ця підписка вважається **нестабільною** і доступна лише якщо validator був
  запущений з прапорцем `--rpc-pubsub-enable-block-subscription`. Формат цієї
  підписки може змінитися в майбутньому.
</Callout>
...
критерії фільтрації для логів для отримання результатів за типом облікового
запису; наразі підтримуються:

- `all` - включити всі транзакції в блоці
- JSON-об'єкт з наступним полем:
  - `mentionsAccountOrProgram: <string>` - повертати лише транзакції, які
    згадують наданий публічний ключ (як рядок у кодуванні base-58). Якщо в
    певному блоці немає згадок, сповіщення не надсилатиметься.
...
формат кодування для кожної поверненої транзакції
...
рівень деталізації транзакції для повернення
...
чи включати масив `rewards`. Якщо параметр не вказано, за замовчуванням включає
винагороди.
...
ідентифікатор підписки (потрібен для відписки)
...
- `err: <object|null>` - Помилка, якщо щось пішло не так під час публікації
  сповіщення, інакше null.
- `block: <object|null>` - Об'єкт блоку, як показано в методі
