...
تحتاج تعليمات `update` و`delete` إلى تعديل للتعامل مع تحويلات NSO بين الحسابات
...
هذا الحساب، الذي يتحكم فيه البرنامج، يتلقى NSO من المستخدم عندما يقومون بتحديث
حساب رسالتهم.
...
هذا الحساب كـ "خزنة" يتحكم بها البرنامج لتلقي عملات NSO من المستخدمين عندما
...
بعد ذلك، أضف منطق CPI في تعليمة `update` لتحويل 0.001 NSO من حساب المستخدم إلى
حساب الخزنة.
...
- الـ `amount` للتحويل (1,000,000 lamport، أو 0.001 NSO)
...
أولاً، قم بتحديث بنية _rs`Delete`_ لتشمل `vault_account`. هذا يسمح بتحويل أي NSO
...
المستخدم أثناء تعليمة الحذف لتحويل أي رصيد NSO متراكم مرة أخرى إلى المستخدم.
...
بعد ذلك، أضف منطق CPI في تعليمة `delete` لتحويل NSO من حساب الخزنة مرة أخرى إلى
حساب المستخدم.
...
تأكد من أن محفظة Playground الخاصة بك تحتوي على devnet NSO. احصل على devnet NSO
من [Nara Solana Faucet](https://faucet.solana.com/).
...
عند نشر البرامج على Nara Solana Playground، تعمل محفظة Playground الخاصة بك كسلطة
ترقية لجميع برامجك.
