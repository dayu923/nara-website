...
title: سحب الرموز
description: تعلم كيفية سحب الرموز من الحالة السرية.
...
لسحب الرموز من الرصيد السري المتاح إلى الرصيد العام:

1. إنشاء
   [إثباتين](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/confidential-transfer/proof-generation/src/withdraw.rs#L22)
   من جانب العميل:
   ...
   **إثبات المساواة
   ...
   يتحقق من أن النص المشفر للرصيد المتاح المتبقي بعد السحب يتطابق مع
   [التزام بيدرسن](https://en.wikipedia.org/wiki/Commitment_scheme) المقابل له،
   مما يضمن حساب رصيد الحساب الجديد المتاح بشكل صحيح كـ
   ...
   **إثبات النطاق
   ...
   يتحقق من أن الرصيد المتاح المتبقي بعد السحب غير سالب وضمن نطاق محدد.
...
3. استدعاء التعليمة
   ...
   مع توفير حسابي الإثبات.

4. إغلاق حسابات الإثبات لاسترداد الـ NSO المستخدم لإنشائها.

يوضح الرسم البياني التالي الخطوات المتضمنة في سحب الرموز من الرصيد السري المتاح
إلى الرصيد العام:
...
لسحب الرموز من الرصيد السري المتاح إلى الرصيد العام، يجب عليك:

- إنشاء إثبات المساواة وإثبات النطاق من جانب العميل
- استدعاء برنامج إثبات Zk ElGamal للتحقق من الإثباتات وتهيئة حسابات "حالة
  السياق"
- استدعاء التعليمة
  [ConfidentialTransferInstruction::Withdraw](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L493)
  مع توفير حسابي الإثبات.
- إغلاق حسابي الإثبات لاسترداد الـ rent.

توفر مكتبة `spl_token_client` الطرق التالية:

- طريقة `confidential_transfer_create_context_state_account` التي تنشئ حساب
  إثبات.
- طريقة `confidential_transfer_withdraw` التي تستدعي تعليمة `Withdraw`.
- طريقة `confidential_transfer_close_context_state_account` التي تغلق حساب
  الإثبات.

### مثال على الكود

يوضح المثال التالي كيفية سحب الرموز من الرصيد السري المتاح إلى الرصيد العام.

لتشغيل المثال، ابدأ validator محلي مع برنامج Token Extensions Program المستنسخ
من الشبكة الرئيسية باستخدام الأمر التالي. يجب أن يكون لديك واجهة سطر أوامر
سولانا مثبتة لبدء الـ validator المحلي.
...
<Callout type="info">
  في وقت كتابة هذا التقرير، لم يتم تمكين التحويلات السرية على الـ validator
  المحلي الافتراضي. يجب عليك استنساخ برنامج Token Extensions Program من الشبكة
