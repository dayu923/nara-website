...
title: التحويل السري
description:
  تعرف على ملحق التحويل السري وكيفية استخدامه لإضافة ميزات اختيارية إلى mint
  account و token account.
...
تتيح التحويلات السرية نقل الرموز بين حسابات الرموز دون الكشف عن مبلغ التحويل.
هذا مفيد للمعاملات التي تحافظ على الخصوصية. فقط مبالغ التحويل وأرصدة الرموز هي
التي تكون خاصة. تظل عناوين حسابات الرموز عامة.
...
- [دليل البدء السريع](https://www.solana-program.com/docs/confidential-balances#setup) -
  الإعداد وأوامر واجهة سطر الأوامر الأساسية
- [كتاب الطبخ للأرصدة السرية](https://github.com/solana-developers/Confidential-Balances-Sample) -
  مقتطفات الكود حول كيفية استخدام ملحق التحويل السري

### كيف يعمل ذلك؟

يضيف ملحق التحويل السري
...
إلى برنامج Token Extensions الذي يسمح لك بتحويل الرموز بين الحسابات دون الكشف عن
مبلغ التحويل.
...
التدفق الأساسي لتحويلات الرموز السرية هو كما يلي:

1. إنشاء mint account مع ملحق التحويل السري.
2. إنشاء token accounts مع ملحق التحويل السري للمرسل والمستلم.
3. سك الرموز إلى حساب المرسل.
4. **إيداع** الرصيد العام للمرسل إلى **الرصيد السري المعلق**.
5. **تطبيق** الرصيد المعلق للمرسل على **الرصيد السري المتاح**.
6. **تحويل** الرموز بسرية من token account الخاص بالمرسل إلى token account الخاص
   بالمستلم.
7. **تطبيق** الرصيد المعلق للمستلم على **الرصيد السري المتاح**.
8. **سحب** الرصيد السري المتاح للمستلم إلى **الرصيد العام**.
...
<Cards>
  ...
  >
    كيفية إعداد token account مع امتداد التحويل السري
  </Card>
  <Card
    title="إيداع الرموز"
    href="/docs/tokens/extensions/confidential-transfer/deposit-tokens"
  >
    كيفية إيداع الرموز إلى الرصيد السري المعلق
  </Card>
  <Card
  ...
  >
    كيفية سحب الرموز من الرصيد السري المتاح
  </Card>
  <Card
    title="تحويل الرموز"
    href="/docs/tokens/extensions/confidential-transfer/transfer-tokens"
  >
    كيفية تحويل الرموز بسرية بين حسابات الرموز
  </Card>
</Cards>

يوضح الرسم البياني أدناه تسلسلاً مفصلاً للتدفق الأساسي لتحويلات الرموز السرية:
...
يوضح مثال Rust أدناه خطوات تحويلات الرموز السرية المقابلة للرسم البياني السابق.
تم فصل الخطوات إلى وظائف مساعدة للتوضيح.

لتشغيل المثال، ابدأ مصادقًا محليًا مع Token Extensions Program المستنسخ من
الشبكة الرئيسية باستخدام الأمر التالي. يجب أن يكون لديك واجهة سطر أوامر سولانا
مثبتة لاستخدام المصادق المحلي.
...
<Callout type="info">
  في وقت كتابة هذا التقرير، لم يتم تمكين التحويلات السرية على المصادق المحلي
  الافتراضي. يجب عليك استنساخ Token Extension Program من الشبكة الرئيسية لتشغيل
  كود المثال.
</Callout>
...
## تعليمات التحويل السري

القائمة الكاملة لتعليمات امتداد التحويل السري
...
| التعليمة                            | الوصف                                                                                                                             |
| ----------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | إعداد mint account للتحويلات السرية. يجب تضمين هذه التعليمة في نفس المعاملة مع تعليمة _rs`TokenInstruction::InitializeMint`_.     |
| _rs`UpdateMint`_                    | تحديث إعدادات التحويل السري للعملة.                                                                                               |
| _rs`ConfigureAccount`_              | إعداد token account للتحويلات السرية.                                                                                             |
| _rs`ApproveAccount`_                | الموافقة على token account للتحويلات السرية إذا كانت العملة تتطلب موافقة لحسابات الرموز الجديدة.                                  |
| _rs`EmptyAccount`_                  | تفريغ الأرصدة السرية المعلقة والمتاحة للسماح بإغلاق token account.                                                                |
| _rs`Deposit`_                       | تحويل رصيد الرموز العام إلى رصيد سري معلق.                                                                                        |
| _rs`Withdraw`_                      | تحويل الرصيد السري المتاح مرة أخرى إلى رصيد عام.                                                                                  |
| _rs`Transfer`_                      | تحويل الرموز بين token accounts بشكل سري.                                                                                         |
| _rs`ApplyPendingBalance`_           | تحويل الرصيد المعلق إلى رصيد متاح بعد الإيداعات أو التحويلات.                                                                     |
| _rs`EnableConfidentialCredits`_     | السماح لـ token account باستلام تحويلات الرموز السرية.                                                                            |
| _rs`DisableConfidentialCredits`_    | حظر التحويلات السرية الواردة مع السماح بالتحويلات العامة.                                                                         |
| _rs`EnableNonConfidentialCredits`_  | السماح لـ token account باستلام تحويلات الرموز العامة.                                                                            |
| _rs`DisableNonConfidentialCredits`_ | حظر التحويلات العادية لجعل الحساب يستقبل فقط التحويلات السرية.                                                                    |
| _rs`TransferWithFee`_               | تحويل الرموز بين token accounts بشكل سري مع رسوم.                                                                                 |
| _rs`ConfigureAccountWithRegistry`_  | طريقة بديلة لإعداد token accounts للتحويلات السرية باستخدام حساب _rs`ElGamalRegistry`_ بدلاً من إثبات _rs`VerifyPubkeyValidity`_. |
