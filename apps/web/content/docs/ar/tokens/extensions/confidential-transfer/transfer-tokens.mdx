...
title: تحويل الرموز
description: تعلم كيفية تحويل الرموز بشكل خاص من حساب رمز إلى آخر.
...
## كيفية تحويل الرموز بشكل سري من حساب رمز إلى آخر

لتحويل الرموز بشكل سري من حساب رمز إلى آخر، يجب أن يكون لدى كل من المرسل
والمستلم حسابات رموز مهيأة بحالة _rs`ConfidentialTransferAccount`_ ومعتمدة
للتحويلات السرية. يجب أيضًا أن يكون لدى حساب token account الخاص بالمرسل رصيد
سري متاح للتحويل منه.

لتحويل الرموز بشكل سري:

1. قم بإنشاء
   [ثلاثة إثباتات](https://github.com/solana-program/token-2022/blob/d073cb89dbcd430387b5c6fb4b7157911351e4a3/confidential-transfer/proof-generation/src/transfer.rs#L35)
   من جانب العميل:
   ...
   **إثبات المساواة
   ...
   يتحقق من أن النص المشفر للرصيد المتاح الجديد بعد التحويل يطابق
   [التزام بيدرسن](https://en.wikipedia.org/wiki/Commitment_scheme) المقابل له،
   مما يضمن أن الرصيد المتاح الجديد لحساب المصدر يتم حسابه بشكل صحيح كـ
   `new_balance = current_balance - transfer_amount`.

   **إثبات صحة النص المشفر
   ...
   يتحقق من أن النصوص المشفرة لمبلغ التحويل تم إنشاؤها بشكل صحيح لجميع الأطراف
   الثلاثة (المصدر والوجهة والمدقق)، مما يضمن تشفير مبلغ التحويل بشكل صحيح تحت
   المفتاح العام لكل طرف.
   ...
   **إثبات النطاق
   ...
   يتحقق من أن الرصيد المتاح الجديد ومبلغ التحويل (مقسم إلى بتات منخفضة/عالية)
   جميعها غير سالبة وضمن نطاق محدد.

2. لكل إثبات:
   - استدعاء برنامج إثبات ZK ElGamal للتحقق من بيانات الإثبات.
   - تخزين البيانات الوصفية الخاصة بالإثبات في حساب "حالة سياق" الإثبات
     لاستخدامها في تعليمات أخرى.
...
3. استدعاء تعليمة
   ...
   مع توفير حسابات حالة سياق الإثبات.

4. إغلاق حسابات حالة سياق الإثبات لاسترداد NSO المستخدم لإنشائها.

يوضح الرسم البياني التالي الخطوات المتضمنة في تحويل الرموز من حساب رموز المرسل
إلى حساب رموز المستلم.
...
لتحويل الرموز بشكل سري من حساب رموز إلى آخر، يجب عليك:

- إنشاء إثبات المساواة، وإثبات صحة النص المشفر، وإثبات النطاق من جانب العميل
- استدعاء برنامج إثبات Zk ElGamal للتحقق من الإثباتات وتهيئة حسابات "حالة
  السياق"
- استدعاء تعليمة
  [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604)
  مع توفير حسابات الإثبات الثلاثة.
- إغلاق حسابات الإثبات الثلاثة لاسترداد الإيجار.

توفر مكتبة `spl_token_client` الطرق التالية:

- طريقة `confidential_transfer_create_context_state_account` التي تنشئ حساب
  إثبات.
- طريقة `confidential_transfer_transfer` التي تستدعي تعليمة `Transfer`.
- طريقة `confidential_transfer_close_context_state_account` التي تغلق حساب
  إثبات.

### مثال على الكود

يوضح المثال التالي كيفية تحويل الرموز بشكل سري بين حسابات الرموز.

لتشغيل المثال، ابدأ مصادق محلي مع برنامج Token Extensions Program المستنسخ من
الشبكة الرئيسية باستخدام الأمر التالي. يجب أن يكون لديك واجهة سطر أوامر سولانا
مثبتة لبدء المصادق المحلي.
...
<Callout type="info">
  في وقت كتابة هذا المقال، لم يتم تمكين التحويلات السرية على المصادق المحلي
  الافتراضي. يجب عليك استنساخ برنامج Token Extensions Program من الشبكة الرئيسية
  لتشغيل كود المثال.
</Callout>
