...
description: تعلم كيفية تطبيق الرصيد المعلق لجعل الأموال متاحة.
...
## كيفية تطبيق الرصيد المعلق على الرصيد المتاح

قبل أن يمكن تحويل الرموز بشكل سري، يجب تحويل رصيد الرموز العام إلى رصيد سري.
تحدث هذه العملية على مرحلتين:

1. **الرصيد السري المعلق**: في البداية، يتم "إيداع" الرموز من الرصيد العام إلى
   رصيد سري "معلق".
2. **الرصيد السري المتاح**: ثم يتم "تطبيق" الرصيد المعلق على الرصيد المتاح، مما
   يجعل الرموز متاحة للتحويلات السرية.

يشرح هذا القسم المرحلة الثانية: تطبيق الرصيد المعلق على الرصيد المتاح.

عندما يتم "إيداع" الرموز من الرصيد العام أو عندما يتم تحويل الرموز بشكل سري من
token account إلى آخر، تتم إضافة الرموز في البداية إلى الرصيد السري المعلق. قبل
أن يمكن استخدام الرموز للتحويلات السرية، يجب "تطبيق" الرصيد المعلق على الرصيد
المتاح.
...
يوضح الرسم البياني التالي الخطوات المتضمنة في تطبيق الرصيد المعلق على الرصيد
المتاح:
...
لتحويل الرصيد المعلق إلى رصيد متاح، قم باستدعاء تعليمة
...
توفر حزمة `spl_token_client` طريقة `confidential_transfer_apply_pending_balance`
التي تقوم ببناء وإرسال معاملة مع تعليمة `ApplyPendingBalance`، كما هو موضح في
المثال أدناه.

### مثال على الكود

يوضح المثال التالي كيفية تطبيق الرصيد السري المعلق على الرصيد السري المتاح.

لتشغيل المثال، قم بتشغيل validator محلي مع برنامج Token Extension Program
المستنسخ من الشبكة الرئيسية باستخدام الأمر التالي. يجب أن يكون لديك واجهة سطر
أوامر سولانا مثبتة لبدء تشغيل validator المحلي.
...
<Callout type="info">
  في وقت كتابة هذا المحتوى، لم يتم تفعيل التحويلات السرية على validator المحلي
