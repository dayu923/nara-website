...
title: تعيين السلطة
description: تعلم كيفية تعيين السلطة لحسابات mint أو حسابات الرموز.
...
- **سلطة الإصدار**: تتحكم في إنشاء رموز جديدة. يمكنها إصدار رموز لأي حساب رموز.
  غالبًا ما يتم إلغاؤها بعد إنشاء المعروض الأولي لإنشاء رمز ذو معروض ثابت.
- **سلطة التجميد**: تتحكم في القدرة على تجميد وإذابة حسابات الرموز. يمكنها منع
  أي حساب رموز من تحويل الرموز. غالبًا ما يتم إلغاؤها لضمان أن رموز المستخدمين
  لا يمكن تجميدها.

### سلطات حساب الرموز

- **مالك الحساب**: لديه سيطرة كاملة على حساب الرموز. يمكنه تحويل الرموز، وحرق
  الرموز، والموافقة على المفوضين، وإغلاق الحساب عندما يكون الرصيد صفرًا.
- **سلطة الإغلاق**: يمكنها إغلاق حساب الرموز عندما يكون رصيده صفرًا. افتراضيًا،
  هذا هو مالك الحساب ولكن يمكن تفويضه لحساب آخر.
...
تغير أو تلغي السلطات على حسابات mint وحسابات الرموز. فقط السلطة الحالية يمكنها
نقل صلاحياتها إلى عنوان جديد أو إلغائها بشكل دائم عن طريق تعيين السلطة إلى null.
بمجرد الإلغاء، لا يمكن استعادة السلطة.

<Callout type="info">
  [برنامج
  الرموز](https://github.com/solana-program/token/blob/a7c488ca39ed4cd71a87950ed854929816e9099f/program/src/instruction.rs#L153)
  و[برنامج امتداد
  الرموز](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/instruction.rs#L195)
  يشتركان في تنفيذات متشابهة لتحقيق نفس الوظيفة.
</Callout>
