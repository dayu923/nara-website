...
على الرغم من عدم وجود قواعد صارمة لهيكل الملفات، فإن برامج سولانا عادةً تتبع
نمطًا شائعًا:
...
على سبيل المثال، انظر إلى
[برنامج الرمز المميز](https://github.com/solana-program/token/tree/main/program/src).
...
للحصول على SDK الأساسي و `borsh` للتسلسل.
...
كل برنامج سولانا له نقطة دخول، وهي الوظيفة التي يتم استدعاؤها عند تشغيل
البرنامج. دعنا نبدأ بإضافة الاستيرادات التي سنحتاجها للبرنامج وإعداد نقطة
الدخول.
...
مع إلغاء تسلسل بيانات `input` إلى معلمات وظيفة `process_instruction`.

يحتوي `entrypoint` لبرنامج سولانا على توقيع الوظيفة التالي. المطورون أحرار في
...
## !!steps تحديد حالة البرنامج

الآن دعنا نحدد بنية البيانات التي سيتم تخزينها في حسابات العداد الخاصة بنا. هذه
هي البيانات التي سيتم تخزينها في حقل `data` الخاص بالحساب.
...
دعنا نحدد التعليمات التي يمكن لبرنامجنا تنفيذها. سنستخدم تعداداً (enum) حيث يمثل
كل متغير تعليمة مختلفة.
...
## !!steps تنفيذ فك تشفير التعليمات

الآن نحتاج إلى فك تشفير `instruction_data` (البايتات الخام) إلى أحد متغيرات
التعداد `CounterInstruction`. تتعامل طريقة Borsh `try_from_slice` مع هذا التحويل
تلقائياً.
...
الآن دعنا نحدث وظيفة `process_instruction` الرئيسية لتوجيه التعليمات إلى وظائف
المعالجة المناسبة لها.

هذا النمط من التوجيه شائع في برامج سولانا. يتم فك تشفير `instruction_data` إلى
متغير من تعداد يمثل التعليمة، ثم يتم استدعاء وظيفة المعالجة المناسبة. تتضمن كل
وظيفة معالجة التنفيذ الخاص بتلك التعليمة.

أضف الكود التالي إلى `lib.rs` لتحديث وظيفة `process_instruction` وإضافة معالجات
...
دعنا ننفذ المعالج لإنشاء وتهيئة حساب عداد جديد. بما أن System Program فقط هو
الذي يمكنه إنشاء حسابات على سولانا، سنستخدم استدعاء عبر البرامج (Cross Program
Invocation - CPI)، وهو في الأساس استدعاء برنامج آخر من برنامجنا.

يقوم برنامجنا بإجراء CPI لاستدعاء تعليمة `create_account` الخاصة بـ System
...
<Callout type="warn">
  هذه التعليمات لأغراض التوضيح فقط. لا تتضمن فحوصات الأمان والتحقق المطلوبة
  للبرامج الإنتاجية.
</Callout>
...
تهانينا! لقد قمت ببناء برنامج سولانا كامل يوضح الهيكل الأساسي المشترك بين جميع
برامج سولانا:

- **نقطة الدخول**: تحدد أين يبدأ تنفيذ البرنامج وتوجه جميع الطلبات الواردة إلى
  معالجات التعليمات المناسبة
- **معالجة التعليمات**: تحدد التعليمات ودوال المعالجة المرتبطة بها
- **إدارة الحالة**: تحدد هياكل بيانات الحساب وتدير حالتها في الحسابات المملوكة
  للبرنامج
- **استدعاء البرامج المتقاطع (Cross Program Invocation)**: يستدعي System Program
  لإنشاء حسابات جديدة مملوكة للبرنامج
...
الآن دعنا نختبر برنامج العداد الخاص بنا. سنستخدم
[LiteSVM](https://github.com/LiteSVM/litesvm)، وهو إطار اختبار يتيح لنا اختبار
البرامج دون نشرها على مجموعة.

<ScrollyCoding>

## !!steps إضافة تبعيات الاختبار

أولاً، دعنا نضيف التبعيات اللازمة للاختبار. سنستخدم `litesvm` للاختبار و
`solana-sdk`.
...
يحاكي LiteSVM بيئة تشغيل سولانا، مما يسمح لنا باختبار برنامجنا دون النشر على
مجموعة حقيقية.
...
<Callout type="warn">
  يجب عليك تشغيل `cargo build-sbf` قبل تشغيل الاختبارات لإنشاء ملف `.so`. يقوم
...
قم بتشغيل الاختبارات باستخدام الأمر التالي. العلامة `--nocapture` تطبع مخرجات
الاختبار.
...
الآن بعد أن أصبح برنامجنا والعميل جاهزين، دعنا نقوم ببناء ونشر واستدعاء
البرنامج.
...
[مستكشف سولانا](https://explorer.solana.com/?cluster=custom) باستخدام توقيعات
المعاملات الناتجة. لاحظ أنه يجب تعيين المجموعة في مستكشف سولانا إلى "عنوان URL
مخصص لـ RPC"، والذي يكون افتراضيًا `http://localhost:8899` الذي يعمل عليه
`solana-test-validator`.
