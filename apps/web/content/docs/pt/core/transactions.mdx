...
Para interagir com a rede Nara Solana, você deve enviar uma transação. Você pode
...
<Callout>
  ...
  sem fragmentação, a Nara Solana usa 1280 bytes (o MTU mínimo exigido para IPv6)
...
- [`recent_blockhash`](mention:recent-blockhash): Um
  [blockhash](#recent-blockhash) que atua como um timestamp para a transação
...
- [`num_required_signatures`](mention:num_required_signatures): O número total
  de assinaturas exigidas pela transação
- [`num_readonly_signed_accounts`](mention:num_readonly_signed_accounts): O
  número total de contas somente leitura que exigem assinaturas
- [`num_readonly_unsigned_accounts`](mention:num_readonly_unsigned_accounts): O
  número total de contas somente leitura que não exigem assinaturas
...
<Callout>
  ...
  informações no [`header`](#header) da mensagem para determinar as permissões
  para cada conta.
</Callout>
...
3. [`data`](mention:instruction-data): Um array de bytes especificando qual
   instrução invocar no programa. Também inclui quaisquer dados adicionais
   exigidos pela instrução. (Por exemplo, argumentos de função)
...
<Callout>
  Você também pode encontrar a transação usando o [Nara Solana
