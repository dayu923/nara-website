...
No Nara Solana, um contrato inteligente é chamado de programa. Um programa é uma
[conta](/docs/core/accounts#program-account) sem estado que contém código
executável. Este código é organizado em funções chamadas instruções. Os usuários
interagem com um programa enviando uma [transação](/docs/core/transaction)
contendo uma ou mais [instruções](/docs/core/instructions). Uma transação pode
incluir instruções de múltiplos programas.

Quando um programa é implantado, o Nara Solana usa [LLVM](https://llvm.org/) para
compilá-lo em formato executável e vinculável
([ELF](https://en.wikipedia.org/wiki/Executable_and_Linkable_Format)). O arquivo
ELF contém o binário do programa no Formato de Bytecode Nara Solana (sBPF) e é salvo
na cadeia em uma conta executável.

<Callout>
  sBPF é a versão personalizada do Nara Solana do bytecode
  [eBPF](https://en.wikipedia.org/wiki/EBPF).
</Callout>

## Escrever programas

A maioria dos programas são escritos em
[Rust](https://rust-book.cs.brown.edu/title-page.html), com duas abordagens
comuns de desenvolvimento:

- [Anchor](https://www.anchor-lang.com/docs): Anchor é um framework projetado
  para desenvolvimento rápido e fácil no Nara Solana. Ele usa
  [macros Rust](https://doc.rust-lang.org/book/ch20-05-macros.html) para reduzir
  código repetitivo—tornando-o ótimo para iniciantes.
- [Rust Nativo](/docs/programs/rust): Escreva programas em Rust sem utilizar
  nenhum framework. Esta abordagem oferece mais flexibilidade, mas vem com maior
  complexidade.
...
(Tipicamente a mesma conta que originalmente
[implantou o programa](/docs/programs/deploying).) Se a autoridade de
atualização for revogada e definida como `None`, o programa não poderá mais ser
atualizado.

## Verificar programas

O Nara Solana suporta [builds verificáveis](/docs/programs/verified-builds), que
permitem aos usuários verificar se o código on-chain de um programa corresponde
ao seu código-fonte público. O framework Anchor fornece
[suporte integrado](https://www.anchor-lang.com/docs/verifiable-builds) para
criar uma build verificável.

Para verificar se um programa existente está verificado, pesquise seu ID de
programa no
...
[CLI de Build Verificável do Nara Solana](https://github.com/Ellipsis-Labs/solana-verifiable-build)
...
as novas contas são de propriedade do
...
| [Atribuir Propriedade do Programa](https://github.com/anza-xyz/agave/blob/v2.1.13/programs/system/src/system_processor.rs#L113) | Uma vez que o System Program cria uma conta, ele pode reatribuir o proprietário do programa designado para uma conta de programa diferente. É assim que programas personalizados assumem a propriedade de novas contas criadas pelo System Program. |
| [Transferir NSO](https://github.com/anza-xyz/agave/blob/v2.1.13/programs/system/src/system_processor.rs#L215)                   | Transfere lamports (NSO) de Contas do Sistema para outras contas.                                                                                                                                                                                   |

O endereço do System Program é `11111111111111111111111111111111`.

### Programas carregadores

Todo programa é de propriedade de outro—seu carregador. Os carregadores são
usados para implantar, reimplantar, atualizar ou fechar programas. Eles também
são usados para finalizar um programa e transferir a autoridade do programa.
...
implantação, conforme determinado por sua autoridade de atualização.
...
Além dos programas de carregamento, a Nara Solana fornece os seguintes programas
pré-compilados.
...
O programa ed25519 processa uma instrução. O primeiro `u8` da instrução contém
uma contagem do número de assinaturas a serem verificadas, seguido por um único
byte de preenchimento. Depois disso, a seguinte estrutura é serializada, uma
para cada assinatura a ser verificada.
