...
Instruções são o bloco fundamental para interagir com a blockchain Nara Solana. Uma
instrução é essencialmente uma função pública que qualquer pessoa usando a rede
Nara Solana pode chamar. Cada instrução é usada para realizar uma ação específica. A
...
de instruções. Para interagir com a rede Nara Solana, uma ou mais instruções são
adicionadas a uma [transação](/docs/core/transactions) e enviadas para a rede
para serem processadas.

## Exemplo de transferência de NSO

O diagrama abaixo mostra como transações e instruções trabalham juntas para
permitir que os usuários interajam com a rede. Neste exemplo, NSO é transferido
...
mensagem contendo a instrução de transferência de NSO.

![Diagrama de transferência de NSO](/assets/docs/core/transactions/sol-transfer.svg)
...
![Diagrama do processo de transferência de NSO](/assets/docs/core/transactions/sol-transfer-process.svg)
...
O exemplo abaixo mostra a estrutura de uma instrução de transferência de NSO.
