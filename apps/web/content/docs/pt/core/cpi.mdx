...
description:
  ...
  chamar instruções em outros programas, lidar com assinantes PDA e compor
  funcionalidades através da rede Solana.

---


Uma invocação entre programas (CPI) ocorre quando um programa Nara Solana invoca
diretamente as instruções de outro programa. Isso permite a composição de
programas. Se você pensar em uma [instrução](/docs/core/instructions) da Nara Solana
como um endpoint de API que um programa expõe para a rede, um CPI é como um
endpoint invocando internamente outro.

Ao fazer um CPI, um programa pode
[assinar](/docs/references/terminology#signature) em nome de um
[PDA](/docs/core/pda) derivado do seu ID de programa. Esses privilégios de
assinatura se estendem do programa chamador para o programa chamado.

![Exemplo de invocação entre programas](/assets/docs/core/cpi/cpi.svg)

Ao fazer um CPI, os privilégios da conta se estendem de um programa para outro.
Vamos dizer que o Programa A recebe uma instrução com uma conta assinante e uma
conta gravável. O Programa A então faz um CPI para o Programa B. Agora, o
Programa B pode usar as mesmas contas que o Programa A, com suas permissões
originais. (Isso significa que o Programa B pode assinar com a conta assinante e
pode escrever na conta gravável.) Se o Programa B fizer seus próprios CPIs, ele
pode passar essas mesmas permissões adiante, até uma profundidade de 4.
...
<Callout>
  ...
  e é definida como a constante [MAX_INSTRUCTION_STACK_DEPTH](https://github.com/anza-xyz/agave/blob/v2.1.13/compute-budget/src/compute_budget.rs#L13)
  de 5.

A altura da pilha começa em 1 para a transação inicial e aumenta em 1 cada vez
que um programa invoca outra instrução, limitando a profundidade de invocação
para CPIs a 4.
...
é usada. Ela recebe as [seeds do assinante](mention:signer-seeds) usadas para
derivar [PDAs](<(/docs/core/pda)>) assinantes. O runtime da Nara Solana internamente
chama
...
Os exemplos abaixo fazem um CPI com assinantes PDA usando Anchor e Rust Nativo.
Cada exemplo inclui uma única instrução para transferir NSO de um PDA para uma
conta destinatária, usando um CPI assinado pelo PDA.

#### Anchor

Os exemplos a seguir mostram três abordagens para implementar CPIs em um
programa [Anchor](https://www.anchor-lang.com/docs). Os exemplos são
funcionalmente equivalentes, mas cada um demonstra um nível diferente de
abstração.

- **Exemplo 1**: Usa o _rs`CpiContext`_ do Anchor e função auxiliar.
- **Exemplo 2**: Usa a função _rs`system_instruction::transfer`_ do crate
  `solana_program`. (O exemplo 1 é uma abstração desta implementação.)
- **Exemplo 3**: Constrói a instrução CPI manualmente. Esta abordagem é útil
  quando não existe um crate disponível para ajudar a construir a instrução que
  você deseja invocar.
...
O exemplo abaixo faz um CPI com assinantes PDA a partir de um programa escrito
em Rust Nativo. Ele inclui uma única instrução que transfere NSO de uma conta
PDA para outra. O CPI é assinado pela conta PDA. (O arquivo de teste usa
...
Os exemplos abaixo fazem um CPI usando Anchor e Rust Nativo. Incluem uma única
instrução que transfere NSO de uma conta para outra.
...
- **Exemplo 2**: Usa a função _rs`system_instruction::transfer`_ do crate
  `solana_program`.
- **Exemplo 3**: Constrói a instrução CPI manualmente. Esta abordagem é útil
  quando não existe um crate para ajudar a construir a instrução que você deseja
  invocar.
...
Rust Nativo. Inclui uma única instrução que transfere NSO de uma conta para
outra. (O arquivo de teste usa [LiteSVM](https://github.com/LiteSVM/litesvm)
