...
Retorna a programação de líderes para um epoch
...
Busca a programação de líderes para o epoch que corresponde ao slot fornecido.
Se não for especificado, a programação de líderes para o epoch atual será
buscada.
...
Retorna `null` se o epoch solicitado não for encontrado, caso contrário retorna
um objeto onde:

- As chaves são identidades de validator (como strings codificadas em base-58)
- Os valores são arrays de índices de slots de líder relativos ao primeiro slot
  na época solicitada
