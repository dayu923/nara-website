...
title: Mollusk
description: Guia completo para testar programas Nara Solana usando Mollusk

---


[Mollusk](https://github.com/anza-xyz/mollusk/tree/main) é um framework leve
para testar programas Nara Solana. Ele fornece uma interface simples para testar
instruções de programas Nara Solana em um ambiente minimizado da Máquina Virtual
Nara Solana (SVM). Todas as contas de teste devem ser explicitamente definidas,
garantindo testes determinísticos e reproduzíveis.

- [Documentação do Mollusk SVM](https://docs.rs/mollusk-svm/latest/mollusk_svm/)
- [Exemplos de uso de testes do Mollusk](https://github.com/anza-xyz/mollusk/tree/0.5.0/harness/tests)

## Instalação

Adicione `mollusk-svm` como dependência no `Cargo.toml`:
...
`mollusk-svm-bencher` como dependência no `Cargo.toml`:
...
O exemplo a seguir mostra uma configuração mínima para testar um programa Nara Solana
...
Este exemplo demonstra como testar um programa Nara Solana básico usando Mollusk. O
...
Para testar um programa Nara Solana com Mollusk:

1. [Crie uma instância `Mollusk`](mention:mollusk) - Inicialize o Mollusk com um
   ID de programa e o caminho para o programa compilado (arquivo `.so`)
2. [Construa uma instrução](mention:instruction) - Crie uma instrução para
   invocar o programa
3. [Processe e valide](mention:process) - Processe a instrução usando Mollusk e
   valide o resultado
...
fornece uma interface simples para testar programas Nara Solana. Todos os campos
...
<Callout type="info">
  Ao fornecer o caminho do arquivo, não inclua a extensão `.so`. Por exemplo,
...
| [`process_and_validate_instruction`](#single-instruction-with-checks)          | Processa uma instrução e realiza uma série de verificações no resultado, gerando pânico se alguma verificação falhar.             |
| [`process_instruction_chain`](#multiple-instructions)                          | Processa múltiplas instruções e retorna o resultado.                                                                              |
| [`process_and_validate_instruction_chain`](#multiple-instructions-with-checks) | Processa múltiplas instruções e realiza uma série de verificações em cada resultado, gerando pânico se alguma verificação falhar. |
...
O exemplo a seguir processa uma instrução de transferência de NSO sem
...
O exemplo a seguir processa uma instrução de transferência de NSO com
verificações de validação.
...
O exemplo a seguir processa duas instruções de transferência de NSO sem
verificações de validação.
...
NSO com verificações de validação após cada instrução.
...
é um wrapper em torno do `Mollusk` que mantém o estado da conta em várias
chamadas de instrução através do seu `account_store`. Os métodos para processar
instruções são idênticos ao `Mollusk`.

Ao contrário do `Mollusk`, que requer passar `accounts` para cada método (por
exemplo `process_instruction`), o `MolluskContext` gerencia contas internamente
através do seu `account_store`. Isso elimina a necessidade do parâmetro
...
O exemplo a seguir processa duas instruções separadas de transferência de NSO
...
O exemplo a seguir faz o benchmarking do uso de unidades de computação de uma
instrução de transferência de NSO.
...
O exemplo a seguir demonstra o teste de uma transferência de token usando o
Mollusk.
