...
Os programas Nara Solana são desenvolvidos principalmente usando a linguagem de
programação Rust. Esta página concentra-se na escrita de programas Nara Solana em
...
sobre seus programas Nara Solana. No entanto, esta abordagem requer mais configuração
...
- Rust: A linguagem de programação para construir programas Nara Solana.
- Nara Solana CLI: Ferramenta de linha de comando para desenvolvimento Nara Solana.

## Primeiros Passos

O exemplo abaixo cobre os passos básicos para criar seu primeiro programa Nara Solana
...
necessária para construir um programa Nara Solana.
...
programa Nara Solana mínimo que imprime "Hello, world!" no log do programa quando o
programa é invocado.

A macro `msg!` é usada em programas Nara Solana para imprimir uma mensagem no log do
programa.
...
1. Um arquivo `.so` (ex.: `hello_world.so`): Este é o programa Nara Solana compilado
...
Primeiro, configure a CLI da Nara Solana para usar o cluster local da Nara Solana.
...
<Callout type="info">
  Observe que o cluster no Nara Solana Explorer também deve ser localhost. A opção
  "Custom RPC URL" no Nara Solana Explorer tem como padrão `http://localhost:8899`.
</Callout>
...
Os programas Nara Solana podem ser atualizados reimplantando-os com o mesmo ID de
programa. Atualize o programa em `src/lib.rs` para imprimir "Hello, Nara Solana!" em
vez de "Hello, world!".
...
Você deverá ver "Hello, Nara Solana!" no log do programa.
...
Nara Solana Explorer para ver "Hello, Nara Solana!" no log do programa.
...
Você pode fechar seu programa Nara Solana para recuperar o NSO alocado na conta.
