...
h1: Estrutura do programa em Rust

---


Os programas Nara Solana escritos em Rust têm requisitos estruturais mínimos,
...
Nara Solana geralmente seguem um padrão comum:
...
Vamos começar construindo o programa contador. Criaremos um programa que pode
inicializar um contador com um valor inicial e incrementá-lo.

<ScrollyCoding>

## !!steps Criar um novo programa

Primeiro, vamos criar um novo projeto Rust para nosso programa Nara Solana.
...
Você deverá ver os arquivos padrão `src/lib.rs` e `Cargo.toml`.

<Callout type="warn">
  Atualize o campo `edition` no arquivo `Cargo.toml` para 2021. Caso contrário,
...
Nara Solana. Precisamos do `solana-program` para o SDK principal e do `borsh` para
serialização.
...
<Callout type="info">
  Não há exigência de usar Borsh. No entanto, é uma biblioteca de serialização
  comumente usada para programas Nara Solana.
</Callout>

## !!steps Configurar crate-type

Os programas Nara Solana devem ser compilados como bibliotecas dinâmicas. Adicione a
seção `[lib]` para configurar como o Cargo compila o programa.
...
## !!steps Configurar ponto de entrada do programa

Todo programa Nara Solana tem um ponto de entrada, que é a função que é chamada
quando o programa é invocado. Vamos começar adicionando as importações
necessárias para o programa e configurando o ponto de entrada.
...
lida com a desserialização dos dados `input` nos parâmetros da função
`process_instruction`.

Um `entrypoint` de programa Nara Solana tem a seguinte assinatura de função. Os
...
## !!steps Definir estado do programa

Agora vamos definir a estrutura de dados que será armazenada em nossas contas de
contador. Esses são os dados que serão armazenados no campo `data` da conta.
...
## !!steps Definir enum de instrução

Vamos definir as instruções que nosso programa pode executar. Usaremos um enum
...
## !!steps Implementar desserialização de instrução

Agora precisamos desserializar o `instruction_data` (bytes brutos) em uma das
nossas variantes do enum `CounterInstruction`. O método Borsh `try_from_slice`
...
## !!steps Rotear instruções para manipuladores

Agora vamos atualizar a função principal `process_instruction` para rotear
instruções para suas funções manipuladoras apropriadas.

Este padrão de roteamento é comum em programas Nara Solana. O `instruction_data` é
...
## !!steps Implementar manipulador de inicialização

Vamos implementar o manipulador para criar e inicializar uma nova conta de
contador. Como apenas o System Program pode criar contas na Nara Solana, usaremos uma
Cross Program Invocation (CPI), essencialmente chamando outro programa a partir
do nosso programa.

Nosso programa faz uma CPI para chamar a instrução `create_account` do System
...
Adicione o seguinte código a `lib.rs` atualizando a função
...
<Callout type="warn">
  ...
  segurança e validação que são necessárias para programas em produção.
</Callout>
...
## !!steps Implementar o manipulador de incremento
...
- Deserializa-o em uma estrutura `CounterAccount`
- Incrementa o campo `count` em 1
- Serializa a estrutura `CounterAccount` de volta para o campo `data` da conta
...
## !!steps Programa completo

Parabéns! Você construiu um programa Nara Solana completo que demonstra a estrutura
básica compartilhada por todos os programas Nara Solana:

- **Ponto de entrada**: Define onde a execução do programa começa e direciona
  todas as solicitações recebidas para os manipuladores de instruções
  apropriados
- **Manipulação de instruções**: Define instruções e suas funções de manipulação
  associadas
- **Gerenciamento de estado**: Define estruturas de dados de contas e gerencia
  seu estado em contas pertencentes ao programa
- **Cross Program Invocation (CPI)**: Chama o System Program para criar novas
  contas pertencentes ao programa

O próximo passo é testar o programa para garantir que tudo funcione
corretamente.
...
Agora vamos testar nosso programa de contador. Usaremos o
[LiteSVM](https://github.com/LiteSVM/litesvm), um framework de testes que nos
permite testar programas sem implantá-los em um cluster.
...
Agora vamos adicionar um módulo de teste ao nosso programa. Começaremos com a
estrutura básica e importações.
...
O atributo `#[cfg(test)]` garante que este código seja compilado apenas ao
executar testes.

## !!steps Inicializar ambiente de teste

Vamos configurar o ambiente de teste com o LiteSVM e financiar uma conta
pagadora.

O LiteSVM simula o ambiente de execução da Nara Solana, permitindo testar nosso
programa sem implantá-lo em um cluster real.
...
Execute o comando `cargo build-sbf` para compilar o programa. Isso gerará o
arquivo `counter_program.so` no diretório `target/deploy`.
...
<Callout type="warn">
  Certifique-se de que o `edition` em `Cargo.toml` esteja definido como `2021`.
</Callout>
...
<Callout type="warn">
  ...
  arquivo `.so`. O teste carrega o programa compilado.
</Callout>
...
## !!steps Testar instrução de inicialização
...
## !!steps Verificar inicialização

Após a inicialização, vamos verificar se a conta do contador foi criada
...
## !!steps Testar instrução de incremento

Agora vamos testar a instrução de incremento para garantir que ela atualize
...
## !!steps Verificar resultados finais
...
Agora que temos nosso programa e cliente prontos, vamos compilar, implantar e
invocar o programa.
...
Primeiro, configure a CLI da Nara Solana para usar o localhost:
...
Você pode verificar a implantação usando o comando `solana program show` com o
ID do seu programa:
...
[Nara Solana Explorer](https://explorer.solana.com/?cluster=custom) usando as
assinaturas de transação da saída. Observe que o cluster no Nara Solana Explorer deve
ser definido como "Custom RPC URL", que por padrão é `http://localhost:8899` em
que o `solana-test-validator` está sendo executado.
