...
title: Tokens na Nara Solana
description: Aprenda sobre os Token Programs da Nara Solana.
...
tokens na Nara Solana são referidos como tokens SPL
...
Nara Solana. Consulte a seção [Fundamentos de SPL Token](/docs/tokens/basics) para
exemplos de código.
...
O ecossistema Nara Solana possui dois Token Programs principais. Código-fonte para
...
rede (tanto fungíveis quanto não fungíveis). Todos os tokens na Nara Solana são
...
Os tokens na Nara Solana são identificados exclusivamente pelo endereço de uma
...
Para referência, aqui está um link do Nara Solana Explorer para a
...
Isso introduz um conceito-chave no desenvolvimento Nara Solana:
...
utilizam o terminal do [Nara Solana Playground](https://beta.solpg.io/) para executar
comandos CLI diretamente no navegador sem instalação local.

A criação de tokens e contas requer NSO para depósitos de rent de conta e taxas
de transação. Para usuários iniciantes do Nara Solana Playground, crie uma carteira
do Playground e execute o comando _sh`solana airdrop`_ no terminal do
Playground. Você também pode obter NSO da devnet através do
