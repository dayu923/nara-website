...
title: Extensões de metadados e ponteiro de metadados
description:
  Aprenda como habilitar a MetadataPointerExtension e a extensão TokenMetadata.

---


## Como habilitar a MetadataPointerExtension e a extensão TokenMetadata
...
permite que uma mint account especifique o endereço de sua conta de metadados.
Este ponteiro pode referenciar qualquer conta pertencente a um programa que
implementa a
[Interface de Metadados de Token](https://github.com/solana-program/token-metadata/tree/main),
proporcionando flexibilidade em relação a onde os metadados são armazenados.

O Token Extensions Program implementa a
[Interface de Metadados de Token](https://github.com/solana-program/token-metadata/blob/main/interface/src/instruction.rs)
diretamente, permitindo que os metadados sejam armazenados na própria mint
account. Isso inclui o nome do token, símbolo, URI e dados descritivos
adicionais. Quando os ponteiros de extensões apontam para a própria mint, as
extensões
...
podem ser usadas para armazenar todos os metadados na mint account.
