...
description: Aprenda como depositar tokens para o estado confidencial.
...
<Callout type="warn">
  ...
  de transferências confidenciais não está disponível no momento. Embora os
  conceitos ainda sejam válidos, os exemplos de código não funcionarão.
</Callout>

## Como depositar tokens para o saldo confidencial pendente
...
1. **Saldo Confidencial Pendente**: Inicialmente, os tokens são "depositados" do
   saldo público para um saldo confidencial "pendente".
2. **Saldo Confidencial Disponível**: O saldo pendente é então "aplicado" ao
   saldo disponível, tornando os tokens disponíveis para transferências
   confidenciais.

Esta seção explica a primeira etapa: depositar o saldo público do token para o
saldo confidencial pendente.

O diagrama a seguir mostra os passos envolvidos no depósito de tokens do saldo
público para o saldo confidencial pendente:
...
Para converter um saldo público em um saldo confidencial pendente, invoque a
instrução
...
A
[quantidade máxima](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/mod.rs#L21)
por instrução de depósito é limitada a 2^48.

O crate `spl_token_client` fornece um método `confidential_transfer_deposit` que
constrói e envia uma transação com a instrução `Deposit`, como demonstrado no
exemplo abaixo.

### Código de exemplo

O exemplo a seguir demonstra como depositar o saldo público do token para o
saldo confidencial pendente.
