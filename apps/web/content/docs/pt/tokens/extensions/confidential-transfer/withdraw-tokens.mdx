...
description: Aprenda como retirar tokens do estado confidencial.
...
Para retirar tokens do saldo confidencial disponível para o saldo público:

1. Crie
   [duas provas](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/confidential-transfer/proof-generation/src/withdraw.rs#L22)
   do lado do cliente:
   ...
   **Prova de Igualdade
   ...
   Verifica se o texto cifrado do saldo disponível restante após a retirada
   corresponde ao seu
   [compromisso de Pedersen](https://en.wikipedia.org/wiki/Commitment_scheme)
   correspondente, garantindo que o novo saldo disponível da conta seja
   ...
   **Prova de Intervalo
   ...
   Verifica se o saldo disponível restante após a retirada é não-negativo e está
...
3. Invoque a instrução
   ...
   fornecendo as duas contas de prova.

4. Feche as contas de prova para recuperar o NSO usado para criá-las.

O diagrama a seguir mostra as etapas envolvidas na retirada de tokens do saldo
confidencial disponível para o saldo público:
...
Para retirar tokens do saldo confidencial disponível para o saldo público, você
deve:

- Gerar uma prova de igualdade e prova de intervalo no lado do cliente
- Invocar o programa de prova Zk ElGamal para verificar as provas e inicializar
  as contas de "estado de contexto"
- Invocar a instrução
  [ConfidentialTransferInstruction::Withdraw](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L493)
  fornecendo as duas contas de prova.
- Fechar as duas contas de prova para recuperar o rent.
...
- Método `confidential_transfer_create_context_state_account` que cria uma conta
  de prova.
- Método `confidential_transfer_withdraw` que invoca a instrução `Withdraw`.
- Método `confidential_transfer_close_context_state_account` que fecha uma conta
  de prova.

### Código de exemplo

O exemplo a seguir demonstra como retirar tokens do saldo confidencial
disponível para o saldo público.
...
Nara Solana instalada para iniciar o validador local.
