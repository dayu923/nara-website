...
description: Aprenda como aplicar o saldo pendente para disponibilizar fundos.

---


## Como aplicar o saldo pendente ao saldo disponível

Antes que os tokens possam ser transferidos confidencialmente, o saldo público
do token deve ser convertido em um saldo confidencial. Esta conversão acontece
em duas etapas:

1. **Saldo pendente confidencial**: Inicialmente, os tokens são "depositados" do
   saldo público para um saldo confidencial "pendente".
2. **Saldo disponível confidencial**: O saldo pendente é então "aplicado" ao
   saldo disponível, tornando os tokens disponíveis para transferências
   confidenciais.

Esta seção explica a segunda etapa: aplicar o saldo pendente ao saldo
disponível.

Quando os tokens são "depositados" do saldo público ou quando os tokens são
transferidos confidencialmente de uma token account para outra, os tokens são
inicialmente adicionados ao saldo pendente confidencial. Antes que os tokens
possam ser usados para transferências confidenciais, o saldo pendente deve ser
"aplicado" ao saldo disponível.
...
O diagrama a seguir mostra as etapas envolvidas na aplicação do saldo pendente
ao saldo disponível:
...
Para converter um saldo pendente em um saldo disponível, invoque a instrução
...
O crate `spl_token_client` fornece um método
`confidential_transfer_apply_pending_balance` que constrói e envia uma transação
com a instrução `ApplyPendingBalance`, como demonstrado no exemplo abaixo.

### Código de exemplo

O exemplo a seguir demonstra como aplicar o saldo pendente confidencial ao saldo
disponível confidencial.
...
Nara Solana instalada para iniciar o validator local.
