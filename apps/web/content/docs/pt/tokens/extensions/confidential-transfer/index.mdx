...
As transferências confidenciais permitem que você transfira tokens entre contas
de token sem revelar o valor da transferência. Isso é útil para transações que
preservam a privacidade. Apenas os valores de transferência e saldos de tokens
são privados. Os endereços das contas de token permanecem públicos.
...
O fluxo básico de transferências confidenciais de tokens é o seguinte:

1. Criar uma mint account com a extensão de transferência confidencial.
2. Criar token accounts com extensão de transferência confidencial para o
   remetente e destinatário.
3. Cunhar tokens para a conta do remetente.
4. **Depositar** o saldo público do remetente para o **saldo pendente
   confidencial**.
5. **Aplicar** o saldo pendente do remetente para o **saldo disponível
   confidencial**.
6. **Transferir** confidencialmente tokens da token account do remetente para a
   token account do destinatário.
7. **Aplicar** o saldo pendente do destinatário para o **saldo disponível
   confidencial**.
8. **Retirar** o saldo disponível confidencial do destinatário para o **saldo
   público**.

Para mais detalhes sobre as etapas no fluxo de transferência confidencial, veja
...
<Cards>
  ...
  >
    Como criar uma mint account com a extensão de Transferência Confidencial
  </Card>
  <Card
  ...
  >
    Como retirar tokens do saldo confidencial disponível
  </Card>
  <Card
...
O exemplo em Rust abaixo mostra os passos para transferências confidenciais de
tokens correspondentes ao diagrama anterior. Os passos estão separados em
...
Nara Solana instalada para usar o validator local.
...
| Instrução                           | Descrição                                                                                                                                                               |
| ----------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | Configura a mint account para transferências confidenciais. Esta instrução deve ser incluída na mesma transação que a instrução _rs`TokenInstruction::InitializeMint`_. |
| _rs`UpdateMint`_                    | Atualiza as configurações de transferência confidencial para uma mint.                                                                                                  |
| _rs`ConfigureAccount`_              | Configura uma token account para transferências confidenciais.                                                                                                          |
| _rs`ApproveAccount`_                | Aprova uma token account para transferências confidenciais se a mint exigir aprovação para novas token accounts.                                                        |
| _rs`EmptyAccount`_                  | Esvazia os saldos confidenciais pendentes e disponíveis para permitir o fechamento de uma token account.                                                                |
...
| _rs`EnableConfidentialCredits`_     | Permite que uma token account receba transferências confidenciais de tokens.                                                                                            |
| _rs`DisableConfidentialCredits`_    | Bloqueia transferências confidenciais de entrada, permitindo ainda transferências públicas.                                                                             |
| _rs`EnableNonConfidentialCredits`_  | Permite que uma token account receba transferências públicas de tokens.                                                                                                 |
| _rs`DisableNonConfidentialCredits`_ | Bloqueia transferências regulares para fazer com que a conta receba apenas transferências confidenciais.                                                                |
...
| _rs`DisableNonConfidentialCredits`_ | Bloqueia transferências regulares para fazer com que a conta receba apenas transferências confidenciais.                                                                |
| _rs`TransferWithFee`_               | Transfere tokens entre token accounts de forma confidencial com uma taxa.                                                                                               |
| _rs`ConfigureAccountWithRegistry`_  | Forma alternativa de configurar token accounts para transferências confidenciais usando uma conta _rs`ElGamalRegistry`_ em vez de prova _rs`VerifyPubkeyValidity`_.     |
