...
que você pode adicionar a um mint account ou token account. Você pode encontrar
a implementação dessas instruções de extensão no
...
durante a criação do mint ou da token account. Ao inicializar qualquer uma das
contas, você pode habilitar extensões específicas simultaneamente para
diferentes funcionalidades. A maioria das extensões não pode ser adicionada após
a inicialização de uma conta. Esta é uma consideração importante ao projetar seu
...
<Callout type="info">
  ...
  simultaneamente no mesmo mint account ou token account. Por exemplo, você não
  pode combinar a extensão _rs`NonTransferable`_ com a extensão
...
que lista todas as extensões disponíveis que você pode adicionar a um mint
account ou token account. Cada variante representa uma extensão diferente com
...
mint ou token account. Todo estado específico de extensão é armazenado no campo
...
que segue o tipo de dados da conta base. Você deve desserializar ainda mais o
