...
title: Fechar conta de token
description: Aprenda como fechar contas de token.
...
fecha permanentemente uma token account e transfere todo o NSO restante (rent)
para uma conta de destino especificada. O saldo da token account deve ser zero
antes de fechá-la. Apenas o proprietário da token account ou a autoridade de
fechamento designada pode executar esta instrução.
