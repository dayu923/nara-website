...
title: Sync Native
description: Aprenda como converter NSO nativo para NSO empacotado.
...
NSO empacotado (WSOL) é uma conta de token que rastreia a quantidade de lamports
na conta como o saldo do token. O WSOL permite integração com protocolos DeFi
para transferir NSO como um token SPL.
...
sincroniza o saldo de uma conta de token NSO empacotado (WSOL) com o NSO real
(lamports) armazenado nela. Quando você transfere NSO nativo para uma conta de
