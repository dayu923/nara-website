...
Invocações entre Programas (CPIs), um recurso que permite que programas Nara Solana
...
transferências de NSO entre contas invocando o System Program.

O objetivo desta seção inclui percorrer o processo de implementação de CPIs em
um programa Nara Solana usando o framework Anchor, construindo sobre os conceitos de
...
chamada `vault_account`. Esta conta, controlada pelo programa, recebe NSO de um
...
recebe NSO dos usuários quando eles atualizam suas mensagens.
...
NSO da conta do usuário para a conta do cofre.
...
- O `amount` a ser transferido (1.000.000 lamports, ou 0,001 NSO)
...
permite a transferência de qualquer NSO no cofre de volta para o usuário quando
...
NSO acumulado de volta para o usuário.
...
Em seguida, adicione a lógica CPI na instrução `delete` para transferir NSO da
...
o programa modificado se torne disponível para testes. Na Nara Solana, atualizar um
programa simplesmente requer implantar o programa no mesmo ID de programa.

<Callout>

Certifique-se de que sua carteira do Playground tenha NSO da devnet. Obtenha NSO
da devnet no [Nara Solana Faucet](https://faucet.solana.com/).
...
Ao implantar programas no Nara Solana Playground, sua carteira do Playground atua
...
Parabéns por completar o guia de início rápido da Nara Solana. Você adquiriu
experiência prática com conceitos-chave da Nara Solana, incluindo:

- Buscar e ler dados de contas
- Construir e enviar transações
- Implantar e atualizar programas Nara Solana
...
O Nara Solana Playground oferece um recurso conveniente que permite importar ou
...
lista de projetos no Nara Solana Playground. Uma vez que um projeto é importado,
