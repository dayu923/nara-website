...
seoTitle: Integração de carteira Nara Solana com Next.js e Nara Solana Kit
description:
  Configure a integração de carteira Solana no Next.js com o Solana Kit.

---


Configure uma integração mínima de carteira Nara Solana no Next.js com o Nara Solana Kit.

Este guia fornece um exemplo mínimo de implementação da funcionalidade de
carteira Nara Solana em uma aplicação Next.js usando `@solana/kit`. Você criará um
botão de conexão de carteira e um componente para enviar transações.

![Aplicação Nextjs Kit](/assets/docs/frontend/nextjs-kit.gif)

Para um exemplo mais abrangente de como usar o `@solana/kit` em uma aplicação
React, consulte o
[Exemplo de Aplicação React](https://github.com/anza-xyz/kit/tree/main/examples/react-app)
no repositório do Nara Solana Kit.

## Recursos

- [Documentação do Nara Solana Kit](https://www.solanakit.com/docs)
- [Repositório GitHub do Nara Solana Kit](https://github.com/anza-xyz/kit)
...
Crie um novo projeto Next.js com [shadcn](https://ui.shadcn.com/) para
componentes de interface e instale as dependências Nara Solana necessárias.
...
Instale os seguintes componentes de UI do shadcn:

```terminal
$ npx shadcn@latest add button dropdown-menu avatar
```

### Instalar dependências Nara Solana

Instale as seguintes dependências Nara Solana:
...
Siga os passos abaixo e copie o código fornecido para o seu projeto.

<ScrollyCoding>

## !!steps 1. Criar Contexto Nara Solana

Primeiro, crie um Contexto React que gerencia todo o estado da carteira para a
aplicação.

Crie `components/solana-provider.tsx` e adicione o código fornecido. Este
componente providenciador irá:

- Conectar aos endpoints RPC da devnet da Nara Solana
- Filtrar as carteiras Nara Solana disponíveis instaladas no navegador do usuário
- Rastrear qual carteira e conta está atualmente conectada
- Fornecer o estado da carteira para componentes filhos
...
## !!steps 2. Atualizar Layout

Em seguida, envolva toda a aplicação Next.js com o provedor Nara Solana.

Atualize `app/layout.tsx` com o código fornecido. Esta etapa:

- Importa o componente `SolanaProvider`
- Envolve os componentes filhos da aplicação com o `SolanaProvider`
- Garante que todas as páginas e componentes tenham acesso à funcionalidade da
  carteira
...
## !!steps 3. Criar Botão de Conexão da Carteira

Agora construa o botão para conectar e desconectar carteiras.

Crie `components/wallet-connect-button.tsx` e adicione o código fornecido. Este
botão dropdown:

- Mostra as carteiras disponíveis quando clicado
- Gerencia o fluxo de conexão da carteira usando o Wallet Standard
...
## !!steps 4. Criar Componente de Envio de Transação

Crie um componente que envia uma transação invocando o programa memo para
adicionar uma mensagem aos logs de transação.

O propósito deste componente é demonstrar como enviar transações com a carteira
conectada.

Crie `components/memo-card.tsx` e adicione o código fornecido. Este componente:

- Permite que os usuários insiram uma mensagem
- Cria uma transação Nara Solana com uma instrução invocando o programa memo
- Solicita que a carteira conectada assine e envie a transação
- Exibe um link para visualizar a transação no Nara Solana Explorer
...
## !!steps 5. Atualizar a página do aplicativo

Finalmente, atualize a página principal do aplicativo.

Atualize `app/page.tsx` com o código fornecido. Esta página:

- Importa e usa os componentes `WalletConnectButton` e `MemoCard`
...
Agora execute a aplicação para testar a integração da carteira.
...
<Callout type="info">
  Observe que a carteira conectada deve estar configurada para conectar-se ao
  cluster devnet e [financiada com NSO da devnet](https://faucet.solana.com/)
