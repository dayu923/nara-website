...
title: Instruksi
description:
  Pelajari tentang instruksi Solana—blok bangunan fundamental untuk berinteraksi
  dengan blockchain Solana. Pahami komposisi instruksi dengan contoh praktis.

---


Instruksi adalah blok bangunan fundamental untuk berinteraksi dengan blockchain
Nara Solana. Instruksi pada dasarnya adalah fungsi publik yang dapat dipanggil oleh
siapa saja yang menggunakan jaringan Nara Solana. Setiap instruksi digunakan untuk
melakukan tindakan tertentu. Logika eksekusi untuk instruksi disimpan pada
[program](/docs/core/programs), di mana setiap program mendefinisikan
serangkaian instruksinya sendiri. Untuk berinteraksi dengan jaringan Nara Solana,
satu atau lebih instruksi ditambahkan ke [transaksi](/docs/core/transactions)
dan dikirim ke jaringan untuk diproses.

## Contoh transfer NSO

Diagram di bawah ini menunjukkan bagaimana transaksi dan instruksi bekerja
bersama untuk memungkinkan pengguna berinteraksi dengan jaringan. Dalam contoh
ini, NSO ditransfer dari satu akun ke akun lainnya.

[Metadata](#account-metadata) akun pengirim menunjukkan bahwa akun tersebut
harus menandatangani transaksi. (Ini memungkinkan System Program untuk
mengurangi [lamport](/docs/references/terminology#lamport).) Baik akun pengirim
maupun penerima harus dapat ditulis, agar saldo lamport mereka dapat berubah.
Untuk mengeksekusi instruksi ini, dompet pengirim mengirimkan transaksi yang
berisi [tanda tangan](/docs/references/terminology#signature) dan pesan yang
berisi instruksi transfer NSO.

![Diagram transfer NSO](/assets/docs/core/transactions/sol-transfer.svg)

Setelah transaksi dikirim, System Program memproses instruksi transfer dan
memperbarui saldo lamport kedua akun.

![Diagram proses transfer NSO](/assets/docs/core/transactions/sol-transfer-process.svg)

Contoh di bawah ini menunjukkan kode yang relevan dengan diagram di atas. (Lihat
[instruksi](https://github.com/anza-xyz/agave/blob/v2.1.13/programs/system/src/system_processor.rs#L183)
...
![Diagram yang menggambarkan transaksi dengan instruksi, dibagi menjadi 3 komponen](/assets/docs/core/transactions/instruction.svg)
...
- [`program_id`](mention:program-id): [ID](#program-id) dari program yang
  dipanggil.
- [`accounts`](mention:accounts): Array dari [metadata akun](#account-metadata)
- [`data`](mention:instruction-data): Array byte dengan [data] tambahan yang
  akan digunakan oleh instruksi.
...
[`program_id`](/docs/references/terminology#program-id) dari instruksi adalah
...
Array `accounts` dari instruksi adalah array dari struktur
...
`accounts` dari instruksi berisi metadata untuk dua akun.

![Transaksi dengan satu instruksi. Instruksi tersebut berisi dua struktur `AccountMeta` dalam array `accounts`.](/assets/docs/core/transactions/accountmeta.svg)

Metadata akun mencakup informasi berikut:

- [pubkey](mention:pubkey): Alamat pubkey dari akun
...
`data` dari instruksi adalah array byte yang menentukan instruksi mana dari
program yang akan dipanggil. Ini juga mencakup argumen apa pun yang diperlukan
oleh instruksi.

## Contoh pembuatan instruksi

Contoh di bawah ini menunjukkan struktur instruksi transfer NSO.
...
informasi yang sama yang diperlukan: [`program_id`](mention:program-id),
...
<Callout type="info">
  Dalam praktiknya, biasanya Anda tidak perlu membuat `Instruction` secara
