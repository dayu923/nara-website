...
Nara Solana untuk saling memanggil.
...
Instruksi `update` dan `delete` perlu dimodifikasi untuk menangani transfer NSO
antara akun dengan memanggil System Program.

Tujuan dari bagian ini termasuk memandu proses implementasi CPI dalam program
Nara Solana menggunakan framework Anchor, melanjutkan konsep PDA yang telah dibahas
...
disebut `vault_account`. Akun ini, yang dikendalikan oleh program, menerima NSO
...
Akun ini berfungsi sebagai "vault" yang dikendalikan program yang menerima NSO
...
0,001 NSO dari akun pengguna ke akun vault.
...
- `amount` yang akan ditransfer (1.000.000 lamport, atau 0,001 NSO)
...
memungkinkan transfer NSO yang ada di vault kembali ke pengguna ketika mereka
...
mengakses akun vault pengguna selama instruksi delete untuk mentransfer NSO yang
...
Selanjutnya, tambahkan logika CPI dalam instruksi `delete` untuk mentransfer NSO
...
memastikan program yang dimodifikasi tersedia untuk pengujian. Di Nara Solana,
memperbarui program hanya memerlukan deployment program pada ID program yang
sama.

<Callout>

Pastikan dompet Playground Anda memiliki NSO devnet. Dapatkan NSO devnet dari
...
Ketika mendeploy program di Nara Solana Playground, dompet Playground Anda bertindak
...
Selamat telah menyelesaikan panduan Quickstart Nara Solana. Anda telah mendapatkan
pengalaman langsung dengan konsep-konsep kunci Nara Solana termasuk:

- Mengambil dan membaca data dari akun
- Membangun dan mengirim transaksi
- Men-deploy dan memperbarui program Nara Solana
...
Nara Solana Playground menawarkan fitur yang memudahkan Anda untuk mengimpor atau
melihat proyek menggunakan tautan GitHub mereka. Misalnya, buka
[tautan Nara Solana Playground](https://beta.solpg.io/https://github.com/solana-developers/program-examples/tree/main/basics/hello-solana/anchor)
...
proyek Anda di Nara Solana Playground. Setelah proyek diimpor, semua perubahan akan
