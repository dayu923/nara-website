...
Program Nara Solana utamanya dikembangkan menggunakan bahasa pemrograman Rust.
Halaman ini berfokus pada penulisan program Nara Solana dalam Rust tanpa menggunakan
...
Nara Solana mereka. Namun, pendekatan ini memerlukan lebih banyak pengaturan manual
...
- Rust: Bahasa pemrograman untuk membangun program Nara Solana.
- Nara Solana CLI: Alat command-line untuk pengembangan Nara Solana.

## Memulai

Contoh di bawah ini mencakup langkah-langkah dasar untuk membuat program Nara Solana
...
minimum yang diperlukan untuk membangun program Nara Solana.
...
Nara Solana minimal yang mencetak "Hello, world!" ke log program ketika program
dijalankan.

Makro `msg!` digunakan dalam program Nara Solana untuk mencetak pesan ke log program.
...
1. File `.so` (misalnya, `hello_world.so`): Ini adalah program Nara Solana yang
...
Pertama, konfigurasikan Nara Solana CLI untuk menggunakan cluster Nara Solana lokal.
...
<Callout type="info">
  Perhatikan bahwa cluster pada Nara Solana Explorer juga harus localhost. Opsi
  "Custom RPC URL" pada Nara Solana Explorer secara default adalah
  `http://localhost:8899`.
</Callout>
...
Program Nara Solana dapat diperbarui dengan men-deploy ulang ke ID program yang sama.
Perbarui program di `src/lib.rs` untuk mencetak "Hello, Nara Solana!" alih-alih
"Hello, world!".
...
Anda akan melihat "Hello, Nara Solana!" di log program.
...
Jalankan kode klien lagi dan periksa tanda tangan transaksi di Nara Solana Explorer
untuk melihat "Hello, Nara Solana!" di log program.
...
Anda dapat menutup program Nara Solana untuk mendapatkan kembali NSO yang
