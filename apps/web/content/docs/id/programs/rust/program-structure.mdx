...
Program Nara Solana yang ditulis dalam Rust memiliki persyaratan struktural minimal,
...
Meskipun tidak ada aturan ketat untuk struktur file, program Nara Solana biasanya
mengikuti pola umum:
...
Mari mulai dengan membangun program counter. Kita akan membuat program yang
dapat menginisialisasi counter dengan nilai awal dan menambahkannya.

<ScrollyCoding>

## !!steps Membuat program baru

Pertama, mari buat proyek Rust baru untuk program Nara Solana kita.
...
<Callout type="warn">
  Perbarui kolom `edition` di `Cargo.toml` menjadi 2021. Jika tidak, Anda
...
Nara Solana. Kita membutuhkan `solana-program` untuk SDK inti dan `borsh` untuk
serialisasi.
...
<Callout type="info">
  Tidak ada keharusan untuk menggunakan Borsh. Namun, ini adalah pustaka
  serialisasi yang umum digunakan untuk program Nara Solana.
</Callout>

## !!steps Konfigurasi crate-type

Program Nara Solana harus dikompilasi sebagai pustaka dinamis. Tambahkan bagian
`[lib]` untuk mengonfigurasi cara Cargo membangun program.
...
## !!steps Siapkan entrypoint program

Setiap program Nara Solana memiliki entrypoint, yaitu fungsi yang dipanggil saat
program dijalankan. Mari mulai dengan menambahkan impor yang kita perlukan untuk
program dan menyiapkan entrypoint.
...
menangani deserialisasi data `input` menjadi parameter dari fungsi
`process_instruction`.

Sebuah `entrypoint` program Nara Solana memiliki tanda tangan fungsi berikut.
...
## !!steps Tentukan state program

Sekarang mari tentukan struktur data yang akan disimpan dalam akun penghitung
kita. Ini adalah data yang akan disimpan di bidang `data` dari akun.
...
## !!steps Tentukan enum instruksi

Mari tentukan instruksi yang dapat dijalankan oleh program kita. Kita akan
menggunakan enum di mana setiap varian mewakili instruksi yang berbeda.
...
Sekarang kita perlu mendeserialkan `instruction_data` (byte mentah) menjadi
salah satu varian enum `CounterInstruction` kita. Metode Borsh `try_from_slice`
...
Sekarang mari perbarui fungsi utama `process_instruction` untuk mengarahkan
instruksi ke fungsi handler yang sesuai.

Pola perutean ini umum dalam program Nara Solana. `instruction_data` dideserialkan
...
## !!steps Implementasikan handler inisialisasi

Mari implementasikan handler untuk membuat dan menginisialisasi akun penghitung
baru. Karena hanya System Program yang dapat membuat akun di Nara Solana, kita akan
menggunakan Cross Program Invocation (CPI), pada dasarnya memanggil program lain
dari program kita.

Program kita membuat CPI untuk memanggil instruksi `create_account` dari System
...
<Callout type="warn">
  Instruksi ini hanya untuk tujuan demonstrasi. Instruksi ini tidak menyertakan
  pemeriksaan keamanan dan validasi yang diperlukan untuk program produksi.
</Callout>
...
Selamat! Anda telah membangun program Nara Solana lengkap yang mendemonstrasikan
struktur dasar yang dimiliki oleh semua program Nara Solana:
...
- **Manajemen State**: Mendefinisikan struktur data akun dan mengelola state
  mereka dalam akun yang dimiliki program
...
memungkinkan kita menguji program tanpa men-deploy ke cluster.
...
## !!steps Menginisialisasi lingkungan pengujian

Mari siapkan lingkungan pengujian dengan LiteSVM dan danai akun pembayar.

LiteSVM mensimulasikan lingkungan runtime Nara Solana, memungkinkan kita menguji
program tanpa perlu men-deploy ke cluster yang sebenarnya.
...
Mari menguji instruksi inisialisasi dengan membuat akun penghitung baru dengan
nilai awal.
...
## !!steps Verifikasi inisialisasi

Setelah inisialisasi, mari verifikasi bahwa akun counter telah dibuat dengan
benar dengan nilai yang diharapkan.
...
## !!steps Uji instruksi increment

Sekarang mari uji instruksi increment untuk memastikan bahwa instruksi tersebut
memperbarui nilai counter dengan benar.
...
## !!steps Verifikasi hasil akhir

Terakhir, mari verifikasi bahwa increment telah berhasil dengan memeriksa nilai
counter yang telah diperbarui.
...
Jalankan pengujian dengan perintah berikut. Flag `--nocapture` akan mencetak
...
Mari kita buat klien Rust untuk berinteraksi dengan program yang telah kita
deploy.
...
Sekarang kita telah memiliki program dan klien yang siap, mari kita build,
deploy, dan panggil programnya.
...
Pertama, konfigurasikan Nara Solana CLI untuk menggunakan localhost:
...
[Nara Solana Explorer](https://explorer.solana.com/?cluster=custom) menggunakan tanda
tangan transaksi yang dihasilkan. Perhatikan bahwa cluster pada Nara Solana Explorer
