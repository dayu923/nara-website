...
title: Mollusk
description:
  Panduan komprehensif untuk menguji program Solana menggunakan Mollusk

---


[Mollusk](https://github.com/anza-xyz/mollusk/tree/main) adalah alat pengujian
ringan untuk menguji program Nara Solana. Mollusk menyediakan antarmuka sederhana
untuk menguji instruksi program Nara Solana dalam lingkungan Nara Solana Virtual Machine
(SVM) yang diminimalkan. Semua akun pengujian harus didefinisikan secara
eksplisit, memastikan tes yang deterministik dan dapat diulang.

- [Dokumentasi Mollusk SVM](https://docs.rs/mollusk-svm/latest/mollusk_svm/)
- [Contoh Penggunaan Tes Mollusk](https://github.com/anza-xyz/mollusk/tree/0.5.0/harness/tests)
...
Contoh berikut menunjukkan pengaturan minimal untuk menguji program Nara Solana dasar
menggunakan Mollusk.
...
Contoh ini mendemonstrasikan cara menguji program Nara Solana dasar menggunakan
Mollusk. Program ini hanya mencetak "Hello, world!" ke log program saat
dipanggil.
...
Untuk menguji program Nara Solana dengan Mollusk:

1. [Buat instance `Mollusk`](mention:mollusk) - Inisialisasi Mollusk dengan
   program ID dan jalur ke program terkompilasi (file `.so`)
2. [Bangun instruksi](mention:instruction) - Buat instruksi untuk memanggil
   program
3. [Proses dan validasi](mention:process) - Proses instruksi menggunakan Mollusk
   dan validasi hasilnya
...
Ketika pengujian berjalan dengan sukses, Anda akan melihat output yang mirip
dengan berikut ini:
...
menyediakan antarmuka sederhana untuk menguji program Nara Solana. Semua bidang dapat
...
| Metode                                                                         | Deskripsi                                                                                                                    |
| ------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------- |
| [`process_instruction`](#single-instruction)                                   | Memproses instruksi dan mengembalikan hasilnya.                                                                              |
| [`process_and_validate_instruction`](#single-instruction-with-checks)          | Memproses instruksi dan melakukan serangkaian pemeriksaan pada hasilnya, panik jika ada pemeriksaan yang gagal.              |
| [`process_instruction_chain`](#multiple-instructions)                          | Memproses beberapa instruksi dan mengembalikan hasilnya.                                                                     |
| [`process_and_validate_instruction_chain`](#multiple-instructions-with-checks) | Memproses beberapa instruksi dan melakukan serangkaian pemeriksaan pada setiap hasil, panik jika ada pemeriksaan yang gagal. |
...
Contoh berikut memproses instruksi transfer NSO tanpa pemeriksaan validasi.
...
Contoh berikut memproses instruksi transfer NSO dengan pemeriksaan validasi.
...
Contoh berikut memproses dua instruksi transfer NSO tanpa pemeriksaan validasi.
...
Contoh berikut memproses rangkaian dua instruksi transfer NSO dengan pemeriksaan
validasi setelah setiap instruksi.
...
Mollusk menyediakan serangkaian
[metode pembantu](https://docs.rs/mollusk-svm-result/0.5.0/mollusk_svm_result/check/struct.Check.html)
untuk memeriksa hasil dari instruksi yang diproses.
...
Contoh berikut memproses dua instruksi transfer NSO terpisah dengan status akun
...
Contoh berikut melakukan benchmark penggunaan unit komputasi dari instruksi
transfer NSO.
