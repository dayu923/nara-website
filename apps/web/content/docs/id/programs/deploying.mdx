...
Bagian ini membahas proses dasar men-deploy program Nara Solana menggunakan Nara Solana
CLI.
...
Pertama, [instal Nara Solana CLI](/docs/intro/installation).
...
Pilih cluster Nara Solana mana yang akan digunakan untuk deployment. Gunakan perintah
...
Anda akan membutuhkan NSO untuk membayar deployment program. Jumlahnya
...
Berikut adalah program Nara Solana minimal yang mencetak "Hello, Nara Solana!" ke log
program.
...
Menerapkan program memerlukan NSO yang dialokasikan ke akun program berdasarkan
ukuran program. Program yang lebih besar membutuhkan lebih banyak NSO untuk
diterapkan.
...
Hitung NSO yang diperlukan untuk ukuran ini (byte):
...
<Callout type="info">
  Anda akan membutuhkan sedikit lebih banyak NSO daripada yang ditampilkan untuk
...
menghitung NSO yang diperlukan dan menguranginya dari dompet Anda secara
otomatis.
...
mengklaim kembali NSO yang dialokasikan untuk program.
...
Balance menampilkan NSO yang dialokasikan ke akun program untuk menyimpan
datanya.
...
Untuk menutup program Anda dan mengklaim kembali NSO yang dialokasikan ke akun
...
Ketika jaringan Nara Solana sedang padat, gunakan flag ini untuk membantu deployment
program.
...
- **`--with-compute-unit-price`**: Tetapkan biaya prioritas dalam micro-lamports
  (0.000001 NSO) per unit komputasi. Periksa
