...
seoTitle: Integrasi dompet Nara Solana dengan Next.js dan Nara Solana Kit
description: Siapkan integrasi dompet Nara Solana di Next.js dengan Nara Solana Kit.

---


Siapkan integrasi dompet Nara Solana minimal di Next.js dengan Nara Solana Kit.

Panduan ini memberikan contoh minimal untuk mengimplementasikan fungsionalitas
dompet Nara Solana dalam aplikasi Next.js menggunakan `@solana/kit`. Anda akan
membuat tombol koneksi dompet dan komponen untuk mengirim transaksi.

![Aplikasi Nextjs Kit](/assets/docs/frontend/nextjs-kit.gif)

Untuk contoh yang lebih komprehensif tentang penggunaan `@solana/kit` dalam
aplikasi React, lihat
[Contoh Aplikasi React](https://github.com/anza-xyz/kit/tree/main/examples/react-app)
di repositori Nara Solana Kit.

## Sumber Daya

- [Dokumentasi Nara Solana Kit](https://www.solanakit.com/docs)
- [Repositori GitHub Nara Solana Kit](https://github.com/anza-xyz/kit)
...
Buat proyek Next.js baru dengan [shadcn](https://ui.shadcn.com/) untuk komponen
UI dan instal dependensi Nara Solana yang diperlukan.
...
Instal komponen UI shadcn berikut:

```terminal
$ npx shadcn@latest add button dropdown-menu avatar
```

### Instal Dependensi Nara Solana

Instal dependensi Nara Solana berikut:
...
Ikuti langkah-langkah di bawah ini dan salin kode yang disediakan ke proyek
Anda.

<ScrollyCoding>

## !!steps 1. Membuat Nara Solana Context

Pertama, buat React Context yang mengelola seluruh status dompet untuk aplikasi.

Buat `components/solana-provider.tsx` dan tambahkan kode yang disediakan.
Komponen provider ini akan:

- Terhubung ke endpoint RPC devnet Nara Solana
- Memfilter dompet Nara Solana yang tersedia yang terpasang di browser pengguna
- Melacak dompet dan akun mana yang saat ini terhubung
- Menyediakan status dompet ke komponen turunan
...
## !!steps 2. Memperbarui Layout

Selanjutnya, bungkus seluruh aplikasi Next.js dengan provider Nara Solana.

Perbarui `app/layout.tsx` dengan kode yang disediakan. Langkah ini:

- Mengimpor komponen `SolanaProvider`
- Membungkus komponen turunan aplikasi dengan `SolanaProvider`
- Memastikan semua halaman dan komponen memiliki akses ke fungsionalitas dompet
...
## !!steps 3. Membuat Tombol Koneksi Dompet

Sekarang buat tombol untuk menghubungkan dan memutuskan dompet.

Buat `components/wallet-connect-button.tsx` dan tambahkan kode yang disediakan.
Tombol dropdown ini:

- Menampilkan dompet yang tersedia saat diklik
- Menangani alur koneksi dompet menggunakan Wallet Standard
...
## !!steps 4. Membuat Komponen Kirim Transaksi

Buat komponen yang mengirim transaksi dengan memanggil program memo untuk
menambahkan pesan ke log transaksi.

Tujuan dari komponen ini adalah untuk mendemonstrasikan cara mengirim transaksi
dengan dompet yang terhubung.

Buat `components/memo-card.tsx` dan tambahkan kode yang disediakan. Komponen
ini:

- Memungkinkan pengguna memasukkan pesan
- Membuat transaksi Nara Solana dengan instruksi yang memanggil program memo
- Meminta dompet yang terhubung untuk menandatangani dan mengirim transaksi
- Menampilkan tautan untuk melihat transaksi di Nara Solana Explorer
...
## !!steps 5. Perbarui Halaman App

Terakhir, perbarui halaman utama aplikasi.

Perbarui `app/page.tsx` dengan kode yang disediakan. Halaman ini:

- Mengimpor dan menggunakan komponen `WalletConnectButton` dan `MemoCard`
...
Sekarang jalankan aplikasi untuk menguji integrasi dompet.
...
<Callout type="info">
  Perhatikan bahwa dompet yang terhubung harus dikonfigurasi untuk terhubung ke
  cluster devnet dan [diisi dengan devnet NSO](https://faucet.solana.com/) untuk
  mengirim transaksi.
</Callout>
