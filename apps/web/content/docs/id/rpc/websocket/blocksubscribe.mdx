...
h1: Metode RPC blockSubscribe

---


Berlangganan untuk menerima notifikasi setiap kali blok baru `confirmed` atau
`finalized`.

<Callout type="warn" title={"Metode Tidak Stabil"}>
  Langganan ini dianggap **tidak stabil** dan hanya tersedia jika validator
  dimulai dengan flag `--rpc-pubsub-enable-block-subscription`. Format langganan
  ini mungkin berubah di masa mendatang.
</Callout>
...
kriteria filter untuk log untuk menerima hasil berdasarkan jenis akun; saat ini
yang didukung:

- `all` - menyertakan semua transaksi dalam blok
- Objek JSON dengan bidang berikut:
  - `mentionsAccountOrProgram: <string>` - hanya mengembalikan transaksi yang
    menyebutkan kunci publik yang disediakan (sebagai string yang dikodekan
    base-58). Jika tidak ada penyebutan dalam blok tertentu, maka tidak ada
...
format pengkodean untuk setiap Transaksi yang dikembalikan
...
- Jika `jsonParsed` diminta tetapi parser tidak dapat ditemukan, instruksi akan
  kembali ke pengkodean JSON reguler (bidang `accounts`, `data`, dan
  `programIdIndex`).
...
apakah akan mengisi array `rewards`. Jika parameter tidak disediakan, defaultnya
menyertakan rewards.
...
- `err: <object|null>` - Error jika terjadi kesalahan saat mempublikasikan
  notifikasi, jika tidak null.
- `block: <object|null>` - Objek blok seperti yang terlihat di metode RPC HTTP
