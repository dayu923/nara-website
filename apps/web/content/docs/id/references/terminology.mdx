...
Istilah-istilah berikut digunakan di seluruh dokumentasi Nara Solana dan ekosistem
pengembangan.

## akun

Sebuah catatan dalam buku besar Nara Solana yang menyimpan data atau merupakan
program yang dapat dieksekusi.

Seperti akun di bank tradisional, akun Nara Solana dapat menyimpan dana yang disebut
...
Aplikasi front-end yang berinteraksi dengan klaster Nara Solana.
...
Nilai unik ([hash](#hash)) yang mengidentifikasi sebuah catatan (block). Nara Solana
...
Program Nara Solana yang memiliki dan memuat
...
Program komputer yang mengakses jaringan server Nara Solana [cluster](#cluster).
...
Juga disebut sebagai `cu`, adalah perkiraan komprehensif dari semua sumber daya
yang diperlukan untuk memproses transaksi, diukur dalam
...
imbalan validasi dan untuk mendanai pengembangan berkelanjutan Nara Solana.
...
Kode yang dapat dieksekusi di blockchain Nara Solana yang menginterpretasikan
...
Runtime paralel Nara Solana untuk [program onchain](#onchain-program).
...
urutan kecil dan S adalah skalar dalam rentang `0 <= S < L`. Persyaratan ini
...
[Slot](#slot) masa lalu yang tidak menghasilkan [block](#block), karena pemimpin
...
muncul sebagai leluhur untuk block pada slot berikutnya, tidak menambah
[tinggi block](#block-height), dan tidak mengakhiri masa berlaku
...
[Native token](#native-token) dari Nara Solana [cluster](#cluster).

## Nara Solana Program Library (SPL)

[Pustaka program](https://spl.solana.com/) di Nara Solana seperti spl-token yang
...
Peserta penuh dalam jaringan [cluster](#cluster) Nara Solana yang menghasilkan
