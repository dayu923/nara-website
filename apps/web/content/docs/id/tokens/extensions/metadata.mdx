...
title: Ekstensi Metadata & Pointer Metadata
description:
  Pelajari cara mengaktifkan ekstensi MetadataPointerExtension dan
  TokenMetadata.
...
memungkinkan mint account untuk menentukan alamat akun metadata-nya. Pointer ini
dapat mereferensikan akun apa pun yang dimiliki oleh program yang
mengimplementasikan
[Token Metadata Interface](https://github.com/solana-program/token-metadata/tree/main),
memberikan fleksibilitas dalam penyimpanan metadata.
...
secara langsung, memungkinkan metadata disimpan pada mint account itu sendiri.
Ini mencakup nama token, simbol, URI, dan data deskriptif tambahan. Ketika
ekstensi pointer mengarah ke mint itu sendiri, ekstensi
...
dapat digunakan untuk menyimpan semua metadata dalam mint account.
