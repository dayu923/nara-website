...
title: Menarik Token
description: Pelajari cara menarik token dari keadaan rahasia.
...
Untuk menarik token dari saldo rahasia yang tersedia ke saldo publik:

1. Buat
   [dua bukti](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/confidential-transfer/proof-generation/src/withdraw.rs#L22)
   di sisi klien:
   ...
   **Bukti Kesetaraan
   ...
   Memverifikasi bahwa ciphertext saldo tersedia yang tersisa setelah penarikan
   cocok dengan
   [Pedersen commitment](https://en.wikipedia.org/wiki/Commitment_scheme) yang
   sesuai, memastikan saldo baru yang tersedia dari akun dihitung dengan benar
   sebagai `remaining_balance = current_balance - withdraw_amount`.
   ...
   **Bukti Rentang
   ...
   Memverifikasi bahwa saldo tersedia yang tersisa setelah penarikan tidak
   negatif dan berada dalam rentang yang ditentukan.
...
3. Panggil instruksi
   ...
   dengan menyediakan dua akun bukti.

4. Tutup akun bukti untuk mendapatkan kembali NSO yang digunakan untuk
   membuatnya.

Diagram berikut menunjukkan langkah-langkah yang terlibat dalam penarikan token
dari saldo rahasia yang tersedia ke saldo publik:
...
Untuk menarik token dari saldo rahasia yang tersedia ke saldo publik, Anda
harus:

- Menghasilkan bukti kesetaraan dan bukti rentang di sisi klien
- Memanggil program bukti Zk ElGamal untuk memverifikasi bukti dan
  menginisialisasi akun "context state"
- Memanggil instruksi
  [ConfidentialTransferInstruction::Withdraw](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L493)
  dengan menyediakan dua akun bukti.
- Menutup kedua akun bukti untuk mendapatkan kembali rent.
...
- Metode `confidential_transfer_create_context_state_account` yang membuat akun
  bukti.
- Metode `confidential_transfer_withdraw` yang memanggil instruksi `Withdraw`.
- Metode `confidential_transfer_close_context_state_account` yang menutup akun
  bukti.

### Contoh Kode

Contoh berikut mendemonstrasikan cara menarik token dari saldo rahasia yang
tersedia ke saldo publik.
