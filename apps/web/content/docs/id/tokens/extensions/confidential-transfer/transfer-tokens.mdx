...
description:
  Pelajari cara mentransfer token secara privat dari satu token account ke token
  account lainnya.
...
## Cara mentransfer token secara konfidensial dari satu token account ke token account lainnya

Untuk mentransfer token secara konfidensial dari satu token account ke token
account lainnya, baik pengirim maupun penerima harus memiliki token account yang
dikonfigurasi dengan state _rs`ConfidentialTransferAccount`_ dan disetujui untuk
transfer konfidensial. Token account pengirim juga harus memiliki saldo
konfidensial yang tersedia untuk ditransfer.

Untuk mentransfer token secara konfidensial:

1. Buat
   [tiga bukti](https://github.com/solana-program/token-2022/blob/d073cb89dbcd430387b5c6fb4b7157911351e4a3/confidential-transfer/proof-generation/src/transfer.rs#L35)
   di sisi klien:
   ...
   **Bukti Kesetaraan
   ...
   Memverifikasi bahwa ciphertext saldo baru yang tersedia setelah transfer
   cocok dengan
   [Pedersen commitment](https://en.wikipedia.org/wiki/Commitment_scheme) yang
   sesuai, memastikan saldo baru yang tersedia pada akun sumber dihitung dengan
   benar sebagai `new_balance = current_balance - transfer_amount`.

   **Bukti Validitas Ciphertext
   ...
   Memverifikasi bahwa ciphertext jumlah transfer dibuat dengan benar untuk
   ketiga pihak (sumber, tujuan, dan auditor), memastikan jumlah transfer
   dienkripsi dengan benar di bawah kunci publik masing-masing pihak.
   ...
   **Bukti Rentang
   ...
   Memverifikasi bahwa saldo baru yang tersedia dan jumlah transfer (dibagi
   menjadi bit rendah/tinggi) semuanya tidak negatif dan berada dalam rentang
   yang ditentukan.

2. Untuk setiap bukti:
   - Panggil program bukti ZK ElGamal untuk memverifikasi data bukti.
   - Simpan metadata khusus bukti dalam akun "context state" bukti untuk
     digunakan dalam instruksi lain.
...
3. Panggil instruksi
   ...
   dengan menyediakan akun context state bukti.

4. Tutup akun status konteks bukti untuk mendapatkan kembali NSO yang digunakan
   untuk membuatnya.

Diagram berikut menunjukkan langkah-langkah yang terlibat dalam mentransfer
token dari akun token pengirim ke akun token penerima.
...
Untuk mentransfer token secara rahasia dari satu akun token ke akun token
lainnya, Anda harus:

- Menghasilkan bukti kesetaraan, bukti validitas ciphertext, dan bukti rentang
  di sisi klien
- Memanggil program Zk ElGamal proof untuk memverifikasi bukti dan
  menginisialisasi akun "context state"
- Memanggil instruksi
  [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604)
  dengan menyediakan tiga akun bukti.
- Menutup ketiga akun bukti untuk mendapatkan kembali rent.

Crate `spl_token_client` menyediakan metode-metode berikut:

- Metode `confidential_transfer_create_context_state_account` yang membuat akun
  bukti.
- Metode `confidential_transfer_transfer` yang memanggil instruksi `Transfer`.
- Metode `confidential_transfer_close_context_state_account` yang menutup akun
  bukti.

### Contoh Kode

Contoh berikut menunjukkan cara mentransfer token secara rahasia antar akun
token.

Untuk menjalankan contoh ini, mulai validator lokal dengan Token Extensions
Program yang dikloning dari mainnet menggunakan perintah berikut. Anda harus
memiliki Nara Solana CLI yang terinstal untuk memulai validator lokal.
