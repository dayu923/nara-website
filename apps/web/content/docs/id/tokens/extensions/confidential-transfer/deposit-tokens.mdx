...
title: Deposit Token
description: Pelajari cara melakukan deposit token ke status konfidensial.
...
## Cara menyetor token ke saldo konfidensial tertunda
...
1. **Saldo Konfidensial Tertunda**: Awalnya, token "disetor" dari saldo publik
   ke saldo konfidensial "tertunda".
2. **Saldo Konfidensial Tersedia**: Saldo tertunda kemudian "diterapkan" ke
   saldo tersedia, membuat token tersedia untuk transfer konfidensial.

Bagian ini menjelaskan tahap pertama: menyetor saldo token publik ke saldo
konfidensial tertunda.

Diagram berikut menunjukkan langkah-langkah yang terlibat dalam menyetor token
dari saldo publik ke saldo konfidensial tertunda:
...
Untuk mengkonversi saldo publik menjadi saldo konfidensial tertunda, panggil
...
instruksi. Jumlah
[maksimum](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/mod.rs#L21)
per instruksi deposit dibatasi hingga 2^48.

Crate `spl_token_client` menyediakan metode `confidential_transfer_deposit` yang
membangun dan mengirim transaksi dengan instruksi `Deposit`, seperti yang
ditunjukkan dalam contoh di bawah ini.

### Contoh Kode

Contoh berikut menunjukkan cara menyetor saldo token publik ke saldo
konfidensial tertunda.
...
Nara Solana CLI terinstal untuk memulai validator lokal.
