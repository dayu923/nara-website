...
title: Sync Native
description: Pelajari cara mengkonversi NSO native menjadi wrapped NSO.

---


## Sinkronisasi NSO Native

Wrapped NSO (WSOL) adalah token account yang melacak jumlah lamports dalam akun
sebagai saldo token. WSOL memungkinkan integrasi dengan protokol DeFi untuk
mentransfer NSO sebagai token SPL.
...
menyinkronkan saldo token account wrapped NSO (WSOL) dengan NSO (lamports)
aktual yang disimpan di dalamnya. Ketika Anda mentransfer NSO native ke token
account WSOL, saldo token tidak otomatis diperbarui. Anda harus memanggil
SyncNative untuk menampilkan saldo WSOL yang benar.
