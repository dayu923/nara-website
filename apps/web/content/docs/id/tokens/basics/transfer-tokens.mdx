...
description: Pelajari cara mentransfer token antar akun token.

---


## Cara Mentransfer Token

Transfer token memindahkan token antar akun token dengan mint yang sama
...
- Kedua akun token harus memiliki jenis token (mint) yang sama
- Hanya pemilik akun sumber atau delegasi yang dapat mengotorisasi transfer
