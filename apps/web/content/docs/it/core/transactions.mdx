...
Per interagire con la rete Nara Solana, devi inviare una transazione. Puoi pensare a
...
<Callout>
  ...
  singolo pacchetto senza frammentazione, Nara Solana utilizza 1280 byte (l'MTU
...
[account firmatario](#account-addresses) incluso in qualsiasi istruzione della
transazione.
...
- [`header`](mention:message-header): L'[intestazione](#header) del messaggio
- [`account_keys`](mention:account-addresses): Un array di
  [indirizzi di account](#account-addresses) richiesti dalle istruzioni della
  transazione
...
- [`num_readonly_signed_accounts`](mention:num_readonly_signed_accounts): Il
  numero totale di account di sola lettura che richiedono firme
- [`num_readonly_unsigned_accounts`](mention:num_readonly_unsigned_accounts): Il
  numero totale di account di sola lettura che non richiedono firme
...
[formato array compatto](/docs/references/terminology#compact-array-format). Il
...
<Callout>
  ...
  con le informazioni nell'[`header`](#header) del messaggio per determinare i
...
<Callout>
  Il metodo RPC [`getLatestBlockhash`](/docs/rpc/http/getlatestblockhash) ti
...
2. [`accounts`](mention:account-indexes): Un array di indici che puntano agli
...
3. [`data`](mention:instruction-data): Un array di byte che specifica quale
   istruzione invocare sul programma. Include anche qualsiasi dato aggiuntivo
...
singola istruzione di trasferimento NSO.
...
<Callout>
  Puoi anche trovare la transazione utilizzando [Nara Solana
