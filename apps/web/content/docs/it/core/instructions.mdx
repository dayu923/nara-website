...
title: Istruzioni
description:
  Scopri le istruzioni di Solana—i blocchi fondamentali per interagire con la
  blockchain Solana. Comprendi la composizione delle istruzioni con esempi
  pratici.

---


Le istruzioni sono il blocco fondamentale per interagire con la blockchain
Nara Solana. Un'istruzione è essenzialmente una funzione pubblica che chiunque
utilizzi la rete Nara Solana può chiamare. Ogni istruzione viene utilizzata per
eseguire un'azione specifica. La logica di esecuzione per le istruzioni è
memorizzata nei [programmi](/docs/core/programs), dove ogni programma definisce
il proprio set di istruzioni. Per interagire con la rete Nara Solana, una o più
istruzioni vengono aggiunte a una [transazione](/docs/core/transactions) e
inviate alla rete per essere elaborate.

## Esempio di trasferimento di NSO

Il diagramma seguente mostra come le transazioni e le istruzioni lavorano
insieme per consentire agli utenti di interagire con la rete. In questo esempio,
NSO viene trasferito da un account a un altro.

I [metadati](#account-metadata) dell'account del mittente indicano che deve
firmare per la transazione. (Questo consente al System Program di detrarre
[lamport](/docs/references/terminology#lamport).) Sia l'account del mittente che
quello del destinatario devono essere scrivibili, per consentire la modifica del
loro saldo in lamport. Per eseguire questa istruzione, il wallet del mittente
invia la transazione contenente la sua
[firma](/docs/references/terminology#signature) e il messaggio contenente
l'istruzione di trasferimento NSO.

![Diagramma di trasferimento NSO](/assets/docs/core/transactions/sol-transfer.svg)
...
![Diagramma del processo di trasferimento NSO](/assets/docs/core/transactions/sol-transfer-process.svg)

L'esempio seguente mostra il codice relativo ai diagrammi sopra. (Vedi
l'istruzione di trasferimento del System Program
[instruction](https://github.com/anza-xyz/agave/blob/v2.1.13/programs/system/src/system_processor.rs#L183).)
...
![Diagramma che rappresenta una transazione con un'istruzione, suddivisa nei suoi 3 componenti](/assets/docs/core/transactions/instruction.svg)
...
- [`data`](mention:instruction-data): Un array di byte con [dati] aggiuntivi da
  utilizzare dall'istruzione.
...
L'[`program_id`](/docs/references/terminology#program-id) dell'istruzione è
...
L'array `accounts` dell'istruzione è un array di strutture
...
![Una transazione con un'istruzione. L'istruzione contiene due strutture `AccountMeta` nel suo array `accounts`.](/assets/docs/core/transactions/accountmeta.svg)
...
<Callout>
  ...
  essere scrivibili, di sola lettura o firmare la transazione, devi fare
  riferimento all'implementazione dell'istruzione, come definita dal programma.
</Callout>
...
Il `data` dell'istruzione è un array di byte che specifica quale istruzione del
programma invocare. Include anche eventuali argomenti richiesti dall'istruzione.

## Esempio di creazione di un'istruzione

L'esempio seguente mostra la struttura di un'istruzione di trasferimento di NSO.
