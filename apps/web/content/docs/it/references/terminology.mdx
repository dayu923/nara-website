...
di sviluppo di Nara Solana.

## account

Un record nel registro di Nara Solana che contiene dati o è un programma eseguibile.

Come un conto in una banca tradizionale, un account Nara Solana può contenere fondi
...
Un'applicazione front-end che interagisce con un cluster Nara Solana.
...
Un valore unico ([hash](#hash)) che identifica un record (blocco). Nara Solana
...
Il programma Nara Solana che possiede e carica
...
Un programma informatico che accede alla rete server Nara Solana [cluster](#cluster).
...
Un insieme di [validatori](#validator) che mantengono un unico
[ledger](#ledger).
...
computazionali utilizzate per elaborare una transazione.

## tempo di conferma

La durata di tempo reale tra la creazione di un [tick entry](#tick) da parte di
...
Anche definite come `cu_consumed`, è una stima completa di tutte le risorse
necessarie per elaborare una transazione, misurate in
...
Una rete multicast utilizzata per validare in modo efficiente gli
[entry](#entry) e ottenere consenso.

## drone

Un servizio off-chain che funge da custode per la chiave privata di un utente.
In genere serve per validare e firmare le transazioni.
...
Un [ledger](#ledger) derivato da entry comuni ma poi divergente.
...
Un aumento della fornitura di token nel tempo utilizzato per finanziare le
ricompense per la validazione e per finanziare lo sviluppo continuo di Nara Solana.

## istruzione interna

Vedi [invocazione cross-program](#cross-program-invocation-cpi).
...
Il ruolo di un [validatore](#validator) quando sta aggiungendo [voci](#entry) al
[registro](#ledger).

## programma dei leader

Una sequenza di [chiavi pubbliche](#public-key-pubkey) dei
[validatori](#validator) mappate agli [slot](#slot). Il cluster utilizza il
programma dei leader per determinare quale validatore è il [leader](#leader) in
qualsiasi momento.
...
[blocco genesi](#genesis-block), ma il registro di un [validatore](#validator)
effettivo potrebbe avere solo blocchi più recenti per ridurre lo spazio di
archiviazione, poiché quelli più vecchi non sono necessari per la validazione
...
[validatore](#validator) che un [blocco](#block) che ha ricevuto è stato
verificato, nonché una promessa di non votare per un [blocco](#block) in
conflitto (cioè [fork](#fork)) per un periodo di tempo specifico, il periodo di
[lockout](#lockout).
...
contiene un'intestazione, un array di indirizzi di account, un
[blockhash](#blockhash) recente e un array di [istruzioni](#instruction).
...
di entità indipendenti che possono agire collettivamente per spegnere una
...
Il codice eseguibile sulla blockchain Nara Solana che interpreta le
...
della prova precedente. Come un [VDF](#verifiable-delay-function-vdf), un Proof
...
necessarie per l'esecuzione per ridurre al minimo le commissioni.
...
sufficiente per pagare il rent, possono essere soggetti a Garbage Collection.

Vedi anche [esenzione dal rent](#rent-exempt) qui sotto. Scopri di più sul rent
qui: [Cos'è il rent?](/docs/intro/rent).
...
memorizzati permanentemente sulla catena fino a quando l'account non viene
chiuso. Non è possibile creare un account che scenda al di sotto della soglia di
esenzione dal rent.

## root

Un [block](#block) o [slot](#slot) che ha raggiunto il massimo
...
Il runtime parallelo di Nara Solana per i [programmi onchain](#onchain-program).

## shred

Una frazione di un [block](#block); l'unità più piccola inviata tra
[validator](#validator).
...
l'[account delle commissioni](#fee-account). Quindi, la prima firma nella
...
Uno [slot](#slot) passato che non ha prodotto un [block](#block), perché il
leader era offline o il [fork](#fork) contenente lo slot è stato abbandonato per
una alternativa migliore dal consenso del cluster. Uno slot saltato non apparirà
...
approssimativamente uguale secondo [PoH](#proof-of-history-poh).
...
Il [token nativo](#native-token) di un [cluster](#cluster) Nara Solana.

## Nara Solana Program Library (SPL)

Una [libreria di programmi](https://spl.solana.com/) su Nara Solana come spl-token
...
comportamento malevolo del [validatore](#validator).

## qualità del servizio ponderata per stake (SWQoS)

SWQoS consente un
[trattamento preferenziale per le transazioni che provengono da validatori con stake](/developers/guides/advanced/stake-weighted-qos).
...
Una [voce](#entry) del registro che stima la durata dell'orologio di sistema.
...
per i loro indirizzi di token mint unici.

## Token Program

Il [Token Program](https://spl.solana.com/token) ha l'ID di programma
`TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA`, e fornisce le capacità di base di
trasferimento, congelamento e conio di token.
...
possibili risultati: successo o fallimento.
...
quando il suo blocco diventa una [root](#root).
...
Un partecipante completo in una rete Nara Solana [cluster](#cluster) che produce
...
Vedi [voto del registro](#ledger-vote).
...
mentre progressivamente diventa efficace. Durante questo periodo, lo stake è
considerato "in attivazione". Maggiori informazioni su:
