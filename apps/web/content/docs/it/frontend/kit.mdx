...
seoTitle: Integrazione del wallet Nara Solana con Next.js e Nara Solana Kit
description:
  Configura l'integrazione del wallet Solana in Next.js con Solana Kit.

---


Configura un'integrazione minima del wallet Nara Solana in Next.js con Nara Solana Kit.

Questa guida fornisce un esempio minimale di implementazione della funzionalità
del wallet Nara Solana in un'applicazione Next.js utilizzando `@solana/kit`. Creerai
un pulsante per connettere il wallet e un componente per inviare transazioni.

![Applicazione Nextjs Kit](/assets/docs/frontend/nextjs-kit.gif)

Per un esempio più completo sull'utilizzo di `@solana/kit` in un'applicazione
React, consulta
l'[Esempio di App React](https://github.com/anza-xyz/kit/tree/main/examples/react-app)
nel repository di Nara Solana Kit.

## Risorse

- [Documentazione di Nara Solana Kit](https://www.solanakit.com/docs)
- [Repository GitHub di Nara Solana Kit](https://github.com/anza-xyz/kit)
...
Crea un nuovo progetto Next.js con [shadcn](https://ui.shadcn.com/) per i
componenti dell'interfaccia utente e installa le dipendenze Nara Solana necessarie.
...
### Installa le dipendenze Nara Solana

Installa le seguenti dipendenze Nara Solana:
...
## !!steps 1. Crea il contesto Nara Solana

Prima, crea un contesto React che gestisca l'intero stato del wallet per
l'applicazione.

Crea `components/solana-provider.tsx` e aggiungi il codice fornito. Questo
componente provider:

- Si connette agli endpoint RPC della devnet di Nara Solana
- Filtra i wallet Nara Solana disponibili installati nel browser dell'utente
...
Successivamente, avvolgi l'intera applicazione Next.js con il provider Nara Solana.

Aggiorna `app/layout.tsx` con il codice fornito. Questo passaggio:

- Importa il componente `SolanaProvider`
- Avvolge i componenti figli dell'applicazione con `SolanaProvider`
- Assicura che tutte le pagine e i componenti abbiano accesso alle funzionalità
  del wallet
...
## !!steps 3. Crea il pulsante di connessione del wallet

Ora costruisci il pulsante per connettere e disconnettere i wallet.

Crea `components/wallet-connect-button.tsx` e aggiungi il codice fornito. Questo
pulsante a discesa:
...
## !!steps 4. Crea il componente per l'invio di transazioni

Crea un componente che invia una transazione invocando il programma memo per
aggiungere un messaggio ai log di transazione.

Lo scopo di questo componente è dimostrare come inviare transazioni con il
wallet connesso.

Crea `components/memo-card.tsx` e aggiungi il codice fornito. Questo componente:

- Consente agli utenti di inserire un messaggio
- Crea una transazione Nara Solana con un'istruzione che invoca il programma memo
- Richiede al wallet connesso di firmare e inviare la transazione
- Mostra un link per visualizzare la transazione su Nara Solana Explorer
...
## !!steps 5. Aggiorna la pagina dell'app

Infine, aggiorna la pagina principale dell'app.

Aggiorna `app/page.tsx` con il codice fornito. Questa pagina:

- Importa e utilizza i componenti `WalletConnectButton` e `MemoCard`
...
<Callout type="info">
  Nota che il wallet connesso deve essere configurato per connettersi al cluster
  devnet e [finanziato con NSO devnet](https://faucet.solana.com/) per inviare
  transazioni.
</Callout>
