...
h1: Metodo RPC blockSubscribe

---


Sottoscrivi per ricevere notifiche ogni volta che un nuovo blocco viene
`confirmed` o `finalized`.

<Callout type="warn" title={"Metodo instabile"}>
  Questa sottoscrizione è considerata **instabile** ed è disponibile solo se il
  validator è stato avviato con il flag
  `--rpc-pubsub-enable-block-subscription`. Il formato di questa sottoscrizione
...
criteri di filtro per i log per ricevere risultati per tipo di account;
attualmente supportati:

- `all` - include tutte le transazioni nel blocco
- Un oggetto JSON con il seguente campo:
  - `mentionsAccountOrProgram: <string>` - restituisce solo le transazioni che
    menzionano la chiave pubblica fornita (come stringa codificata in base-58).
    Se non ci sono menzioni in un determinato blocco, non verrà inviata alcuna
    notifica.
...
formato di codifica per ogni transazione restituita
...
- Se viene richiesto `jsonParsed` ma non è possibile trovare un parser,
  l'istruzione ricade nella codifica JSON regolare (campi `accounts`, `data` e
  `programIdIndex`).
...
livello di dettaglio della transazione da restituire

- Se viene richiesto `accounts`, i dettagli della transazione includono solo
...
se popolare l'array `rewards`. Se il parametro non viene fornito, per
impostazione predefinita include le ricompense.
...
- `err: <object|null>` - Errore se qualcosa è andato storto durante la
  pubblicazione della notifica, altrimenti null.
- `block: <object|null>` - Un oggetto blocco come visto nel metodo RPC HTTP
