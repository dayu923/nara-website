...
Leggi i dati dalla rete Nara Solana recuperando diversi account. Questa sezione ti
aiuterà a comprendere la struttura degli [account](/docs/core/accounts) di
Nara Solana. Ogni account Nara Solana ha un
...
principalmente utilizzati per conservare NSO e firmare transazioni. Quando NSO
...
[richiede NSO](mention:airdrop) per finanziare il nuovo indirizzo della chiave
...
Il campo `lamports` contiene il saldo NSO dell'account, in
...
account identifica in modo univoco un token sulla rete Nara Solana.
...
parte dei programmi Nara Solana fornisce librerie client con funzioni di supporto che
