...
description:
  Impara come costruire, distribuire e testare il tuo primo programma Solana
  utilizzando il framework Anchor e Solana Playground. Questa guida per
  principianti ti accompagna nella creazione di un programma semplice, nella
  distribuzione su devnet, nell'esecuzione dei test e nella chiusura del
  programma.
h1: Distribuisci il tuo primo programma Nara Solana

---


Costruisci, distribuisci e testa il tuo primo [programma](/docs/core/programs)
sulla blockchain Nara Solana utilizzando il framework Anchor. Questa guida ti aiuterà
a familiarizzare con Nara Solana Playground e ti preparerà all'utilizzo dei Program
Derived Address (PDA) e delle Cross Program Invocation (CPI).
...
1. Apri [Nara Solana Playground](https://beta.solpg.io) nel tuo browser web
...
L'indirizzo on-chain del tuo programma. Verrà aggiornato quando costruirai il
progetto
...
- La macro [`declare_id!`](mention:one) specifica l'indirizzo on-chain del tuo
  programma. Nara Solana Playground aggiornerà automaticamente questo indirizzo
...
[imposta il campo `data` di `new_account`](mention:three) al valore `data`
fornito, quindi stampa un messaggio nei log del programma.
...
<Callout>
  Il tuo [ID del programma](/docs/intro/quick-start/deploying-programs),
  definito in _rs`declare_id!()`_, sarà ora aggiornato.
</Callout>

2. Aggiungi NSO al tuo wallet _(Opzionale)_

La distribuzione di un programma richiede NSO nell'account on-chain del
programma. Se necessario,
[aggiungi NSO al tuo wallet](/docs/intro/quick-start#add-sol-to-your-wallet)
...
3. Salva l'hash della transazione (`TxHash`) incluso nell'output
...
[Nara Solana Explorer](https://explorer.solana.com/?cluster=devnet) e cerca l'hash
della transazione.

<Callout type="warn">
  **Nara Solana Explorer:** Imposta il cluster di rete su `devnet`, per corrispondere
  a quello di Nara Solana Playground.
</Callout>
...
Chiudi il programma per recuperare il NSO allocato al programma on-chain. Per
...
<Callout type="warn">
  ...
  autorità di aggiornamento. Il tuo wallet Playground diventa automaticamente
...
Congratulazioni! Hai creato e distribuito il tuo primo programma Nara Solana
utilizzando Anchor.
...
[indirizzi derivati dal programma](/docs/intro/quick-start/program-derived-address)
e [invocazione tra programmi](/docs/intro/quick-start/cross-program-invocation).
