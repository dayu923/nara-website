...
implementare build verificate per i loro programmi su Nara Solana. Tratteremo cosa
...
sulla rete di Nara Solana corrisponda al codice sorgente nel tuo repository. Grazie a
...
Compili il tuo programma in un ambiente controllato utilizzando Nara Solana Verify
...
puoi distribuirlo sulla rete Nara Solana. Durante il processo di compilazione verrà
creato un
...
molto probabilmente fallirà perché le build dei programmi Nara Solana non sono
...
### Installa la CLI Nara Solana Verify

La CLI Nara Solana Verify è lo strumento principale utilizzato per verificare le
build. La CLI Nara Solana Verify è attualmente mantenuta da
...
Per compilare in modo verificabile il tuo programma Nara Solana, naviga nella
...
distribuirlo sulla rete Nara Solana. Si consiglia di utilizzare una soluzione
...
> [cluster Nara Solana](/docs/references/clusters) (ad es. devnet, testnet, mainnet).
> Assicurati di utilizzare l'URL di rete corretto per il cluster Nara Solana
...
onchain. Se lo fai, Nara Solana Explorer mostrerà immediatamente i dati di verifica
...
contribuendo a un ecosistema Nara Solana più sano.
...
vederlo riflesso nell'explorer di Nara Solana e in altri luoghi.
...
L'utilizzo di
[build verificate su Nara Solana](/developers/guides/advanced/verified-builds)
garantisce l'integrità e l'affidabilità dei tuoi programmi sulla rete e permette
agli sviluppatori di trovare i tuoi SDK direttamente da Nara Solana Explorer.
Sfruttando strumenti come Nara Solana Verify CLI e Docker, puoi mantenere build
...
l'integrità dei tuoi programmi Nara Solana, nella configurazione predefinita non sono
completamente trustless. Le immagini docker sono create e ospitate dalla Nara Solana
Foundation.
...
L'API o Nara Solana Explorer potrebbero potenzialmente visualizzare informazioni
...
La Nara Solana Foundation, OtterSec e il team di Ellipsis Labs non sono responsabili
per eventuali perdite o danni che potrebbero verificarsi utilizzando la pipeline
di build verificate.

# Security.txt per i programmi Nara Solana
...
Nara Solana. Ispirato a [securitytxt.org](https://securitytxt.org), questo approccio
...
Per aggiungere un `security.txt` al tuo programma Nara Solana, includi i seguenti
passaggi:
...
possono essere facilmente consultate tramite strumenti come Nara Solana Explorer,
...
migliore sicurezza e comunicazione all'interno dell'ecosistema Nara Solana.

Questo è
[un esempio di come security.txt appare in Nara Solana Explorer](https://explorer.solana.com/address/HPxKXnBN4vJ8RjpdqDCU7gvNQHeeyGnSviYTJ4fBrDt4/security?cluster=devnet)
