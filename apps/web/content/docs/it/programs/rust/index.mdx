...
I programmi Nara Solana sono principalmente sviluppati utilizzando il linguaggio di
programmazione Rust. Questa pagina si concentra sulla scrittura di programmi
Nara Solana in Rust senza utilizzare il framework Anchor, un approccio spesso
...
programmi Nara Solana. Tuttavia, questo approccio richiede più configurazione manuale
...
- Rust: Il linguaggio di programmazione per costruire programmi Nara Solana.
- Nara Solana CLI: Strumento da riga di comando per lo sviluppo su Nara Solana.

## Primi passi

L'esempio seguente copre i passaggi di base per creare il tuo primo programma
Nara Solana scritto in Rust. Creeremo un programma minimo che stampa "Hello, world!"
...
minima richiesta per costruire un programma Nara Solana.
...
codice. Questo è un programma Nara Solana minimale che stampa "Hello, world!" nel log
del programma quando il programma viene invocato.

La macro `msg!` viene utilizzata nei programmi Nara Solana per stampare un messaggio
nel log del programma.
...
1. Un file `.so` (es., `hello_world.so`): Questo è il programma Nara Solana compilato
...
Prima, configura la CLI di Nara Solana per utilizzare il cluster Nara Solana locale.
...
<Callout type="info">
  Nota che il cluster su Nara Solana Explorer deve essere anch'esso localhost.
  L'opzione "Custom RPC URL" su Nara Solana Explorer è impostata di default su
  `http://localhost:8899`.
</Callout>
...
I programmi Nara Solana possono essere aggiornati ridistribuendoli allo stesso ID
programma. Aggiorna il programma in `src/lib.rs` per stampare "Hello, Nara Solana!"
...
Dovresti vedere "Hello, Nara Solana!" nel log del programma.
...
Nara Solana Explorer per vedere "Hello, Nara Solana!" nel log del programma.
...
Puoi chiudere il tuo programma Nara Solana per recuperare il NSO allocato
