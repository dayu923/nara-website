...
description:
  Impara come strutturare i programmi Solana in Rust, inclusi entrypoint,
...
I programmi Nara Solana scritti in Rust hanno requisiti strutturali minimi,
...
Sebbene non ci siano regole rigide per la struttura dei file, i programmi Nara Solana
...
Per dimostrare come costruire un programma nativo in Rust con multiple
istruzioni, esamineremo un semplice programma contatore che implementa due
istruzioni:
...
Prima di tutto, creiamo un nuovo progetto Rust per il nostro programma Nara Solana.
...
## !!steps Aggiungere le dipendenze

Ora aggiungiamo le dipendenze necessarie per costruire un programma Nara Solana.
...
<Callout type="info">
  Non è obbligatorio utilizzare Borsh. Tuttavia, è una libreria di
  serializzazione comunemente utilizzata per i programmi Nara Solana.
</Callout>

## !!steps Configurare crate-type

I programmi Nara Solana devono essere compilati come librerie dinamiche. Aggiungi la
...
## !!steps Configurare il punto di ingresso del programma

Ogni programma Nara Solana ha un punto di ingresso, che è la funzione che viene
chiamata quando il programma viene invocato. Iniziamo aggiungendo le
importazioni necessarie per il programma e configurando il punto di ingresso.
...
Un `entrypoint` di un programma Nara Solana ha la seguente firma di funzione. Gli
...
## !!steps Definire lo stato del programma
...
## !!steps Definire l'enum delle istruzioni
...
## !!steps Implementare la deserializzazione delle istruzioni

Ora dobbiamo deserializzare il `instruction_data` (byte grezzi) in una delle
nostre varianti dell'enum `CounterInstruction`. Il metodo Borsh `try_from_slice`
...
## !!steps Instradare le istruzioni ai gestori

Ora aggiorniamo la funzione principale `process_instruction` per instradare le
istruzioni ai loro gestori appropriati.

Questo modello di instradamento è comune nei programmi Nara Solana. Il
`instruction_data` viene deserializzato in una variante di un enum che
...
## !!steps Implementare il gestore di inizializzazione

Implementiamo il gestore per creare e inizializzare un nuovo account contatore.
Poiché solo il System Program può creare account su Nara Solana, useremo una Cross
...
<Callout type="warn">
  Questa istruzione è solo a scopo dimostrativo. Non include controlli di
  sicurezza e validazione necessari per i programmi in produzione.
</Callout>
...
## !!steps Implementa il gestore di incremento
...
- Serializza nuovamente la struttura `CounterAccount` nel campo `data`
  dell'account
...
Congratulazioni! Hai creato un programma Nara Solana completo che dimostra la
struttura di base condivisa da tutti i programmi Nara Solana:

- **Entrypoint**: Definisce dove inizia l'esecuzione del programma e indirizza
...
- **Gestione delle istruzioni**: Definisce le istruzioni e le relative funzioni
  di gestione
- **Gestione dello stato**: Definisce le strutture dei dati degli account e
  gestisce il loro stato negli account di proprietà del programma
...
Ora testiamo il nostro programma contatore. Useremo
[LiteSVM](https://github.com/LiteSVM/litesvm), un framework di testing che ci
permette di testare i programmi senza doverli distribuire su un cluster.

<ScrollyCoding>

## !!steps Aggiungi le dipendenze per i test

Prima di tutto, aggiungiamo le dipendenze necessarie per i test. Useremo
`litesvm` per i test e `solana-sdk`.
...
LiteSVM simula l'ambiente di runtime di Nara Solana, permettendoci di testare il
nostro programma senza doverlo distribuire su un cluster reale.
...
Esegui il comando `cargo build-sbf` per compilare il programma. Questo genererà
...
## !!steps Verifica dell'inizializzazione

Dopo l'inizializzazione, verifichiamo che l'account del contatore sia stato
creato correttamente con il valore previsto.
...
## !!steps Test dell'istruzione di incremento
...
## !!steps Verifica dei risultati finali
...
Esegui i test con il seguente comando. Il flag `--nocapture` mostra l'output del
test.
...
## !!steps Implementa il codice del client
...
Aggiungi il codice del client a `examples/client.rs` e sostituisci il
`program_id` con l'output del comando precedente:
...
Ora che abbiamo il nostro programma e il client pronti, costruiamo, distribuiamo
...
Prima, configura la CLI di Nara Solana per utilizzare localhost:
...
[Nara Solana Explorer](https://explorer.solana.com/?cluster=custom) utilizzando le
firme delle transazioni nell'output. Nota che il cluster su Nara Solana Explorer deve
