...
Questa sezione illustra il processo base di distribuzione di un programma Nara Solana
utilizzando la CLI di Nara Solana.
...
### Installare la CLI di Nara Solana

Prima, [installa la CLI di Nara Solana](/docs/intro/installation).
...
Scegli su quale cluster Nara Solana effettuare il deployment. Usa il comando
...
Avrai bisogno di NSO per pagare i deployment dei programmi. La quantità dipende
...
Ecco un programma Nara Solana minimale che stampa "Hello, Nara Solana!" nei log del
programma.
...
La distribuzione di un programma richiede che del NSO venga allocato all'account
del programma in base alla dimensione del programma. Programmi più grandi
costano più NSO per essere distribuiti.
...
Calcola il NSO necessario per questa dimensione (in byte):
...
<Callout type="info">
  Avrai bisogno di un po' più NSO di quanto mostrato per coprire le commissioni
...
nuovo programma. La CLI calcolerà automaticamente il NSO necessario e lo
...
programma per recuperare il NSO allocato al programma.
...
Il Balance visualizza il NSO allocato all'account del programma per memorizzare
i suoi dati.
...
Per chiudere il tuo programma e recuperare il NSO allocato all'account del
...
Quando la rete Nara Solana è congestionata, usa questi flag per facilitare la
...
- **`--with-compute-unit-price`**: Imposta la commissione prioritaria in
  micro-lamports (0.000001 NSO) per unità di calcolo. Controlla
