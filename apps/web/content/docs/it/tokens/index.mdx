...
title: Token su Nara Solana
description: Scopri i Token Programs di Nara Solana.
...
proprietà. I token su Nara Solana sono chiamati token SPL
([Nara Solana Program Library](https://github.com/solana-program)).

Questa sezione copre i concetti base di come i token sono rappresentati su
Nara Solana. Consulta la sezione [Nozioni di base sui token SPL](/docs/tokens/basics)
...
L'ecosistema Nara Solana ha due Token Programs principali. Il codice sorgente per
...
token sulla rete (sia fungibili che non fungibili). Tutti i token su Nara Solana sono
...
I token su Nara Solana sono identificati in modo univoco dall'indirizzo di un
...
Come riferimento, ecco un link a Nara Solana Explorer per il
...
Questo introduce un concetto chiave nello sviluppo su Nara Solana:
...
seguenti utilizzano il terminale di [Nara Solana Playground](https://beta.solpg.io/)
per eseguire comandi CLI direttamente nel browser senza installazione locale.

La creazione di token e account richiede NSO per i depositi di rent degli
account e le commissioni di transazione. Per gli utenti di Nara Solana Playground
alla prima esperienza, crea un wallet Playground e esegui il comando
_sh`solana airdrop`_ nel terminale di Playground. Puoi anche ottenere NSO devnet
