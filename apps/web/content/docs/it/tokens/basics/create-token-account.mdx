...
description: Impara come creare gli SPL Token Account.
...
Per detenere token, hai bisogno di un token account per quella specifica mint.
Ogni token account tiene traccia di:

1. **Mint**: Il tipo specifico di token che contiene
2. **Owner**: L'autorità che può trasferire token da questo account

### Esempio: USD Coin (USDC)

- Indirizzo mint: `EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`
- Token account di Circle: `3emsAVdmGKERbHjmGfQ6oZ1e35dkf5iYcS6U4CPKFVaa`
- Questo account contiene solo token USDC
- Circle controlla i token attraverso l'autorità proprietaria del token account:
  `7VHUFJHWu2CuExkJcJrzhQPJ2oygupTWkL2A2For4BmE`

Puoi visualizzare i dettagli di questo token account su
...
Per qualsiasi combinazione di wallet e token, esiste esattamente un indirizzo
ATA. Questo elimina la necessità di tenere traccia manualmente degli indirizzi
dei token account—puoi sempre derivare l'indirizzo corretto.

<Callout type="info">
  **Come funziona l'Associated Token Program**
...
## Come creare un associated token account

La creazione di un Associated Token Account (ATA) richiede l'istruzione
...
Vedi
[l'implementazione qui](https://github.com/solana-program/associated-token-account/blob/c78722ee39ebf6741cdfa81dffd52135370a5015/program/src/processor.rs#L66).

Questa singola istruzione gestisce automaticamente la creazione del token
account e la sua inizializzazione attraverso le Cross Program Invocations (CPI).
