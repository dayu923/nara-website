...
title: Imposta autorità
description:
  Impara come impostare l'autorità per i mint account o i token account.

---


## Comprendere le autorità dei token

### Autorità del mint account

- **Autorità di conio (Mint Authority)**: Controlla la creazione di nuovi token.
  Può coniare token per qualsiasi token account. Spesso viene revocata dopo la
  creazione della fornitura iniziale per creare un token con fornitura fissa.
- **Autorità di congelamento (Freeze Authority)**: Controlla la capacità di
  congelare e scongelare i token account. Può impedire a qualsiasi token account
  di trasferire token. Spesso viene revocata per garantire agli utenti che i
  loro token non possano essere congelati.

### Autorità del token account

- **Proprietario dell'account (Account Owner)**: Ha il controllo completo sul
  token account. Può trasferire token, bruciare token, approvare delegati e
  chiudere l'account quando il saldo è zero.
- **Autorità di chiusura (Close Authority)**: Può chiudere il token account
  quando il suo saldo è zero. Per impostazione predefinita, questo è il
  proprietario dell'account, ma può essere delegato a un altro account.

## Come impostare l'autorità
...
modifica o revoca le autorità sui mint account e token account. Solo l'autorità
corrente può trasferire i propri permessi a un nuovo indirizzo o revocarli
permanentemente impostando l'autorità a null. Una volta revocata, l'autorità non
...
<Callout type="info">
  ...
  condividono implementazioni simili per ottenere la stessa funzionalità.
</Callout>
