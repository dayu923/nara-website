...
title: Sync Native
description: Impara come convertire NSO nativo in wrapped NSO.
...
Wrapped NSO (WSOL) è un token account che tiene traccia della quantità di
lamports nell'account come saldo del token. WSOL consente l'integrazione con i
protocolli DeFi per trasferire NSO come token SPL.
...
sincronizza il saldo di un token account wrapped NSO (WSOL) con il NSO nativo
(lamports) effettivamente memorizzato in esso. Quando trasferisci NSO nativo a
un token account WSOL, il saldo del token non si aggiorna automaticamente. Devi
chiamare SyncNative per riflettere il saldo WSOL corretto.
