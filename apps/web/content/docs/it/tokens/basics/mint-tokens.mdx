...
title: Conia Token
description: Impara come coniare nuove unità di un token.
...
La coniazione crea nuove unità di un token utilizzando l'istruzione
...
- Solo l'autorità di conio può coniare nuovi token
- Deve esistere un token account di destinazione per ricevere i token coniati
