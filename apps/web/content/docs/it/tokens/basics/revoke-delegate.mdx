...
title: Revoca del delegato
description: Scopri come revocare il delegato di un token account.

---


## Come revocare un delegato

L'istruzione
[`Revoke`](https://github.com/solana-program/token/blob/a7c488ca39ed4cd71a87950ed854929816e9099f/instruction.rs#L133C5-L133C11)
rimuove tutti i permessi di trasferimento dal delegato attualmente approvato.
Dopo la revoca, il delegato non può più trasferire alcun token dal tuo account.
Solo il proprietario del token account può revocare un delegato.
...
// Create and sign transaction with mint creation, ATA creation, minting, and approval
...
// Create and sign transaction for revoking delegation
...
const approveSignature = await approveChecked(
  connection, // connection
  feePayer, // payer
  mintPubkey, // mint
  associatedTokenAccount, // token account
...
const transactionSignature2 = await revoke(
  connection,
  feePayer,
