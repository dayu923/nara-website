...
description: Impara come trasferire token tra account token.
...
I trasferimenti di token spostano token tra account token dello stesso mint
...
- Entrambi gli account token devono contenere lo stesso tipo di token (mint)
- Solo il proprietario dell'account di origine o il delegato può autorizzare i
  trasferimenti
