...
title: Estensioni Metadata & Metadata Pointer
description:
  Scopri come abilitare l'estensione MetadataPointerExtension e TokenMetadata.

---


## Come abilitare l'estensione MetadataPointerExtension e TokenMetadata
...
consente a un mint account di specificare l'indirizzo del suo account di
metadati. Questo puntatore può fare riferimento a qualsiasi account posseduto da
un programma che implementa l'
[Interfaccia Token Metadata](https://github.com/solana-program/token-metadata/tree/main),
offrendo flessibilità su dove archiviare i metadati.

Il Token Extensions Program implementa direttamente l'
[Interfaccia Token Metadata](https://github.com/solana-program/token-metadata/blob/main/interface/src/instruction.rs),
consentendo di archiviare i metadati direttamente nel mint account stesso.
Questo include il nome del token, il simbolo, l'URI e dati descrittivi
aggiuntivi. Quando i puntatori delle estensioni puntano al mint stesso,
l'estensione
...
può essere utilizzata per archiviare tutti i metadati nell'account del mint.
