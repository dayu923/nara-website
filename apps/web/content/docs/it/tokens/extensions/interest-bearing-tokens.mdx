...
consente di impostare un tasso di interesse direttamente su un mint account.
L'interesse si accumula continuamente in base al timestamp della rete, ma il
saldo effettivo dei token memorizzato in ciascun token account rimane invariato.
Il valore totale (saldo del token più interessi maturati) è solo un calcolo.
Nessun nuovo token viene creato con questa estensione.
