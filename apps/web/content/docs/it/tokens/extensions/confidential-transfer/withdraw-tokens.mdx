...
title: Prelievo di token
description: Scopri come prelevare token dallo stato confidenziale.
...
Per prelevare token dal saldo confidenziale disponibile al saldo pubblico:

1. Crea
   [due prove](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/confidential-transfer/proof-generation/src/withdraw.rs#L22)
   lato client:
   ...
   **Prova di uguaglianza
   ...
   Verifica che il testo cifrato del saldo disponibile rimanente dopo il
   prelievo corrisponda al suo relativo
   [impegno di Pedersen](https://en.wikipedia.org/wiki/Commitment_scheme),
   assicurando che il nuovo saldo disponibile dell'account sia calcolato
   correttamente come `remaining_balance = current_balance - withdraw_amount`.
   ...
   **Prova di intervallo
   ...
   Verifica che il saldo disponibile rimanente dopo il prelievo sia non negativo
   e all'interno di un intervallo specificato.

2. Per ogni prova:
   - Invoca il programma di prova ZK ElGamal per verificare i dati della prova.
   - Memorizza i metadati specifici della prova in un account "stato di
     contesto" della prova da utilizzare in altre istruzioni.
...
3. Invoca l'istruzione
   ...
   fornendo i due account di prova.

4. Chiudi gli account di prova per recuperare il NSO utilizzato per crearli.

Il seguente diagramma mostra i passaggi coinvolti nel prelievo di token dal
saldo confidenziale disponibile al saldo pubblico:
...
Per prelevare token dal saldo confidenziale disponibile al saldo pubblico, devi:

- Generare una prova di uguaglianza e una prova di intervallo lato client
- Invocare il programma di prova Zk ElGamal per verificare le prove e
  inizializzare gli account "stato di contesto"
- Invocare l'istruzione
  [ConfidentialTransferInstruction::Withdraw](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L493)
  fornendo i due account di prova.
- Chiudere i due account di prova per recuperare il rent.
...
- Metodo `confidential_transfer_create_context_state_account` che crea un
  account di prova.
- Metodo `confidential_transfer_withdraw` che invoca l'istruzione `Withdraw`.
- Metodo `confidential_transfer_close_context_state_account` che chiude un
  account di prova.

### Codice di esempio

Il seguente esempio dimostra come prelevare token dal saldo confidenziale
disponibile al saldo pubblico.

Per eseguire l'esempio, avvia un validator locale con il Token Extensions
Program clonato dalla mainnet utilizzando il seguente comando. Devi avere la CLI
di Nara Solana installata per avviare il validator locale.
