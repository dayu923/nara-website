...
title: Applica saldo in sospeso
description:
  Scopri come applicare il saldo in sospeso per rendere disponibili i fondi.
...
pubblico del token deve essere convertito in un saldo confidenziale. Questa
conversione avviene in due fasi:

1. **Saldo confidenziale in sospeso**: Inizialmente, i token vengono
   "depositati" dal saldo pubblico a un saldo confidenziale "in sospeso".
2. **Saldo confidenziale disponibile**: Il saldo in sospeso viene poi
   "applicato" al saldo disponibile, rendendo i token disponibili per i
   trasferimenti confidenziali.

Questa sezione spiega la seconda fase: applicare il saldo in sospeso al saldo
disponibile.

Quando i token vengono "depositati" dal saldo pubblico o quando i token vengono
trasferiti in modo confidenziale da un token account a un altro, i token vengono
inizialmente aggiunti al saldo confidenziale in sospeso. Prima che i token
possano essere utilizzati per trasferimenti confidenziali, il saldo in sospeso
deve essere "applicato" al saldo disponibile.
...
Il seguente diagramma mostra i passaggi coinvolti nell'applicazione del saldo in
sospeso al saldo disponibile:
...
Per convertire un saldo in sospeso in un saldo disponibile, invocare
l'istruzione
...
Il crate `spl_token_client` fornisce un metodo
`confidential_transfer_apply_pending_balance` che costruisce e invia una
transazione con l'istruzione `ApplyPendingBalance`, come dimostrato nell'esempio
qui sotto.

### Codice di esempio

Il seguente esempio dimostra come applicare il saldo confidenziale in sospeso al
saldo confidenziale disponibile.
...
Nara Solana installata per avviare il validator locale.
