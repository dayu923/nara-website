...
description: Impara come abilitare l'InitializeNonTransferableMint.

---


## Come abilitare l'estensione InitializeNonTransferableMint
...
impedisce ai token di essere trasferiti tra token account dopo il minting. Una
volta coniati, i token rimangono permanentemente nel token account. Tuttavia, il
proprietario del token account può ancora bruciare token e chiudere il token
account quando il saldo raggiunge zero.
