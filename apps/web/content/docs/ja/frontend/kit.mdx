...
seoTitle: Next.jsとNara Solana Kitを使用したNara Solanaウォレット連携
description: Next.jsでNara Solana Kitを使用してNara Solanaウォレット連携を設定します。

---


Next.jsでNara Solana Kitを使用して最小限のNara Solanaウォレット連携を設定します。

このガイドでは、Next.jsアプリケーションで`@solana/kit`を使用してNara Solanaウォレット機能を実装する最小限の例を提供します。ウォレット接続ボタンとトランザクション送信用のコンポーネントを作成します。

![Nextjs Kitアプリケーション](/assets/docs/frontend/nextjs-kit.gif)

Reactアプリケーションで`@solana/kit`を使用するより包括的な例については、Nara Solana
Kitリポジトリの[Reactアプリの例](https://github.com/anza-xyz/kit/tree/main/examples/react-app)を参照してください。

## リソース

- [Nara Solana Kit ドキュメント](https://www.solanakit.com/docs)
- [Nara Solana Kit GitHubリポジトリ](https://github.com/anza-xyz/kit)
...
UIコンポーネント用の[shadcn](https://ui.shadcn.com/)を使用して新しいNext.jsプロジェクトを作成し、必要なNara Solana依存関係をインストールします。
...
以下のshadcn uiコンポーネントをインストールします：

```terminal
$ npx shadcn@latest add button dropdown-menu avatar
```

### Nara Solana依存関係のインストール

以下のNara Solana依存関係をインストールします：
...
以下の手順に従い、提供されたコードをプロジェクトにコピーしてください。

<ScrollyCoding>

## !!steps 1. Nara Solanaコンテキストを作成する

まず、アプリケーション全体のウォレット状態を管理するReactコンテキストを作成します。

`components/solana-provider.tsx`を作成し、提供されたコードを追加します。このプロバイダーコンポーネントは以下を行います：

- Nara SolanaのdevnetのRPCエンドポイントに接続する
- ユーザーのブラウザにインストールされている利用可能なNara Solanaウォレットをフィルタリングする
- 現在接続されているウォレットとアカウントを追跡する
- 子コンポーネントにウォレットの状態を提供する
...
次に、Next.jsアプリケーション全体をNara Solanaプロバイダーでラップします。

`app/layout.tsx`を提供されたコードで更新します。このステップでは：

- `SolanaProvider`コンポーネントをインポートする
- アプリケーションの子コンポーネントを`SolanaProvider`でラップする
- すべてのページとコンポーネントがウォレット機能にアクセスできるようにする
...
## !!steps 3. ウォレット接続ボタンを作成する

次に、ウォレットの接続と切断を行うボタンを構築します。

`components/wallet-connect-button.tsx`を作成し、提供されたコードを追加します。このドロップダウンボタンは：

- クリックすると利用可能なウォレットを表示する
- Wallet Standardを使用してウォレット接続フローを処理する
...
## !!steps 4. トランザクション送信コンポーネントを作成する

メモプログラムを呼び出してトランザクションログにメッセージを追加するトランザクションを送信するコンポーネントを作成します。

このコンポーネントの目的は、接続されたウォレットでトランザクションを送信する方法を示すことです。

`components/memo-card.tsx`を作成し、提供されたコードを追加します。このコンポーネントは：

- ユーザーがメッセージを入力できるようにします
- メモプログラムを呼び出すinstructionsを含むNara Solanaトランザクションを作成します
- 接続されたウォレットにトランザクションの署名と送信を要求します
- Nara Solana Explorerでトランザクションを表示するためのリンクを表示します
...
提供されたコードで`app/page.tsx`を更新します。このページは：

- `WalletConnectButton`と`MemoCard`コンポーネントをインポートして使用します
...
ウォレット統合をテストするためにアプリケーションを実行します。
...
<Callout type="info">
  接続されたウォレットはdevnetクラスターに接続するように設定され、トランザクションを送信するために[devnet
  NSOで資金提供](https://faucet.solana.com/)されている必要があることに注意してください。
</Callout>
