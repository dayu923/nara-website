...
title: トークンアカウントの作成
description: SPLトークンアカウントの作成方法を学びましょう。
...
トークンアカウントは特定のトークンの残高を保存します。各トークンアカウントは正確に1つのミントに関連付けられ、トークン残高と追加の詳細を追跡します。
...
1. **ミント**：保有する特定のトークンタイプ
2. **オーナー**：このアカウントからトークンを転送できる権限者

### 例：USD Coin (USDC)

- ミントアドレス：`EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`
- Circleのトークンアカウント：`3emsAVdmGKERbHjmGfQ6oZ1e35dkf5iYcS6U4CPKFVaa`
- このアカウントはUSDCトークンのみを保有
- Circleはトークンアカウントオーナー権限を通じてトークンを管理：
  `7VHUFJHWu2CuExkJcJrzhQPJ2oygupTWkL2A2For4BmE`

このトークンアカウントの詳細は[Nara Solana Explorer](https://explorer.solana.com/address/3emsAVdmGKERbHjmGfQ6oZ1e35dkf5iYcS6U4CPKFVaa)で確認できます。

<Callout type="info">
  **「オーナー」という用語の理解**

「オーナー」という用語は2つの異なる文脈で使用されることがあります：

1. **トークンアカウントオーナー**：トークンの転送、焼却、委任ができる権限者。これはトークンアカウントのデータに保存されています。

2. **プログラムオーナー**：アカウントを所有するプログラム（トークンアカウントの場合は常にToken
   ProgramまたはToken Extensions Program）。

トークンアカウントを扱う場合、「オーナー」は通常、アカウントを所有するプログラムではなく、トークンを転送できる権限者を指します。
...
instruction が必要です。
[実装はこちら](https://github.com/solana-program/token/blob/a7c488ca39ed4cd71a87950ed854929816e9099f/program/src/processor.rs#L83)をご覧ください。

トークンアカウントを作成するには2つのinstructionsが必要です：

1. System
   Program: トークンアカウント用の領域を確保したアカウントを作成し、所有権をToken
   Programに移転する
2. Token Program: トークンアカウントデータを初期化する
...
## アソシエイテッドトークンアカウントの作成方法

アソシエイテッドトークンアカウント（ATA）の作成には
...
この単一の命令で、クロスプログラム呼び出し（CPI）を通じてトークンアカウントの作成と初期化を自動的に処理します。
