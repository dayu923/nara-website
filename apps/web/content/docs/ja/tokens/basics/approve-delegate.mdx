...
title: 委任の承認
description: トークンアカウントの委任を承認する方法を学びましょう。
...
instructionは、別のアカウント（委任先）に対して、あなたのトークンアカウントから特定の量のトークンを転送する権限を付与します。委任先は承認された金額までトークンを転送できますが、それを超えることはできません。トークンアカウントの所有者のみが委任先を承認でき、各トークンアカウントは一度に1つの委任先しか持てません。新しい承認は以前の承認を上書きします。

<Callout type="info">
  [Token
  Program](https://github.com/solana-program/token/blob/a7c488ca39ed4cd71a87950ed854929816e9099f/program/src/instruction.rs#L295)と[Token
  Extension
  Program](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/instruction.rs#L361)は同じ機能を実現するために類似の実装を共有しています。
</Callout>
