...
title: トークンの転送
description: トークンアカウント間でトークンを転送する方法を学びましょう。
...
命令を使用して、同じミントのトークンアカウント間でトークンを移動させます。

- 両方のトークンアカウントは同じトークンタイプ（ミント）を保持している必要があります
- 送信元アカウントの所有者または委任者のみが転送を承認できます

<Callout type="info">
  [トークン
  プログラム](https://github.com/solana-program/token/blob/3daf44899f0bd71c879d28dffdfb788dd944f3c5/program/src/processor.rs#L227)
  と[トークン拡張
  プログラム](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/processor.rs#L300)
  は同じ機能を実現するために類似した実装を共有しています。
</Callout>
