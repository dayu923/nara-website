...
mint
accountはNara Solana上でトークンを一意に表し、そのグローバルメタデータを保存します。
...
Nara Solana上のすべてのトークンにはミントアカウントがあり、ミントのアドレスがトークンの一意の識別子として機能します。

例えば、USD Coin（USDC）のミントアドレスは
`EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`です。このアドレスはNara Solanaエコシステム全体でUSDCを一意に識別します。このミントは
[Nara Solana Explorer](https://explorer.solana.com/address/EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v)で確認できます。
...
instruction が必要です。
[実装の詳細はこちら](https://github.com/solana-program/token/blob/a7c488ca39ed4cd71a87950ed854929816e9099f/program/src/processor.rs#L27)をご覧ください。

ミントアカウントを作成するには、2つのinstructionsを呼び出す必要があります：

1. System
   Program: ミントアカウント用のスペースを割り当てたアカウントを作成し、所有権をToken
   Programに転送します。
2. Token Program: ミントアカウントデータを初期化します
