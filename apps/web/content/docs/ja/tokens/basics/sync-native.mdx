...
title: ネイティブの同期
description: ネイティブNSOをラップドNSOに変換する方法を学びましょう。
...
ラップドNSO（WSOL）は、アカウント内のlamportsの量をトークン残高として追跡するトークンアカウントです。WSOLはDeFiプロトコルとの統合を可能にし、NSOをSPLトークンとして転送できるようにします。

[`SyncNative`](https://github.com/solana-program/token/blob/a7c488ca39ed4cd71a87950ed854929816e9099f/interface/src/instruction.rs#L387)命令は、ラップドNSO（WSOL）トークンアカウントの残高を、そこに保存されている実際のNSO（lamports）と同期させます。ネイティブNSOをWSOLトークンアカウントに転送する場合、トークン残高は自動的に更新されません。正確なWSOL残高を反映させるには、SyncNativeを呼び出す必要があります。

<Callout type="info">
  [トークンプログラム](https://github.com/solana-program/token/blob/a7c488ca39ed4cd71a87950ed854929816e9099f/interface/src/instruction.rs#L387)と[トークン拡張プログラム](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/instruction.rs#L182)は、同じ機能を実現するために類似の実装を共有しています。
</Callout>
