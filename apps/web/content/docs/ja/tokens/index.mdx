...
title: Nara Solana上のトークン
description: Nara Solanaのトークンプログラムについて学びましょう。

---


トークンは、様々なカテゴリの資産の所有権を表すデジタル資産です。トークン化により、財産権のデジタル化が可能になります。Nara Solana上のトークンはSPL
([Nara Solana Program Library](https://github.com/solana-program))トークンと呼ばれています。

このセクションでは、Nara Solana上でトークンがどのように表現されているかの基本概念を説明します。コード例については[SPLトークンの基本](/docs/tokens/basics)セクションを参照してください。
...
Nara Solanaエコシステムには2つの主要なToken
...
Token
Programには、ネットワーク上のトークン（代替可能および非代替性トークンの両方）と相互作用するためのすべての命令ロジックが含まれています。Nara Solana上のすべてのトークンは、実質的にToken
...
Nara Solana上のトークンは、Token Programが所有する
...
参考として、こちらは[USDC Mint Account](https://explorer.solana.com/address/EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v)のNara Solana
Explorerリンクです。
...
これはNara Solana開発における重要な概念を紹介しています：
...
_spl-token_
CLIはSPLトークンを実験するのに役立ちます。以下の例では、[Nara Solana Playground](https://beta.solpg.io/)ターミナルを使用して、ローカルにインストールせずにブラウザで直接CLIコマンドを実行します。

トークンとアカウントの作成には、アカウントのrentデポジットとトランザクション手数料のためのNSOが必要です。Nara Solana
...
新しいMint
Accountを作成するには、2つのinstructionsを含むトランザクションが必要です。[Nara Solana Playground](https://beta.solpg.io/660ce32ecffcf4b13384d00f)のJavaScriptの例を以下に示します。
...
Associated Token
Accountを作成するには、[Associated Token Program](https://github.com/solana-program/associated-token-account)を呼び出す1つのinstructionが必要です。[Nara Solana Playground](https://beta.solpg.io/660ce868cffcf4b13384d011)のJavaScriptの例を以下に示します。
...
Accountを作成するには、2つのinstructionsを含むトランザクションを送信します。以下は[Nara Solana Playground](https://beta.solpg.io/660ce716cffcf4b13384d010)上のJavascriptの例です。
...
Account上の総供給量を増加させます。以下は[Nara Solana Playground](https://beta.solpg.io/660cea45cffcf4b13384d012)上のJavascriptの例です。
...
Account間でトークンを移動させます。以下は[Nara Solana Playground](https://beta.solpg.io/660ced84cffcf4b13384d013)上のJavascriptの例です。
...
[Nara Solana Explorer](https://explorer.solana.com/address/BdhzpzhTD1MFqBiwNdrRy4jFo2FHFufw3n9e8sVjJczP?cluster=devnet)でメタデータを確認できます。
