...
title: 利子付きトークン
description: InterestBearingMintExtensionの有効化方法について学びます。

---


## InterestBearingExtensionを有効にする方法

[`InterestBearingMintExtension`](https://github.com/solana-program/token-2022/blob/6f2473344d70271f632c3e9b7e945be00186c536/interface/src/instruction.rs#L654)を使用すると、mint
accountに直接金利を設定できます。利子はネットワークのタイムスタンプに基づいて継続的に複利計算されますが、各token
accountに保存されている実際のトークン残高は変更されません。総価値（トークン残高と発生した利子）は計算上のものに過ぎません。この拡張機能では新しいトークンは発行されません。
