...
description:
  Confidential
  Transfer拡張機能について学び、トークンミントとアカウントにオプション機能を追加する方法を理解しましょう。
...
機密転送により、転送金額を明かさずにトークンアカウント間でトークンを転送することができます。これはプライバシーを保護するトランザクションに役立ちます。転送金額とトークン残高のみが非公開となり、トークンアカウントのアドレスは公開されたままです。

- [プロトコル概要](https://www.solana-program.com/docs/confidential-balances/overview) - 基盤となる暗号プロトコルの詳細
- [クイックスタートガイド](https://www.solana-program.com/docs/confidential-balances#setup) - セットアップと基本的なCLIコマンド
- [機密残高クックブック](https://github.com/solana-developers/Confidential-Balances-Sample) - 機密転送拡張機能の使用方法に関するコードスニペット

### どのように機能しますか？

機密転送拡張機能は、Token Extension
programに[命令](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/instruction.rs#L29)を追加し、転送金額を明かさずにアカウント間でトークンを転送できるようにします。
...
機密トークン転送の基本的な流れは次のとおりです：

1. 機密転送拡張機能を持つmint accountを作成します。
2. 送信者と受信者のために機密転送拡張機能を持つtoken accountを作成します。
3. 送信者アカウントにトークンをミントします。
4. 送信者の**公開残高**を**機密保留残高**に**デポジット**します。
5. 送信者の保留残高を**機密利用可能残高**に**適用**します。
6. 送信者のtoken accountから受信者のtoken
   accountにトークンを機密に**転送**します。
7. 受信者の保留残高を**機密利用可能残高**に**適用**します。
8. 受信者の機密利用可能残高を**公開残高**に**引き出し**ます。

機密転送フローのステップに関する詳細については、対応するページを参照してください：
...
<Cards>
  ...
  >
    Confidential Transfer拡張機能を持つmint accountを作成する方法
  </Card>
  <Card
    title="token accountの作成"
    href="/docs/tokens/extensions/confidential-transfer/create-token-account"
  >
    Confidential Transfer拡張機能でtoken accountを設定する方法
  </Card>
  <Card
    title="トークンの入金"
  ...
  >
    token account間で機密にトークンを転送する方法
  </Card>
</Cards>

以下の図は、機密トークン転送の基本的なフローの詳細なシーケンスを示しています：
...
以下のRustの例は、前述の図に対応する機密トークン転送の手順を示しています。わかりやすくするために、手順はヘルパー関数に分けられています。

この例を実行するには、以下のコマンドを使用して、メインネットからクローンしたToken
Extensions
Programでローカルバリデータを起動します。ローカルバリデータを使用するには、Nara Solana
...
<Callout type="info">
  執筆時点では、Confidential
  Transfersはデフォルトのローカルバリデータでは有効になっていません。サンプルコードを実行するには、メインネットのToken
...
| instruction                         | 説明                                                                                                                                                        |
| ----------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | 機密転送のためのmint accountを設定します。このinstructionは*rs`TokenInstruction::InitializeMint`* instructionと同じトランザクションに含める必要があります。 |
| _rs`UpdateMint`_                    | ミントの機密転送設定を更新します。                                                                                                                          |
| _rs`ConfigureAccount`_              | 機密転送のためのtoken accountを設定します。                                                                                                                 |
| _rs`ApproveAccount`_                | ミントが新しいtoken accountの承認を要求する場合、token accountを機密転送用に承認します。                                                                    |
| _rs`EmptyAccount`_                  | token accountを閉じることができるように、保留中および利用可能な機密残高を空にします。                                                                       |
| _rs`Deposit`_                       | 公開トークン残高を保留中の機密残高に変換します。                                                                                                            |
| _rs`Withdraw`_                      | 利用可能な機密残高を公開残高に戻します。                                                                                                                    |
| _rs`Transfer`_                      | token account間でトークンを機密に転送します。                                                                                                               |
| _rs`ApplyPendingBalance`_           | 入金または転送後、保留残高を利用可能な残高に変換します。                                                                                                    |
| _rs`EnableConfidentialCredits`_     | token accountが機密トークン転送を受け取ることを許可します。                                                                                                 |
| _rs`DisableConfidentialCredits`_    | 公開転送を許可しながら、着信機密転送をブロックします。                                                                                                      |
| _rs`EnableNonConfidentialCredits`_  | token accountが公開トークン転送を受け取ることを許可します。                                                                                                 |
| _rs`DisableNonConfidentialCredits`_ | 通常の転送をブロックして、アカウントが機密転送のみを受け取るようにします。                                                                                  |
| _rs`TransferWithFee`_               | 手数料付きでtoken account間でトークンを機密に転送します。                                                                                                   |
| _rs`ConfigureAccountWithRegistry`_  | *rs`VerifyPubkeyValidity`*証明の代わりに*rs`ElGamalRegistry`*アカウントを使用して、機密転送用のtoken accountを設定する代替方法です。                        |
