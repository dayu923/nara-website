...
title: トークンの転送
description: あるトークンアカウントから別のトークンアカウントへプライベートにトークンを転送する方法を学びましょう。
...
<Callout type="warn">
  ZK
  ElGamalプログラムは、セキュリティ監査を受けている間、メインネットとdevnetで一時的に無効化されています。これは、機密転送拡張機能が現在利用できないことを意味します。概念は依然として有効ですが、コード例は実行されません。
</Callout>

## あるトークンアカウントから別のトークンアカウントへ機密転送する方法

あるトークンアカウントから別のトークンアカウントへ機密転送するには、送信者と受信者の両方が*rs`ConfidentialTransferAccount`*状態で構成され、機密転送が承認されたトークンアカウントを持っている必要があります。送信者のトークンアカウントには、転送元として利用可能な機密残高も必要です。

トークンを機密転送するには：

1. クライアント側で[3つの証明](https://github.com/solana-program/token-2022/blob/d073cb89dbcd430387b5c6fb4b7157911351e4a3/confidential-transfer/proof-generation/src/transfer.rs#L35)を作成します：

   **等価性証明 ([CiphertextCommitmentEqualityProofData](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/ciphertext_commitment_equality.rs#L56))**: 転送後の新しい利用可能残高の暗号文が対応する[Pedersenコミットメント](https://en.wikipedia.org/wiki/Commitment_scheme)と一致することを検証し、送信元アカウントの新しい利用可能残高が`new_balance = current_balance - transfer_amount`として正しく計算されていることを確認します。

   **暗号文有効性証明 ([BatchedGroupedCiphertext3HandlesValidityProofData](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/batched_grouped_ciphertext_validity/handles_3.rs#L63))**: 転送額の暗号文が3つの関係者（送信元、送信先、監査人）すべてに対して適切に生成されていることを検証し、転送額が各関係者の公開鍵で正しく暗号化されていることを確認します。

   **範囲証明 ([BatchedRangeProofU128Data](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/batched_range_proof/batched_range_proof_u128.rs#L37))**: 新しい利用可能残高と転送額（低/高ビットに分割）がすべて非負であり、指定された範囲内にあることを検証します。

2. 各証明について：
   - ZK ElGamal証明プログラムを呼び出して、証明データを検証します。
   - 証明固有のメタデータを証明「コンテキスト状態」アカウントに保存し、他のinstructionsで使用します。
...
3. [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604)
   instructionを呼び出し、証明コンテキスト状態アカウントを提供します。

4. プルーフコンテキスト状態アカウントを閉じて、作成に使用したNSOを回収します。

次の図は、送信者のトークンアカウントから受信者のトークンアカウントへトークンを転送する際の手順を示しています。
...
あるトークンアカウントから別のトークンアカウントへ機密転送するには、以下が必要です：

- クライアント側で等価性証明、暗号文有効性証明、範囲証明を生成する
- Zk
  ElGamal証明プログラムを呼び出して証明を検証し、「コンテキスト状態」アカウントを初期化する
- [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604)
  instructionを呼び出し、3つの証明アカウントを提供する
- 3つの証明アカウントを閉じてrentを回収する

`spl_token_client` クレートは以下のメソッドを提供しています：

- `confidential_transfer_create_context_state_account`
  メソッドは証明アカウントを作成します。
- `confidential_transfer_transfer` メソッドは `Transfer`
  instructionを呼び出します。
...
以下の例は、トークンアカウント間で機密転送を行う方法を示しています。

この例を実行するには、以下のコマンドを使用してメインネットからクローンしたToken
Extensions
Programでローカルバリデータを起動します。ローカルバリデータを起動するにはNara Solana
...
<Callout type="info">
  執筆時点では、機密転送はデフォルトのローカルバリデータでは有効になっていません。サンプルコードを実行するには、メインネットのToken
