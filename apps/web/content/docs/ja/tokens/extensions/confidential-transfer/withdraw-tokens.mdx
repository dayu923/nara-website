...
title: トークンの引き出し
description: 機密状態からトークンを引き出す方法を学びます。
...
機密利用可能残高から公開残高にトークンを引き出すには：

1. クライアント側で
   [2つの証明](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/confidential-transfer/proof-generation/src/withdraw.rs#L22)
   を作成します：

   **等価性証明 ([CiphertextCommitmentEqualityProofData](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/ciphertext_commitment_equality.rs#L56))**：引き出し後の残りの利用可能残高の暗号文が対応する
   [Pedersenコミットメント](https://en.wikipedia.org/wiki/Commitment_scheme)と一致することを検証し、アカウントの新しい利用可能残高が
   `remaining_balance = current_balance - withdraw_amount`として正しく計算されていることを確認します。

   **範囲証明 ([BatchedRangeProofU64Data](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/batched_range_proof/batched_range_proof_u64.rs#L37))**：引き出し後の残りの利用可能残高が負でなく、指定された範囲内にあることを検証します。
...
4. 証明アカウントを閉じて、それらを作成するために使用されたNSOを回収します。

以下の図は、機密利用可能残高から公開残高にトークンを引き出す手順を示しています：
...
機密利用可能残高から公開残高にトークンを引き出すには、以下が必要です：
...
- 2つの証明アカウントを提供する
  [ConfidentialTransferInstruction::Withdraw](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L493)
  instructionを呼び出す
- 2つの証明アカウントを閉じてrentを回収する

`spl_token_client`クレートは以下のメソッドを提供します：

- 証明アカウントを作成する`confidential_transfer_create_context_state_account`メソッド
- `Withdraw`instructionを呼び出す`confidential_transfer_withdraw`メソッド
- 証明アカウントを閉じる`confidential_transfer_close_context_state_account`メソッド

### サンプルコード

以下の例は、機密利用可能残高から公開残高にトークンを引き出す方法を示しています。

例を実行するには、以下のコマンドを使用してメインネットからクローンしたToken
Extension
Programを持つローカルvalidatorを起動します。ローカルvalidatorを起動するにはNara Solana
...
<Callout type="info">
  執筆時点では、機密転送（Confidential
  Transfers）はデフォルトのローカルvalidatorでは有効になっていません。サンプルコードを実行するには、メインネットからToken
