...
title: トークンの入金
description: 機密状態へトークンを入金する方法を学びましょう。
...
1. **機密保留残高**：最初に、トークンは公開残高から「保留中」の機密残高に「入金」されます。
2. **機密利用可能残高**：その後、保留残高が利用可能残高に「適用」され、トークンが機密転送に利用できるようになります。

このセクションでは、最初の段階について説明します：公開トークン残高を機密保留残高に入金する方法です。

以下の図は、公開残高から機密保留残高にトークンを入金する手順を示しています：
...
公開残高を機密保留残高に変換するには、[ConfidentialTransferInstruction::Deposit](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L386)
instructionを呼び出します。入金instructionごとの[最大金額](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/mod.rs#L21)は2^48に制限されています。

`spl_token_client` クレートは、以下の例で示すように、`Deposit`
instructionを含むトランザクションを構築して送信する
`confidential_transfer_deposit` メソッドを提供しています。

### サンプルコード

以下の例は、公開トークン残高を機密保留残高に入金する方法を示しています。

この例を実行するには、以下のコマンドを使用してメインネットからクローンしたToken
Extensions
Programでローカルvalidatorを起動します。ローカルvalidatorを起動するにはNara Solana
