...
title: 保留残高を適用する
description: 資金を利用可能にするために保留残高を適用する方法を学びます。
...
<Callout type="warn">
  ZK
  ElGamalプログラムは、セキュリティ監査を受けている間、メインネットとdevnetで一時的に無効化されています。これは、機密転送拡張機能が現在利用できないことを意味します。概念自体は有効ですが、コード例は実行できません。
</Callout>

## 保留残高を利用可能残高に適用する方法

トークンを機密転送する前に、公開トークン残高を機密残高に変換する必要があります。この変換は2段階で行われます：

1. **機密保留残高**：最初に、トークンは公開残高から「保留」機密残高に「預け入れ」されます。
2. **機密利用可能残高**：その後、保留残高が利用可能残高に「適用」され、トークンが機密転送に利用できるようになります。

このセクションでは、2番目の段階について説明します：保留残高を利用可能残高に適用する方法です。

トークンが公開残高から「預け入れ」される場合や、あるtoken accountから別のtoken
accountに機密転送される場合、トークンは最初に機密保留残高に追加されます。トークンを機密転送に使用する前に、保留残高を利用可能残高に「適用」する必要があります。
...
以下の図は、保留残高を利用可能残高に適用する手順を示しています：
...
保留残高を利用可能残高に変換するには、[ConfidentialTransferInstruction::ApplyPendingBalance](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L1152)
instructionを呼び出します。

`spl_token_client` クレートは、以下の例で示すように、`ApplyPendingBalance`
instructionを含むトランザクションを構築して送信する
`confidential_transfer_apply_pending_balance` メソッドを提供しています。

### コード例

以下の例は、機密保留残高を機密利用可能残高に適用する方法を示しています。

この例を実行するには、以下のコマンドを使用して、メインネットからクローンされたToken
Extensions
Programを使用してローカルvalidatorを起動します。ローカルvalidatorを起動するには、Nara Solana
