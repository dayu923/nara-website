...
title: Mollusk
description: Molluskを使用したNara Solanaプログラムのテストに関する包括的ガイド

---


[Mollusk](https://github.com/anza-xyz/mollusk/tree/main)はNara Solanaプログラムをテストするための軽量なテストハーネスです。Nara Solana仮想マシン（SVM）の簡易環境でNara Solanaプログラムのinstructionsをテストするためのシンプルなインターフェースを提供します。すべてのテストアカウントは明示的に定義する必要があり、決定論的で再現可能なテストを保証します。
...
以下の例は、Molluskを使用して基本的なNara Solanaプログラムをテストするための最小限のセットアップを示しています。

<ScrollyCoding>

## !!steps Hello Worldプログラム

この例では、Molluskを使用して基本的なNara Solanaプログラムをテストする方法を示しています。このプログラムは呼び出されると単に「Hello,
...
Molluskを使用してNara Solanaプログラムをテストするには：
...
[`Mollusk`](https://docs.rs/mollusk-svm/0.5.0/mollusk_svm/struct.Mollusk.html)構造体は、Nara Solanaプログラムをテストするためのシンプルなインターフェースを提供します。すべてのフィールドは少数のヘルパーメソッドを通じて操作できますが、ユーザーはより細かい制御が必要な場合、直接アクセスして変更することもできます。
...
次の例では、検証チェックなしでNSO転送instructionを処理します。
...
次の例では、検証チェック付きのNSO転送instructionsを処理しています。
...
次の例では、検証チェックなしで2つのNSO転送instructionsを処理しています。
...
次の例では、各instructionsの後に検証チェックを行いながら、2つのNSO転送instructionsのチェーンを処理しています。
...
以下の例では、`account_store`を通じてinstructions間で永続的なアカウント状態を維持しながら、2つの別々のNSO転送instructionsを処理しています。
...
以下の例はNSO転送instructionのコンピュートユニット使用量をベンチマークしています。
