...
このガイドは、Nara Solana上でプログラムの検証済みビルドを実装したい開発者向けのリファレンスです。検証済みビルドとは何か、その使用方法、特別な考慮事項、およびオンチェーンでのプログラムの信頼性を確保するためのベストプラクティスについて説明します。

# 検証済みビルドとは何ですか？

検証済みビルドは、Nara Solanaネットワークにデプロイする実行可能なプログラムが、リポジトリ内のソースコードと一致することを保証します。これにより、開発者とユーザーは、オンチェーンで実行されているプログラムが公開されているコードベースと完全に一致していることを確信でき、透明性とセキュリティを促進します。
...
検証プロセスは、オンチェーンプログラムのハッシュとソースコードからローカルでビルドされたプログラムのハッシュを比較することで行われます。Nara Solana
Verify
CLIとDockerを使用して、制御された環境でプログラムをビルドします。これにより、ビルドプロセスが決定論的になり、異なるシステム間で一貫性が保たれます。実行可能ファイルができたら、Nara Solanaネットワークにデプロイできます。ビルドプロセス中に[verify program](https://github.com/otter-sec/otter-verify)の[PDA](https://explorer.solana.com/address/63XDCHrwZu3mXsw2RUFb4tbNpChuUHx4eA5aJMnHkpQQ/anchor-account)が作成されます。このPDAにはプログラムを検証するために必要なすべてのデータが含まれています。PDAにはプログラムアドレス、gitのURL、コミットハッシュ、およびプログラムをビルドするために使用された引数が含まれています。

PDAのデータを使用して、誰でもローカルで検証プログラムコマンドを実行し、プログラムが提供されたソースコードからビルドされたかどうかを確認できます。その後、完全に信頼性を確保しながら自分自身で検証したり、[OtterSec](https://github.com/otter-sec)が維持している独自の[verify API](https://github.com/otter-sec/solana-verified-programs-api)を実行して、ユーザーが検証を簡単に確認できるアクセスポイントを提供することができます。これらの[APIコール](https://verify.osec.io/status/PhoeNiXZ8ByJGLkxNfZRnkUfjvmuYqLR89jjFHGqdXY)は、[Nara Solana Explorer](https://explorer.solana.com/address/E1fcPKuV1UJEsTJPpab2Jr8y87ZN73i4CHTPLbWQE6CA/verified-build)や[SolanaFM](https://solana.fm/address/E1fcPKuV1UJEsTJPpab2Jr8y87ZN73i4CHTPLbWQE6CA/transactions?cluster=mainnet-alpha)など、さまざまな場所ですでに使用されています。
...
Dockerコンテナ内で構築されていないプログラムを検証しようとすると、Nara Solanaプログラムのビルドは異なるシステム間で決定論的ではないため、ほとんどの場合失敗します。
...
### Nara Solana Verify CLIのインストール

Nara Solana Verify CLIは、ビルドを検証するために使用される主要なツールです。Nara Solana
Verify
...
Nara Solanaプログラムを検証可能な方法でビルドするには、ワークスペースの `Cargo.toml`
...
プログラムをビルドしてハッシュを取得したら、Nara Solanaネットワークにデプロイできます。安全なデプロイのためには、[Squads Protocol](https://squads.so/protocol)のようなマルチシグネチャやガバナンスソリューションを使用することが推奨されますが、以下のように直接デプロイすることもできます：
...
> 異なるバージョンが異なる[Nara Solanaクラスター](/docs/references/clusters)（つまり、devnet、testnet、mainnet）にデプロイされている場合があります。検証したいNara Solanaクラスターに対して正しいネットワークURLを使用していることを確認してください。リモート検証はメインネットでのみ機能します。
...
コマンドの最後に、検証データをオンチェーンにアップロードするかどうか尋ねられます。アップロードすると、Nara Solana
...
[Nara Solana Explorer](https://explorer.solana.com/address/PhoeNiXZ8ByJGLkxNfZRnkUfjvmuYqLR89jjFHGqdXY/verified-build)、
...
にも表示され、より健全なNara Solanaエコシステムに貢献します。
...
APIにリモートジョブを送信しました。これでNara Solanaエクスプローラーや他の場所で反映されているのが確認できるはずです。
...
[Nara Solanaの検証済みビルド](/developers/guides/advanced/verified-builds)を使用することで、ネットワーク上のプログラムの整合性と信頼性を確保し、開発者はNara Solana
Explorerから直接SDKを見つけることができます。Nara Solana Verify
...
検証済みビルドはNara Solanaプログラムの整合性を確保するための強力なツールですが、デフォルトの設定では完全に信頼性が担保されているわけではありません。Dockerイメージは、Nara Solana
...
APIと[Nara Solana Explorer](https://explorer.solana.com/address/PhoeNiXZ8ByJGLkxNfZRnkUfjvmuYqLR89jjFHGqdXY)をある程度信頼する必要があります。
...
Nara Solana財団、OtterSec、およびEllipsis
Labsチームは、検証済みビルドパイプラインの使用によって発生する可能性のある損失や損害について責任を負いません。

# Nara Solanaプログラム用のsecurity.txt
...
`security.txt`機能により、開発者はNara Solanaスマートコントラクト内に連絡先とセキュリティ情報を直接埋め込むことができます。[securitytxt.org](https://securitytxt.org)にインスパイアされたこのアプローチは、セキュリティ研究者がコントラクトのアドレスしか知らない場合でも、プロジェクトのメンテナに連絡するための標準化された方法を提供します。
...
Nara Solanaプログラムに`security.txt`を追加するには、次の手順を含めます：
...
`security.txt` 情報がプログラムに埋め込まれると、Nara Solana
...
セキュリティ連絡先情報をコントラクトに直接埋め込むことで、研究者があなたに連絡しやすくなり、Nara Solanaエコシステム内でのセキュリティとコミュニケーションが向上します。

これは[Nara Solana Explorerでのsecurity.txtの表示例](https://explorer.solana.com/address/HPxKXnBN4vJ8RjpdqDCU7gvNQHeeyGnSviYTJ4fBrDt4/security?cluster=devnet)です
