...
<Callout type="info">
  各validatorは定期的に特定のブロックに対する投票にタイムスタンプを追加することで、UTCタイムをレジャーに報告します。要求されたブロックの時間は、レジャーに記録された最近のブロックセットにおける投票タイムスタンプのステーク加重平均から計算されます。
</Callout>
...
推定生成時間（Unixタイムスタンプ、Unixエポックからの秒数）
