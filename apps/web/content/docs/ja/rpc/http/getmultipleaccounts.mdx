---

title: getMultipleAccounts
hideTableOfContents: true
h1: getMultipleAccounts RPCメソッド

---


複数のPubkeyに対するアカウント情報を返します。

<APIMethod>

```jsonc !!request curl
{
  "jsonrpc": "2.0",
  "id": 1,
  "method": "getMultipleAccounts",
  "params": [
    // !hover(1:4) 0
    [
      "vines1vzrYbzLMRdu58ou5XTby4qAqVRLmqo36NKPTg",
      "4fYNw3dojWmQ4dXtSGE9epjRGy9pFSx62YypT7avPYvA"
    ],
    // !hover(1:3) 1
    {
      // !hover encoding
      "encoding": "base58",
      // !hover commitment
      "commitment": "finalized"
    }
  ]
}
```

```ts !!request title="Kit"
import { address, createSolanaRpc } from "@solana/kit";

const rpc_url = "https://api.devnet.solana.com";
const rpc = createSolanaRpc(rpc_url);

// !hover(1:4) 0
let addresses = [
  address("vines1vzrYbzLMRdu58ou5XTby4qAqVRLmqo36NKPTg"),
  address("4fYNw3dojWmQ4dXtSGE9epjRGy9pFSx62YypT7avPYvA")
];

// !hover(1:3) 1
let config = {
  // !hover encoding
  encoding: "base58",
  // !hover commitment
  commitment: "finalized"
};

let accounts = await rpc.getMultipleAccounts(addresses, config).send();

console.log(accounts);
```

```ts !!request title="web3.js"
import {
  Connection,
  PublicKey,
  clusterApiUrl,
  type GetMultipleAccountsConfig
} from "@solana/web3.js";

const connection = new Connection(clusterApiUrl("devnet"), "confirmed");

// !hover(1:4) 0
let addresses = [
  new PublicKey("vines1vzrYbzLMRdu58ou5XTby4qAqVRLmqo36NKPTg"),
  new PublicKey("4fYNw3dojWmQ4dXtSGE9epjRGy9pFSx62YypT7avPYvA")
];

// !hover(1:3) 1
let config: GetMultipleAccountsConfig = {
  // !hover commitment
  commitment: "finalized"
};

let accounts = await connection.getMultipleAccountsInfo(addresses, config);

console.log(accounts);
```

```rs !!request title="Rust"
use anyhow::Result;
use solana_account_decoder::UiAccountEncoding;
use solana_client::{nonblocking::rpc_client::RpcClient, rpc_config::RpcAccountInfoConfig};
use solana_sdk::{commitment_config::CommitmentConfig, pubkey};

#[tokio::main]
async fn main() -> Result<()> {
    let client = RpcClient::new_with_commitment(
        String::from("https://api.devnet.solana.com"),
        CommitmentConfig::confirmed(),
    );

    // !hover(1:4) 0
    let addresses = [
        pubkey!("vines1vzrYbzLMRdu58ou5XTby4qAqVRLmqo36NKPTg"),
        pubkey!("4fYNw3dojWmQ4dXtSGE9epjRGy9pFSx62YypT7avPYvA"),
    ];

    // !hover(1:6) 1
    let config = RpcAccountInfoConfig {
        // !hover encoding
        encoding: UiAccountEncoding::Base58.into(),
        // !hover dataSlice
        data_slice: None,
        // !hover commitment
        commitment: CommitmentConfig::finalized().into(),
        // !hover minContextSlot
        min_context_slot: None,
    };

    let accounts = client
        .get_multiple_accounts_with_config(&addresses, config)
        .await?;

    println!("{:#?}", accounts);

    Ok(())
}
```

### !params

#### !! 0

!type array
!required

クエリするPubkeyの配列（base-58エンコードされた文字列として）（最大100個まで）

#### !! 1

!type object

以下のフィールドを含む設定オブジェクト：

##### !! commitment

!type string

コミットメントは、その時点でブロックがどの程度確定しているかを示します。[ステートコミットメントの設定](/docs/rpc#configuring-state-commitment)を参照してください。

##### !! minContextSlot

!type number

リクエストが評価される最小のslot

##### !! dataSlice

!type object

アカウントデータのスライスをリクエストします。

- `length: <usize>` - 返すバイト数
- `offset: <usize>` - 読み取りを開始するバイトオフセット

<Callout type="info">
  データスライシングは `base58`、`base64`、または `base64+zstd`
  エンコーディングでのみ利用可能です。
</Callout>

##### !! encoding

!type string
!values jsonParsed base58 base64 base64+zstd
!default base64

返されるアカウントデータのエンコーディング形式

- `base58` は処理が遅く、アカウントデータが129バイト未満に制限されています。
- `base64`
  はどのサイズのアカウントデータでもbase64エンコードされたデータを返します。
- `base64+zstd`
  は[Zstandard](https://facebook.github.io/zstd/)を使用してアカウントデータを圧縮し、その結果をbase64エンコードします。
- `jsonParsed`
  エンコーディングはプログラム固有の状態パーサーを使用して、より読みやすく明示的なアカウント状態データを返そうとします。
- `jsonParsed` が要求されたがパーサーが見つからない場合、フィールドは `base64`
  エンコーディングにフォールバックします。これは `data` フィールドが `<string>`
  型の場合に検出できます。

### !!result

```jsonc !response
{
  "jsonrpc": "2.0",
  "result": {
    "context": { "apiVersion": "2.0.15", "slot": 341197247 },
    // !hover(1:18) result
    "value": [
      {
        // !hover data
        "data": ["", "base58"],
        // !hover executable
        "executable": false,
        // !hover lamports
        "lamports": 88849814690250,
        // !hover owner
        "owner": "11111111111111111111111111111111",
        // !hover rentEpoch
        "rentEpoch": 18446744073709551615,
        // !hover space
        "space": 0
      },
      {
        "data": ["", "base58"],
        "executable": false,
        "lamports": 998763433,
        "owner": "2WRuhE4GJFoE23DYzp2ij6ZnuQ8p9mJeU6gDgfsjR4or",
        "rentEpoch": 18446744073709551615,
        "space": 0
      }
    ]
  },
  "id": 1
}
```

!type array

結果は以下のいずれかを含む配列になります：

- `null` - そのpubkeyのアカウントが存在しない場合、または
- 以下のフィールドを持つアカウントオブジェクト：

##### !! data

!type [string,encoding] | object

アカウントに関連付けられたデータ。エンコードされたバイナリデータまたはJSON形式
`{<program>: <state>}` - エンコーディングパラメータによって異なります

##### !! executable

!type bool

アカウントがプログラムを含むかどうかを示すブール値（含む場合は厳密に読み取り専用）

##### !! lamports

!type u64

このアカウントに割り当てられたlamportの数

##### !! owner

!type string

このアカウントが割り当てられたプログラムのBase-58エンコードされたpubkey

##### !! rentEpoch

!type u64

このアカウントが次にrentを支払うepoch

##### !! space

!type u64

アカウントのデータサイズ

</APIMethod>
