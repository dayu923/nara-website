...
InstructionsはNara Solanaブロックチェーンとやり取りするための基本的な構成要素です。instructionは本質的に、Nara Solanaネットワークを使用する誰もが呼び出せる公開関数です。各instructionは特定のアクションを実行するために使用されます。instructionsの実行ロジックは[プログラム](/docs/core/programs)に格納され、各プログラムは独自のinstructionsセットを定義します。Nara Solanaネットワークとやり取りするには、1つ以上のinstructionsを[トランザクション](/docs/core/transactions)に追加し、処理のためにネットワークに送信します。

## NSO送金の例

以下の図は、トランザクションとinstructionsがどのように連携してユーザーがネットワークとやり取りできるようにするかを示しています。この例では、NSOがあるアカウントから別のアカウントに送金されます。

送信者アカウントの[メタデータ](#account-metadata)は、トランザクションに署名する必要があることを示しています。（これによりSystem
Programが[lamport](/docs/references/terminology#lamport)を差し引くことができます。）送信者と受信者の両方のアカウントは、lamportの残高を変更するために書き込み可能である必要があります。このinstructionを実行するために、送信者のウォレットは[署名](/docs/references/terminology#signature)とNSO送金instructionを含むメッセージを含むトランザクションを送信します。

![NSO送金図](/assets/docs/core/transactions/sol-transfer.svg)

トランザクションが送信された後、System
Programは送金instructionを処理し、両方のアカウントのlamport残高を更新します。

![NSO送金処理図](/assets/docs/core/transactions/sol-transfer-process.svg)
...
以下の例は、NSO転送instructionの構造を示しています。
