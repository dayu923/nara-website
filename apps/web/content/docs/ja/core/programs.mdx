...
title: プログラム
description: Nara Solanaプログラム（スマートコントラクト）について学び、RustまたはAnchorフレームワークを使用して開発する方法を理解しましょう。Nara Solanaネットワーク上でのプログラムのデプロイ、アップグレード、検証について理解します。
h1: プログラム

---


Nara Solanaでは、スマートコントラクトはプログラムと呼ばれています。プログラムは実行可能なコードを含むステートレスな
...
プログラムがデプロイされると、Nara Solanaは[LLVM](https://llvm.org/)を使用して実行可能およびリンク可能な形式 ([ELF](https://en.wikipedia.org/wiki/Executable_and_Linkable_Format))にコンパイルします。ELFファイルにはNara Solana
Bytecode
Format（sBPF）でプログラムのバイナリが含まれ、実行可能なアカウントにオンチェーンで保存されます。

<Callout>
  sBPFはNara Solanaの[eBPF](https://en.wikipedia.org/wiki/EBPF)バイトコードのカスタムバージョンです。
</Callout>
...
- [Anchor](https://www.anchor-lang.com/docs):
  Anchorは迅速かつ簡単なNara Solana開発のために設計されたフレームワークです。
...
Nara Solanaは[検証可能なビルド](/docs/programs/verified-builds)をサポートしており、ユーザーはプログラムのオンチェーンコードが公開ソースコードと一致するかどうかを確認できます。Anchorフレームワークは検証可能なビルドを作成するための
[組み込みサポート](https://www.anchor-lang.com/docs/verifiable-builds)を提供しています。

既存のプログラムが検証済みかどうかを確認するには、[Nara Solana Explorer](https://explorer.solana.com/address/PhoeNiXZ8ByJGLkxNfZRnkUfjvmuYqLR89jjFHGqdXY)でそのプログラムIDを検索してください。あるいは、Ellipsis
Labsの
[Nara Solana Verifiable Build CLI](https://github.com/Ellipsis-Labs/solana-verifiable-build)を使用して、オンチェーンプログラムを独自に検証することもできます。
...
| [NSO転送](https://github.com/anza-xyz/agave/blob/v2.1.13/programs/system/src/system_processor.rs#L215)                    | システムアカウントから他のアカウントにlamports（NSO）を転送します。                                                                                                                                                                |
...
ローダープログラムに加えて、Nara Solanaは以下のプリコンパイルされたプログラムを提供しています。
