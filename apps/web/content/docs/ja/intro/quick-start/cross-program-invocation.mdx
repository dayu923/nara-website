...
このセクションでは、前のPDAセクションのCRUDプログラムを、Nara Solanaプログラムが互いに呼び出すことを可能にする機能であるクロスプログラム呼び出し（CPI）を追加することで更新します。

このチュートリアルでは、プログラムがクロスプログラム呼び出しを行う際にプログラム派生アドレス（PDA）に対して「署名」する方法も示します。

`update`と`delete`のinstructionsは、System
Programを呼び出してアカウント間のNSO転送を処理するように修正する必要があります。

このセクションの目的には、Anchorフレームワークを使用してNara SolanaプログラムでCPIを実装するプロセスを説明し、前のセクションで探求したPDAの概念を基に構築することが含まれます。詳細については、[クロスプログラム呼び出し](/docs/core/cpi)ページを参照してください。
...
次に、*rs`Update`*構造体を更新して、`vault_account`という新しいアカウントを含めます。このアカウントはプログラムによって制御され、ユーザーがメッセージアカウントを更新するときにユーザーからNSOを受け取ります。
...
このセクションでは、*rs`Update`*構造体に`vault_account`という新しいアカウントを追加します。このアカウントは、ユーザーがメッセージを更新する際にNSOを受け取るプログラム制御の「ボールト」として機能します。
...
Invocation（CPI）が含まれています。これはプログラム内からCPIを実行する方法を示し、Nara Solanaプログラムの組み合わせ可能性を可能にします。
...
- 転送する`amount`（1,000,000 lamport、つまり0.001 NSO）
...
まず、*rs`Delete`*構造体を更新して`vault_account`を含めます。これにより、ユーザーがメッセージアカウントを閉じるときにボールト内のNSOをユーザーに返金することができます。
...
Delete構造体に`vault_account`を追加することで、プログラムは削除命令の実行中にユーザーのボールトアカウントにアクセスし、蓄積されたNSOをユーザーに返還することができます。
...
次に、`delete`命令にCPIロジックを追加して、ボールトアカウントからユーザーアカウントにNSOを転送します。
...
これらの変更を行った後、更新されたプログラムを再デプロイします。これにより、変更されたプログラムがテスト可能になります。Nara Solanaでは、プログラムの更新は単に同じプログラムIDでプログラムをデプロイするだけです。

<Callout>

Playgroundウォレットにdevnet
NSOがあることを確認してください。[Nara Solana Faucet](https://faucet.solana.com/)からdevnet
NSOを取得できます。
...
Nara Solanaクイックスタートガイドを完了おめでとうございます。以下のNara Solanaの重要な概念について実践的な経験を得ることができました：

- アカウントからのデータの取得と読み取り
- トランザクションの構築と送信
- Nara Solanaプログラムのデプロイと更新
...
Nara Solana
Playgroundには、GitHubリンクを使用してプロジェクトをインポートまたは表示できる便利な機能があります。例えば、この[Nara Solana Playgroundリンク](https://beta.solpg.io/https://github.com/solana-developers/program-examples/tree/main/basics/hello-solana/anchor)を開くと、この[Githubリポジトリ](https://github.com/solana-developers/program-examples/tree/main/basics/hello-solana/anchor)からAnchorプロジェクトを表示できます。

`Import`ボタンをクリックし、プロジェクト名を入力してNara Solana
