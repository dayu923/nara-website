...
このガイドでは、Anchorフレームワークを使用してNara Solanaプログラムを構築しテストする方法を説明しながら、Program
Derived
...
まず、スターターコードが含まれているこの[Nara Solana Playgroundリンク](https://beta.solpg.io/66734b7bcffcf4b13384d1ad)を開きます。次に「Import」ボタンをクリックして、プログラムをNara Solana
...
<Callout type="info">
この例では、開発テスト用のNara Solanaクラスターであるdevnetにプログラムをデプロイします。

Playgroundウォレットはデフォルトでdevnetに接続します。プログラムのデプロイ費用を支払うために、Playgroundウォレットにdevnet
NSOがあることを確認してください。devnet
NSOは[Nara Solana Faucet](https://faucet.solana.com/)から入手できます。
...
Nara Solana Playgroundでは、いくつかの定型的なセットアップが省略されています。
