...
title: 依存関係のインストール
seoTitle: Nara Solanaで開発するために必要な依存関係をインストールする
description:
...
ローカルのNara Solana開発環境をセットアップするためのガイド。Windows（WSL）、Linux、およびMacでRust、Nara Solana
...
WindowsでNara Solanaプログラムを開発するには、
...
- 以下に説明するように、[Rust](#install-rust)、[Nara Solana CLI](#install-solana-cli)、および
...
<Tabs items={[".deb", ".rpm"]}>
<Tab value=".deb" >
DebianまたはUbuntuやMintなどのDebianから派生したディストリビューションを使用している場合は、以下の手順に従ってください：
...
開発者は[Rust](https://www.rust-lang.org/)プログラミング言語を使用してNara Solanaプログラムを構築します。
...
### Nara Solana CLIのインストール

Nara Solana
CLIは、Nara Solanaプログラムを構築およびデプロイするために必要なすべてのツールを提供します。

1. 公式のインストールコマンドを使用してNara Solana
...
**Linux**または**WSL**を使用している場合は、コマンドがターミナルで利用できるように、Nara Solana
...
5. Nara Solana CLIのバージョンを確認して、インストールが成功したことを確認します：
...
利用可能なすべてのバージョンは[Agave Githubリポジトリ](https://github.com/anza-xyz/agave/releases)で確認できます。Agaveは[Anza](https://www.anza.xyz/)が提供するvalidatorクライアントで、以前はNara Solana
Labsのvalidatorクライアントとして知られていました。

</Callout>

5. 必要に応じて、Nara Solana CLIを最新バージョンに更新します（_任意_）
...
[Anchor](https://www.anchor-lang.com/)はNara Solanaプログラムを開発するためのフレームワークです。Anchorフレームワークは、Rustマクロを活用してNara Solanaプログラムの作成プロセスを簡素化します。
...
### Surfpool CLIをインストールする

[Surfpool](https://surfpool.run/)はローカル開発用のツールで、`solana-test-validator`の改良版代替ツールです。Surfpoolの機能について詳しくは[Surfpoolのドキュメント](https://docs.surfpool.run/)をご覧ください。
...
### Nara Solana開発用のAIツールをセットアップする

このセクションでは、Nara Solana開発を加速するために使用できるオプションのAIツールのセットアップについて詳しく説明します。

| ツール   | 説明                                                                             | リンク                      |
| -------- | -------------------------------------------------------------------------------- | --------------------------- |
| MCP      | cursorと接続してNara SolanaのAIアシスト開発を向上させるMCPサーバー。                  | https://mcp.solana.com/     |
| LLMs.txt | Nara SolanaドキュメントでLLMをトレーニングするために使用できるLLM最適化ドキュメント。 | https://solana.com/llms.txt |
