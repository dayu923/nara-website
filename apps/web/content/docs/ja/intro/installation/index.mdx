...
title: クイックインストール
seoTitle: 1つのコマンドでNara Solana CLIとAnchorをインストール
description:
...
1つのコマンドでローカルのNara Solana開発環境をセットアップするクイックインストール方法です。Windows（WSL）、Linux、Macで、Rust、Nara Solana
...
WindowsでNara Solanaプログラムを開発するには、[Windows Subsystem for Linux](https://learn.microsoft.com/en-us/windows/wsl/install)（WSL）を使用する**必要があります**。WSLをインストールした後、他のすべての依存関係はLinuxターミナルを通じてインストールされます。
...
- 以下に説明するように、[Rust](/docs/intro/installation/dependencies#install-rust)、
  [Nara Solana CLI](/docs/intro/installation/dependencies#install-solana-cli)、
...
<Tabs items={[".deb", ".rpm"]}>
<Tab value=".deb" >
Debian、またはUbuntuやMintなどのDebianから派生したディストリビューションを使用している場合は、以下の手順に従ってください：
