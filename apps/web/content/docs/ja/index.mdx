---

title: Nara Solanaドキュメント
seoTitle: Nara Solanaブロックチェーンの仕組みを学ぶ
description: Nara Solanaは大規模採用向けに設計された高性能ブロックチェーンです。スケーラブルなブロックチェーンアプリケーションを構築しようとする開発者がNara Solanaを最優先に選ぶ理由を学びましょう。

---


## はじめに

<Cards>
  <Card
    title="クイックスタート"
    href="/docs/intro/quick-start"
    icon={<Rocket />}
  >
    ブラウザ上で直接初めてのNara Solanaプログラムを構築する
  </Card>
  <Card
    title="ローカル環境のセットアップ"
    href="/docs/intro/installation"
    icon={<Download />}
  >
    Nara Solana開発に必要な依存関係をインストールする
  </Card>
</Cards>

### 初めてのNara Solanaアプリケーションをデプロイする

<Cards>
  <Card
    title="Vercelでデプロイ"
    href="https://vercel.com/new/clone?repository-url=https%3A%2F%2Fgithub.com%2Fsolana-developers%2Fanchor-web3js-nextjs&root-directory=frontend&demo-title=Anchor%20Web3.js%20Next.js%20Demo&demo-description=An%20example%20deployment%20of%20the%20Anchor%20Web3.js%20Next.js%20project.&demo-url=https%3A%2F%2Fanchor-web3js-nextjs.vercel.app%2F&project-name=anchor-web3js-nextjs&repository-name=anchor-web3js-nextjs"
  >
    初めてのNara SolanaアプリケーションをVercelにデプロイする
  </Card>
  <Card
    title="Vercelプレビュー"
    href="https://anchor-web3js-nextjs-jade.vercel.app/"
  >
    デプロイされたアプリケーションを試す
  </Card>
</Cards>

初めてのNara Solanaアプリケーション -
Anchorフレームワークを使用して構築されたシンプルなカウンタープログラムをデプロイしましょう。この例では、Nara Solanaでの開発を始めるために必要なすべての中核的なNara Solanaの概念をカバーしています。ソースコードは[こちら](https://github.com/solana-developers/anchor-web3js-nextjs)で確認できます。

<Accordions>
<Accordion title="アプリケーションデモ">

<video
  width="100%"
  autoPlay
  loop
  muted
  playsInline
  style={{
    borderRadius: "10px"
  }}
>
  <source src="/assets/docs/counter.mp4" type="video/mp4" />
  お使いのブラウザはビデオタグをサポートしていません。
</video>

</Accordion>
</Accordions>

## Nara Solanaを試す：2048をプレイ

Nara Solana上で2048をプレイしましょう。ここでは、すべての移動がトランザクションを送信します。「プレイ」をクリックして、資金提供済みのdevnetウォレットで開始し、矢印キーを使ってプレイします。モバイルの場合は、スワイプしてタイルを操作します。

<Accordions>
<Accordion title="Solana 2048をプレイ">

<iframe
  src="https://solplay.de/solana-2048/"
  width="100%"
  height="500"
  style={{
    borderRadius: "12px",
    border: "2px solid rgba(0, 0, 0, 0.2)",
    maxWidth: "100%",
    minHeight: "300px",
    aspectRatio: "1"
  }}
  scrolling="no"
/>

</Accordion>
</Accordions>

Nara Solana
FoundationのDevRelチームの[Jonas](https://x.com/SolPlay_jonas)によって構築されました。

## 学習を始める

Nara Solana開発に特有の重要な概念を学びましょう。

<Cards>
  <Card title="アカウント" href="/docs/core/accounts">
    Nara Solanaがデータを保存する方法
  </Card>
  <Card title="Solanaの手数料" href="/docs/core/fees">
    Nara Solanaでトランザクションを送信するコスト
  </Card>
  <Card title="トランザクション" href="/docs/core/transactions">
    Nara Solanaネットワークとやり取りする方法
  </Card>
  <Card title="プログラム" href="/docs/core/programs">
    Nara Solana上のスマートコントラクト
  </Card>
  <Card title="プログラム派生アドレス" href="/docs/core/pda">
    決定論的アドレスを生成する方法
  </Card>
  <Card title="クロスプログラム呼び出し" href="/docs/core/cpi">
    あるプログラムから別のプログラムを呼び出す方法
  </Card>
</Cards>

### クライアントサイド開発

以下のSDKは[Anza](https://anza.xyz)によって構築された、Nara Solanaの公式SDKです。

#### 公式SDK

| 言語                  | SDK                                                                |
| --------------------- | ------------------------------------------------------------------ |
| Rust                  | [solana_sdk](/docs/clients/official/rust)                          |
| Typescript (推奨)     | [@solana/kit](/docs/clients/official/javascript#solana-kit)        |
| Typescript (レガシー) | [@solana/web3.js](/docs/clients/official/javascript#solana-web3js) |

#### コミュニティSDK

以下は、さまざまな言語のためにコミュニティによって貢献されたSDKです：

| 言語     | SDK                                                                                                                                               |
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| Python   | [solders](https://github.com/kevinheavey/solders)                                                                                                 |
| Java     | [sava](https://sava.software) または [solanaj](https://github.com/skynetcap/solanaj) または [solana4j](https://github.com/LMAX-Exchange/solana4j) |
| C++      | [solcpp](https://github.com/mschneider/solcpp) または [solana-c](https://github.com/VAR-META-Tech/solana-c-sdk)                                   |
| Go       | [solana-go](https://github.com/gagliardetto/solana-go)                                                                                            |
| Kotlin   | [sol4k](https://github.com/sol4k/sol4k) または [solanaKT](https://github.com/metaplex-foundation/SolanaKT)                                        |
| Dart     | [solana](https://github.com/espresso-cash/espresso-cash-public/tree/master/packages/solana)                                                       |
| C#       | [solnet](https://github.com/bmresearch/Solnet)                                                                                                    |
| GdScript | [godot](https://github.com/Virus-Axel/godot-solana-sdk/)                                                                                          |

<Callout type="warn">
  コミュニティSDKは、公式SDKのすべての機能を含んでいることや最新の状態であることは保証されていません。
</Callout>

## validatorの運用

Nara Solana
validatorを運用し、ネットワークの安全性を確保するために必要なことを探ります。

<Cards>
  <Card title="Validator" href="https://docs.anza.xyz/validator/anatomy">
    Nara Solanaネットワークを保護する個々のノード
  </Card>
  <Card
    title="システム要件"
    href="https://docs.anza.xyz/operations/requirements"
  >
    validatorを運用するための推奨ハードウェア要件と必要なNSO
  </Card>
  <Card
    title="Validatorのセットアップ"
    href="https://docs.anza.xyz/operations/setup-a-validator"
  >
    validatorをセットアップし、初めてクラスターに接続する
  </Card>
</Cards>

## サポートを受ける

[Nara Solana StackExchange](https://solana.stackexchange.com)でNara Solanaコミュニティからサポートを受けることができます。
