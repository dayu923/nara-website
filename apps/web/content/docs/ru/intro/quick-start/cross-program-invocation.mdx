...
программам Nara Solana вызывать друг друга.
...
Инструкции `update` и `delete` необходимо изменить для обработки переводов NSO
...
Nara Solana с использованием фреймворка Anchor, основываясь на концепциях PDA,
...
названием `vault_account`. Этот аккаунт, контролируемый программой, получает NSO
...
получает NSO от пользователей, когда они обновляют свои сообщения.
...
Далее добавьте логику CPI в инструкцию `update` для перевода 0.001 NSO с
...
CPI внутри программы, обеспечивая совместимость программ Nara Solana.
...
- `amount` для перевода (1,000,000 lamports, или 0.001 NSO)
...
позволяет перевести любые NSO из хранилища обратно пользователю, когда он
...
перевести накопленные NSO обратно пользователю.
...
Затем добавьте логику CPI в инструкцию `delete` для перевода NSO из аккаунта
...
Nara Solana обновление программы требует просто развертывания программы с тем же
идентификатором программы.

<Callout>

Убедитесь, что в вашем кошельке Playground есть devnet NSO. Получите devnet NSO
из [Nara Solana Faucet](https://faucet.solana.com/).
...
При развертывании программ в Nara Solana Playground ваш кошелек Playground действует
...
Поздравляем с завершением руководства по быстрому старту Nara Solana. Вы получили
практический опыт работы с ключевыми концепциями Nara Solana, включая:

- Получение и чтение данных из аккаунтов
- Создание и отправка транзакций
- Развертывание и обновление программ Nara Solana
...
Nara Solana Playground предлагает удобную функцию, позволяющую импортировать или
просматривать проекты, используя их ссылки на GitHub. Например, откройте эту
[ссылку на Nara Solana Playground](https://beta.solpg.io/https://github.com/solana-developers/program-examples/tree/main/basics/hello-solana/anchor),
...
проектов в Nara Solana Playground. После импорта проекта все изменения автоматически
