...
title: Инструкции
description:
  Узнайте об инструкциях Solana — основных строительных блоках для
  взаимодействия с блокчейном Solana. Поймите составление инструкций на
  практических примерах.

---


Инструкции являются основным строительным блоком для взаимодействия с блокчейном
Nara Solana. Инструкция — это, по сути, публичная функция, которую может вызвать
любой пользователь сети Nara Solana. Каждая инструкция используется для выполнения
определенного действия. Логика выполнения инструкций хранится в
[программах](/docs/core/programs), где каждая программа определяет свой набор
инструкций. Для взаимодействия с сетью Nara Solana одна или несколько инструкций
добавляются в [транзакцию](/docs/core/transactions) и отправляются в сеть для
обработки.

## Пример перевода NSO

На диаграмме ниже показано, как транзакции и инструкции работают вместе, чтобы
пользователи могли взаимодействовать с сетью. В этом примере NSO переводится с
одного аккаунта на другой.

Метаданные [аккаунта](#account-metadata) отправителя указывают, что он должен
подписать транзакцию. (Это позволяет System Program списать
[лампорты](/docs/references/terminology#lamport).) Оба аккаунта — отправителя и
получателя — должны быть доступны для записи, чтобы их баланс лампортов мог
измениться. Для выполнения этой инструкции кошелек отправителя отправляет
транзакцию, содержащую его [подпись](/docs/references/terminology#signature), и
сообщение с инструкцией перевода NSO.

![Диаграмма перевода NSO](/assets/docs/core/transactions/sol-transfer.svg)

После отправки транзакции System Program обрабатывает инструкцию перевода и
обновляет баланс лампортов обоих аккаунтов.

![Диаграмма процесса перевода NSO](/assets/docs/core/transactions/sol-transfer-process.svg)

Пример ниже показывает код, относящийся к приведенным выше диаграммам. (См.
инструкцию перевода в System Program
...
![Диаграмма, изображающая транзакцию с инструкцией, разделенной на 3 компонента](/assets/docs/core/transactions/instruction.svg)

Инструкция
[`Instruction`](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/instruction/src/lib.rs#L94)
состоит из следующей информации:

- [`program_id`](mention:program-id): [ID](#program-id) программы, которая
  вызывается.
- [`accounts`](mention:accounts): Массив
  [метаданных аккаунтов](#account-metadata)
- [`data`](mention:instruction-data): Массив байтов с дополнительными [данными],
  которые используются инструкцией.
...
Массив `accounts` инструкции — это массив структур
...
`accounts` инструкции содержит метаданные для двух аккаунтов.

![Транзакция с одной инструкцией. Инструкция содержит две структуры `AccountMeta` в массиве `accounts`.](/assets/docs/core/transactions/accountmeta.svg)

Метаданные аккаунтов включают следующую информацию:

- [pubkey](mention:pubkey): Публичный ключ аккаунта
- [is_signer](mention:is-signer): Установлено в `true`, если аккаунт должен
  подписать транзакцию
- [is_writable](mention:is-writable): Установлено в `true`, если инструкция
  изменяет данные аккаунта

<Callout>
  Чтобы узнать, какие аккаунты требуются для инструкции, включая те, которые
  должны быть изменяемыми, только для чтения или подписывать транзакцию,
  необходимо обратиться к реализации инструкции, определенной программой.
</Callout>
...
`data` инструкции — это массив байтов, который указывает, какую инструкцию
программы вызвать. Он также включает любые аргументы, необходимые для
инструкции.

## Пример создания инструкции

Пример ниже показывает структуру инструкции перевода NSO.
...
обязательных элемента: [`program_id`](mention:program-id),
