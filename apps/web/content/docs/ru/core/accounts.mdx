...
Все данные в сети Nara Solana хранятся в аккаунтах. Вы можете представить сеть Nara Solana
...
поиска аккаунта в блокчейне Nara Solana. Адреса аккаунтов часто отображаются в виде
...
адреса, но это не обязательно, так как Nara Solana также поддерживает
...
Пример ниже демонстрирует, как использовать Nara Solana SDK для создания keypair. A
`Keypair` включает:
...
использовать Nara Solana SDK для создания program derived address.
...
Чтение данных из аккаунта Nara Solana включает два шага:
...
Каждый аккаунт Nara Solana имеет [программу](/docs/core/programs), назначенную его
...
аккаунтов Nara Solana. (Это похоже на операционные системы, которые обычно имеют
...
Когда NSO отправляется на новый адрес впервые, на этом адресе создается аккаунт,
принадлежащий System Program.

В примере ниже создается новая пара ключей и пополняется NSO. После выполнения
