...
title: Mollusk
description:
  Полное руководство по тестированию программ Solana с использованием Mollusk

---


[Mollusk](https://github.com/anza-xyz/mollusk/tree/main) — это легковесный
фреймворк для тестирования программ Nara Solana. Он предоставляет простой интерфейс
для тестирования инструкций программ Nara Solana в минимизированной среде Nara Solana
Virtual Machine (SVM). Все тестовые аккаунты должны быть явно определены, что
обеспечивает детерминированные и воспроизводимые тесты.
...
Добавьте `mollusk-svm` в качестве зависимости в `Cargo.toml`:
...
Для оценки использования вычислительных единиц добавьте `mollusk-svm-bencher` в
качестве зависимости в `Cargo.toml`:
...
Для использования token program, token2022 program (расширений токенов) и
associated token program для тестирования с Mollusk добавьте
`mollusk-svm-programs-token` в качестве зависимости в `Cargo.toml`:
...
Следующий пример демонстрирует минимальную настройку для тестирования базовой
программы Nara Solana с использованием Mollusk.

<ScrollyCoding>

## !!steps Программа Hello World

Этот пример демонстрирует, как протестировать базовую программу Nara Solana с
использованием Mollusk. Программа просто выводит "Hello, world!" в логи
программы при вызове.

Запуск `cargo build-sbf` генерирует скомпилированную программу в
...
Для тестирования программы Nara Solana с использованием Mollusk:

1. [Создайте экземпляр `Mollusk`](mention:mollusk) - Инициализируйте Mollusk с
   ID программы и путем к скомпилированной программе (файл `.so`)
2. [Создайте инструкцию](mention:instruction) - Создайте инструкцию для вызова
   программы
3. [Обработайте и проверьте](mention:process) - Обработайте инструкцию с
   использованием Mollusk и проверьте результат
...
предоставляет простой интерфейс для тестирования программ Nara Solana. Все поля можно
изменять с помощью нескольких вспомогательных методов, но пользователи также
могут напрямую получать доступ к ним и изменять их для большего контроля.
...
<Callout type="info">
  При указании пути к файлу не включайте расширение `.so`. Например,
  `"path/to/my_program"` корректно, а `"path/to/my_program.so"` нет.
</Callout>
...
| [`process_and_validate_instruction`](#single-instruction-with-checks)          | Обрабатывает инструкцию и выполняет серию проверок результата, завершая выполнение при ошибке.                       |
| [`process_instruction_chain`](#multiple-instructions)                          | Обрабатывает несколько инструкций и возвращает результат.                                                            |
| [`process_and_validate_instruction_chain`](#multiple-instructions-with-checks) | Обрабатывает несколько инструкций и выполняет серию проверок для каждого результата, завершая выполнение при ошибке. |
...
проверок результата. Вы можете вручную проверить результаты после обработки.
...
Следующий пример обрабатывает инструкцию перевода NSO без проверок результата.

<Callout type="info">

Примеры ниже запускают Mollusk в функции `main` для демонстрационных целей. На
практике вы обычно будете использовать Mollusk в тестовом модуле, аннотированном
атрибутом `#[test]`.
...
#### Одиночная инструкция с проверками

Используйте метод `process_and_validate_instruction` для обработки одной
инструкции с проверками валидации. Этот метод вызовет панику, если любая
проверка не пройдет.
...
Следующий пример обрабатывает инструкцию перевода NSO с проверками валидации.
...
Следующий пример обрабатывает две инструкции перевода NSO без проверок
валидации.
...
нескольких инструкций с проверками валидации после каждой инструкции. Каждая
инструкция имеет свой набор проверок, которые должны быть пройдены.
...
Следующий пример обрабатывает цепочку из двух инструкций перевода NSO с
проверками валидации после каждой инструкции.
...
Используйте следующие методы для проверки результатов инструкций:
...
### Постоянное состояние аккаунта

[`MolluskContext`](https://docs.rs/mollusk-svm/0.5.0/mollusk_svm/struct.MolluskContext.html)
— это оболочка вокруг `Mollusk`, которая поддерживает состояние аккаунта в
нескольких вызовах инструкций через `account_store`. Методы для обработки
инструкций идентичны `Mollusk`.
...
внутренне через `account_store`. Это устраняет необходимость в параметре
`accounts` при обработке инструкций.
...
Следующий пример обрабатывает две отдельные инструкции перевода NSO с постоянным
состоянием аккаунта между инструкциями через `account_store`.
...
Mollusk предоставляет пользовательскую
[структуру `Sysvars`](https://docs.rs/mollusk-svm/0.5.0/mollusk_svm/sysvar/struct.Sysvars.html)
для изменения её значений в тестировании.

Используйте метод `warp_to_slot` для обновления системной переменной времени,
чтобы симулировать перемещение вперёд или назад во времени до определённого
слота.
...
Следующий пример показывает, как напрямую изменить системную переменную Mollusk,
обратившись к полю `sysvars` для изменения параметров rent. Вы можете
аналогичным образом изменить другие значения системных переменных.
...
Результаты записываются в файл в формате markdown.

Требуется
[`mollusk-svm-bencher`](https://docs.rs/mollusk-svm-bencher/0.5.0/mollusk_svm_bencher/index.html)
в качестве зависимости.

Следующий пример измеряет использование вычислительных единиц инструкцией
перевода NSO.
...
Результаты бенчмарка записываются в указанный `out_dir` в виде файла markdown с
...
crate для добавления token program, token2022 program (расширения токенов) и
associated token program в Mollusk для тестирования.
...
Следующий пример демонстрирует тестирование перевода токенов с использованием
Mollusk.

<Callout type="info">
  Пример ниже вручную определяет тестовые аккаунты для демонстрационных целей.
  `mollusk-svm-programs-token` также включает вспомогательные функции для
  создания mint и token аккаунтов.
</Callout>
