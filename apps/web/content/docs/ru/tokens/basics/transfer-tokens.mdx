...
description: Узнайте, как переводить токены между токен-аккаунтами.
...
Переводы токенов перемещают токены между токен-аккаунтами одного и того же
выпуска (mint) с использованием инструкции
...
- Оба токен-аккаунта должны содержать токены одного типа (выпуска)
- Только владелец исходного аккаунта или делегат может авторизовать переводы
