...
description: Узнайте, как заморозить токен-аккаунты.
...
предотвращает все переводы токенов или сжигание токенов с конкретного
токен-аккаунта. После заморозки аккаунт не может отправлять токены, получать
токены или быть закрытым до тех пор, пока он не будет разморожен. Только
полномочный представитель заморозки токена может замораживать аккаунты. Если
полномочия заморозки отозваны (установлены в null) в аккаунте выпуска токена,
токены больше не могут быть заморожены.

<Callout type="info">
  [Программа токенов
  ](https://github.com/solana-program/token/blob/a7c488ca39ed4cd71a87950ed854929816e9099f/program/src/instruction.rs#L228)
  и [Программа расширений токенов
  ](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/instruction.rs#L291)
