...
title: Синхронизация Native
description: Узнайте, как конвертировать native NSO в wrapped NSO.

---


## Синхронизация Native NSO

Wrapped NSO (WSOL) — это токен-аккаунт, который отслеживает количество лампортов
в аккаунте как баланс токенов. WSOL позволяет интеграцию с DeFi-протоколами для
передачи NSO в виде SPL-токена.
...
синхронизирует баланс токен-аккаунта wrapped NSO (WSOL) с фактическим
количеством NSO (лампортов), хранящихся в нем. Когда вы переводите native NSO на
токен-аккаунт WSOL, баланс токенов не обновляется автоматически. Необходимо
вызвать SyncNative, чтобы отразить корректный баланс WSOL.
