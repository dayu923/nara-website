...
title: Применение ожидаемого баланса
description:
  Узнайте, как применить ожидаемый баланс, чтобы сделать средства доступными.
...
## Как применить ожидаемый баланс к доступному балансу

Прежде чем токены можно будет переводить конфиденциально, публичный баланс
токенов должен быть преобразован в конфиденциальный баланс. Это преобразование
происходит в два этапа:

1. **Конфиденциальный ожидаемый баланс**: Сначала токены "депонируются" из
   публичного баланса в "ожидаемый" конфиденциальный баланс.
2. **Конфиденциальный доступный баланс**: Затем ожидаемый баланс "применяется" к
   доступному балансу, делая токены доступными для конфиденциальных переводов.

Этот раздел объясняет второй этап: применение ожидаемого баланса к доступному
балансу.

Когда токены "депонируются" из публичного баланса или когда токены
конфиденциально переводятся с одного токен-аккаунта на другой, токены сначала
добавляются в конфиденциальный ожидаемый баланс. Прежде чем токены можно будет
использовать для конфиденциальных переводов, ожидаемый баланс должен быть
"применен" к доступному балансу.
...
Следующая диаграмма показывает шаги, связанные с применением ожидаемого баланса
к доступному балансу:
...
Чтобы преобразовать ожидаемый баланс в доступный баланс, вызовите инструкцию
...
Crate `spl_token_client` предоставляет метод
`confidential_transfer_apply_pending_balance`, который создает и отправляет
транзакцию с инструкцией `ApplyPendingBalance`, как показано в примере ниже.

### Пример кода

Следующий пример демонстрирует, как применить конфиденциальный ожидаемый баланс
к конфиденциальному доступному балансу.
...
запуска локального валидатора у вас должен быть установлен Nara Solana CLI.
...
<Callout type="info">
  ...
  локальный валидатор. Чтобы запустить пример кода, необходимо клонировать
  программу Token Extensions из основной сети.
</Callout>
