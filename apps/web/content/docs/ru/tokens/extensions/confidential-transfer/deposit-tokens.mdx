...
description: Узнайте, как внести токены в конфиденциальное состояние.
...
1. **Конфиденциальный ожидающий баланс**: Сначала токены "депонируются" из
   публичного баланса в "ожидающий" конфиденциальный баланс.
2. **Конфиденциальный доступный баланс**: Затем ожидающий баланс "применяется" к
   доступному балансу, делая токены доступными для конфиденциальных переводов.

В этом разделе объясняется первый этап: внесение публичного баланса токенов в
конфиденциальный ожидающий баланс.

Следующая диаграмма показывает шаги, связанные с внесением токенов из публичного
баланса в конфиденциальный ожидающий баланс:
...
Чтобы преобразовать публичный баланс в конфиденциальный ожидающий баланс,
вызовите инструкцию
...
[Максимальная сумма](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/mod.rs#L21)
на одну инструкцию депозита ограничена 2^48.

Crate `spl_token_client` предоставляет метод `confidential_transfer_deposit`,
который создает и отправляет транзакцию с инструкцией `Deposit`, как показано в
примере ниже.

### Пример кода

Следующий пример демонстрирует, как внести публичный баланс токенов в
конфиденциальный ожидающий баланс.
...
запуска локального валидатора необходимо установить Nara Solana CLI.
...
<Callout type="info">
  На момент написания конфиденциальные переводы не включены в стандартном
  локальном валидаторе. Вам необходимо клонировать основную сеть Token
  Extensions Program, чтобы запустить пример кода.
</Callout>
