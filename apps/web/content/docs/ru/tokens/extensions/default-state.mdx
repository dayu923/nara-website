...
description: Узнайте, как включить DefaultAccountStateExtension.

---


## Как включить DefaultAccountStateExtension
...
устанавливает состояние по умолчанию для всех новых токен-аккаунтов, созданных
для конкретного токена (mint). При включении это расширение автоматически
инициализирует каждый новый токен-аккаунт с указанным состоянием, например,
замороженным. Это позволяет создателям токенов применять определённые правила,
такие как требование, чтобы все новые аккаунты по умолчанию были заморожены до
их явного размораживания.
