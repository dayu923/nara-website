...
позволяет mint аккаунту указывать адрес своего аккаунта метаданных. Этот
указатель может ссылаться на любой аккаунт, принадлежащий программе, реализующей
[Token Metadata Interface](https://github.com/solana-program/token-metadata/tree/main),
обеспечивая гибкость в том, где хранятся метаданные.
