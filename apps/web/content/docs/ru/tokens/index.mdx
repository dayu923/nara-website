...
title: Токены в Nara Solana
description: Узнайте о Token Programs в Nara Solana.
...
Токены в Nara Solana называются SPL
([Nara Solana Program Library](https://github.com/solana-program)) токенами.

Этот раздел охватывает основные концепции представления токенов в Nara Solana.
...
Экосистема Nara Solana включает два основных Token Programs. Исходный код обоих
...
сети (как с заменяемыми, так и с незаменяемыми). Все токены в Nara Solana фактически
...
Токены в Nara Solana уникально идентифицируются адресом
...
Для справки, вот ссылка на Nara Solana Explorer для
...
Это вводит ключевую концепцию в разработке на Nara Solana:
...
используют терминал [Nara Solana Playground](https://beta.solpg.io/) для выполнения
CLI-команд прямо в браузере без локальной установки.

Создание токенов и аккаунтов требует NSO для депозита аренды аккаунта и оплаты
комиссий за транзакции. Для новых пользователей Nara Solana Playground создайте
кошелёк Playground и выполните команду _sh`solana airdrop`_ в терминале
Playground. Вы также можете получить devnet NSO из публичного
