...
h1: Метод RPC getBlockTime

---


Возвращает предполагаемое время создания блока.

<Callout type="info">
  Каждый валидатор регулярно сообщает своё время по UTC в реестр, периодически
  добавляя временную метку к голосу за определённый блок. Время запрашиваемого
  блока рассчитывается как средневзвешенное по ставке значение временных меток
  голосов в наборе недавних блоков, записанных в реестр.
</Callout>
