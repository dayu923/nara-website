...
h1: Метод RPC getFeeForMessage

---


Получите комиссию, которую сеть взимает за определённое сообщение

<Callout type="warn" title={"Ограничение версии"}>
  Этот метод доступен только в `solana-core` версии 1.9 или новее. Пожалуйста,
  используйте [getFees](/docs/rpc/deprecated/getfees) для `solana-core` версии
  1.8 и ниже.
</Callout>
...
Сообщение, закодированное в Base-64
...
Подтверждение описывает, насколько завершён блок на данный момент. См.
...
Комиссия, соответствующая сообщению с указанным blockhash
