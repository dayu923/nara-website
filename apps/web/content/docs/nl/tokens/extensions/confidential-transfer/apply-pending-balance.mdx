...
description:
  Leer hoe je wachtend saldo kunt toepassen om fondsen beschikbaar te maken.

---


## Hoe wachtend saldo toepassen op beschikbaar saldo
...
Dit gedeelte legt de tweede fase uit: het toepassen van het wachtende saldo op
het beschikbare saldo.

Wanneer tokens worden "gestort" vanuit het openbare saldo of wanneer tokens
vertrouwelijk worden overgedragen van het ene token account naar het andere,
worden de tokens aanvankelijk toegevoegd aan het vertrouwelijke wachtende saldo.
Voordat tokens kunnen worden gebruikt voor vertrouwelijke overdrachten, moet het
wachtende saldo worden "toegepast" op het beschikbare saldo.
...
Het volgende diagram toont de stappen die betrokken zijn bij het toepassen van
het wachtende saldo op het beschikbare saldo:
...
Om een wachtend saldo om te zetten naar een beschikbaar saldo, roep de
...
De `spl_token_client` crate biedt een
`confidential_transfer_apply_pending_balance` methode die een transactie bouwt
en verzendt met de `ApplyPendingBalance` instructie, zoals gedemonstreerd in het
voorbeeld hieronder.

### Voorbeeldcode

Het volgende voorbeeld laat zien hoe je het vertrouwelijke wachtende saldo kunt
toepassen op het vertrouwelijke beschikbare saldo.
...
de Nara Solana CLI geïnstalleerd hebben om de lokale validator te starten.
