...
description:
  Leer over de Vertrouwelijke Overdracht-extensie en hoe je deze kunt gebruiken
  om optionele functies toe te voegen aan token mints en accounts.
...
Vertrouwelijke overdrachten stellen je in staat om tokens tussen token accounts
over te dragen zonder het overdrachtsbedrag bekend te maken. Dit is nuttig voor
transacties waarbij privacy belangrijk is. Alleen de overdrachtsbedragen en
token-saldi zijn privé. De token account-adressen blijven openbaar.

- [Protocol overzicht](https://www.solana-program.com/docs/confidential-balances/overview) -
...
- [Snelstartgids](https://www.solana-program.com/docs/confidential-balances#setup) -
  Setup en basis CLI-commando's
- [Vertrouwelijke saldi kookboek](https://github.com/solana-developers/Confidential-Balances-Sample) -
  Codevoorbeelden over het gebruik van de Vertrouwelijke Overdracht-extensie

### Hoe werkt het?

De Vertrouwelijke Overdracht-extensie voegt
...
toe aan het Token Extensions Program waarmee je tokens tussen accounts kunt
overdragen zonder het overdrachtsbedrag bekend te maken.
...
De basisstroom van vertrouwelijke token-overdrachten is als volgt:

1. Maak een mint account aan met de vertrouwelijke overdracht-extensie.
2. Maak token accounts aan met vertrouwelijke overdracht-extensie voor de
   verzender en ontvanger.
3. Mint tokens naar het account van de verzender.
4. **Stort** het openbare saldo van de verzender naar het **vertrouwelijke
   wachtende saldo**.
5. **Pas** het wachtende saldo van de verzender toe op het **vertrouwelijke
   beschikbare saldo**.
6. Draag vertrouwelijk tokens over van het token account van de verzender naar
   het token account van de ontvanger.
7. **Pas** het wachtende saldo van de ontvanger toe op het **vertrouwelijke
   beschikbare saldo**.
8. **Neem** het vertrouwelijke beschikbare saldo van de ontvanger **op** naar
   het **openbare saldo**.

Voor meer details over de stappen in de vertrouwelijke overdrachtsflow, zie de
...
<Cards>
  ...
  >
    Hoe je een mint account aanmaakt met de Vertrouwelijke Overdracht-extensie
  </Card>
  <Card
    title="Token account aanmaken"
    href="/docs/tokens/extensions/confidential-transfer/create-token-account"
  >
    Hoe je een token account configureert met de Vertrouwelijke
    Overdracht-extensie
  </Card>
  <Card
  ...
  >
    Hoe je tokens stort naar het vertrouwelijke wachtende saldo
  </Card>
  <Card
    title="Wachtend saldo toepassen"
    href="/docs/tokens/extensions/confidential-transfer/apply-pending-balance"
  >
    Hoe je wachtend saldo toepast op het beschikbare vertrouwelijke saldo
  </Card>
  <Card
    title="Tokens opnemen"
    href="/docs/tokens/extensions/confidential-transfer/withdraw-tokens"
  >
    Hoe je tokens opneemt van het vertrouwelijke beschikbare saldo
  </Card>
  <Card
    title="Tokens overdragen"
    href="/docs/tokens/extensions/confidential-transfer/transfer-tokens"
  >
    Hoe je vertrouwelijk tokens overdraagt tussen token accounts
  </Card>
</Cards>

Het onderstaande diagram toont een gedetailleerde sequentie van de basisstroom
voor vertrouwelijke tokenoverboekingen:
...
Het onderstaande Rust-voorbeeld toont de stappen voor vertrouwelijke
tokenoverboekingen die overeenkomen met het voorgaande diagram. De stappen zijn
voor de duidelijkheid opgedeeld in hulpfuncties.
...
Je moet de Nara Solana CLI geïnstalleerd hebben om de lokale validator te gebruiken.
...
## Confidential Transfer-instructies
...
| ----------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | Stelt mint account in voor vertrouwelijke overboekingen. Deze instructie moet in dezelfde transactie worden opgenomen als de _rs`TokenInstruction::InitializeMint`_ instructie.           |
| _rs`UpdateMint`_                    | Werkt vertrouwelijke overboekingsinstellingen bij voor een mint.                                                                                                                          |
| _rs`ConfigureAccount`_              | Stelt een token account in voor vertrouwelijke overboekingen.                                                                                                                             |
| _rs`ApproveAccount`_                | Keurt een token account goed voor vertrouwelijke overboekingen als de mint goedkeuring vereist voor nieuwe token accounts.                                                                |
| _rs`EmptyAccount`_                  | Maakt de wachtende en beschikbare vertrouwelijke saldi leeg om het sluiten van een token account mogelijk te maken.                                                                       |
| _rs`Deposit`_                       | Zet openbaar tokensaldo om in wachtend vertrouwelijk saldo.                                                                                                                               |
| _rs`Withdraw`_                      | Zet beschikbaar vertrouwelijk saldo terug naar openbaar saldo.                                                                                                                            |
| _rs`Transfer`_                      | Draagt tokens vertrouwelijk over tussen token accounts.                                                                                                                                   |
| _rs`ApplyPendingBalance`_           | Zet wachtend saldo om in beschikbaar saldo na stortingen of overboekingen.                                                                                                                |
| _rs`EnableConfidentialCredits`_     | Staat een token account toe om vertrouwelijke tokenoverboekingen te ontvangen.                                                                                                            |
| _rs`DisableConfidentialCredits`_    | Blokkeert inkomende vertrouwelijke overboekingen terwijl openbare overboekingen nog steeds zijn toegestaan.                                                                               |
| _rs`EnableNonConfidentialCredits`_  | Staat een token account toe om openbare tokenoverboekingen te ontvangen.                                                                                                                  |
| _rs`DisableNonConfidentialCredits`_ | Blokkeert reguliere overboekingen om het account alleen vertrouwelijke overboekingen te laten ontvangen.                                                                                  |
| _rs`TransferWithFee`_               | Draagt tokens vertrouwelijk over tussen token accounts met een vergoeding.                                                                                                                |
| _rs`ConfigureAccountWithRegistry`_  | Alternatieve manier om token accounts te configureren voor vertrouwelijke overboekingen met behulp van een _rs`ElGamalRegistry`_ account in plaats van _rs`VerifyPubkeyValidity`_ bewijs. |

| Instructie                          | Beschrijving                                                                                                                                                                             |
| ----------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | Stelt mint account in voor vertrouwelijke overdrachten. Deze instructie moet in dezelfde transactie worden opgenomen als de _rs`TokenInstruction::InitializeMint`_ instructie.           |
| _rs`UpdateMint`_                    | Werkt de instellingen voor vertrouwelijke overdrachten voor een mint bij.                                                                                                                |
...
| _rs`EmptyAccount`_                  | Maakt de openstaande en beschikbare vertrouwelijke saldi leeg om het sluiten van een token account mogelijk te maken.                                                                    |
| _rs`Deposit`_                       | Zet openbaar token saldo om in openstaand vertrouwelijk saldo.                                                                                                                           |
| _rs`Withdraw`_                      | Zet beschikbaar vertrouwelijk saldo terug naar openbaar saldo.                                                                                                                           |
| _rs`Transfer`_                      | Draagt tokens vertrouwelijk over tussen token accounts.                                                                                                                                  |
| _rs`ApplyPendingBalance`_           | Zet openstaand saldo om in beschikbaar saldo na stortingen of overdrachten.                                                                                                              |
| _rs`EnableConfidentialCredits`_     | Staat een token account toe om vertrouwelijke token overdrachten te ontvangen.                                                                                                           |
| _rs`DisableConfidentialCredits`_    | Blokkeert inkomende vertrouwelijke overdrachten terwijl openbare overdrachten nog steeds mogelijk zijn.                                                                                  |
| _rs`EnableNonConfidentialCredits`_  | Staat een token account toe om openbare token overdrachten te ontvangen.                                                                                                                 |
| _rs`DisableNonConfidentialCredits`_ | Blokkeert reguliere overdrachten om het account alleen vertrouwelijke overdrachten te laten ontvangen.                                                                                   |
| _rs`TransferWithFee`_               | Draagt tokens vertrouwelijk over tussen token accounts met een vergoeding.                                                                                                               |
