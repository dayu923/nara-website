...
description: Leer hoe je tokens naar vertrouwelijke status kunt storten.
...
## Hoe tokens storten naar vertrouwelijk wachtend saldo

Voordat tokens vertrouwelijk kunnen worden overgedragen, moet het openbare
tokensaldo worden omgezet naar een vertrouwelijk saldo. Deze conversie gebeurt
in twee fasen:

1. **Vertrouwelijk wachtend saldo**: In eerste instantie worden tokens "gestort"
   van openbaar saldo naar een "wachtend" vertrouwelijk saldo.
2. **Vertrouwelijk beschikbaar saldo**: Het wachtende saldo wordt vervolgens
   "toegepast" op het beschikbare saldo, waardoor de tokens beschikbaar worden
   voor vertrouwelijke overdrachten.

Dit gedeelte legt de eerste fase uit: het storten van openbaar tokensaldo naar
het vertrouwelijk wachtend saldo.

Het volgende diagram toont de stappen die betrokken zijn bij het storten van
tokens van het openbare saldo naar het vertrouwelijk wachtend saldo:
...
Om een openbaar saldo om te zetten naar een vertrouwelijk wachtend saldo, roep
je de
...
instructie aan. Het
[maximale bedrag](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/mod.rs#L21)
per stortingsinstructie is beperkt tot 2^48.

De `spl_token_client` crate biedt een `confidential_transfer_deposit` methode
die een transactie bouwt en verzendt met de `Deposit` instructie, zoals
gedemonstreerd in het onderstaande voorbeeld.

### Voorbeeldcode

Het volgende voorbeeld laat zien hoe je openbaar tokensaldo naar vertrouwelijk
wachtend saldo kunt storten.
...
<Callout type="info">
  Op het moment van schrijven zijn de vertrouwelijke overdrachten niet
  ingeschakeld op de standaard lokale validator. Je moet het Token Extension
  Program van het mainnet klonen om de voorbeeldcode uit te voeren.
</Callout>
