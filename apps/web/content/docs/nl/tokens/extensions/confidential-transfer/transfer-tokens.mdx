...
title: Tokens overmaken
description:
  Leer hoe je tokens privé van de ene token account naar de andere kunt
  overmaken.
...
<Callout type="warn">
  ...
  terwijl het een beveiligingsaudit ondergaat. Dit betekent dat de
  vertrouwelijke overdrachtsextensie momenteel niet beschikbaar is. Hoewel de
  concepten nog steeds geldig zijn, zullen de code voorbeelden niet werken.
</Callout>

## Hoe tokens vertrouwelijk overmaken van de ene token account naar de andere

Om tokens vertrouwelijk over te maken van de ene token account naar de andere,
moeten zowel de verzender als de ontvanger token accounts hebben die
geconfigureerd zijn met de _rs`ConfidentialTransferAccount`_ status en
goedgekeurd zijn voor vertrouwelijke overdrachten. De token account van de
verzender moet ook een beschikbaar vertrouwelijk saldo hebben om van over te
maken.

Om tokens vertrouwelijk over te maken:

1. Creëer
   [drie bewijzen](https://github.com/solana-program/token-2022/blob/d073cb89dbcd430387b5c6fb4b7157911351e4a3/confidential-transfer/proof-generation/src/transfer.rs#L35)
   aan de clientzijde:
   ...
   **Gelijkheidsbewijs
   ...
   Verifieert dat de nieuwe beschikbare saldo-ciphertekst na de overdracht
   overeenkomt met de bijbehorende
   [Pedersen commitment](https://en.wikipedia.org/wiki/Commitment_scheme), wat
   ervoor zorgt dat het nieuwe beschikbare saldo van de bronaccount correct
   wordt berekend als `new_balance = current_balance - transfer_amount`.

   **Ciphertekst Validiteitsbewijs
   ...
   Verifieert dat de cipherteksten van het overdrachtsbedrag correct zijn
   gegenereerd voor alle drie de partijen (bron, bestemming en auditor), wat
   ervoor zorgt dat het overdrachtsbedrag correct is versleuteld onder de
   publieke sleutel van elke partij.
   ...
   **Bereikbewijs
   ...
   Verifieert dat het nieuwe beschikbare saldo en overdrachtsbedrag (opgesplitst
   in lage/hoge bits) allemaal niet-negatief zijn en binnen een gespecificeerd
   bereik vallen.

2. Voor elk bewijs:
   - Roep het ZK ElGamal bewijsprogramma aan om de bewijsgegevens te verifiëren.
   - Sla de bewijsspecifieke metadata op in een bewijs "context state" account
     om te gebruiken in andere instructies.
...
3. Roep de
   ...
   instructie aan, waarbij je de bewijs context state accounts verstrekt.

4. Sluit de proof context state accounts om de NSO te herstellen die gebruikt is
   om ze aan te maken.

Het volgende diagram toont de stappen die betrokken zijn bij het overmaken van
tokens van een token account van de verzender naar een token account van de
ontvanger.
...
Om vertrouwelijk tokens over te maken van het ene token account naar het andere,
moet je:

- Een gelijkheidsbewijs, ciphertext-validatiebewijs en bereikbewijs genereren
  aan de clientzijde
- Het Zk ElGamal proof programma aanroepen om de bewijzen te verifiëren en de
  "context state" accounts te initialiseren
- De
  [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604)
  instructie aanroepen en daarbij de drie proof accounts verstrekken.
- De drie proof accounts sluiten om de rent terug te krijgen.

De `spl_token_client` crate biedt de volgende methoden:

- `confidential_transfer_create_context_state_account` methode die een proof
  account aanmaakt.
- `confidential_transfer_transfer` methode die de `Transfer` instructie
  aanroept.
- `confidential_transfer_close_context_state_account` methode die een proof
  account sluit.

### Voorbeeldcode

Het volgende voorbeeld laat zien hoe je vertrouwelijk tokens kunt overmaken
tussen token accounts.

Om het voorbeeld uit te voeren, start je een lokale validator met het Token
Extensions Program gekloond van het mainnet met behulp van het volgende
commando. Je moet de Nara Solana CLI geïnstalleerd hebben om de lokale validator te
starten.
...
<Callout type="info">
  ...
  op de standaard lokale validator. Je moet het Token Extensions Program van het
