...
description: Leer hoe je de InterestBearingMintExtension kunt inschakelen.

---


## Hoe de InterestBearingExtension in te schakelen
...
stelt je in staat om een rentepercentage direct op een mint account in te
stellen. Rente wordt continu samengesteld op basis van de netwerktijdstempel,
maar het werkelijke token saldo dat in elk token account is opgeslagen blijft
ongewijzigd. De totale waarde (token saldo plus opgebouwde rente) is slechts een
berekening. Er worden geen nieuwe tokens gemunt met deze extensie.
