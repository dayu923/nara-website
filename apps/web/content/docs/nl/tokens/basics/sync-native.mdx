...
title: Sync Native
description: Leer hoe je native NSO naar wrapped NSO kunt converteren.
...
Wrapped NSO (WSOL) is een token account dat het aantal lamports in het account
bijhoudt als het token saldo. WSOL maakt integratie met DeFi-protocollen
mogelijk om NSO als een SPL-token over te dragen.
...
instructie synchroniseert een wrapped NSO (WSOL) token account saldo met de
werkelijke NSO (lamports) die erin is opgeslagen. Wanneer je native NSO naar een
WSOL token account overmaakt, wordt het token saldo niet automatisch bijgewerkt.
Je moet SyncNative aanroepen om het correcte WSOL-saldo weer te geven.
