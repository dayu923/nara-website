...
De volgende termen worden gebruikt in de Nara Solana-documentatie en het
ontwikkelingsecosysteem.

## account

Een record in het Nara Solana-grootboek dat ofwel gegevens bevat of een uitvoerbaar
programma is.

Net als een rekening bij een traditionele bank kan een Nara Solana-account fondsen
...
Een front-end applicatie die communiceert met een Nara Solana-cluster.
...
Een unieke waarde ([hash](#hash)) die een record (block) identificeert. Nara Solana
...
Het Nara Solana programma dat [BPF](/docs/core/programs#berkeley-packet-filter-bpf)
[onchain programma's](#onchain-program) bezit en laadt, waardoor het programma
kan communiceren met de runtime.

## client

Een computerprogramma dat toegang heeft tot het Nara Solana server netwerk
[cluster](#cluster).
...
Ook wel aangeduid als `transaction cost`, is een uitgebreide schatting van alle
...
validatie te financieren en om de voortdurende ontwikkeling van Nara Solana te
bekostigen.
...
De uitvoerbare code op de Nara Solana blockchain die de [instructies](#instruction)
...
Nara Solana's parallelle runtime voor [onchain programs](#onchain-program).
...
het bereik van `0 <= S < L`. Deze vereiste zorgt ervoor dat er geen handtekening
manipulatie mogelijk is. Elke transactie moet ten minste één handtekening hebben
voor [fee account](#fee-account). Daarom kan de eerste handtekening in een
...
Een voorbije [slot](#slot) dat geen [block](#block) heeft geproduceerd, omdat de
leader offline was of de [fork](#fork) die de slot bevatte werd verlaten voor
een betere alternatief door cluster consensus. Een overgeslagen slot zal niet
verschijnen als een voorouder voor blokken in volgende slots, noch de
...
De [native token](#native-token) van een Nara Solana [cluster](#cluster).

## Nara Solana Program Library (SPL)

Een [bibliotheek van programma's](https://spl.solana.com/) op Nara Solana zoals
...
Een volledige deelnemer in een Nara Solana-netwerk [cluster](#cluster) die nieuwe
