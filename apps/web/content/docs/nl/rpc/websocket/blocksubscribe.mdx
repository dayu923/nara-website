...
Abonneer je om een melding te ontvangen wanneer een nieuw blok `confirmed` of
`finalized` is.

<Callout type="warn" title={"Onstabiele methode"}>
  Dit abonnement wordt beschouwd als **onstabiel** en is alleen beschikbaar als
  de validator is gestart met de `--rpc-pubsub-enable-block-subscription` vlag.
  Het formaat van dit abonnement kan in de toekomst veranderen.
</Callout>
...
filtercriteria voor de logs om resultaten te ontvangen per accounttype;
momenteel ondersteund:

- `all` - bevat alle transacties in het blok
- Een JSON-object met het volgende veld:
  - `mentionsAccountOrProgram: <string>` - retourneert alleen transacties die de
    opgegeven publieke sleutel vermelden (als base-58 gecodeerde string). Als er
    geen vermeldingen zijn in een bepaald blok, dan wordt er geen melding
    verzonden.
...
coderingsformaat voor elke geretourneerde transactie

- `jsonParsed` probeert programmaspecifieke instructieparsers te gebruiken om
...
niveau van transactiedetails om terug te geven
...
Momenteel is de enige geldige waarde voor deze parameter `0`. Door deze in te
stellen op `0` kunt u alle transacties ophalen, inclusief zowel versioned als
legacy transacties.
...
waarde, wordt er een fout geretourneerd. Als u deze parameter weglaat, worden
alleen legacy transacties geretourneerd—elke versioned transactie zal resulteren
in een fout.
...
of de `rewards` array moet worden gevuld. Als de parameter niet wordt opgegeven,
worden standaard beloningen meegenomen.
...
De notificatie is een object met de volgende velden:

- `slot: <u64>` - De bijbehorende slot.
- `err: <object|null>` - Fout als er iets misging bij het publiceren van de
  notificatie, anders null.
- `block: <object|null>` - Een blokobject zoals te zien in de
