...
title: Mollusk
description: Uitgebreide gids voor het testen van Nara Solana-programma's met Mollusk

---


[Mollusk](https://github.com/anza-xyz/mollusk/tree/main) is een lichtgewicht
test framework voor het testen van Nara Solana-programma's. Het biedt een eenvoudige
interface voor het testen van Nara Solana-programma-instructies in een
geminimaliseerde Nara Solana Virtual Machine (SVM) omgeving. Alle testaccounts moeten
expliciet worden gedefinieerd, wat zorgt voor deterministische en herhaalbare
tests.

- [Mollusk SVM-documentatie](https://docs.rs/mollusk-svm/latest/mollusk_svm/)
- [Mollusk testvoorbeelden](https://github.com/anza-xyz/mollusk/tree/0.5.0/harness/tests)

## Installatie

Voeg `mollusk-svm` toe als afhankelijkheid in `Cargo.toml`:
...
Om het gebruik van rekeneenheden te benchmarken, voeg `mollusk-svm-bencher` toe
als afhankelijkheid in `Cargo.toml`:
...
Om het Token Program, Token2022 Program (Token Extensions) en Associated Token
Program te gebruiken voor testen met Mollusk, voeg `mollusk-svm-programs-token`
toe als afhankelijkheid in `Cargo.toml`:
...
Het volgende voorbeeld toont een minimale setup voor het testen van een
eenvoudig Nara Solana-programma met Mollusk.

<ScrollyCoding>

## !!steps Hello World-programma

Dit voorbeeld laat zien hoe je een eenvoudig Nara Solana-programma kunt testen met
Mollusk. Het programma print simpelweg "Hello, world!" naar de programmalogs
wanneer het wordt aangeroepen.

Het uitvoeren van `cargo build-sbf` genereert het gecompileerde programma op
...
Om een Nara Solana-programma te testen met Mollusk:

1. [Maak een `Mollusk`instantie aan](mention:mollusk) - Initialiseer Mollusk met
   een programma-ID en het pad naar het gecompileerde programma (`.so`bestand)
2. [Bouw een instructie](mention:instruction) - Maak een instructie om het
   programma aan te roepen
3. [Verwerk en valideer](mention:process) - Verwerk de instructie met Mollusk en
...
Om de test uit te voeren, gebruik `cargo test`.

Wanneer de test succesvol wordt uitgevoerd, zie je uitvoer vergelijkbaar met het
volgende:
...
struct biedt een eenvoudige interface voor het testen van Nara Solana programma's.
Alle velden kunnen worden gemanipuleerd via een handvol hulpmethoden, maar
gebruikers kunnen ze ook direct benaderen en wijzigen als ze meer controle
wensen.

Om Mollusk te initialiseren met een standaardinstantie gebruik je de
`Mollusk::default` methode.
...
Om Mollusk te initialiseren met een specifiek programma gebruik je de
`Mollusk::new` methode.
...
Om een programma toe te voegen aan Mollusk gebruik je de `Mollusk::add_program`
methode.
...
<Callout type="info">
  Bij het opgeven van het bestandspad, voeg niet de `.so` extensie toe.
  Bijvoorbeeld, `"path/to/my_program"` is correct, maar
  `"path/to/my_program.so"` is niet correct.
</Callout>
...
| ------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------- |
| [`process_instruction`](#single-instruction)                                   | Verwerk een instructie en geef het resultaat terug.                                                                    |
| [`process_and_validate_instruction`](#single-instruction-with-checks)          | Verwerk een instructie en voer een reeks controles uit op het resultaat, met een panic als een controle mislukt.       |
| [`process_instruction_chain`](#multiple-instructions)                          | Verwerk meerdere instructies en geef het resultaat terug.                                                              |
| [`process_and_validate_instruction_chain`](#multiple-instructions-with-checks) | Verwerk meerdere instructies en voer een reeks controles uit op elk resultaat, met een panic als een controle mislukt. |

De
[`InstructionResult`](https://docs.rs/mollusk-svm-result/0.5.0/mollusk_svm_result/types/struct.InstructionResult.html)
bevat de details van een verwerkte instructie.

#### Enkele instructie

Gebruik de `process_instruction` methode om een enkele instructie te verwerken
zonder controles op het resultaat. Je kunt de resultaten handmatig valideren na
verwerking.
...
Het volgende voorbeeld verwerkt een NSO overdrachts-instructie zonder validatie
controles.

<Callout type="info">

De onderstaande voorbeelden voeren Mollusk uit in de `main` functie voor
demonstratiedoeleinden. In de praktijk zul je Mollusk meestal gebruiken in een
testmodule die is geannoteerd met het `#[test]` attribuut.
...
Gebruik de `process_and_validate_instruction` methode om een enkele instructie
te verwerken met validatiecontroles. Deze methode zal crashen als een controle
mislukt.
...
Het volgende voorbeeld verwerkt een NSO-overdrachtsinstructie met
validatiecontroles.
...
Gebruik de `process_instruction_chain` methode om meerdere instructies
opeenvolgend te verwerken zonder validatiecontroles.
...
Het volgende voorbeeld verwerkt twee NSO-overdrachtsinstructies zonder
validatiecontroles.
...
Gebruik de `process_and_validate_instruction_chain` methode om meerdere
instructies te verwerken met validatiecontroles na elke instructie. Elke
instructie heeft zijn eigen set controles waaraan moet worden voldaan.
...
Het volgende voorbeeld verwerkt een reeks van twee NSO-overdrachtsinstructies
met validatiecontroles na elke instructie.
...
Mollusk biedt een reeks
[hulpmethoden](https://docs.rs/mollusk-svm-result/0.5.0/mollusk_svm_result/check/struct.Check.html)
om de resultaten van een verwerkte instructie te controleren.
...
is een wrapper rond `Mollusk` die de accountstatus behoudt tussen meerdere
instructie-aanroepen via zijn `account_store`. De methoden voor het verwerken
van instructies zijn identiek aan `Mollusk`.

In tegenstelling tot `Mollusk`, waarbij `accounts` aan elke methode moet worden
doorgegeven (bijv. `process_instruction`), beheert `MolluskContext` accounts
intern via zijn `account_store`. Hierdoor is de `accounts` parameter niet meer
nodig bij het verwerken van instructies.

Maak een `account_store` met behulp van de `with_context` methode:
...
Het volgende voorbeeld verwerkt twee afzonderlijke NSO-overdrachtsinstructies
met permanente accountstatus tussen de instructies via de `account_store`.
...
Mollusk biedt een aangepaste
[`Sysvars`](https://docs.rs/mollusk-svm/0.5.0/mollusk_svm/sysvar/struct.Sysvars.html)
struct voor het wijzigen van waarden voor testdoeleinden.

Gebruik de `warp_to_slot` methode om de sysvar-klok bij te werken om het vooruit
of achteruit gaan in de tijd naar een specifieke slot te simuleren.
...
Het volgende voorbeeld laat zien hoe je de Mollusk sysvar direct kunt wijzigen
door toegang te krijgen tot het `sysvars` veld om de rent-parameters te
wijzigen. Je kunt andere sysvar-waarden op dezelfde manier wijzigen.
...
houdt het gebruik van compute units bij van de instructies van een programma.
Resultaten worden weggeschreven naar een markdown-bestand.

Vereist
[`mollusk-svm-bencher`](https://docs.rs/mollusk-svm-bencher/0.5.0/mollusk_svm_bencher/index.html)
als afhankelijkheid.

Het volgende voorbeeld benchmarkt het compute unit gebruik van een NSO transfer
instructie.
...
De benchmark-resultaten worden weggeschreven naar het opgegeven `out_dir` als
een markdown-bestand genaamd `compute_units.md`.
...
crate om het Token Program, token2022 programma (Token Extensions) en Associated
Token Program toe te voegen aan Mollusk voor testen.
...
Het volgende voorbeeld demonstreert het testen van een token-overdracht met
Mollusk.

<Callout type="info">
  Het onderstaande voorbeeld definieert handmatig de testaccounts voor
  demonstratie- doeleinden. De `mollusk-svm-programs-token` bevat ook
  hulpfuncties om de mint- en token accounts aan te maken.
</Callout>
