...
Nara Solana-programma's worden voornamelijk ontwikkeld met de programmeertaal Rust.
Deze pagina richt zich op het schrijven van Nara Solana-programma's in Rust zonder
...
Nara Solana-programma's. Deze aanpak vereist echter meer handmatige configuratie en
...
- Rust: De programmeertaal voor het bouwen van Nara Solana-programma's.
- Nara Solana CLI: Command-line tool voor Nara Solana-ontwikkeling.

## Aan de slag

Het onderstaande voorbeeld behandelt de basisstappen om je eerste
Nara Solana-programma geschreven in Rust te maken. We zullen een minimaal programma
...
dependency die nodig is om een Nara Solana-programma te bouwen.
...
minimaal Nara Solana-programma dat "Hello, world!" naar het programmalogboek print
wanneer het programma wordt aangeroepen.

De `msg!` macro wordt in Nara Solana-programma's gebruikt om een bericht naar het
...
1. Een `.so` bestand (bijv. `hello_world.so`): Dit is het gecompileerde
   Nara Solana-programma dat naar het netwerk wordt gedeployed als een "smart
   contract".
...
Configureer eerst de Nara Solana CLI om de lokale Nara Solana cluster te gebruiken.
...
[Nara Solana Explorer](https://explorer.solana.com/?cluster=custom&customUrl=http%3A%2F%2Flocalhost%3A8899).

<Callout type="info">
  Let op dat de cluster op Nara Solana Explorer ook localhost moet zijn. De optie
  "Custom RPC URL" op Nara Solana Explorer staat standaard op
  `http://localhost:8899`.
</Callout>
...
Nara Solana-programma's kunnen worden bijgewerkt door opnieuw te implementeren naar
dezelfde programma-ID. Werk het programma in `src/lib.rs` bij om "Hello,
Nara Solana!" af te drukken in plaats van "Hello, world!".
...
Je zou "Hello, Nara Solana!" in het programmalogboek moeten zien.
...
Voer de clientcode opnieuw uit en inspecteer de transactiehandtekening op Nara Solana
Explorer om "Hello, Nara Solana!" in het programmalogboek te zien.
...
Je kunt je Nara Solana-programma sluiten om de NSO die aan het account is toegewezen
