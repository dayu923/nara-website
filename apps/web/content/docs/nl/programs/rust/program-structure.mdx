...
description:
  Leer hoe je Solana-programma's in Rust structureert, inclusief entrypoints,
...
Nara Solana-programma's geschreven in Rust hebben minimale structurele vereisten, wat
...
Nara Solana-programma's meestal een gemeenschappelijk patroon:
...
Voor de eenvoud wordt het programma geïmplementeerd in één `lib.rs`bestand,
...
Laten we eerst een nieuw Rust-project maken voor ons Nara Solana-programma.
...
Nara Solana-programma. We hebben `solana-program` nodig voor de core SDK en `borsh`
voor serialisatie.
...
<Callout type="info">
  ...
  serialisatiebibliotheek voor Nara Solana-programma's.
</Callout>

## !!steps Configureer crate-type

Nara Solana-programma's moeten worden gecompileerd als dynamische bibliotheken. Voeg
...
<Callout type="warn">
  Als je deze configuratie niet opneemt, wordt de target/deploy directory niet
  gegenereerd wanneer je het programma bouwt.
</Callout>

## !!steps Instellen van programma-entrypoint

Elk Nara Solana-programma heeft een entrypoint, dit is de functie die wordt
aangeroepen wanneer het programma wordt gestart. Laten we beginnen met het
toevoegen van de imports die we nodig hebben voor het programma en het instellen
van het entrypoint.
...
macro handelt de deserialisatie van de `input` data naar de parameters van de
`process_instruction` functie af.

Een Nara Solana-programma `entrypoint` heeft de volgende functiehandtekening.
...
## !!steps Definieer programmastatus

Laten we nu de datastructuur definiëren die in onze counter-accounts wordt
opgeslagen. Dit zijn de gegevens die worden opgeslagen in het `data` veld van
het account.
...
## !!steps Definieer instructie enum

Laten we de instructies definiëren die ons programma kan uitvoeren. We gebruiken
een enum waarin elke variant een verschillende instructie vertegenwoordigt.
...
## !!steps Implementeer instructie deserialisatie
...
Laten we nu de hoofd `process_instruction` functie updaten om instructies naar
hun bijbehorende handlefuncties te routeren.

Dit routeringspatroon is gebruikelijk in Nara Solana-programma's. De
`instruction_data` wordt gedeserialiseerd naar een variant van een enum die de
...
Voeg de volgende code toe aan `lib.rs` om de `process_instruction` functie te
updaten en de handlers voor de `InitializeCounter` en `IncrementCounter`
...
te initialiseren. Aangezien alleen het System Program accounts kan aanmaken op
Nara Solana, gebruiken we een Cross Program Invocation (CPI), waarbij we in feite een
ander programma aanroepen vanuit ons programma.
...
Voeg de volgende code toe aan `lib.rs` om de `process_initialize_counter`
...
Voeg de volgende code toe aan `lib.rs` om de `process_increment_counter` functie
bij te werken:
...
Gefeliciteerd! Je hebt een compleet Nara Solana-programma gebouwd dat de
basisstructuur toont die alle Nara Solana-programma's delen:

- **Entrypoint**: Definieert waar de programma-uitvoering begint en routeert
  alle binnenkomende verzoeken naar de juiste instructiehandlers
- **Instruction Handling**: Definieert instructies en hun bijbehorende handler
  functies
- **State Management**: Definieert accountdatastructuren en beheert hun status
  in programma-eigen accounts
- **Cross Program Invocation (CPI)**: Roept het System Program aan om nieuwe
  programma-eigen accounts te creëren

De volgende stap is het testen van het programma om te zorgen dat alles correct
werkt.
...
Laten we nu ons tellerprogramma testen. We zullen
[LiteSVM](https://github.com/LiteSVM/litesvm) gebruiken, een testframework
waarmee we programma's kunnen testen zonder ze op een cluster te implementeren.
...
`litesvm` gebruiken voor testen en `solana-sdk`.
...
## !!steps Testmodule maken

Laten we nu een testmodule aan ons programma toevoegen. We beginnen met de
...
## !!steps Testomgeving initialiseren

Laten we de testomgeving opzetten met LiteSVM en een betaalaccount financieren.

LiteSVM simuleert de Nara Solana runtime-omgeving, waardoor we ons programma kunnen
testen zonder het naar een echte cluster te implementeren.

Voeg de volgende code toe aan `lib.rs` en werk de functie `test_counter_program`
bij:
...
## !!steps Het programma laden

Nu moeten we ons programma bouwen en in de testomgeving laden. Voer het commando
`cargo build-sbf` uit om het programma te bouwen. Dit genereert het bestand
`counter_program.so` in de map `target/deploy`.
...
Werk de functie `test_counter_program` bij om het programma in de testomgeving
te laden.
...
<Callout type="warn">
  Je moet `cargo build-sbf` uitvoeren voordat je tests uitvoert om het bestand
  `.so` te genereren. De test laadt het gecompileerde programma.
</Callout>
...
## !!steps Initialisatie-instructie testen

Laten we de initialisatie-instructie testen door een nieuw counter-account met
een startwaarde aan te maken.
...
## !!steps Initialisatie verifiëren

Na initialisatie gaan we controleren of het counter-account correct is
aangemaakt met de verwachte waarde.
...
## !!steps Increment-instructie testen

Laten we nu de increment-instructie testen om er zeker van te zijn dat deze de
...
## !!steps Eindresultaten verifiëren

Tot slot gaan we controleren of de increment correct heeft gewerkt door de
...
## !!steps Client code implementeren

Nu gaan we de client implementeren die ons geïmplementeerde programma zal
aanroepen.
...
Voeg de client-code toe aan `examples/client.rs` en vervang de `program_id` met
...
Nu we ons programma en de client klaar hebben, laten we het programma bouwen,
...
Configureer eerst de Nara Solana CLI om localhost te gebruiken:
...
[Nara Solana Explorer](https://explorer.solana.com/?cluster=custom) met behulp van de
handtekeningen van de uitgevoerde transacties. Let op dat de cluster op Nara Solana
Explorer moet worden ingesteld op "Custom RPC URL", die standaard is ingesteld
op `http://localhost:8899` waarop de `solana-test-validator` draait.
