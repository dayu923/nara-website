...
willen implementeren voor hun programma's op Nara Solana. We behandelen wat
...
Nara Solana- netwerk implementeert, overeenkomt met de broncode in je repository.
...
bouwt je programma in een gecontroleerde omgeving met behulp van de Nara Solana
...
kun je het implementeren op het Nara Solana-netwerk. Tijdens het bouwproces wordt een
...
het waarschijnlijk mislukken omdat Nara Solana-programmabuilds niet deterministisch
...
### Installeer de Nara Solana Verify CLI

De Nara Solana Verify CLI is het belangrijkste hulpmiddel voor het verifiëren van
builds. Nara Solana Verify CLI wordt momenteel onderhouden door
...
Om je Nara Solana-programma verifieerbaar te bouwen, navigeer naar de map die je
...
implementeren op het Nara Solana-netwerk. Het wordt aanbevolen om een multi-signature
...
> [Nara Solana-clusters](/docs/references/clusters) (bijv. devnet, testnet, mainnet).
> Zorg ervoor dat je de juiste netwerk-URL gebruikt voor de gewenste
> Nara Solana-cluster waartegen je een programma wilt verifiëren. Verificatie op
...
onchain wilt uploaden. Als je dat doet, zal de Nara Solana Explorer onmiddellijk je
...
waarmee wordt bijgedragen aan een gezonder Nara Solana-ecosysteem.
...
in de Nara Solana explorer en andere plaatsen.
...
Het gebruik van
[geverifieerde builds op Nara Solana](/developers/guides/advanced/verified-builds)
waarborgt de integriteit en betrouwbaarheid van je programma's op het netwerk en
stelt ontwikkelaars in staat om je SDK's rechtstreeks via Nara Solana Explorer te
vinden. Door gebruik te maken van tools zoals de Nara Solana Verify CLI en Docker,
...
je Nara Solana-programma's te waarborgen, is het in de standaardopzet niet volledig
trustless. De docker images worden gebouwd en gehost door de Nara Solana Foundation.
...
De API of Nara Solana Explorer kan mogelijk onjuiste informatie weergeven als deze
gecompromitteerd is.
...
De Nara Solana Foundation, OtterSec en het Ellipsis Labs team zijn niet
...
# Security.txt voor Nara Solana programma's
...
beveiligingsinformatie rechtstreeks in hun Nara Solana smart contracts op te nemen.
...
Om een `security.txt` toe te voegen aan je Nara Solana programma, volg je de volgende
stappen:
...
gemakkelijk worden opgevraagd via tools zoals de Nara Solana Explorer, waardoor je
...
beveiliging en communicatie binnen het Nara Solana-ecosysteem.

Dit is
[een voorbeeld van hoe security.txt eruitziet in de Nara Solana Explorer](https://explorer.solana.com/address/HPxKXnBN4vJ8RjpdqDCU7gvNQHeeyGnSviYTJ4fBrDt4/security?cluster=devnet)
