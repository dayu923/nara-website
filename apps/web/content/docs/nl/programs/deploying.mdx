...
- [Nara Solana accountmodel](/core/accounts)
- [Nara Solana programma's in het algemeen](/core/programs)
- [Ontwikkelen van aangepaste Nara Solana programma's](/programs/rust/)
...
De toolchain bevatte een snelkoppeling, deze wordt echter uitgefaseerd /
afgeschaft:
...
Tijdens periodes van congestie zijn er enkele aanvullende flags die je kunt
...
- `--use-rpc`: Stuur schrijftransacties naar de geconfigureerde RPC in plaats
  van validator TPUs. Deze flag vereist een
  [stake-weighted](/developers/guides/advanced/stake-weighted-qos) RPC-
  verbinding zoals [Helius](https://www.helius.dev/) of
  [Triton](https://triton.one/). Deze flag kan ook worden geconfigureerd als
  standaard met: `solana config set --url <RPC_URL>`.
...
Bij mislukte implementaties wordt een foutmelding weergegeven met de seed-zin
...
Voer de 12-woorden seed-zin in wanneer daarom wordt gevraagd.
...
Om een enkele programdata account te sluiten:
...
Om een andere autoriteit te specificeren:
...
wordt de standaard keypair gebruikt als autoriteit. Daarom was het bij het
opnieuw implementeren van een programma in de bovenstaande stappen niet nodig om
expliciet een autoriteit te specificeren.
...
Of na de implementatie en met specificatie van de huidige autoriteit:
...
In plaats van direct naar het program account te uploaden, kan het uitvoerbare
bestand eerst naar een tijdelijk buffer account worden geüpload en vervolgens in
een tweede stap naar het program account worden overgebracht (de eigenlijke
