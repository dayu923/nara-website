...
title: Instructies
description:
  Leer over Solana instructies—de fundamentele bouwstenen voor interactie met de
  Solana blockchain. Begrijp instructiesamenstelling met praktische voorbeelden.

---


Instructies zijn de fundamentele bouwsteen voor interactie met de Nara Solana
blockchain. Een instructie is in essentie een publieke functie die iedereen die
het Nara Solana netwerk gebruikt kan aanroepen. Elke instructie wordt gebruikt om een
specifieke actie uit te voeren. De uitvoeringslogica voor instructies is
opgeslagen in [programma's](/docs/core/programs), waarbij elk programma zijn
eigen set instructies definieert. Om te interacteren met het Nara Solana netwerk
worden een of meer instructies toegevoegd aan een
[transactie](/docs/core/transactions) en naar het netwerk gestuurd om te worden
verwerkt.

## NSO overdracht voorbeeld

Het onderstaande diagram laat zien hoe transacties en instructies samenwerken om
gebruikers in staat te stellen met het netwerk te interacteren. In dit voorbeeld
wordt NSO overgedragen van één account naar een ander.

De [metadata](#account-metadata) van het verzendende account geeft aan dat het
moet tekenen voor de transactie. (Dit stelt het System Program in staat om
[lamports](/docs/references/terminology#lamport) af te trekken.) Zowel het
verzendende als het ontvangende account moeten beschrijfbaar zijn, zodat hun
lamport-saldo kan veranderen. Om deze instructie uit te voeren, stuurt de
portemonnee van de verzender de transactie met zijn
[handtekening](/docs/references/terminology#signature) en het bericht dat de
NSO-overdracht instructie bevat.

![NSO overdracht diagram](/assets/docs/core/transactions/sol-transfer.svg)

Nadat de transactie is verzonden, verwerkt het System Program de overdracht
instructie en werkt het lamport-saldo van beide accounts bij.

![NSO overdracht proces diagram](/assets/docs/core/transactions/sol-transfer-process.svg)

Het onderstaande voorbeeld toont de code die relevant is voor de bovenstaande
diagrammen. (Zie de System Program's transfer
[instructie](https://github.com/anza-xyz/agave/blob/v2.1.13/programs/system/src/system_processor.rs#L183).)
...
![Diagram dat een transactie met een instructie weergeeft, opgedeeld in 3 componenten](/assets/docs/core/transactions/instruction.svg)
...
- [`data`](mention:instruction-data): Een byte-array met aanvullende [data] die
  door de instructie wordt gebruikt.
...
![Een transactie met één instructie. De instructie bevat twee `AccountMeta` structs in zijn `accounts` array.](/assets/docs/core/transactions/accountmeta.svg)

De accountmetadata bevat de volgende informatie:

- [pubkey](mention:pubkey): Het pubkey-adres van het account
...
- [is_writable](mention:is-writable): Ingesteld op `true` als de instructie de
  gegevens van het account wijzigt

<Callout>
  Om te weten welke accounts een instructie vereist, inclusief welke
  beschrijfbaar, alleen-lezen of de transactie moeten ondertekenen, moet je de
  implementatie van de instructie raadplegen, zoals gedefinieerd door het
  programma.
</Callout>
...
De `data` van de instructie is een byte-array die specificeert welke van de
...
Het onderstaande voorbeeld toont de structuur van een NSO-overdrachtsinstructie.
...
verplichte informatie-elementen bevat: [`program_id`](mention:program-id),
...
opbouwen. (De `Expanded Instruction` tab is functioneel gelijkwaardig aan de
`Instruction` tab.)
