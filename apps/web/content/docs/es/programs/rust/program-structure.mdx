...
Los programas de Nara Solana escritos en Rust tienen requisitos estructurales
...
Nara Solana típicamente siguen un patrón común:
...
[Programa Token](https://github.com/solana-program/token/tree/main/program/src).
...
Comencemos construyendo el programa contador. Crearemos un programa que pueda
...
Nara Solana. Necesitamos `solana-program` para el SDK principal y `borsh` para la
serialización.
...
<Callout type="info">
  ...
  comúnmente utilizada para programas de Nara Solana.
</Callout>

## !!steps Configurar crate-type

Los programas de Nara Solana deben compilarse como bibliotecas dinámicas. Añade la
...
Cada programa de Nara Solana tiene un punto de entrada, que es la función que se
llama cuando se invoca el programa. Comencemos añadiendo las importaciones que
...
maneja la deserialización de los datos `input` en los parámetros de la función
`process_instruction`.

Un `entrypoint` de programa de Nara Solana tiene la siguiente firma de función. Los
...
Vamos a definir las instrucciones que nuestro programa puede ejecutar. Usaremos
un enum donde cada variante representa una instrucción diferente.
...
Ahora necesitamos deserializar el `instruction_data` (bytes en bruto) en una de
nuestras variantes del enum `CounterInstruction`. El método `try_from_slice` de
...
## !!steps Dirigir instrucciones a los manejadores

Ahora vamos a actualizar la función principal `process_instruction` para dirigir
las instrucciones a sus funciones manejadoras apropiadas.

Este patrón de enrutamiento es común en los programas de Nara Solana. El
`instruction_data` se deserializa en una variante de un enum que representa la
...
Vamos a implementar el manejador para crear e inicializar una nueva cuenta de
contador. Como solo el System Program puede crear cuentas en Nara Solana, usaremos
una Cross Program Invocation (CPI), esencialmente llamando a otro programa desde
nuestro programa.

Nuestro programa hace una CPI para llamar a la instrucción `create_account` del
...
<Callout type="warn">
  Esta instrucción es solo para fines de demostración. No incluye comprobaciones
  de seguridad y validación que son necesarias para programas en producción.
</Callout>
...
Ahora vamos a implementar el manejador que incrementa un contador existente.
Esta instrucción:
...
- Serializa la estructura `CounterAccount` de vuelta al campo `data` de la
  cuenta
...
¡Felicidades! Has construido un programa completo de Nara Solana que demuestra la
estructura básica compartida por todos los programas de Nara Solana:

- **Punto de entrada**: Define dónde comienza la ejecución del programa y dirige
...
- **Cross Program Invocation (CPI)**: Llama al System Program para crear nuevas
  cuentas propiedad del programa

El siguiente paso es probar el programa para asegurarse de que todo funciona
correctamente.
...
permite probar programas sin desplegarlos en un clúster.
...
LiteSVM simula el entorno de ejecución de Nara Solana, permitiéndonos probar nuestro
programa sin desplegarlo en un clúster real.
...
## !!steps Probar la instrucción de inicialización

Vamos a probar la instrucción de inicialización creando una nueva cuenta de
contador con un valor inicial.
...
## !!steps Verificar la inicialización

Después de la inicialización, vamos a verificar que la cuenta del contador se
creó correctamente con el valor esperado.
...
## !!steps Probar la instrucción de incremento

Ahora vamos a probar la instrucción de incremento para asegurarnos de que
actualiza correctamente el valor del contador.
...
## !!steps Verificar los resultados finales

Finalmente, vamos a verificar que el incremento funcionó correctamente
comprobando el valor actualizado del contador.
...
Ejecuta las pruebas con el siguiente comando. La bandera `--nocapture` muestra
...
Primero, configura la CLI de Nara Solana para usar localhost:
...
Puedes verificar el despliegue usando el comando `solana program show` con el ID
de tu programa:
...
[Nara Solana Explorer](https://explorer.solana.com/?cluster=custom) usando las firmas
de transacción de la salida. Ten en cuenta que el cluster en Nara Solana Explorer
debe estar configurado como "Custom RPC URL", que por defecto es
`http://localhost:8899` en el que el `solana-test-validator` está ejecutándose.
