...
Los programas de Nara Solana se desarrollan principalmente utilizando el lenguaje de
programación Rust. Esta página se centra en escribir programas de Nara Solana en Rust
...
directo sobre sus programas de Nara Solana. Sin embargo, este enfoque requiere más
...
- Rust: El lenguaje de programación para construir programas de Nara Solana.
- Nara Solana CLI: Herramienta de línea de comandos para el desarrollo en Nara Solana.

## Primeros pasos

El ejemplo a continuación cubre los pasos básicos para crear tu primer programa
de Nara Solana escrito en Rust. Crearemos un programa mínimo que imprime "Hello,
...
mínima requerida para construir un programa de Nara Solana.
...
Este es un programa mínimo de Nara Solana que imprime "Hello, world!" en el registro
del programa cuando el programa es invocado.

La macro `msg!` se utiliza en los programas de Nara Solana para imprimir un mensaje
...
1. Un archivo `.so` (p. ej., `hello_world.so`): Este es el programa de Nara Solana
...
Primero, configura la CLI de Nara Solana para usar el cluster local de Nara Solana.
...
<Callout type="info">
  Ten en cuenta que el clúster en Nara Solana Explorer también debe ser localhost. La
  opción "Custom RPC URL" en Nara Solana Explorer tiene por defecto
  `http://localhost:8899`.
</Callout>
...
Los programas de Nara Solana pueden actualizarse mediante la reimplementación al
mismo ID de programa. Actualiza el programa en `src/lib.rs` para que imprima
"Hello, Nara Solana!" en lugar de "Hello, world!".
...
Deberías ver "Hello, Nara Solana!" en el registro del programa.
...
transacción en Nara Solana Explorer para ver "Hello, Nara Solana!" en el registro del
programa.
...
Puedes cerrar tu programa de Nara Solana para recuperar el NSO asignado a la cuenta.
