...
Nara Solana utilizando la CLI de Nara Solana.
...
### Instalar la CLI de Nara Solana

Primero, [instala la CLI de Nara Solana](/docs/intro/installation).
...
Elige a qué clúster de Nara Solana desplegar. Usa el comando `solana config get` para
...
Necesitarás NSO para pagar los despliegues del programa. La cantidad depende del
...
Aquí hay un programa mínimo de Nara Solana que imprime "Hello, Nara Solana!" en los
...
Implementar un programa requiere NSO que se asignará a la cuenta del programa
según el tamaño del programa. Los programas más grandes cuestan más NSO para
implementar.
...
Calcula el NSO requerido para este tamaño (bytes):
...
<Callout type="info">
  Necesitarás un poco más de NSO que lo mostrado para cubrir las comisiones de
...
calculará el NSO requerido y lo deducirá de tu billetera automáticamente.
...
programa para recuperar el NSO asignado al programa.
...
El balance muestra el NSO asignado a la cuenta del programa para almacenar sus
datos.
...
Para cerrar tu programa y recuperar el NSO asignado a la cuenta del programa,
...
Cuando la red de Nara Solana está congestionada, utiliza estos indicadores para
...
- **`--with-compute-unit-price`**: Establece la tarifa prioritaria en
  micro-lamports (0.000001 NSO) por unidad de cómputo. Consulta la
  [API de tarifas prioritarias de Helius](https://docs.helius.dev/guides/priority-fee-api)
  para conocer las tarifas actuales. Una tarifa prioritaria es una tarifa
...
- **`--use-rpc`**: Envía transacciones al RPC configurado. Este indicador
  requiere una conexión RPC
