...
title: Mollusk
description: Guía completa para probar programas de Nara Solana usando Mollusk

---


[Mollusk](https://github.com/anza-xyz/mollusk/tree/main) es un entorno ligero de
pruebas para probar programas de Nara Solana. Proporciona una interfaz simple para
probar instrucciones de programas de Nara Solana en un entorno minimizado de la
Máquina Virtual de Nara Solana (SVM). Todas las cuentas de prueba deben definirse
...
- [Ejemplos de uso de pruebas de Mollusk](https://github.com/anza-xyz/mollusk/tree/0.5.0/harness/tests)
...
Para evaluar el uso de unidades de cómputo, añade `mollusk-svm-bencher` como
...
básico de Nara Solana usando Mollusk.

<ScrollyCoding>

## !!steps Programa Hola Mundo

Este ejemplo demuestra cómo probar un programa básico de Nara Solana usando Mollusk.
El programa simplemente imprime "Hello, world!" en los registros del programa
cuando se invoca.
...
Para probar un programa de Nara Solana con Mollusk:
...
proporciona una interfaz simple para probar programas de Nara Solana. Todos los
campos pueden ser manipulados a través de un puñado de métodos auxiliares, pero
los usuarios también pueden acceder y modificarlos directamente si desean más
control.

Para inicializar Mollusk con una instancia predeterminada, utiliza el método
`Mollusk::default`.
...
Para inicializar Mollusk con un programa específico, utiliza el método
`Mollusk::new`.
...
Para añadir un programa a Mollusk, utiliza el método `Mollusk::add_program`.
...
| [`process_and_validate_instruction`](#single-instruction-with-checks)          | Procesa una instrucción y realiza una serie de comprobaciones en el resultado, entrando en pánico si alguna comprobación falla.           |
| [`process_instruction_chain`](#multiple-instructions)                          | Procesa múltiples instrucciones y devuelve el resultado.                                                                                  |
| [`process_and_validate_instruction_chain`](#multiple-instructions-with-checks) | Procesa múltiples instrucciones y realiza una serie de comprobaciones en cada resultado, entrando en pánico si alguna comprobación falla. |
...
Utiliza el método `process_instruction` para procesar una sola instrucción sin
...
El siguiente ejemplo procesa una instrucción de transferencia de NSO sin
comprobaciones de validación.

<Callout type="info">

Los ejemplos a continuación ejecutan Mollusk en la función `main` con fines de
demostración. En la práctica, normalmente usarás Mollusk en un módulo de prueba
...
#### Instrucción única con verificaciones

Usa el método `process_and_validate_instruction` para procesar una sola
instrucción con verificaciones de validación. Este método generará un error si
...
El siguiente ejemplo procesa una instrucción de transferencia de NSO con
...
secuencialmente sin verificaciones de validación.
...
El siguiente ejemplo procesa dos instrucciones de transferencia de NSO sin
verificaciones de validación.
...
#### Múltiples instrucciones con verificaciones

Usa el método `process_and_validate_instruction_chain` para procesar múltiples
instrucciones con verificaciones de validación después de cada instrucción. Cada
instrucción tiene su propio conjunto de verificaciones que deben pasar.
...
NSO con verificaciones de validación después de cada instrucción.
...
para verificar los resultados de una instrucción procesada.

```rust title="Example"
use mollusk_svm::result::Check;
```

Utiliza los siguientes métodos para validar los resultados de las instrucciones:
...
Utiliza lo siguiente para validar los estados de las cuentas:
...
Crea un `account_store` utilizando el método `with_context`:
...
El siguiente ejemplo procesa dos instrucciones separadas de transferencia de NSO
...
Utiliza el método `warp_to_slot` para actualizar el reloj sysvar y simular el
avance o retroceso en el tiempo a un slot específico.
...
El siguiente ejemplo muestra cómo modificar el sysvar de Mollusk directamente
...
## Evaluación comparativa de unidades de cómputo
...
El siguiente ejemplo evalúa el uso de unidades de cómputo de una instrucción de
transferencia de NSO.
...
Los resultados de la evaluación comparativa se escriben en el `out_dir`
especificado como un archivo markdown llamado `compute_units.md`.
...
El siguiente ejemplo demuestra cómo probar una transferencia de token usando
Mollusk.

<Callout type="info">
  El ejemplo a continuación define manualmente las cuentas de prueba con fines
  de demostración. El paquete `mollusk-svm-programs-token` también incluye
  funciones auxiliares para crear las cuentas de mint y token.
</Callout>
