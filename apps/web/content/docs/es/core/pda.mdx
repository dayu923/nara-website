...
Una [dirección de cuenta](/docs/core/accounts#account-address) de Nara Solana apunta
...
El tiempo de ejecución de Nara Solana permite a los programas firmar PDAs sin
...
de Nara Solana son puntos en la [curva Ed25519](https://ed25519.cr.yp.to/)
...
Nara Solana permite a los programas firmar PDAs sin necesidad de una clave privada.
...
Los SDK de Nara Solana admiten la creación de PDA con las funciones mostradas en la
...
Esto significa que dadas las mismas seeds opcionales y `programId`, un bump seed
con un valor diferente todavía puede derivar un PDA válido.
...
Los ejemplos a continuación derivan un PDA utilizando los SDKs de Nara Solana. Haz
...
El ejemplo a continuación utiliza el
...
[bump seed](mention:bump) utilizados para derivar el PDA.
...
La restricción [`init`](mention:init) le indica a Anchor que
[invoque al System Program](/docs/core/programs#the-system-program) para crear
una nueva cuenta utilizando el PDA como dirección. Las [seeds](mention:seeds)
utilizadas para crear el PDA son:

- La dirección de la cuenta de usuario proporcionada en la instrucción
- La cadena fija: "data"
- El [bump seed](mention:bump) canónico

En este ejemplo, la restricción de bump no tiene asignado un valor, por lo que
Anchor utilizará `find_program_address` para derivar el PDA y encontrar el bump.
...
<Callout type="warn">
  Si invocas la instrucción `initialize` nuevamente con la misma seed de
  dirección `user`, la transacción fallará. Esto ocurre porque ya existe una
