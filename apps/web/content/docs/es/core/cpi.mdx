...
Una invocación entre programas (CPI) ocurre cuando un programa de Nara Solana invoca
directamente las instrucciones de otro programa. Esto permite la composición de
programas. Si piensas en una [instrucción](/docs/core/instructions) de Nara Solana
...
Esta toma las [semillas de firmante](mention:signer-seeds) utilizadas para
derivar [PDAs](<(/docs/core/pda)>) firmantes. El runtime de Nara Solana internamente
llama a
...
[añadido como firmante válido](https://github.com/anza-xyz/agave/blob/v2.1.13/programs/bpf_loader/src/syscalls/cpi.rs#L554).
...
y Rust nativo. Cada ejemplo incluye una única instrucción para transferir NSO
...
- **Ejemplo 1**: Utiliza _rs`CpiContext`_ de Anchor y función auxiliar.
...
escrito en Rust nativo. Incluye una única instrucción que transfiere NSO desde
...
Incluyen una única instrucción que transfiere NSO de una cuenta a otra.
...
- **Ejemplo 1**: Utiliza _rs`CpiContext`_ de Anchor y una función auxiliar.
- **Ejemplo 2**: Utiliza la función _rs`system_instruction::transfer`_ del crate
  `solana_program`.
- **Ejemplo 3**: Construye la instrucción CPI manualmente. Este enfoque es útil
...
Rust nativo. Incluye una única instrucción que transfiere NSO de una cuenta a
