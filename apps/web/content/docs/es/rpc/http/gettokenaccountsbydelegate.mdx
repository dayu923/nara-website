...
Devuelve todas las cuentas de Token SPL por Delegado aprobado.
...
Pubkey del delegado de la cuenta a consultar, como cadena codificada en base-58
...
Pubkey del token Mint específico para limitar las cuentas, como cadena
codificada en base-58 ; o
...
- Si se solicita `jsonParsed` pero no se puede encontrar un analizador, el campo
  recurre a la codificación `base64`, detectable cuando el campo `data` es de
  tipo `string`.
