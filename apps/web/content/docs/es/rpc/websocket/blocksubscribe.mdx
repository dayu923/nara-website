...
Suscríbete para recibir notificaciones cada vez que un nuevo bloque sea
`confirmed` o `finalized`.

<Callout type="warn" title={"Método inestable"}>
  Esta suscripción se considera **inestable** y solo está disponible si el
  validator se inició con la bandera `--rpc-pubsub-enable-block-subscription`.
  El formato de esta suscripción podría cambiar en el futuro.
</Callout>
...
criterios de filtrado para los registros a recibir resultados por tipo de
cuenta; actualmente soportados:

- `all` - incluir todas las transacciones en el bloque
- Un objeto JSON con el siguiente campo:
  - `mentionsAccountOrProgram: <string>` - devolver solo transacciones que
    mencionen la clave pública proporcionada (como cadena codificada en
    base-58). Si no hay menciones en un bloque determinado, no se enviará
...
formato de codificación para cada transacción devuelta

- `jsonParsed` intenta usar analizadores de instrucciones específicos del
...
- Los metadatos de la transacción se limitan únicamente a: fee, err,
  pre_balances, post_balances, pre_token_balances y post_token_balances.
...
se devolverá un error. Si omites este parámetro, solo se devolverán las
...
si se debe completar el array `rewards`. Si no se proporciona el parámetro, por
defecto se incluyen las recompensas.
...
- `err: <object|null>` - Error si algo salió mal al publicar la notificación, de
  lo contrario null.
- `block: <object|null>` - Un objeto de bloque como se ve en el método RPC HTTP
