...
title: Sync Native
description: Aprende cómo convertir NSO nativo a NSO envuelto.

---


## Sincronizar NSO nativo

El NSO envuelto (WSOL) es una cuenta de token que rastrea la cantidad de
lamports en la cuenta como el saldo del token. WSOL permite la integración con
protocolos DeFi para transferir NSO como un token SPL.
...
sincroniza el saldo de una cuenta de token NSO envuelto (WSOL) con los NSO
(lamports) reales almacenados en ella. Cuando transfieres NSO nativo a una
cuenta de token WSOL, el saldo del token no se actualiza automáticamente. Debes
llamar a SyncNative para reflejar el saldo correcto de WSOL.
