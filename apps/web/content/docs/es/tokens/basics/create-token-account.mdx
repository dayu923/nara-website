...
### Ejemplo: USD Coin (USDC)

- Dirección del mint: `EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`
- Token account de Circle: `3emsAVdmGKERbHjmGfQ6oZ1e35dkf5iYcS6U4CPKFVaa`
- Esta cuenta contiene solo tokens USDC
- Circle controla los tokens a través de la autoridad propietaria de la token
  account: `7VHUFJHWu2CuExkJcJrzhQPJ2oygupTWkL2A2For4BmE`
...
2. **Program Owner**: El programa que posee la cuenta (siempre el Token Program
   o Token Extensions Program para las token accounts).

Cuando se trabaja con token accounts, "owner" típicamente se refiere a la
autoridad que puede transferir tokens, no al programa que posee la cuenta.
...
<Callout type="info">
  Solo las token accounts creadas por el Associated Token Program se denominan
...
estructura estándar `Account`.

</Callout>

## Cómo crear una token account

La creación de una token account requiere la instrucción
...
Consulta la
[implementación aquí](https://github.com/solana-program/token/blob/a7c488ca39ed4cd71a87950ed854929816e9099f/program/src/processor.rs#L83).

Necesitas dos instrucciones para crear una token account:

1. System Program: Crear una cuenta con espacio asignado para una token account
   y transferir la propiedad al Token Program
2. Token Program: Inicializar los datos de la token account
...
La creación de una associated token account (ATA) requiere la instrucción
...
su inicialización a través de Invocaciones entre Programas (CPI).
