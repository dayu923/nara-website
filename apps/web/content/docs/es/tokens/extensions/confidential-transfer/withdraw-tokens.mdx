...
description: Aprende cómo retirar tokens del estado confidencial.

---


## Cómo retirar tokens del saldo disponible confidencial

Para retirar tokens del saldo disponible confidencial al saldo público:

1. Crea
   [dos pruebas](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/confidential-transfer/proof-generation/src/withdraw.rs#L22)
   ...
   **Prueba de igualdad
   ...
   Verifica que el texto cifrado del saldo disponible restante después de la
   retirada coincide con su correspondiente
   [compromiso de Pedersen](https://en.wikipedia.org/wiki/Commitment_scheme),
   asegurando que el nuevo saldo disponible de la cuenta se calcula
   correctamente como `remaining_balance = current_balance - withdraw_amount`.
   ...
   **Prueba de rango
   ...
   Verifica que el saldo disponible restante después de la retirada es no
   negativo y dentro de un rango específico.

2. Para cada prueba:
   - Invoca el programa de prueba ZK ElGamal para verificar los datos de la
     prueba.
   - Almacena los metadatos específicos de la prueba en una cuenta de "estado de
     contexto" de prueba para usar en otras instrucciones.

3. Invoca la instrucción
   ...
   proporcionando las dos cuentas de prueba.

4. Cierra las cuentas de prueba para recuperar el NSO utilizado para crearlas.

El siguiente diagrama muestra los pasos involucrados en la retirada de tokens
del saldo disponible confidencial al saldo público:
...
Para retirar tokens del saldo disponible confidencial al saldo público, debes:

- Generar una prueba de igualdad y una prueba de rango del lado del cliente
- Invocar el programa de prueba Zk ElGamal para verificar las pruebas e
  inicializar las cuentas de "estado de contexto"
- Invocar la instrucción
  [ConfidentialTransferInstruction::Withdraw](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L493)
  proporcionando las dos cuentas de prueba.
- Cerrar las dos cuentas de prueba para recuperar el rent.
...
- Método `confidential_transfer_create_context_state_account` que crea una
  cuenta de prueba.
- Método `confidential_transfer_withdraw` que invoca la instrucción `Withdraw`.
- Método `confidential_transfer_close_context_state_account` que cierra una
  cuenta de prueba.

### Código de ejemplo

El siguiente ejemplo demuestra cómo retirar tokens del saldo disponible
confidencial al saldo público.
...
instalada la CLI de Nara Solana para iniciar el validator local.
