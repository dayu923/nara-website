...
description:
  Aprende cómo transferir tokens de forma privada de una cuenta de token a otra.
...
## Cómo transferir tokens confidencialmente de una cuenta de token a otra

Para transferir tokens confidencialmente de una cuenta de token a otra, tanto el
remitente como el destinatario deben tener cuentas de token configuradas con el
estado _rs`ConfidentialTransferAccount`_ y aprobadas para transferencias
confidenciales. La cuenta de token del remitente también debe tener un saldo
confidencial disponible para transferir.

Para transferir tokens confidencialmente:

1. Crear
   [tres pruebas](https://github.com/solana-program/token-2022/blob/d073cb89dbcd430387b5c6fb4b7157911351e4a3/confidential-transfer/proof-generation/src/transfer.rs#L35)
   ...
   **Prueba de igualdad
   ...
   Verifica que el nuevo texto cifrado del saldo disponible después de la
   transferencia coincida con su correspondiente
   [compromiso Pedersen](https://en.wikipedia.org/wiki/Commitment_scheme),
   asegurando que el nuevo saldo disponible de la cuenta de origen se calcule
   correctamente como `new_balance = current_balance - transfer_amount`.
   ...
   **Prueba de validez de texto cifrado
   ...
   Verifica que los textos cifrados del monto de transferencia estén generados
   correctamente para las tres partes (origen, destino y auditor), asegurando
   que el monto de transferencia esté cifrado correctamente bajo la clave
   pública de cada parte.
   ...
   **Prueba de rango
   ...
   Verifica que el nuevo saldo disponible y el monto de transferencia (dividido
   en bits altos/bajos) sean todos no negativos y estén dentro de un rango
   específico.

2. Para cada prueba:
   - Invocar el programa de prueba ZK ElGamal para verificar los datos de la
     prueba.
   - Almacenar los metadatos específicos de la prueba en una cuenta de "estado
     de contexto" de prueba para usar en otras instrucciones.

3. Invocar la instrucción
   ...
   proporcionando las cuentas de estado de contexto de prueba.

4. Cerrar las cuentas de estado de contexto de prueba para recuperar el NSO
   utilizado para crearlas.

El siguiente diagrama muestra los pasos involucrados en la transferencia de
tokens desde la cuenta de token del remitente a la cuenta de token del
destinatario.
...
Para transferir tokens confidencialmente de una cuenta de token a otra, debes:

- Generar una prueba de igualdad, prueba de validez de texto cifrado y prueba de
  rango desde el lado del cliente
- Invocar el programa Zk ElGamal proof para verificar las pruebas e inicializar
  las cuentas de "estado de contexto"
- Invocar la instrucción
  [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604)
  proporcionando las tres cuentas de prueba.
- Cerrar las tres cuentas de prueba para recuperar el rent.

El crate `spl_token_client` proporciona los siguientes métodos:

- Método `confidential_transfer_create_context_state_account` que crea una
  cuenta de prueba.
- Método `confidential_transfer_transfer` que invoca la instrucción `Transfer`.
- Método `confidential_transfer_close_context_state_account` que cierra una
  cuenta de prueba.

### Código de ejemplo

El siguiente ejemplo demuestra cómo transferir tokens confidencialmente entre
cuentas de token.
...
instalado el Nara Solana CLI para iniciar el validador local.
