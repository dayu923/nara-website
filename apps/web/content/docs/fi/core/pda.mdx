...
kryptografisia toimintoja. (Kuten allekirjoituksen tarjoaminen.) Nara Solana
...
Nara Solana SDK:t tukevat PDA:n luomista alla olevassa taulukossa näytetyillä
...
Alla olevat esimerkit johtavat PDA:n käyttäen Nara Solana SDK:ta. Napsauta **&#9655;
...
[`initialize`](mention:initialize) -ohjeen uuden tilin luomiseksi, joka
tallentaa [käyttäjän osoitteen](mention:user-address) ja PDA:n johtamiseen
käytetyn [bump seedin](mention:bump).
...
[`init`](mention:init) -rajoite kertoo Anchorille, että sen tulee
...
[`initialize`](mention:initialize) -ohjetta luodakseen uuden tilin
ohjelmajohdannaisella osoitteella. Tiedosto sisältää koodin
[PDA:n johtamiseen](mention:pda).
