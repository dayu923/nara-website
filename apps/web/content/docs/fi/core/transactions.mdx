...
Vuorovaikuttaaksesi Nara Solana-verkon kanssa sinun täytyy lähettää transaktio. Voit
...
- [`signatures`](mention:signatures): Taulukko [allekirjoituksia](#signatures)
- [`message`](mention:message): Transaktion tiedot, mukaan lukien luettelo
...
tavua. Tämä rajoitus sisältää sekä [`signatures`](#signatures) -taulukon että
...
<Callout>
  ...
  pakettiin ilman pirstoutumista, Nara Solana käyttää 1280 tavua (IPv6:n vaatima
...
[allekirjoittajatilille](#account-addresses), joka sisältyy mihinkään
transaktion ohjeista.

Ensimmäinen allekirjoitus kuuluu tilille, joka maksaa tapahtuman
[perusmaksun](docs/core/fees#base-fee) ja on tapahtuman allekirjoitus.
Tapahtuman allekirjoitusta voidaan käyttää tapahtuman tietojen hakemiseen
verkosta.
...
- [`account_keys`](mention:account-addresses): Taulukko
  [tilien osoitteista](#account-addresses), joita tapahtuman ohjeet vaativat
- [`recent_blockhash`](mention:recent-blockhash):
  [Lohkohash](#recent-blockhash), joka toimii tapahtuman aikaleimana
...
- [`num_required_signatures`](mention:num_required_signatures): Tapahtuman
  vaatimien allekirjoitusten kokonaismäärä
- [`num_readonly_signed_accounts`](mention:num_readonly_signed_accounts): Vain
  luku -tilien kokonaismäärä, jotka vaativat allekirjoituksia
- [`num_readonly_unsigned_accounts`](mention:num_readonly_unsigned_accounts):
  Vain luku -tilien kokonaismäärä, jotka eivät vaadi allekirjoituksia
...
[kompaktissa taulukkomuodossa](/docs/references/terminology#compact-array-format).
...
1. [`program_id_index`](mention:program-id-index): Indeksi, joka osoittaa
   ...
   osoittaa ohjelman osoitteen, joka käsittelee ohjeen.
2. [`accounts`](mention:account-indexes): Taulukko indeksejä, jotka osoittavat
   osoitteisiin `account_keys` -taulukossa. Jokainen indeksi osoittaa tämän
   ohjeen vaatiman tilin osoitteeseen.
...
<Callout>
  Voit myös löytää transaktion käyttämällä [Nara Solana
