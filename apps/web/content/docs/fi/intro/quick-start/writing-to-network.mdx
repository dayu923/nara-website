...
Edellisessä osiossa opit lukemaan tietoja Nara Solana-verkosta. Nyt opit
kirjoittamaan tietoja verkkoon. Nara Solana-verkkoon kirjoittaminen tapahtuu
...
[transaktion](/docs/core/transactions), Nara Solana-ajoympäristö suorittaa jokaisen
ohjeen järjestyksessä ja atomisesti. Tämän osion esimerkit näyttävät, miten
rakennetaan ja lähetetään transaktioita Nara Solana-ohjelmien kutsumiseksi, mukaan
lukien:

1. NSO-tokenien siirtäminen tilien välillä
2. Uuden tokenin luominen

## NSO-tokenien siirtäminen

Alla oleva esimerkki siirtää NSO-tokeneita kahden tilin välillä. Jokaisella
tilillä on omistaja- ohjelma, joka on ainoa ohjelma, joka voi vähentää tilin
NSO-saldoa.

Kaikki lompakkotilit ovat System Program -ohjelman omistamia. NSO-tokenien
siirtämiseksi sinun täytyy kutsua System Program -ohjelman
...
Luo yhteys Nara Solana-klusteriin.

### !sender

Luo uusi avainpari käytettäväksi `sender`na.

### !receiver

Luo uusi avainpari käytettäväksi `receiver`na.

### !airdrop

Pyydä NSO-tokenien airdrop `sender`n rahoittamiseksi.
...
Luo uudet [avainparit](/docs/core/accounts#public-key) käytettäväksi lähettäjän
ja vastaanottajan tileinä.
...
Lisää NSO lähettäjän tilille. Muilla verkoilla kuin pääverkolla voit käyttää
`requestAirdrop` -metodia saadaksesi NSO-tokeneita testausta varten.
...
`SystemProgram.transfer()` -metodi luo ohjeen, joka siirtää NSO-tokeneita
[`fromPubkey`](mention:from) -tililtä [`toPubkey`](mention:to) -tilille
määritetyn määrän [`lamports`](mention:lamports) -yksiköitä.
...
jossa on yksi ohje. Voit kuitenkin lisätä transaktioon useita ohjeita.
...
valtuuttamaan NSO:n siirto heidän tililtään.
...
transaktion etsimiseen Nara Solana Explorerissa.
...
Alla oleva esimerkki luo uuden tokenin Solanassa käyttäen Token Extensions
Programia. Tämä vaatii kaksi ohjetta:
...
Laske mint-tilille vaadittavien lamport-yksiköiden määrä.

### !rentExemptionLamports

Laske rent-vapautukseen tarvittavien lamport-yksiköiden määrä
...
- [Generoi keypair](mention:wallet) transaktion maksamiseksi
- [Pyytää airdropin](mention:airdrop) keypair-avaimen rahoittamiseksi
...
Generoi keypair mint-tiliä varten. Julkista avainta käytetään mint-tilin
osoitteena.
...
1. Varaa [tavujen määrä](mention:space), joka tarvitaan mint-tietojen
   tallentamiseen.
2. [Siirrä lamportseja](mention:lamports) lompakosta uuden tilin
   rahoittamiseksi.
3. [Määritä omistajuus](mention:programId) tilille
   [Token Extensions program](/docs/tokens/extensions) -ohjelmalle.
...
Lisää molemmat ohjeet yhteen transaktioon. Tämä varmistaa, että tilin luominen
ja alustaminen tapahtuvat atomisesti. (Joko molemmat ohjeet onnistuvat tai
kumpikaan ei onnistu.)

Tämä lähestymistapa on yleinen rakennettaessa monimutkaisia
Nara Solana-transaktioita, sillä se takaa, että kaikki ohjeet suoritetaan yhdessä.
...
- [Lompakko](mention:wallet) -tili allekirjoittaa maksajana
  [transaktiomaksuja](/docs/core/fees) ja tilin luomista varten
