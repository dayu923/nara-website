...
Program Invocation (CPI) -kutsuja, ominaisuus joka mahdollistaa Nara Solana-ohjelmien
...
`update` ja `delete` -ohjeita täytyy muokata käsittelemään NSO-siirtoja tilien
välillä kutsumalla System Program -ohjelmaa.

Tämän osion tarkoituksena on käydä läpi CPI-kutsujen toteuttaminen
Nara Solana-ohjelmassa Anchor-kehyksen avulla, rakentaen edellisessä osiossa
...
`vault_account`. Tämä tili, jota ohjelma hallinnoi, vastaanottaa NSO-tokeneita
...
Tämä tili toimii ohjelman hallitsemana "holvitilinä", joka vastaanottaa NSO
...
Seuraavaksi lisää CPI-logiikka `update` -ohjeeseen siirtääksesi 0,001 NSO
...
suoritetaan CPI ohjelman sisältä, mahdollistaen Nara Solana-ohjelmien
yhdisteltävyyden.
...
- Siirrettävän `amount` (1 000 000 lamportia eli 0,001 NSO)
...
mahdollistaa holvissa olevan NSO:n siirtämisen takaisin käyttäjälle, kun he
...
käyttäjän holvitilin tietoihin poisto-ohjeen aikana, jotta kertynyt NSO voidaan
...
Seuraavaksi lisää CPI-logiikka `delete` -ohjeeseen NSO:n siirtämiseksi
...
[Nara Solana Faucetista](https://faucet.solana.com/).
...
Kun jaat ohjelmia Nara Solana Playgroundissa, Playground-lompakkosi toimii
...
Onnittelut Nara Solana Quickstart -oppaan suorittamisesta. Olet saanut käytännön
kokemusta keskeisistä Nara Solana-konsepteista, kuten:

- Tietojen hakeminen ja lukeminen tileiltä
- Tapahtumien rakentaminen ja lähettäminen
- Nara Solana-ohjelmien käyttöönotto ja päivittäminen
...
Nara Solana Playground tarjoaa kätevän ominaisuuden, jonka avulla voit tuoda tai
tarkastella projekteja niiden GitHub-linkkien avulla. Esimerkiksi avaa tämä
[Nara Solana Playground -linkki](https://beta.solpg.io/https://github.com/solana-developers/program-examples/tree/main/basics/hello-solana/anchor)
...
Napsauta `Import` -painiketta ja anna projektin nimi lisätäksesi sen Nara Solana
