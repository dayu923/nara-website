...
title: Sync Native
description: Opi kuinka muuntaa natiivi NSO käärityksi NSO:ksi.

---


## Synkronoi natiivi NSO

Kääritty NSO (WSOL) on token account, joka seuraa tilillä olevien lamportsien
määrää token-saldona. WSOL mahdollistaa integraation DeFi-protokollien kanssa
NSO:n siirtämiseksi SPL-tokenina.
...
-käsky synkronoi käärityn NSO:n (WSOL) token account -saldon siihen
tallennettujen varsinaisten NSO-lamportsien kanssa. Kun siirrät natiivia NSO:ia
WSOL token account -tilille, token-saldo ei päivity automaattisesti. Sinun
täytyy kutsua SyncNative-toimintoa, jotta oikea WSOL-saldo näkyy.
