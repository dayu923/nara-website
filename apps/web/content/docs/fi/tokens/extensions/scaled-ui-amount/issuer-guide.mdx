...
käyttöön token mintissä tai token accountissa. Huomaa, että kun token on luotu,
et voi muokata käytössä olevia laajennuksia.
...
tilissä. Tässä on esimerkki siitä, miten luoda token, jossa Skaalatun
...
`update-ui-amount-multiplier` komentoa. Aikaleima sekunteina Unix epochista on
valinnainen ja sitä voidaan käyttää asettamaan mukautettu aloitusaika uudelle
kertoimelle. Jos aikaleimaa ei anneta, käytetään nykyistä aikaleimaa.

**Tärkeä huomautus:** Tällä hetkellä skaalatun käyttöliittymäkertoimen
päivitysprosessissa on erikoisuus. Tällä hetkellä jos teet seuraavasti:

1. Aseta skaalattu käyttöliittymäkerroin tulevaisuuteen
2. Aika kuluu tämän päivitysajan yli
3. Aseta toinen skaalattu käyttöliittymäkerroin tulevaisuuteen
4. Tällöin edellinen skaalattu käyttöliittymäkerroin korvataan uudella, mikä
   käytännössä poistaa sen.

Tähän on
[yhdistetty PR korjaukseksi](https://github.com/solana-program/token-2022/pull/522),
jotta kerroin päivittyy asianmukaisesti nykyiseen kertoimeen sen sijaan, että se
korvattaisiin, mutta tämä ei ole vielä käytössä.

Tähän asti on välttämätöntä tehdä 2 kertoimen päivitystä vaiheen 3 aikana:

1. Aseta edellinen kerroin uudelleen aikaleimalla samaan aikaleimaan, joka alun
   perin asetettiin vaiheessa 1 (tämä asettaa kertoimen oikein nykyiseksi
   kertoimeksi)
2. Aseta uusi kerroin kuten kuvattu yllä vaiheessa 3
...
Saldon hakemiseen sinun täytyy käyttää `balance` komentoa.
