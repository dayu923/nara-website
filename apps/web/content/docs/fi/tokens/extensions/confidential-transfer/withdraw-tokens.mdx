...
title: Tokenien nostaminen
description: Opi miten nostaa tokeneita luottamuksellisesta tilasta.
...
Nostaaksesi tokeneita luottamuksellisesta käytettävissä olevasta saldosta
julkiseen saldoon:

1. Luo
   [kaksi todistetta](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/confidential-transfer/proof-generation/src/withdraw.rs#L22)
   asiakaspuolella:
   ...
   **Yhtäläisyystodiste
   ...
   Varmistaa, että jäljellä oleva käytettävissä olevan saldon salateksti noston
   jälkeen vastaa sen vastaavaa
   [Pedersen-sitoumusta](https://en.wikipedia.org/wiki/Commitment_scheme),
   varmistaen että tilin uusi käytettävissä oleva saldo on oikein laskettu
   ...
   **Aluetodiste
   ...
   Varmistaa, että jäljellä oleva käytettävissä oleva saldo noston jälkeen on
   ei-negatiivinen ja määritetyllä alueella.

2. Jokaiselle todisteelle:
...
3. Kutsu
   ...
   -ohjetta tarjoamalla kaksi todistustiliä.

4. Sulje todistustilit saadaksesi takaisin NSO, jota käytettiin niiden
   luomiseen.

Seuraava kaavio näyttää vaiheet, jotka liittyvät tokenien nostamiseen
luottamuksellisesta käytettävissä olevasta saldosta julkiseen saldoon:
...
Nostaaksesi tokeneita luottamuksellisesta käytettävissä olevasta saldosta
julkiseen saldoon, sinun täytyy:

- Luoda yhtäläisyystodiste ja aluetodiste asiakaspuolella
- Kutsua Zk ElGamal -todistusohjelmaa todisteiden vahvistamiseksi ja
  "kontekstitila"-tilien alustamiseksi
- Kutsua
  [ConfidentialTransferInstruction::Withdraw](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L493)
  -ohjetta tarjoamalla kaksi todistustiliä.
- Sulkea kaksi todistustiliä saadaksesi rent-maksun takaisin.

`spl_token_client` crate tarjoaa seuraavat metodit:

- `confidential_transfer_create_context_state_account` -metodi, joka luo
  todistustilin.
- `confidential_transfer_withdraw` -metodi, joka kutsuu `Withdraw` -ohjetta.
- `confidential_transfer_close_context_state_account` -metodi, joka sulkee
  todistus- tilin.

### Esimerkkikoodi

Seuraava esimerkki näyttää, miten nostaa tokeneita luottamuksellisesta
käytettävissä olevasta saldosta julkiseen saldoon.

Suorittaaksesi esimerkin, käynnistä paikallinen validator Token Extension
Program -ohjelmalla, joka on kloonattu mainnetistä seuraavalla komennolla.
Sinulla täytyy olla Nara Solana CLI asennettuna paikallisen validatorin
käynnistämiseksi.
