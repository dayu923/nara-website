...
description:
  Opi miten siirtää tokeneita yksityisesti yhdeltä token-tililtä toiselle.
...
## Kuinka siirtää tokeneita luottamuksellisesti yhdeltä token accountilta toiselle

Siirtääksesi tokeneita luottamuksellisesti yhdeltä token accountilta toiselle,
sekä lähettäjällä että vastaanottajalla on oltava token accountit, jotka on
määritetty _rs`ConfidentialTransferAccount`_ -tilalla ja hyväksytty
luottamuksellisia siirtoja varten. Lähettäjän token accountilla on myös oltava
käytettävissä oleva luottamuksellinen saldo siirtoa varten.

Tokenien siirtäminen luottamuksellisesti:

1. Luo
   [kolme todistusta](https://github.com/solana-program/token-2022/blob/d073cb89dbcd430387b5c6fb4b7157911351e4a3/confidential-transfer/proof-generation/src/transfer.rs#L35)
   asiakaspuolella:
   ...
   **Tasa-arvotodistus
   ...
   Vahvistaa, että uusi käytettävissä oleva saldo-salateksti siirron jälkeen
   vastaa sen vastaavaa
   [Pedersen-sitoumusta](https://en.wikipedia.org/wiki/Commitment_scheme),
   varmistaen että lähdetilin uusi käytettävissä oleva saldo on oikein laskettu
   kaavalla `new_balance = current_balance - transfer_amount`.

   **Salatekstin kelvollisuustodistus
   ...
   Vahvistaa, että siirtosumman salatekstit on asianmukaisesti luotu kaikille
   kolmelle osapuolelle (lähde, kohde ja auditoija), varmistaen että siirtosumma
   on oikein salattu kunkin osapuolen julkisella avaimella.
   ...
   **Aluetodistus
   ...
   Vahvistaa, että uusi käytettävissä oleva saldo ja siirtosumma (jaettuna
   ala-/yläbitteihin) ovat kaikki ei-negatiivisia ja määritetyllä alueella.

2. Jokaiselle todistukselle:
   - Kutsu ZK ElGamal -todistusohjelmaa todistustietojen vahvistamiseksi.
   - Tallenna todistuskohtaiset metatiedot todistuksen "kontekstitila"-tilille
     käytettäväksi muissa ohjeissa.
...
3. Kutsu
   ...
   -ohjetta tarjoamalla todistuksen kontekstitilatilit.

4. Sulje todistuksen kontekstitilatilit NSO:n palauttamiseksi, jota käytettiin
   niiden luomiseen.

Seuraava kaavio näyttää vaiheet, jotka liittyvät tokenien siirtämiseen
lähettäjän token-tililtä vastaanottajan token-tilille.
...
Siirtääksesi tokeneita luottamuksellisesti yhdeltä token-tililtä toiselle, sinun
täytyy:

- Luoda yhtäläisyystodiste, salatekstin voimassaolotodiste ja aluetodiste
  asiakaspuolella
- Kutsua Zk ElGamal -todistusohjelmaa todisteiden vahvistamiseksi ja
  "kontekstitila"-tilien alustamiseksi
- Kutsua
  [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604)
  -ohjetta tarjoamalla kolme todistustiliä
- Sulkea kolme todistustiliä rent-maksun palauttamiseksi

`spl_token_client` -crate tarjoaa seuraavat metodit:

- `confidential_transfer_create_context_state_account` -metodi, joka luo
  todistustilin.
- `confidential_transfer_transfer` -metodi, joka kutsuu `Transfer` -ohjetta.
- `confidential_transfer_close_context_state_account` -metodi, joka sulkee
  todistustilin.

### Esimerkkikoodi

Seuraava esimerkki näyttää, miten tokeneita siirretään luottamuksellisesti
token-tilien välillä.

Suorittaaksesi esimerkin, käynnistä paikallinen validator Token Extensions
Program -ohjelmalla, joka on kloonattu mainnetistä käyttäen seuraavaa komentoa.
Sinulla täytyy olla Nara Solana CLI asennettuna käynnistääksesi paikallisen
validatorin.
...
<Callout type="info">
  Tätä kirjoitettaessa luottamukselliset siirrot eivät ole käytössä
  oletusarvoisessa paikallisessa validatorissa. Sinun täytyy kloonata mainnetin
  Token Extensions Program suorittaaksesi esimerkkikoodin.
</Callout>
