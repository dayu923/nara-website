...
title: Luottamuksellinen siirto
description:
  Opi luottamuksellisesta siirtolaajennuksesta ja kuinka käyttää sitä
  lisätäksesi valinnaisia ominaisuuksia token-mintteihin ja -tileihin.
...
-tilien välillä paljastamatta siirron määrää. Tämä on hyödyllistä yksityisyyttä
suojaavissa transaktioissa. Vain siirtosummat ja token-saldot ovat yksityisiä.
Token account -osoitteet pysyvät julkisina.

- [Protokollan yleiskatsaus](https://www.solana-program.com/docs/confidential-balances/overview) -
  Tietoa taustalla olevasta kryptografisesta protokollasta
- [Pikaopas](https://www.solana-program.com/docs/confidential-balances#setup) -
  Asennus ja peruskomentorivikäskyt
- [Luottamuksellisten saldojen keittokirja](https://github.com/solana-developers/Confidential-Balances-Sample) -
  Koodiesimerkkejä siitä, miten käyttää luottamuksellisten siirtojen laajennusta

### Miten se toimii?

Luottamuksellisten siirtojen laajennus lisää
...
Token Extensions Program -ohjelmaan, mikä mahdollistaa tokenien siirtämisen
tilien välillä paljastamatta siirron määrää.
...
Luottamuksellisten tokensiirtojen peruskulku on seuraava:

1. Luo mint account luottamuksellisten siirtojen laajennuksella.
2. Luo token account -tilit luottamuksellisten siirtojen laajennuksella
...
5. **Sovella** lähettäjän odottava saldo **luottamukselliseen käytettävissä
   olevaan saldoon**.
6. Siirrä luottamuksellisesti tokeneita lähettäjän token account -tililtä
...
Lisätietoja luottamuksellisen siirron vaiheista löydät vastaavista sivuista:
...
<Cards>
  ...
  >
    Miten luoda mint account Confidential Transfer -laajennuksella
  </Card>
  <Card
    title="Luo Token Account"
    href="/docs/tokens/extensions/confidential-transfer/create-token-account"
  >
    Miten määrittää token account Confidential Transfer -laajennuksella
  </Card>
  <Card
    title="Talleta tokeneita"
    href="/docs/tokens/extensions/confidential-transfer/deposit-tokens"
  >
    Miten tallettaa tokeneita luottamukselliseen odottavaan saldoon
  </Card>
  <Card
    title="Käytä odottavaa saldoa"
    href="/docs/tokens/extensions/confidential-transfer/apply-pending-balance"
  >
    Miten siirtää odottava saldo käytettävissä olevaan luottamukselliseen
    saldoon
  </Card>
  <Card
  ...
  >
    Miten nostaa tokeneita luottamuksellisesta käytettävissä olevasta saldosta
  </Card>
  <Card
    title="Siirrä tokeneita"
    href="/docs/tokens/extensions/confidential-transfer/transfer-tokens"
  >
    Miten siirtää tokeneita luottamuksellisesti token accountien välillä
  </Card>
</Cards>

Alla oleva kaavio näyttää yksityiskohtaisen järjestyksen luottamuksellisten
tokenien siirtojen peruskulusta:
...
Alla oleva Rust-esimerkki näyttää luottamuksellisten tokenien siirtojen vaiheet,
jotka vastaavat edellä esitettyä kaaviota. Vaiheet on selkeyden vuoksi jaettu
apufunktioihin.

Suorittaaksesi esimerkin käynnistä paikallinen validaattori Token Extensions
Program -ohjelmalla, joka on kloonattu mainnetistä seuraavalla komennolla.
Sinulla täytyy olla Nara Solana CLI asennettuna käyttääksesi paikallista
validaattoria.
...
<Callout type="info">
  ...
  oletusarvoisessa paikallisessa validaattorissa. Sinun täytyy kloonata
  mainnetin Token Extensions Program suorittaaksesi esimerkkikoodin.
</Callout>
...
| Ohje                                | Kuvaus                                                                                                                                                              |
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | Määrittää mint accountin luottamuksellisia siirtoja varten. Tämä ohje on sisällytettävä samaan transaktioon kuin _rs`TokenInstruction::InitializeMint`_ -ohje.      |
| _rs`UpdateMint`_                    | Päivittää luottamuksellisten siirtojen asetuksia mintille.                                                                                                          |
| _rs`ConfigureAccount`_              | Määrittää token accountin luottamuksellisia siirtoja varten.                                                                                                        |
| _rs`ApproveAccount`_                | Hyväksyy token accountin luottamuksellisia siirtoja varten, jos mint vaatii hyväksynnän uusille token accounteille.                                                 |
| _rs`EmptyAccount`_                  | Tyhjentää odottavan ja käytettävissä olevan luottamuksellisen saldon, jotta token account voidaan sulkea.                                                           |
...
| _rs`Transfer`_                      | Siirtää tokeneita token accountien välillä luottamuksellisesti.                                                                                                     |
| _rs`ApplyPendingBalance`_           | Muuntaa odottavan saldon käytettävissä olevaksi saldoksi talletusten tai siirtojen jälkeen.                                                                         |
| _rs`EnableConfidentialCredits`_     | Sallii token accountin vastaanottaa luottamuksellisia tokenien siirtoja.                                                                                            |
| _rs`DisableConfidentialCredits`_    | Estää saapuvat luottamukselliset siirrot, mutta sallii edelleen julkiset siirrot.                                                                                   |
| _rs`EnableNonConfidentialCredits`_  | Sallii token accountin vastaanottaa julkisia tokenien siirtoja.                                                                                                     |
| _rs`DisableNonConfidentialCredits`_ | Estää tavalliset siirrot, jotta tili vastaanottaa vain luottamuksellisia siirtoja.                                                                                  |
| _rs`TransferWithFee`_               | Siirtää tokeneita token accountien välillä luottamuksellisesti maksulla.                                                                                            |
| _rs`ConfigureAccountWithRegistry`_  | Vaihtoehtoinen tapa määrittää token accountit luottamuksellisia siirtoja varten käyttäen _rs`ElGamalRegistry`_ -tiliä _rs`VerifyPubkeyValidity`_ -todisteen sijaan. |
