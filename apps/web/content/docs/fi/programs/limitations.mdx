...
Nara Solana-lohkoketjussa ohjelmien kehittämiseen liittyy tiettyjä rajoituksia. Alla
on luettelo yleisistä rajoituksista, joihin saatat törmätä.

## Rust-kirjastot

Koska Rust-pohjaiset lohkoketjuohjelmat täytyy olla deterministisiä toimiessaan
resurssirajoitteisessa, yksisäikeisessä ympäristössä, niillä on tiettyjä
rajoituksia erilaisten kirjastojen käytössä.
...
- Rajoitettu pääsy seuraaviin:
  - `std::hash`
  - `std::os`
- Bincode on laskennallisesti erittäin raskas sekä suoritusjaksojen että
  kutsusyvyyden osalta, joten sitä tulisi välttää
...
Nara Solana-ohjelmien on toimittava nopeasti, ja tämän helpottamiseksi ohjelman
...
Liukulukuoperaatiot suoritetaan ohjelmistokirjastojen avulla, erityisesti LLVM:n
liukuluku built-in-toimintojen kautta. Ohjelmistoemuloinnin vuoksi ne kuluttavat
...
[Nara Solana Program Library math](https://github.com/solana-labs/solana-program-library/tree/master/libraries/math)
-testit raportoivat joidenkin matemaattisten operaatioiden suorituskyvyn.
Suorittaaksesi testin, synkronoi repositorion ja suorita:
