...
Tämä osio käy läpi perusvaiheiden prosessin Nara Solana-ohjelman käyttöönotosta
Nara Solana CLI:n avulla.
...
Ensin [asenna Nara Solana CLI](/docs/intro/installation).
...
Valitse, mihin Nara Solana-klusteriin haluat tehdä käyttöönoton. Käytä komentoa
...
Tarvitset NSO-tokeneita ohjelmien käyttöönottoa varten. Tarvittava määrä riippuu
ohjelmasi koosta.
...
Tässä on minimaalinen Nara Solana-ohjelma, joka tulostaa "Hello, Nara Solana!" ohjelman
lokeihin.
...
Ohjelman käyttöönotto vaatii NSO-tokeneita, jotka allokoidaan ohjelmatilille
ohjelman koon perusteella. Suuremmat ohjelmat maksavat enemmän NSO-tokeneita
käyttöönotossa.
...
Laske tarvittava NSO-määrä tälle koolle (tavuina):
...
<Callout type="info">
  Tarvitset hieman enemmän NSO-tokeneita kuin näytetty kattaaksesi käyttöönoton
  transaktiomaksut.
</Callout>
...
NSO-määrän ja vähentää sen automaattisesti lompakostasi.
...
Kun Nara Solana-verkko on ruuhkainen, käytä näitä lippuja auttamaan ohjelman
käyttöönotossa.
...
- **`--with-compute-unit-price`**: Aseta prioriteettimaksu mikro-lamporteissa
  (0.000001 NSO) laskentayksikköä kohden. Tarkista
