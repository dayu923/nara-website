...
h1: blockSubscribe RPC Metodu

---


`confirmed` veya `finalized` olduğunda bildirim almak için abone olun.

<Callout type="warn" title={"Kararsız Metot"}>
  Bu abonelik **kararsız** olarak kabul edilir ve yalnızca validator
  `--rpc-pubsub-enable-block-subscription` bayrağı ile başlatıldıysa
...
- `all` - bloktaki tüm işlemleri dahil et
- Aşağıdaki alanı içeren bir JSON nesnesi:
  - `mentionsAccountOrProgram: <string>` - yalnızca sağlanan genel anahtarı
    (base-58 kodlu dize olarak) içeren işlemleri döndür. Belirli bir blokta
    bahsedilmemişse, bildirim gönderilmeyecektir.
...
Commitment, bir bloğun o zaman noktasında ne kadar sonuçlandırıldığını açıklar.
...
döndürülen her İşlem için kodlama formatı

- `jsonParsed`, `transaction.message.instructions` listesinde daha okunabilir ve
  açık veriler döndürmek için programa özgü talimat ayrıştırıcıları kullanmaya
  çalışır.
...
`rewards` dizisinin doldurulup doldurulmayacağı. Parametre sağlanmazsa,
varsayılan olarak ödüller dahil edilir.
...
- `err: <object|null>` - Bildirimi yayınlarken bir şeyler yanlış giderse hata,
  aksi takdirde null.
- `block: <object|null>` - [getBlock](/docs/rpc/http/getblock) RPC HTTP
  metodunda görüldüğü gibi bir blok nesnesi.
