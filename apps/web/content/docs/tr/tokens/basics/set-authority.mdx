...
title: Yetki Ayarla
description: Mint veya token hesapları için yetki ayarlamayı öğrenin.

---


## Token Yetkilerini Anlama

### Mint Account Yetkileri

- **Mint Yetkisi**: Yeni token oluşturulmasını kontrol eder. Herhangi bir token
  hesabına token mint edebilir. Genellikle sabit arz token oluşturmak için
  başlangıç arzı oluşturulduktan sonra iptal edilir.
- **Dondurma Yetkisi**: Token hesaplarını dondurma ve çözme yeteneğini kontrol
  eder. Herhangi bir token hesabının token transferini engelleyebilir.
  Genellikle kullanıcılara tokenlarının dondurulamayacağını garanti etmek için
  iptal edilir.

### Token Hesap Yetkileri

- **Hesap Sahibi**: Token hesabı üzerinde tam kontrole sahiptir. Token transfer
  edebilir, yakabilir, delegeleri onaylayabilir ve bakiye sıfır olduğunda hesabı
  kapatabilir.
- **Kapatma Yetkisi**: Bakiyesi sıfır olduğunda token hesabını kapatabilir.
  Varsayılan olarak bu hesap sahibidir, ancak başka bir hesaba devredilebilir.

## Yetki nasıl ayarlanır
...
talimatı, mint ve token hesaplarındaki yetkileri değiştirir veya iptal eder.
Yalnızca mevcut yetki sahibi, izinlerini yeni bir adrese aktarabilir veya
yetkiyi null olarak ayarlayarak kalıcı olarak iptal edebilir. Bir kez iptal
edildikten sonra, yetki geri yüklenemez.
