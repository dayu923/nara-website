...
title: Aktarılamaz Tokenlar
description: InitializeNonTransferableMint'i nasıl etkinleştireceğinizi öğrenin.
...
tokenların basıldıktan sonra token account'lar arasında transfer edilmesini
engeller. Bir kez basıldıktan sonra, tokenlar kalıcı olarak token account'ta
kalır. Ancak, token account sahibi yine de tokenları yakabilir ve bakiye sıfıra
ulaştığında token account'ı kapatabilir.
