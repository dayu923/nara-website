...
description:
  Gizli Transfer uzantısı hakkında bilgi edinin ve token mint'lerine ve
  hesaplarına isteğe bağlı özellikler eklemek için nasıl kullanacağınızı
  öğrenin.
...
Gizli transferler, token hesapları arasında transfer miktarını açıklamadan token
transferi yapmanızı sağlar. Bu, gizlilik koruyan işlemler için kullanışlıdır.
Yalnızca transfer miktarları ve token bakiyeleri gizlidir. Token hesap adresleri
herkese açık kalır.

- [Protokol Genel Bakışı](https://www.solana-program.com/docs/confidential-balances/overview) -
  Altta yatan kriptografik protokol hakkında detaylar
...
- [Gizli Bakiyeler Yemek Kitabı](https://github.com/solana-developers/Confidential-Balances-Sample) -
  Gizli Transfer uzantısını nasıl kullanacağınıza dair kod parçaları

### Nasıl çalışır?

Gizli Transfer uzantısı, Token Extensions Program'a transfer miktarını
açıklamadan hesaplar arasında token transferi yapmanıza olanak tanıyan
...
1. Gizli transfer uzantısına sahip bir mint account oluşturun.
2. Gönderen ve alıcı için gizli transfer uzantısına sahip token hesapları
   oluşturun.
3. Gönderen hesaba token mint edin.
4. Gönderenin açık bakiyesini **gizli bekleyen bakiyeye yatırın**.
5. Gönderenin bekleyen bakiyesini **gizli kullanılabilir bakiyeye uygulayın**.
6. Gönderen token hesabından alıcı token hesabına gizli olarak token **transfer
   edin**.
7. Alıcının bekleyen bakiyesini **gizli kullanılabilir bakiyeye uygulayın**.
8. Alıcının gizli kullanılabilir bakiyesini **açık bakiyeye çekin**.

Gizli transfer akışındaki adımlar hakkında daha fazla bilgi için ilgili
sayfalara bakın:

<Cards>
  <Card
    title="Mint Hesabı Oluştur"
    href="/docs/tokens/extensions/confidential-transfer/create-mint"
  >
    Gizli Transfer uzantısına sahip bir mint hesabı nasıl oluşturulur
  </Card>
  <Card
    title="Token Hesabı Oluştur"
    href="/docs/tokens/extensions/confidential-transfer/create-token-account"
  >
    Gizli Transfer uzantısıyla bir token hesabı nasıl yapılandırılır
  </Card>
  <Card
    title="Token Yatır"
  ...
  >
    Gizli kullanılabilir bakiyeden tokenler nasıl çekilir
  </Card>
  <Card
    title="Token Transfer Et"
    href="/docs/tokens/extensions/confidential-transfer/transfer-tokens"
  >
    Token hesapları arasında gizli olarak tokenler nasıl transfer edilir
  </Card>
</Cards>

Aşağıdaki diyagram, gizli token transferleri için temel akışın detaylı bir
...
Aşağıdaki Rust örneği, önceki diyagrama karşılık gelen gizli token transferleri
için adımları göstermektedir. Adımlar, netlik sağlamak için yardımcı
fonksiyonlara ayrılmıştır.
...
kullanmak için Nara Solana CLI'nin kurulu olması gerekir.
...
<Callout type="info">
  ...
  etkinleştirilmemiştir. Örnek kodu çalıştırmak için mainnet Token Extension
  Program'ını klonlamanız gerekir.
</Callout>
...
Gizli Transfer uzantısının tam
[talimatlar](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/instruction.rs#L29)
listesi aşağıdaki gibidir:

| Talimat                             | Açıklama                                                                                                                                                       |
| ----------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | Mint hesabını gizli transferler için ayarlar. Bu talimat, _rs`TokenInstruction::InitializeMint`_ talimatı ile aynı işlemde yer almalıdır.                      |
| _rs`UpdateMint`_                    | Bir mint için gizli transfer ayarlarını günceller.                                                                                                             |
| _rs`ConfigureAccount`_              | Bir token hesabını gizli transferler için ayarlar.                                                                                                             |
| _rs`ApproveAccount`_                | Mint yeni token hesapları için onay gerektiriyorsa, bir token hesabını gizli transferler için onaylar.                                                         |
| _rs`EmptyAccount`_                  | Bir token hesabını kapatmaya izin vermek için bekleyen ve mevcut gizli bakiyeleri boşaltır.                                                                    |
| _rs`Deposit`_                       | Genel token bakiyesini bekleyen gizli bakiyeye dönüştürür.                                                                                                     |
| _rs`Withdraw`_                      | Mevcut gizli bakiyeyi genel bakiyeye geri dönüştürür.                                                                                                          |
| _rs`Transfer`_                      | Token hesapları arasında gizli olarak token transferi yapar.                                                                                                   |
| _rs`ApplyPendingBalance`_           | Yatırma veya transferlerden sonra bekleyen bakiyeyi mevcut bakiyeye dönüştürür.                                                                                |
| _rs`EnableConfidentialCredits`_     | Bir token hesabının gizli token transferleri almasına izin verir.                                                                                              |
| _rs`DisableConfidentialCredits`_    | Genel transferlere izin verirken gelen gizli transferleri engeller.                                                                                            |
| _rs`EnableNonConfidentialCredits`_  | Bir token hesabının genel token transferleri almasına izin verir.                                                                                              |
...
| _rs`TransferWithFee`_               | Token hesapları arasında ücretli olarak gizli token transferi yapar.                                                                                           |
| _rs`ConfigureAccountWithRegistry`_  | _rs`VerifyPubkeyValidity`_ kanıtı yerine _rs`ElGamalRegistry`_ hesabı kullanarak token hesaplarını gizli transferler için yapılandırmanın alternatif bir yolu. |

| Talimat                             | Açıklama                                                                                                                                                       |
| ----------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | Gizli transferler için mint account'u kurar. Bu talimat, _rs`TokenInstruction::InitializeMint`_ talimatı ile aynı işlemde bulunmalıdır.                        |
| _rs`UpdateMint`_                    | Bir mint için gizli transfer ayarlarını günceller.                                                                                                             |
| _rs`ConfigureAccount`_              | Gizli transferler için bir token account'u kurar.                                                                                                              |
| _rs`ApproveAccount`_                | Mint, yeni token hesapları için onay gerektiriyorsa, bir token account'u gizli transferler için onaylar.                                                       |
| _rs`EmptyAccount`_                  | Bir token hesabını kapatmaya izin vermek için bekleyen ve mevcut gizli bakiyeleri boşaltır.                                                                    |
| _rs`Deposit`_                       | Açık token bakiyesini bekleyen gizli bakiyeye dönüştürür.                                                                                                      |
| _rs`Withdraw`_                      | Mevcut gizli bakiyeyi açık bakiyeye geri dönüştürür.                                                                                                           |
| _rs`Transfer`_                      | Token'ları token hesapları arasında gizli olarak transfer eder.                                                                                                |
| _rs`ApplyPendingBalance`_           | Yatırma veya transfer işlemlerinden sonra bekleyen bakiyeyi mevcut bakiyeye dönüştürür.                                                                        |
| _rs`EnableConfidentialCredits`_     | Bir token hesabının gizli token transferleri almasına izin verir.                                                                                              |
| _rs`DisableConfidentialCredits`_    | Açık transferlere izin verirken gelen gizli transferleri engeller.                                                                                             |
| _rs`EnableNonConfidentialCredits`_  | Bir token hesabının açık token transferleri almasına izin verir.                                                                                               |
| _rs`DisableNonConfidentialCredits`_ | Hesabın yalnızca gizli transferleri almasını sağlamak için normal transferleri engeller.                                                                       |
| _rs`TransferWithFee`_               | Token'ları token hesapları arasında ücretli olarak gizli şekilde transfer eder.                                                                                |
| _rs`ConfigureAccountWithRegistry`_  | _rs`VerifyPubkeyValidity`_ kanıtı yerine _rs`ElGamalRegistry`_ hesabı kullanarak token hesaplarını gizli transferler için yapılandırmanın alternatif bir yolu. |
