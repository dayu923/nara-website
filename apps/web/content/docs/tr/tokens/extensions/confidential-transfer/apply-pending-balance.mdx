...
description:
  Fonları kullanılabilir hale getirmek için bekleyen bakiyeyi nasıl
  uygulayacağınızı öğrenin.

---


## Bekleyen bakiyeyi kullanılabilir bakiyeye uygulama

Tokenler gizli olarak transfer edilmeden önce, genel token bakiyesinin gizli
bakiyeye dönüştürülmesi gerekir. Bu dönüşüm iki aşamada gerçekleşir:

1. **Gizli Bekleyen Bakiye**: Başlangıçta, tokenler genel bakiyeden "bekleyen"
...
Bu bölüm, ikinci aşamayı açıklar: bekleyen bakiyeyi kullanılabilir bakiyeye
uygulama.

Tokenler genel bakiyeden "yatırıldığında" veya bir token account'tan diğerine
gizli olarak transfer edildiğinde, tokenler başlangıçta gizli bekleyen bakiyeye
eklenir. Tokenler gizli transferler için kullanılmadan önce, bekleyen bakiyenin
kullanılabilir bakiyeye "uygulanması" gerekir.
...
Aşağıdaki diyagram, bekleyen bakiyeyi kullanılabilir bakiyeye uygulamada yer
alan adımları göstermektedir:
...
`ApplyPendingBalance` talimatı içeren bir işlem oluşturup gönderen bir
`confidential_transfer_apply_pending_balance` metodu sağlar.

### Örnek kod

Aşağıdaki örnek, gizli bekleyen bakiyeyi gizli kullanılabilir bakiyeye nasıl
uygulayacağınızı göstermektedir.
...
<Callout type="info">
  ...
  Program'ı klonlamanız gerekmektedir.
</Callout>
