...
Bir token hesabından diğerine gizli token transferi yapmak için, hem gönderen
hem de alıcının _rs`ConfidentialTransferAccount`_ durumu ile yapılandırılmış ve
gizli transferler için onaylanmış token hesaplarına sahip olması gerekir.
Gönderenin token hesabının ayrıca transfer edilecek kullanılabilir bir gizli
bakiyeye sahip olması gerekir.
...
1. İstemci tarafında
   ...
   **Eşitlik Kanıtı
   ...
   Transfer sonrası yeni kullanılabilir bakiye şifre metninin, karşılık gelen
   [Pedersen taahhüdüyle](https://en.wikipedia.org/wiki/Commitment_scheme)
   eşleştiğini doğrular ve kaynak hesabın yeni kullanılabilir bakiyesinin
   `new_balance = current_balance - transfer_amount` olarak doğru hesaplandığını
   sağlar.
...
- `confidential_transfer_create_context_state_account` bir kanıt hesabı
  oluşturan yöntem.
- `confidential_transfer_transfer` `Transfer` talimatını çağıran yöntem.
- `confidential_transfer_close_context_state_account` bir kanıt hesabını kapatan
  yöntem.
