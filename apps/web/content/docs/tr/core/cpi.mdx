...
Bir cross-program invocation (CPI), bir Nara Solana programının doğrudan başka bir
programın talimatlarını çağırdığında gerçekleşir. Bu, program
birleştirilebilirliğine olanak tanır. Bir Nara Solana
...
<Callout>
  Program talimatı çağrısının maksimum yüksekliği [`max_instruction_stack_depth`](https://github.com/anza-xyz/agave/blob/v2.1.13/compute-budget/src/compute_budget.rs#L38) olarak adlandırılır ve [MAX_INSTRUCTION_STACK_DEPTH](https://github.com/anza-xyz/agave/blob/v2.1.13/compute-budget/src/compute_budget.rs#L13) sabitine 5 olarak ayarlanmıştır.
...
fonksiyonu kullanılır. Bu fonksiyon, imzalayan [PDA'ları](<(/docs/core/pda)>)
türetmek için kullanılan [imzalayan seed'lerini](mention:signer-seeds) alır.
Nara Solana çalışma zamanı, dahili olarak çağıran programın `signers_seeds` ve
...
NSO'u bir PDA'dan alıcı hesaba aktaran tek bir talimat içerir.
...
CPI yapmayı göstermektedir. Program, PDA hesabından başka bir hesaba NSO aktaran
...
Her örnek, bir hesaptan diğerine NSO aktaran tek bir talimat içerir.
...
göstermektedir. Örnek, bir hesaptan diğerine NSO aktaran tek bir talimat içerir.
