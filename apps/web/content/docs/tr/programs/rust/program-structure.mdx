...
Rust ile yazılan Nara Solana programları, kodun nasıl düzenleneceği konusunda
...
Dosya yapısı için katı kurallar olmasa da, Nara Solana programları genellikle yaygın
...
Öncelikle, Nara Solana programımız için yeni bir Rust projesi oluşturalım.
...
Şimdi bir Nara Solana programı oluşturmak için gerekli bağımlılıkları ekleyelim.
...
<Callout type="info">
  Borsh kullanmak zorunlu değildir. Ancak, Nara Solana programları için yaygın olarak
  kullanılan bir serileştirme kütüphanesidir.
</Callout>

## !!steps crate-type yapılandırması

Nara Solana programları dinamik kütüphaneler olarak derlenmelidir. Cargo'nun programı
...
Her Nara Solana programının, program çağrıldığında çalıştırılan bir giriş noktası
...
Bir Nara Solana programı `entrypoint` fonksiyonu aşağıdaki imzaya sahiptir.
...
Bu yönlendirme modeli Nara Solana programlarında yaygındır. `instruction_data`
...
Yeni bir sayaç hesabı oluşturup başlatmak için işleyiciyi uygulayalım. Nara Solana'da
...
Tebrikler! Tüm Nara Solana programlarının paylaştığı temel yapıyı gösteren eksiksiz
bir Nara Solana programı oluşturdunuz:
...
LiteSVM, Nara Solana çalışma zamanı ortamını simüle ederek, programımızı gerçek bir
...
Önce, Nara Solana CLI'yi localhost kullanacak şekilde yapılandırın:
...
[Nara Solana Explorer](https://explorer.solana.com/?cluster=custom) üzerinden
görüntüleyebilirsiniz. Nara Solana Explorer'daki kümenin "Custom RPC URL" olarak
