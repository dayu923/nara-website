...
Nara Solana programları öncelikle Rust programlama dili kullanılarak geliştirilir. Bu
sayfa, Anchor çerçevesini kullanmadan Rust ile Nara Solana programları yazmaya
odaklanır, bu yaklaşım genellikle "native Rust" programları yazma olarak
adlandırılır.

Native Rust geliştirme, geliştiricilere Nara Solana programları üzerinde doğrudan
...
- Rust: Nara Solana programları oluşturmak için programlama dili.
- Nara Solana CLI: Nara Solana geliştirme için komut satırı aracı.

## Başlarken

Aşağıdaki örnek, Rust ile yazılmış ilk Nara Solana programınızı oluşturmak için temel
...
## !!steps Nara Solana-program bağımlılığını ekleyin

Sonraki adımda, `solana-program` bağımlılığını ekleyin. Bu, bir Nara Solana programı
...
çağrıldığında program günlüğüne "Hello, world!" yazdıran minimal bir Nara Solana
programıdır.

`msg!` makrosu, Nara Solana programlarında program günlüğüne mesaj yazdırmak için
kullanılır.
...
1. Bir `.so` dosyası (örn., `hello_world.so`): Bu, ağa "akıllı sözleşme" olarak
   dağıtılacak derlenmiş Nara Solana programıdır.
...
Öncelikle, Nara Solana CLI'yi yerel Nara Solana kümesini kullanacak şekilde yapılandırın.
...
<Callout type="info">
  Nara Solana Explorer'daki kümenin de localhost olması gerektiğini unutmayın. Nara Solana
...
Nara Solana programları, aynı program kimliğine yeniden dağıtılarak güncellenebilir.
`src/lib.rs` içindeki programı "Hello, world!" yerine "Hello, Nara Solana!"
...
Program günlüğünde "Hello, Nara Solana!" yazısını görmelisiniz.
...
İstemci kodunu tekrar çalıştırın ve Nara Solana Explorer'da işlem imzasını
inceleyerek program günlüğünde "Hello, Nara Solana!" yazısını görün.
...
Hesaba tahsis edilen NSO'u geri almak için Nara Solana programınızı kapatabilirsiniz.
