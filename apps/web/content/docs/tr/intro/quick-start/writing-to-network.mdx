...
Önceki bölümde, Nara Solana ağından veri okumayı öğrendiniz. Şimdi ağa nasıl veri
yazacağınızı öğreneceksiniz. Nara Solana ağına yazmak, bir veya daha fazla talimat
içeren işlemler göndermeyi içerir.

Programlar, her [talimatın](/docs/core/instructions) ne yapacağına dair iş
mantığını tanımlar. Bir [işlem](/docs/core/transactions) gönderdiğinizde, Nara Solana
çalışma zamanı her talimatı sırayla ve atomik olarak yürütür. Bu bölümdeki
örnekler, Nara Solana programlarını çağırmak için işlemlerin nasıl oluşturulacağını
ve gönderileceğini göstermektedir, bunlar şunları içerir:

1. Hesaplar arasında NSO transferi
2. Yeni bir token oluşturma

## NSO Transferi

Aşağıdaki örnek, iki hesap arasında NSO transferi yapar. Her hesabın bir sahip
programı vardır ve bu program, hesabın NSO bakiyesini düşürebilen tek
programdır.

Tüm cüzdan hesapları System Program tarafından sahiplenilir. NSO transfer etmek
...
Bir Nara Solana kümesine bağlantı oluşturun.
...
`sender` hesabını fonlamak için NSO airdrop'u isteyin.
...
Gönderen hesaba NSO ekleyin. Ana ağ dışındaki ağlarda, test için NSO almak üzere
...
[`lamports`](mention:lamports) için [`fromPubkey`](mention:from) hesabından
[`toPubkey`](mention:to) hesabına NSO transfer eden bir talimat oluşturur.
...
[Gönderen](mention:sender) keypair, kendi hesabından NSO transferini
...
İşlem imzası, Nara Solana Explorer'da işlemi aramak için kullanılabilecek benzersiz
bir tanımlayıcıdır.
...
Aşağıdaki örnek, Token Extensions Program kullanarak Nara Solana'da yeni bir token
...
[Token Extensions Program'ın](mention:programId)
...
Bu yaklaşım, karmaşık Nara Solana işlemleri oluştururken yaygındır, çünkü tüm
...
Döndürülen işlem imzası, işlemi Nara Solana Explorer'da incelemek için
kullanılabilir.
