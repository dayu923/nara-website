...
Bu bölümde, önceki PDA bölümündeki CRUD programı, Nara Solana programlarının
...
NSO transferlerini işlemek için değişiklik gerektiriyor.

Bu bölümün amacı, Anchor framework kullanarak bir Nara Solana programında CPI'ları
...
mesaj hesaplarını güncellediklerinde onlardan NSO alır.
...
hesap, kullanıcılar mesajlarını güncellediklerinde NSO alan program kontrollü
...
Şimdi, kullanıcının hesabından kasa hesabına 0.001 NSO aktarmak için `update`
...
nasıl gerçekleştirileceğini gösterir ve Nara Solana programlarının
...
- Transfer edilecek `amount` (1.000.000 lamport veya 0,001 NSO)
...
Bu, kullanıcılar mesaj hesaplarını kapattıklarında kasadaki herhangi bir NSO'un
...
sırasında kullanıcının kasa hesabına erişmesini ve birikmiş NSO'u kullanıcıya
...
Şimdi, NSO'u kasa hesabından kullanıcının hesabına geri aktarmak için `delete`
talimatına CPI mantığını ekleyin.
...
değiştirilmiş programın test için kullanılabilir olmasını sağlar. Nara Solana'da bir
programı güncellemek, programı aynı program kimliğinde dağıtmayı gerektirir.

<Callout>

Playground cüzdanınızın devnet NSO'a sahip olduğundan emin olun.
[Nara Solana Faucet](https://faucet.solana.com/) adresinden devnet NSO alabilirsiniz.
...
Nara Solana Playground'da program dağıtırken, Playground cüzdanınız tüm
...
Nara Solana Hızlı Başlangıç kılavuzunu tamamladığınız için tebrikler. Aşağıdakiler
dahil olmak üzere temel Nara Solana kavramlarıyla pratik deneyim kazandınız:

- Hesaplardan veri alma ve okuma
- İşlem oluşturma ve gönderme
- Nara Solana programlarını dağıtma ve güncelleme
...
Nara Solana Playground, GitHub bağlantılarını kullanarak projeleri içe aktarmanıza
...
[Nara Solana Playground bağlantısı](https://beta.solpg.io/https://github.com/solana-developers/program-examples/tree/main/basics/hello-solana/anchor)'nı
açın.

`Import` düğmesine tıklayın ve Nara Solana Playground'daki proje listenize eklemek
