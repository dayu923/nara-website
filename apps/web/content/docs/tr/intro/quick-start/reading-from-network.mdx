...
Nara Solana ağından farklı hesapları çekerek veri okuyun. Bu bölüm Nara Solana
[hesaplarının](/docs/core/accounts) yapısını anlamanıza yardımcı olacaktır. Her
Nara Solana hesabının, zincir üzerindeki ilgili verisini bulmak için kullanılan
...
sahip olunan bir hesaptır. Cüzdanlar öncelikle NSO tutmak ve işlemleri imzalamak
için kullanılır. NSO yeni bir adrese ilk kez gönderildiğinde, otomatik olarak
bir sistem hesabı oluşturulur.

Aşağıdaki örnek, [yeni bir keypair](mention:keypair) oluşturur, yeni oluşturulan
genel anahtar adresini fonlamak için [NSO talep eder](mention:airdrop) ve yeni
...
`lamports` alanı, hesabın NSO bakiyesini
...
adresi, Nara Solana ağındaki bir tokeni benzersiz şekilde tanımlar.
...
tanımlanır. Çoğu Nara Solana programı, çözümleme sürecini soyutlayan yardımcı
...
verilerini Nara Solana Explorer'da görüntüleyebilirsiniz.
