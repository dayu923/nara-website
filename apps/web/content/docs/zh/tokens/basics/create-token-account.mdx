...
<Callout type="info">
  请注意，在源代码中，开发者将 Token Account 称为 `Account` 类型。无论是 [Token
  ...
  还是 [Token Extension
  Program](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/state.rs#L102-L123)，它们的
...
此单一指令通过跨程序调用 (CPI) 自动处理创建 token account 并对其进行初始化。
