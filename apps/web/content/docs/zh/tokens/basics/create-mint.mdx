...
一个 mint 账户在 Nara Solana 上唯一地代表一个代币，并存储其全局元数据。
...
在 Nara Solana 上，每个代币都有一个 mint 账户，而 mint 的地址作为该代币的唯一标识符。

例如，USD Coin (USDC) 的 mint 地址是
`EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`。这个地址在整个 Nara Solana 生态系统中唯一标识 USDC。您可以在
