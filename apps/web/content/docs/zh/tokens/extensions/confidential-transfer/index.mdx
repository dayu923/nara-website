...
title: 保密转账
description: 了解保密转账扩展以及如何使用它为代币铸币和账户添加可选功能。
...
保密转账使您能够在代币账户之间转移代币，而无需透露转账金额。这对于保护隐私的交易非常有用。只有转账金额和代币余额是保密的，代币账户地址仍然是公开的。

- [协议概述](https://www.solana-program.com/docs/confidential-balances/overview) - 底层加密协议的详细信息
- [快速入门指南](https://www.solana-program.com/docs/confidential-balances#setup) - 设置和基本 CLI 命令
- [保密余额手册](https://github.com/solana-developers/Confidential-Balances-Sample) - 使用保密转账扩展的代码示例

### 它是如何工作的？

保密转账扩展为
[Token Extensions Program](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/instruction.rs#L29)
添加了指令，使您能够在账户之间转移代币而不透露转账金额。
...
6. 从发送方代币账户**保密转账**代币到接收方代币账户。
7. **应用**接收方的待处理余额到**保密可用余额**。
8. **提取**接收方的保密可用余额到**公开余额**。
...
下面的 Rust 示例展示了与前述图表对应的保密代币转账步骤。这些步骤被分解为辅助函数以便于理解。

要运行此示例，请使用以下命令启动一个从主网克隆的包含 Token Extensions
Program 的本地 validator。您必须安装 Nara Solana CLI 才能使用本地 validator。
...
<Callout type="info">
  在撰写本文时，保密转账功能尚未在默认的本地 validator 上启用。您必须克隆主网的
...
完整的保密转账扩展[指令](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/instruction.rs#L29)如下：

| 指令                                | 描述                                                                                                                 |
| ----------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | 设置 mint account 以支持保密转账。此指令必须与 _rs`TokenInstruction::InitializeMint`_ 指令在同一事务中包含。         |
| _rs`UpdateMint`_                    | 更新 mint 的保密转账设置。                                                                                           |
| _rs`ConfigureAccount`_              | 设置 token account 以支持保密转账。                                                                                  |
| _rs`ApproveAccount`_                | 如果 mint 要求新 token account 获得批准，则批准该 token account 以支持保密转账。                                     |
| _rs`EmptyAccount`_                  | 清空待处理和可用的保密余额以允许关闭 token account。                                                                 |
| _rs`Deposit`_                       | 将公共代币余额转换为待处理的保密余额。                                                                               |
| _rs`Withdraw`_                      | 将可用的保密余额转换回公共余额。                                                                                     |
| _rs`Transfer`_                      | 在 token account 之间保密地转移代币。                                                                                |
| _rs`ApplyPendingBalance`_           | 在存款或转账后将待处理余额转换为可用余额。                                                                           |
| _rs`EnableConfidentialCredits`_     | 允许 token account 接收保密代币转账。                                                                                |
| _rs`DisableConfidentialCredits`_    | 阻止传入的保密转账，同时仍允许公共转账。                                                                             |
| _rs`EnableNonConfidentialCredits`_  | 允许 token account 接收公共代币转账。                                                                                |
| _rs`DisableNonConfidentialCredits`_ | 阻止常规转账，使账户仅接收保密转账。                                                                                 |
| _rs`TransferWithFee`_               | 在 token account 之间保密地转移代币并收取费用。                                                                      |
| _rs`ConfigureAccountWithRegistry`_  | 使用 _rs`ElGamalRegistry`_ 账户而不是 _rs`VerifyPubkeyValidity`_ 证明来配置 token account 以支持保密转账的替代方法。 |

| 指令                                | 描述                                                                                                           |
| ----------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | 设置用于保密转账的 mint account。此指令必须与 _rs`TokenInstruction::InitializeMint`_ 指令在同一交易中包含。    |
| _rs`UpdateMint`_                    | 更新 mint 的保密转账设置。                                                                                     |
| _rs`ConfigureAccount`_              | 设置用于保密转账的 token account。                                                                             |
| _rs`ApproveAccount`_                | 如果 mint 要求新 token account 需要批准，则批准该 token account 用于保密转账。                                 |
| _rs`EmptyAccount`_                  | 清空待处理和可用的保密余额以关闭 token account。                                                               |
| _rs`Deposit`_                       | 将公共 token 余额转换为待处理的保密余额。                                                                      |
| _rs`Withdraw`_                      | 将可用的保密余额转换回公共余额。                                                                               |
| _rs`Transfer`_                      | 在 token account 之间进行保密转账。                                                                            |
| _rs`ApplyPendingBalance`_           | 在存款或转账后，将待处理余额转换为可用余额。                                                                   |
| _rs`EnableConfidentialCredits`_     | 允许 token account 接收保密 token 转账。                                                                       |
| _rs`DisableConfidentialCredits`_    | 阻止接收保密转账，同时仍允许公共转账。                                                                         |
| _rs`EnableNonConfidentialCredits`_  | 允许 token account 接收公共 token 转账。                                                                       |
| _rs`DisableNonConfidentialCredits`_ | 阻止常规转账，使账户仅接收保密转账。                                                                           |
| _rs`TransferWithFee`_               | 在 token account 之间进行带手续费的保密转账。                                                                  |
| _rs`ConfigureAccountWithRegistry`_  | 使用 _rs`ElGamalRegistry`_ 账户而非 _rs`VerifyPubkeyValidity`_ 证明来配置 token account 的保密转账的替代方法。 |
