...
title: 转移代币
description:
  了解如何将代币从一个 token account 私密地转移到另一个 token account。
...
<Callout type="warn">
  ...
  这意味着机密转移扩展目前不可用。尽管概念仍然有效，但代码示例将无法运行。
</Callout>

## 如何将代币从一个 token account 私密地转移到另一个 token account

要将代币私密地从一个 token account 转移到另一个 token
account，发送方和接收方都必须配置了 _rs`ConfidentialTransferAccount`_
状态并获得机密转移的批准。发送方的 token
account 还必须有可用的机密余额以进行转移。

要私密地转移代币：

1. 在客户端创建
   [三个证明](https://github.com/solana-program/token-2022/blob/d073cb89dbcd430387b5c6fb4b7157911351e4a3/confidential-transfer/proof-generation/src/transfer.rs#L35)：

   **等式证明 ([CiphertextCommitmentEqualityProofData](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/ciphertext_commitment_equality.rs#L56))**：验证转移后的新可用余额密文是否与其对应的
   [Pedersen 承诺](https://en.wikipedia.org/wiki/Commitment_scheme)匹配，确保源账户的新可用余额正确计算为
   `new_balance = current_balance - transfer_amount`。

   **密文有效性证明 ([BatchedGroupedCiphertext3HandlesValidityProofData](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/batched_grouped_ciphertext_validity/handles_3.rs#L63))**：验证转移金额密文是否为所有三方（源、目标和审计方）正确生成，确保转移金额在每方的公钥下正确加密。

   **范围证明 ([BatchedRangeProofU128Data](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/batched_range_proof/batched_range_proof_u128.rs#L37))**：验证新可用余额和转移金额（分为低/高位）均为非负数且在指定范围内。
...
4. 关闭证明上下文状态账户以回收用于创建它们的 NSO。

下图展示了从发送者的 token 账户向接收者的 token 账户转移 token 所涉及的步骤。
...
要在两个 token 账户之间机密地转移 token，您需要：
...
以下示例展示了如何在 token 账户之间机密地转移 token。

要运行该示例，请使用以下命令启动一个从主网克隆的包含 Token Extension
Program 的本地 validator。您必须安装 Nara Solana CLI 才能启动本地 validator。
...
<Callout type="info">
  在撰写本文时，机密转移功能尚未在默认的本地 validator 上启用。您必须克隆主网的
