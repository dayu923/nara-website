...
在代币能够进行机密转账之前，公共代币余额必须转换为机密余额。此转换分为两个阶段：

1. **机密待处理余额**：最初，代币从公共余额“存入”到“待处理”的机密余额中。
2. **机密可用余额**：然后将待处理余额“应用”到可用余额中，使代币可用于机密转账。

本节将解释第二阶段：将待处理余额应用为可用余额。

当代币从公共余额“存入”或从一个代币账户机密转账到另一个代币账户时，代币最初会被添加到机密待处理余额中。在代币能够用于机密转账之前，必须将待处理余额“应用”到可用余额中。
...
`spl_token_client` crate 提供了一个
`confidential_transfer_apply_pending_balance` 方法，用于构建并发送包含
`ApplyPendingBalance` 指令的交易，如以下示例所示。

### 示例代码

以下示例演示了如何将机密待处理余额应用为机密可用余额。
...
<Callout type="info">
  在撰写本文时，Confidential Transfers 尚未在默认的本地 validator
  上启用。您必须克隆主网的 Token Extensions Program 才能运行示例代码。
</Callout>
