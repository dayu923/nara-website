...
title: 默认状态
description: 了解如何启用 DefaultAccountStateExtension。

---


## 如何启用 DefaultAccountStateExtension
...
为所有为某个 mint 创建的新 token
account 设置默认状态。启用后，此扩展会自动将每个新 token
account 初始化为指定的状态，例如冻结。这使得 token 创建者可以强制执行特定行为，例如要求所有新账户默认被冻结，直到明确解冻。
