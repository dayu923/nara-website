...
title: 关闭 Mint
description: 了解如何启用 MintCloseAuthority 扩展。

---


## 如何启用 MintCloseAuthority 扩展
...
扩展允许关闭 mint 账户并回收其 rent。此扩展必须在创建 mint 时初始化。

只有指定的关闭权限持有者才能关闭 mint 账户。类似于只有在代币余额为零时才能关闭代币账户，具有此扩展的 mint 账户只有在整个代币供应量减少到零时才能关闭。
