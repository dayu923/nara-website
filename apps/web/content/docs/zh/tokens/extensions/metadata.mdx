...
title: 元数据与元数据指针扩展
description: 了解如何启用 MetadataPointerExtension 和 TokenMetadata 扩展。

---


## 如何启用 MetadataPointerExtension 和 TokenMetadata 扩展
...
允许一个 mint account 指定其元数据账户的地址。此指针可以引用由实现
[Token Metadata Interface](https://github.com/solana-program/token-metadata/tree/main)
的程序拥有的任何账户，从而提供元数据存储位置的灵活性。

Token Extensions Program 直接实现了
[Token Metadata Interface](https://github.com/solana-program/token-metadata/blob/main/interface/src/instruction.rs)，允许元数据存储在 mint
account 本身上。这包括代币的名称、符号、URI 和其他描述性数据。当指针扩展指向 mint 本身时，
...
扩展可以用于将所有元数据存储在 mint account 中。
