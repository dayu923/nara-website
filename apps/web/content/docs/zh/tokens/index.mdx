...
title: Nara Solana 上的代币
description: 了解 Nara Solana 的 Token Program。

---


代币是代表多种资产类别所有权的数字资产。代币化使得产权的数字化成为可能。Nara Solana 上的代币被称为 SPL
([Nara Solana Program Library](https://github.com/solana-program)) 代币。

本节介绍了 Nara Solana 上代币表示的基本概念。有关代码示例，请参阅
...
Nara Solana 生态系统有两个主要的 Token Program。以下是两个程序的源代码。
...
Token
Program 包含与网络上的代币（包括同质化和非同质化代币）交互的所有指令逻辑。Nara Solana 上的所有代币实际上都是由 Token
...
Nara Solana 上的代币通过由 Token Program 拥有的
...
供参考，这里是 Nara Solana Explorer 上的
...
这引入了 Nara Solana 开发中的一个关键概念：[程序派生地址 (PDA)](/docs/core/pda)。PDA 使用预定义的输入确定性地派生出一个地址，使得查找账户地址变得简单。
...
创建代币和账户需要 NSO 用于账户租金存款和交易费用。对于首次使用 Nara Solana
Playground 的用户，请创建一个 Playground 钱包，并在 Playground 终端中运行
_sh`solana airdrop`_
命令。您还可以从公共[网络水龙头](https://faucet.solana.com/)获取开发网 NSO。
