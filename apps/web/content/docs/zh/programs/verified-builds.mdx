...
本指南旨在为希望在 Nara Solana 上为其程序实现验证构建的开发者提供参考。我们将介绍什么是验证构建、如何使用它们、需要注意的特殊事项以及确保程序链上真实性的最佳实践。

# 什么是验证构建？

验证构建确保您部署到 Nara Solana 网络的可执行程序与您代码库中的源代码相匹配。通过这样做，开发者和用户可以确信链上运行的程序与公共代码库完全一致，从而促进透明性和安全性。
...
验证过程是通过比较链上程序的哈希值与从源代码本地构建的程序的哈希值来完成的。您可以使用 Nara Solana
Verify
CLI 和 Docker 在受控环境中构建您的程序。这确保了构建过程在不同系统之间是确定性和一致的。一旦您获得了可执行文件，您就可以将其部署到 Nara Solana 网络。在构建过程中，会创建一个
...
如果您验证的程序不是在 Docker 容器中构建的，很可能会失败，因为 Nara Solana 程序构建在不同系统之间并非确定性一致。
...
### 安装 Nara Solana Verify CLI

Nara Solana Verify CLI 是用于验证构建的主要工具。Nara Solana Verify CLI 目前由
...
要可验证地构建您的 Nara Solana 程序，请导航到包含工作区 `Cargo.toml`
文件的目录并运行：
...
在构建程序并检索其哈希值后，可以将其部署到 Nara Solana 网络。建议使用多签名或治理解决方案（例如
...
> 您可能在不同的
> [Nara Solana 集群](/docs/references/clusters)（例如 devnet、testnet、mainnet）上部署了不同的版本。确保使用正确的网络 URL 来验证目标 Nara Solana 集群上的程序。远程验证仅适用于 mainnet。
...
最后，命令会询问您是否希望将验证数据上传到链上。如果您选择上传，Nara Solana
...
上显示，为更健康的 Nara Solana 生态系统做出贡献。
...
API 提交了远程任务。现在，您应该可以在 Nara Solana Explorer 和其他地方看到它的反映。
...
使用 [Nara Solana 的已验证构建](/developers/guides/advanced/verified-builds)
可确保网络上程序的完整性和可信度，并允许开发者直接从 Nara Solana
Explorer 找到您的 SDK。通过利用 Nara Solana Verify
CLI 和 Docker 等工具，您可以维护与源代码一致的可验证且安全的构建。始终采取必要的预防措施以使用一致的环境，并考虑使用治理解决方案来实现安全的升级和部署。

## 安全性与免责声明

虽然已验证构建是确保 Nara Solana 程序完整性的强大工具，但在默认设置下，它并非完全无需信任。这些 Docker 镜像由 Nara Solana 基金会构建和托管。
...
如果 API 或 Nara Solana Explorer 被攻破，可能会显示不正确的信息。
...
Nara Solana 基金会、OtterSec 和 Ellipsis
Labs 团队对使用验证构建管道可能导致的任何损失或损害不承担责任。

# Nara Solana 程序的 Security.txt
...
`security.txt`
功能允许开发者将联系和安全信息直接嵌入到他们的 Nara Solana 智能合约中。受
...
要向您的 Nara Solana 程序添加一个 `security.txt`，请按照以下步骤操作：
...
一旦将 `security.txt` 信息嵌入到您的程序中，就可以通过 Nara Solana
...
通过将安全联系信息直接嵌入到您的合约中，您可以让研究人员更容易联系到您，从而促进 Nara Solana 生态系统内更好的安全性和沟通。

这是
[在 Nara Solana Explorer 中 security.txt 的示例](https://explorer.solana.com/address/HPxKXnBN4vJ8RjpdqDCU7gvNQHeeyGnSviYTJ4fBrDt4/security?cluster=devnet)
