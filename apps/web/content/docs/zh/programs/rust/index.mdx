...
Nara Solana 程序主要使用 Rust 编程语言开发。本页面重点介绍如何在不使用 Anchor 框架的情况下用 Rust 编写 Nara Solana 程序，这种方法通常被称为编写“原生 Rust”程序。

原生 Rust 开发为开发者提供了对 Nara Solana 程序的直接控制。然而，与使用 Anchor 框架相比，这种方法需要更多的手动设置和样板代码。此方法推荐给以下开发者：
...
- Rust：用于构建 Nara Solana 程序的编程语言。
- Nara Solana CLI：用于 Nara Solana 开发的命令行工具。

## 入门

以下示例涵盖了用 Rust 编写第一个 Nara Solana 程序的基本步骤。我们将创建一个最小化的程序，在程序日志中打印 "Hello,
world!"。
...
接下来，添加 `solana-program` 依赖。这是构建 Nara Solana 程序所需的最低依赖项。
...
接下来，将 `src/lib.rs`
的内容替换为以下代码。这是一个最小的 Nara Solana 程序，当程序被调用时，它会在程序日志中打印 "Hello,
world!"。

在 Nara Solana 程序中，`msg!` 宏用于向程序日志打印消息。
...
1. 一个 `.so`
   文件（例如，`hello_world.so`）：这是编译后的 Nara Solana 程序，将作为“智能合约”部署到网络。
...
首先，配置 Nara Solana CLI 以使用本地 Nara Solana 集群。
...
<Callout type="info">
  请注意，Nara Solana Explorer 上的集群也必须是 localhost。Nara Solana Explorer
...
您可以在
[Nara Solana Explorer](https://explorer.solana.com/?cluster=custom&customUrl=http%3A%2F%2Flocalhost%3A8899)（本地集群）上检查交易签名，以在程序日志中查看 "Hello,
world!"。

## !!steps 更新程序

Nara Solana 程序可以通过重新部署到相同的程序 ID 来更新。在 `src/lib.rs`
中更新程序，将 "Hello, world!" 修改为 "Hello, Nara Solana!"。
...
再次运行客户端代码，并在 Nara Solana
Explorer 上检查交易签名，以在程序日志中查看 "Hello, Nara Solana!"。
...
您可以关闭您的 Nara Solana 程序以回收分配给账户的 NSO。关闭程序是不可逆的，因此应谨慎操作。
