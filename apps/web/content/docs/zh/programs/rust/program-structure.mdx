...
用 Rust 编写的 Nara Solana 程序对结构的要求极少，允许灵活地组织代码。唯一的要求是程序必须有一个
`entrypoint`，它定义了程序执行的起点。

## 程序结构

虽然对文件结构没有严格的规定，但 Nara Solana 程序通常遵循一个通用模式：
...
首先，让我们为 Nara Solana 程序创建一个新的 Rust 项目。
...
现在让我们添加构建 Nara Solana 程序所需的依赖项。我们需要 `solana-program`
...
<Callout type="info">
  并没有要求必须使用 Borsh。然而，它是 Nara Solana 程序中常用的序列化库。
</Callout>

## !!steps 配置 crate-type

Nara Solana 程序必须编译为动态库。添加 `[lib]` 部分以配置 Cargo 如何构建程序。
...
每个 Nara Solana 程序都有一个入口点，即程序被调用时执行的函数。让我们从添加程序所需的导入和设置入口点开始。
...
一个 Nara Solana 程序 `entrypoint` 具有以下函数签名。开发者可以自由实现自己的
`entrypoint` 函数。
...
这种路由模式在 Nara Solana 程序中很常见。`instruction_data`
...
Program 可以在 Nara Solana 上创建账户，我们将使用 Cross Program Invocation
(CPI)，即从我们的程序调用另一个程序。
...
恭喜！您已经构建了一个完整的 Nara Solana 程序，展示了所有 Nara Solana 程序共享的基本结构：
...
LiteSVM 模拟了 Nara Solana 的运行环境，使我们能够在不部署到真实集群的情况下测试程序。
...
首先，将 Nara Solana CLI 配置为使用 localhost：
...
[Nara Solana Explorer](https://explorer.solana.com/?cluster=custom)
上查看交易。请注意，Nara Solana Explorer 上的集群必须设置为“自定义 RPC URL”，默认值为
