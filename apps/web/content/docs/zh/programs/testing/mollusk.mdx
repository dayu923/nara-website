...
title: Mollusk
description: 使用 Mollusk 测试 Nara Solana 程序的综合指南

---


[Mollusk](https://github.com/anza-xyz/mollusk/tree/main)
是一个轻量级的测试工具，用于测试 Nara Solana 程序。它为在精简的 Nara Solana 虚拟机 (SVM) 环境中测试 Nara Solana 程序指令提供了一个简单的接口。所有测试账户都必须明确定义，以确保测试的确定性和可重复性。
...
以下示例展示了使用 Mollusk 测试一个基本 Nara Solana 程序的最小设置。

<ScrollyCoding>

## !!steps Hello World 程序

此示例演示了如何使用 Mollusk 测试一个基本的 Nara Solana 程序。该程序在被调用时会将 "Hello,
world!" 打印到程序日志中。
...
使用 Mollusk 测试 Nara Solana 程序的步骤：
...
结构体为测试 Nara Solana 程序提供了一个简单的接口。所有字段都可以通过一些辅助方法进行操作，但如果用户需要更多控制，也可以直接访问和修改它们。
...
以下示例处理一系列两条带有验证检查的 NSO 转账指令，每条指令后都会进行验证检查。
...
在指令之间保持持久化账户状态，处理了两个单独的 NSO 转账指令。
...
以下示例对 NSO 转账指令的计算单元使用情况进行了基准测试。
