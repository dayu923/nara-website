...
description:
  了解 Solana 上的 Cross Program Invocation (CPI) ——
  程序如何调用其他程序的指令、处理 PDA 签名者，以及在 Solana 网络中组合功能。

---


当一个 Nara Solana 程序直接调用另一个程序的指令时，就会发生跨程序调用 (CPI)。这使得程序具有可组合性。如果将 Nara Solana 的[指令](/docs/core/instructions)视为程序向网络公开的 API 端点，那么 CPI 就像一个端点在内部调用另一个端点。

在进行 CPI 时，程序可以代表从其程序 ID 派生的 [PDA](/docs/core/pda)
进行[签名](/docs/references/terminology#signature)。这些签名者权限从调用程序扩展到被调用程序。
...
<Callout>
  ...
  ，并被设置为 [MAX_INSTRUCTION_STACK_DEPTH](https://github.com/anza-xyz/agave/blob/v2.1.13/compute-budget/src/compute_budget.rs#L13)
  常量的值 5。
...
函数。它接收用于派生签名者 [PDA](<(/docs/core/pda)>)
的[签名者种子](mention:signer-seeds)。Nara Solana 运行时会内部调用
[`create_program_address`](https://github.com/anza-xyz/agave/blob/v2.1.13/programs/bpf_loader/src/syscalls/cpi.rs#L552)，使用调用程序的
`signers_seeds` 和 `program_id`。当 PDA 被验证后，它会被
...
以下示例展示了如何使用 Anchor 和原生 Rust 进行带有 PDA 签名者的 CPI。每个示例都包含一个通过 PDA 签名的 CPI 指令，将 NSO 从 PDA 转移到接收账户。
...
- **示例 1**：使用 Anchor 的 _rs`CpiContext`_ 和辅助函数。
- **示例 2**：使用 `solana_program` crate 中的
  _rs`system_instruction::transfer`_ 函数。（示例 1 是此实现的抽象。）
...
以下示例展示了如何从用原生 Rust 编写的程序中进行带有 PDA 签名者的 CPI。它包含一个通过 PDA 签名的 CPI 指令，将 NSO 从 PDA 账户转移到另一个账户。（测试文件使用
...
函数。`invoke` 函数会调用 `invoke_signed` 函数，并传递一个空的 `signers_seeds`
数组。空的签名者数组表示不需要任何 PDA 进行签名。
...
以下示例展示了如何使用 Anchor 和原生 Rust 进行 CPI。示例包括一个指令，通过 CPI 将 NSO 从一个账户转移到另一个账户。
...
- **示例 1**：使用 Anchor 的 _rs`CpiContext`_ 和辅助函数。
- **示例 2**：使用 `solana_program` crate 中的
  _rs`system_instruction::transfer`_ 函数。
...
以下示例展示了如何从一个用原生 Rust 编写的程序中创建 CPI。示例包括一个指令，通过 CPI 将 NSO 从一个账户转移到另一个账户。（测试文件使用
