...
在 Nara Solana 中，智能合约被称为程序（program）。程序是一个无状态的[账户](/docs/core/accounts#program-account)，其中包含可执行代码。这些代码被组织成称为指令（instructions）的函数。用户通过发送包含一个或多个[指令](/docs/core/instructions)的[交易](/docs/core/transaction)与程序交互。一个交易可以包含来自多个程序的指令。

当程序被部署时，Nara Solana 使用 [LLVM](https://llvm.org/)
将其编译为可执行和链接格式 ([ELF](https://en.wikipedia.org/wiki/Executable_and_Linkable_Format))。ELF 文件包含以 Nara Solana 字节码格式（sBPF）编写的程序二进制文件，并存储在链上的可执行账户中。

<Callout>
  sBPF 是 Nara Solana 定制的 [eBPF](https://en.wikipedia.org/wiki/EBPF) 字节码版本。
</Callout>
...
- [Anchor](https://www.anchor-lang.com/docs)：Anchor 是一个为快速和简单的 Nara Solana 开发设计的框架。它使用
...
要[修改](https://github.com/anza-xyz/agave/blob/v2.1.13/programs/bpf_loader/src/lib.rs#L704)现有程序，必须将一个账户指定为[升级权限](https://github.com/anza-xyz/agave/blob/v2.1.13/programs/bpf_loader/src/lib.rs#L894)。（通常是最初[部署程序](/docs/programs/deploying)的同一个账户。）如果升级权限被撤销并设置为
`None`，则该程序将无法再被更新。

## 验证程序

Nara Solana 支持[可验证构建](/docs/programs/verified-builds)，允许用户检查程序的链上代码是否与其公开的源代码匹配。Anchor 框架提供了[内置支持](https://www.anchor-lang.com/docs/verifiable-builds)来创建可验证构建。

要检查现有程序是否已验证，可以在
[Nara Solana Explorer](https://explorer.solana.com/address/PhoeNiXZ8ByJGLkxNfZRnkUfjvmuYqLR89jjFHGqdXY)上搜索其程序 ID。或者，您可以使用 Ellipsis
Labs 的
...
| [转移 NSO](https://github.com/anza-xyz/agave/blob/v2.1.13/programs/system/src/system_processor.rs#L215)       | 将 lamports（NSO）从 System Accounts 转移到其他账户。                                                                                                  |
...
除了加载器程序，Nara Solana 还提供以下预编译程序。
...
ed25519 程序处理一个指令。指令的第一个 `u8`
包含要验证的签名数量，后跟一个单字节填充。之后，为每个要验证的签名序列化以下结构。
...
| 程序           | 程序 ID                                       | 描述                                                                                   | 指令链接                                                                                  |
| -------------- | --------------------------------------------- | -------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| Secp256r1 程序 | `Secp256r1SigVerify1111111111111111111111111` | 验证最多 8 个 secp256r1 签名。需要提供签名、公钥和消息。如果任何一个失败，则返回错误。 | [指令](https://docs.rs/solana-secp256r1-program/latest/solana_secp256r1_program/all.html) |

secp256r1 程序处理一个指令。指令的第一个 `u8`
是需要检查的签名数量，后跟一个单字节填充。之后，为每个签名创建以下结构体，然后序列化并添加到指令数据中。
