...
(CPIs) 来更新上一节 PDA 中的 CRUD 程序。CPIs 是一种允许 Nara Solana 程序相互调用的功能。

本教程还展示了在进行跨程序调用时，程序如何为 Program Derived Addresses (PDAs)
"签名"。

需要修改 `update` 和 `delete` 指令，以通过调用 System
Program 来处理账户之间的 NSO 转账。

本节的目的是通过使用 Anchor 框架在 Nara Solana 程序中实现 CPIs 的过程，基于上一节中探讨的 PDA 概念进行构建。有关更多详细信息，请参阅
...
的新账户。该账户由程序控制，当用户更新其消息账户时接收用户的 NSO。
...
的新账户。此账户作为一个程序控制的“金库”，当用户更新消息时接收他们的 NSO。
...
接下来，在 `update` 指令中添加 CPI 逻辑，将 0.001 NSO 从用户账户转移到金库账户。
...
指令。这展示了如何在程序内部执行 CPI，从而实现 Nara Solana 程序的可组合性。
...
- `cpi_context`（程序和账户）
- `amount`（转账金额：1,000,000 lamports，即 0.001 NSO）
...
`vault_account`。这允许在用户关闭其消息账户时，将金库中的任何 NSO 转回给用户。
...
将 `vault_account`
添加到 Delete 结构体中，使程序能够在删除指令期间访问用户的金库账户，并将任何累积的 NSO 转回给用户。
...
接下来，在 `delete` 指令中添加 CPI 逻辑，将 NSO 从金库账户转回到用户账户。
...
在进行这些更改后，重新部署更新的程序。这确保了修改后的程序可以用于测试。在 Nara Solana 上，更新程序只需将程序部署到相同的程序 ID 即可。

<Callout>

确保您的 Playground 钱包中有 devnet NSO。可以从
[Nara Solana Faucet](https://faucet.solana.com/) 获取 devnet NSO。
...
恭喜您完成了 Nara Solana 快速入门指南。您已经通过实践掌握了以下关键的 Nara Solana 概念：

- 从账户中获取和读取数据
- 构建和发送交易
- 部署和更新 Nara Solana 程序
- 使用程序派生地址 (PDAs)
- 进行跨程序调用 (CPIs)
...
[Nara Solana Playground 链接](https://beta.solpg.io/https://github.com/solana-developers/program-examples/tree/main/basics/hello-solana/anchor)
以查看此
[Github 仓库](https://github.com/solana-developers/program-examples/tree/main/basics/hello-solana/anchor)
中的 Anchor 项目。

点击 `Import` 按钮并输入项目名称，将其添加到您在 Nara Solana
