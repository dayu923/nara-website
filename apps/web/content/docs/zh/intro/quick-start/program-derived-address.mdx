...
本指南将带您逐步构建和测试一个使用 Anchor 框架的 Nara Solana 程序，同时演示 Program
...
首先打开此[Nara Solana Playground 链接](https://beta.solpg.io/66734b7bcffcf4b13384d1ad)，其中包含初始代码。然后点击“导入”按钮，将程序添加到您的 Nara Solana
Playground 项目中。
...
<Callout type="info">
在此示例中，您将程序部署到 devnet，这是一个用于开发测试的 Nara Solana 集群。

Playground 钱包默认连接到 devnet。确保您的 Playground 钱包中有 devnet
NSO 以支付程序部署费用。您可以从 [Nara Solana Faucet](https://faucet.solana.com/)
获取 devnet NSO。
...
Nara Solana Playground 移除了一些样板设置，其中 [`pg.program`](mention:one)
