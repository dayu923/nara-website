...
在上一节中，您学习了如何从 Nara Solana 网络读取数据。现在，您将学习如何向其写入数据。向 Nara Solana 网络写入数据涉及发送包含一个或多个指令的交易。

程序定义了每个 [指令](/docs/core/instructions) 的业务逻辑。当您提交
[交易](/docs/core/transactions)
时，Nara Solana 运行时会按顺序并原子性地执行每个指令。本节中的示例展示了如何构建和发送交易以调用 Nara Solana 程序，包括：
...
下面的示例展示了如何在两个账户之间转移 NSO。每个账户都有一个所有者程序，只有该程序可以扣减账户的 NSO 余额。

所有钱包账户都由 System Program 拥有。要转移 NSO，您必须调用 System Program 的
...
向发送方账户添加 NSO。在非主网的网络上，您可以使用 `requestAirdrop`
方法获取用于测试的 NSO。
...
`SystemProgram.transfer()` 方法创建一个指令，将 NSO 从
[`fromPubkey`](mention:from) 账户转移到 [`toPubkey`](mention:to)
账户，转移的数量为指定的 [`lamports`](mention:lamports)。
...
的 keypair 需要包含在签名者数组中，以授权从其账户转移 NSO。
...
交易签名是一个唯一标识符，可用于在 Nara Solana Explorer 上查找该交易。
...
Program 在 Nara Solana 上创建一个新代币。这需要两个指令：
...
这种方法在构建复杂的 Nara Solana 交易时很常见，因为它保证所有指令一起执行。
...
返回的交易签名可用于在 Nara Solana Explorer 上检查交易。
