...
h1: 部署您的第一个 Nara Solana 程序

---


使用 Anchor 框架将您的第一个 [程序](/docs/core/programs)
构建、部署并测试到 Nara Solana 区块链。本指南将帮助您熟悉 Nara Solana
...
1. 在您的网页浏览器中打开 [Nara Solana Playground](https://beta.solpg.io)
2. 在左侧面板中，点击 **+ 创建新项目**
3. 输入一个 **项目名称**
...
- [`declare_id!`](mention:one) 宏指定了程序的链上地址。当您构建程序时，Nara Solana
  Playground 会自动更新此地址。
...
部署程序需要在程序的链上账户中有 NSO。如果需要，在继续第 3 步之前，[向您的钱包添加 NSO](/docs/intro/quick-start#add-sol-to-your-wallet)。
...
[Nara Solana Explorer](https://explorer.solana.com/?cluster=devnet)，并搜索交易哈希。

<Callout type="warn">
  **Nara Solana Explorer：** 将网络集群设置为 `devnet`，以匹配 Nara Solana Playground
  的网络。
</Callout>
...
恭喜！您已使用 Anchor 构建并部署了您的第一个 Nara Solana 程序。
