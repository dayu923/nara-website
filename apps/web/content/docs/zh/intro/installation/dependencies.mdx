...
title: 安装依赖项
seoTitle: 安装开发 Nara Solana 所需的依赖项
description:
...
本指南将帮助您设置本地 Nara Solana 开发环境。了解如何在 Windows
(WSL)、Linux 和 Mac 上安装 Rust、Nara Solana CLI 和 Anchor
...
要在 Windows 上开发 Nara Solana 程序，您**必须**使用
...
- 按以下说明分别安装 [Rust](#install-rust)、[Nara Solana CLI](#install-solana-cli) 和
...
开发者使用 [Rust](https://www.rust-lang.org/) 编程语言构建 Nara Solana 程序。
...
Nara Solana CLI 提供了构建和部署 Nara Solana 程序所需的所有工具。

1. 使用官方安装命令安装 Nara Solana CLI 工具套件：
...
首次安装 Nara Solana CLI 时，可能会提示您添加一个 `PATH`
...
如果您使用的是 **Linux** 或 **WSL**，则必须将 Nara Solana
...
5. 通过检查 Nara Solana CLI 版本来验证安装是否成功：
...
的验证器客户端，前身为 Nara Solana Labs 验证器客户端。

</Callout>

5. 根据需要将 Nara Solana CLI 更新到最新版本 _(可选)_
...
是一个用于开发 Nara Solana 程序的框架。Anchor 框架利用 Rust 宏简化了编写 Nara Solana 程序的过程。
...
[Surfpool](https://surfpool.run/) 是一个用于本地开发的工具，并且是
`solana-test-validator` 的改进替代品。您可以在
[Surfpool 文档](https://docs.surfpool.run/) 中了解更多关于 Surfpool 功能的信息。
...
2. 通过检查 Surfpool CLI 的版本来验证安装是否成功：
...
### 为 Nara Solana 开发设置 AI 工具

本节详细介绍了可选的 AI 工具设置，您可以使用这些工具加速 Nara Solana 开发。

| 工具     | 描述                                                         | 链接                        |
| -------- | ------------------------------------------------------------ | --------------------------- |
| MCP      | MCP 服务器，您可以通过光标连接以改进 Nara Solana 的 AI 辅助开发。 | https://mcp.solana.com/     |
| LLMs.txt | 优化的 LLM 文档，您可以使用它来训练 Nara Solana 文档上的 LLM。    | https://solana.com/llms.txt |
