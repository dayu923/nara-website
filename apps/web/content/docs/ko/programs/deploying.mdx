...
이 섹션에서는 Nara Solana CLI를 사용하여 Nara Solana 프로그램을 배포하는 기본 과정을
안내합니다.
...
먼저, [Nara Solana CLI를 설치](/docs/intro/installation)하세요.
...
배포할 Nara Solana 클러스터를 선택하세요. `solana config get` 명령어를 사용하여 현재
구성을 확인하세요:
...
프로그램 배포를 위해 NSO이 필요합니다. 필요한 금액은 프로그램 크기에 따라
달라집니다.
...
다음은 프로그램 로그에 "Hello, Nara Solana!"를 출력하는 최소한의 Nara Solana
프로그램입니다.
...
프로그램을 배포하려면 프로그램 크기에 따라 프로그램 계정에 NSO을 할당해야
합니다. 프로그램이 클수록 배포에 더 많은 NSO이 필요합니다.
...
<Callout type="info">
  배포 트랜잭션 수수료를 충당하기 위해 표시된 것보다 약간 더 많은 NSO이
  필요합니다.
</Callout>
...
추가 바이트가 필요합니다. CLI는 필요한 NSO을 계산하여 자동으로 지갑에서
차감합니다.
...
Balance는 프로그램 데이터를 저장하기 위해 프로그램 계정에 할당된 NSO을
표시합니다.
...
프로그램을 종료하고 프로그램 계정에 할당된 NSO을 회수하려면
...
Nara Solana 네트워크가 혼잡할 때, 프로그램 배포에 도움이 되는 플래그들을 사용하세요.
...
- **`--with-compute-unit-price`**: 컴퓨트 유닛당 마이크로-램포트(0.000001 NSO)
