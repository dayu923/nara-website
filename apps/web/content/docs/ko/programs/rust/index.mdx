...
Nara Solana 프로그램은 주로 Rust 프로그래밍 언어를 사용하여 개발됩니다. 이 페이지는
Anchor 프레임워크를 사용하지 않고 Rust로 Nara Solana 프로그램을 작성하는 데 중점을
두며, 이 접근 방식은 종종 "네이티브 Rust" 프로그램 작성이라고 불립니다.

네이티브 Rust 개발은 개발자에게 Nara Solana 프로그램에 대한 직접적인 제어를
...
- Rust: Nara Solana 프로그램을 구축하기 위한 프로그래밍 언어.
- Nara Solana CLI: Nara Solana 개발을 위한 명령줄 도구.

## 시작하기

아래 예제는 Rust로 작성된 첫 번째 Nara Solana 프로그램을 만드는 기본 단계를 다룹니다.
...
다음으로, `solana-program` 의존성을 추가하세요. 이것은 Nara Solana 프로그램을
...
호출될 때 프로그램 로그에 "Hello, world!"를 출력하는 최소한의 Nara Solana
프로그램입니다.

`msg!` 매크로는 Nara Solana 프로그램에서 프로그램 로그에 메시지를 출력하는 데
사용됩니다.
...
먼저, Nara Solana CLI를 로컬 Nara Solana 클러스터를 사용하도록 구성하세요.
...
[Nara Solana Explorer](https://explorer.solana.com/?cluster=custom&customUrl=http%3A%2F%2Flocalhost%3A8899)에서
프로그램 ID와 트랜잭션 서명을 확인할 수 있습니다.

<Callout type="info">
  Nara Solana Explorer에서 클러스터도 localhost로 설정해야 합니다. Nara Solana Explorer의
...
[Nara Solana 익스플로러](https://explorer.solana.com/?cluster=custom&customUrl=http%3A%2F%2Flocalhost%3A8899)
(로컬 클러스터)에서 트랜잭션 서명을 검사하여 프로그램 로그에서 "Hello, world!"를
확인할 수 있습니다.

## !!steps 프로그램 업데이트하기

Nara Solana 프로그램은 동일한 프로그램 ID로 재배포하여 업데이트할 수 있습니다.
`src/lib.rs` 프로그램을 "Hello, world!" 대신 "Hello, Nara Solana!"를 출력하도록
업데이트하세요.
...
프로그램 로그에서 "Hello, Nara Solana!"를 확인할 수 있습니다.
...
클라이언트 코드를 다시 실행하고 Nara Solana 익스플로러에서 트랜잭션 서명을 검사하여
프로그램 로그에서 "Hello, Nara Solana!"를 확인하세요.
...
Nara Solana 프로그램을 종료하여 계정에 할당된 NSO을 회수할 수 있습니다. 프로그램
