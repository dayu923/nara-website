...
Rust로 작성된 Nara Solana 프로그램은 최소한의 구조적 요구사항만 있어 코드 구성 방식에
유연성을 제공합니다. 유일한 요구사항은 프로그램이 `entrypoint`를 가져야 한다는
것인데, 이는 프로그램 실행이 시작되는 지점을 정의합니다.

## 프로그램 구조

파일 구조에 대한 엄격한 규칙은 없지만, Nara Solana 프로그램은 일반적으로 다음과 같은
공통 패턴을 따릅니다:
...
먼저, Nara Solana 프로그램을 위한 새로운 Rust 프로젝트를 만들어 보겠습니다.
...
이제 Nara Solana 프로그램을 구축하는 데 필요한 의존성을 추가해 보겠습니다. 핵심 SDK를
...
<Callout type="info">
  Borsh를 사용해야 하는 필수 요건은 없습니다. 그러나 Nara Solana 프로그램에서
  일반적으로 사용되는 직렬화 라이브러리입니다.
</Callout>

## !!steps crate-type 구성하기

Nara Solana 프로그램은 동적 라이브러리로 컴파일되어야 합니다. Cargo가 프로그램을
...
모든 Nara Solana 프로그램에는 프로그램이 호출될 때 실행되는 함수인 진입점이 있습니다.
...
Nara Solana 프로그램 `entrypoint`는 다음과 같은 함수 시그니처를 가집니다. 개발자는
...
이러한 라우팅 패턴은 Nara Solana 프로그램에서 일반적입니다. `instruction_data`는
...
새 카운터 계정을 생성하고 초기화하는 핸들러를 구현해 보겠습니다. Nara Solana에서는
...
축하합니다! 모든 Nara Solana 프로그램이 공유하는 기본 구조를 보여주는 완전한 Nara Solana
프로그램을 구축했습니다:
...
LiteSVM은 Nara Solana 런타임 환경을 시뮬레이션하여 실제 클러스터에 배포하지 않고도
...
먼저, Nara Solana CLI를 localhost로 구성합니다:
...
[Nara Solana Explorer](https://explorer.solana.com/?cluster=custom)에서 트랜잭션을
확인할 수 있습니다. Nara Solana Explorer에서 클러스터를 "Custom RPC URL"로 설정해야
