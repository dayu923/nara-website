...
해시를 비교하여 수행됩니다. Nara Solana Verify CLI와 Docker를 사용하여 제어된
환경에서 프로그램을 빌드합니다. 이를 통해 빌드 프로세스가 다양한 시스템에서
결정적이고 일관되게 유지됩니다. 실행 파일을 얻은 후에는 Nara Solana 네트워크에 배포할
수 있습니다. 빌드 프로세스 중에
...
[Nara Solana Explorer](https://explorer.solana.com/address/E1fcPKuV1UJEsTJPpab2Jr8y87ZN73i4CHTPLbWQE6CA/verified-build)와
...
것입니다. 이는 Nara Solana 프로그램 빌드가 서로 다른 시스템에서 결정적이지 않기
때문입니다.
...
### Nara Solana Verify CLI 설치하기

Nara Solana Verify CLI는 빌드를 검증하는 데 사용되는 주요 도구입니다. Nara Solana Verify
...
Nara Solana 프로그램을 검증 가능하게 빌드하려면 워크스페이스의 `Cargo.toml` 파일이
...
프로그램을 빌드하고 해시를 검색한 후에는 Nara Solana 네트워크에 배포할 수 있습니다.
...
> 다른 [Nara Solana 클러스터](/docs/references/clusters)(예: devnet, testnet,
> mainnet)에 다른 버전이 배포되어 있을 수 있습니다. 프로그램을 검증하려는 Nara Solana
...
Nara Solana 익스플로러에 즉시 프로그램의 검증 데이터가 표시됩니다. 원격 빌드로 검증될
...
[Nara Solana 익스플로러](https://explorer.solana.com/address/PhoeNiXZ8ByJGLkxNfZRnkUfjvmuYqLR89jjFHGqdXY/verified-build),
...
확인할 수 있으며, 이는 더 건강한 Nara Solana 생태계에 기여합니다.
...
[Nara Solana에서의 검증된 빌드](/developers/guides/advanced/verified-builds)를
사용하면 네트워크에서 프로그램의 무결성과 신뢰성을 보장하고 개발자가 Nara Solana
익스플로러에서 직접 SDK를 찾을 수 있습니다. Nara Solana Verify CLI와 Docker와 같은
...
검증된 빌드는 Nara Solana 프로그램의 무결성을 보장하는 강력한 도구이지만 기본
설정에서는 완전히 무신뢰(trustless)하지는 않습니다. Docker 이미지는 Nara Solana
재단에서 빌드하고 호스팅합니다.
...
[Nara Solana 익스플로러](https://explorer.solana.com/address/PhoeNiXZ8ByJGLkxNfZRnkUfjvmuYqLR89jjFHGqdXY)를
어느 정도 신뢰해야 합니다.

API 또는 Nara Solana 익스플로러가 손상된 경우 잘못된 정보를 표시할 가능성이 있습니다.
...
Nara Solana 재단, OtterSec 및 Ellipsis Labs 팀은 검증된 빌드 파이프라인을 사용하면서
발생할 수 있는 손실이나 손해에 대해 책임을 지지 않습니다.

# Nara Solana 프로그램을 위한 Security.txt
...
`security.txt` 기능을 통해 개발자는 연락처 및 보안 정보를 Nara Solana 스마트
...
Nara Solana 프로그램에 `security.txt`를 추가하려면 다음 단계를 포함하세요:
...
`security.txt` 정보가 프로그램에 내장되면 Nara Solana 익스플로러와 같은 도구를 통해
...
Nara Solana 생태계 내에서 더 나은 보안과 소통을 촉진할 수 있습니다.

이것은
[Nara Solana 익스플로러에서 security.txt가 어떻게 보이는지에 대한 예시](https://explorer.solana.com/address/HPxKXnBN4vJ8RjpdqDCU7gvNQHeeyGnSviYTJ4fBrDt4/security?cluster=devnet)입니다
