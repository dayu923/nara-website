...
<Callout>
  프로그램 명령어 호출의 최대 높이는 [`max_instruction_stack_depth`](https://github.com/anza-xyz/agave/blob/v2.1.13/compute-budget/src/compute_budget.rs#L38)라고 하며, [MAX_INSTRUCTION_STACK_DEPTH](https://github.com/anza-xyz/agave/blob/v2.1.13/compute-budget/src/compute_budget.rs#L13) 상수인 5로 설정되어 있습니다.
...
함수가 사용됩니다. 이 함수는 서명자 [PDA](<(/docs/core/pda)>)를 도출하는 데
사용되는 [서명자 시드](mention:signer-seeds)를 받습니다. Nara Solana 런타임은
내부적으로 호출 프로그램의 `signers_seeds`와 `program_id`를 사용하여
...
보여줍니다. 각 예제는 PDA가 서명한 CPI를 사용하여 PDA에서 수신자 계정으로 NSO을
전송하는 단일 명령을 포함합니다.
...
- **예제 3**: CPI 명령어를 수동으로 구성합니다. 이 접근 방식은 호출하려는
  명령어를 구성하는 데 도움이 되는 크레이트가 없을 때 유용합니다.
...
만드는 방법을 보여줍니다. 이 프로그램은 PDA 계정에서 다른 계정으로 NSO을
...
계정으로 NSO을 전송하는 단일 명령어가 포함되어 있습니다.
...
- **예제 2**: `solana_program` 크레이트의 _rs`system_instruction::transfer`_
  함수를 사용합니다.
- **예제 3**: CPI 명령어를 수동으로 구성합니다. 이 접근 방식은 호출하려는
  명령어를 구축하는 데 도움이 되는 크레이트가 없을 때 유용합니다.
...
보여줍니다. 한 계정에서 다른 계정으로 NSO을 전송하는 단일 명령어가 포함되어
