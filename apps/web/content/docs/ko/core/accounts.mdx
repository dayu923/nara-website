...
아래 예제는 Nara Solana SDK를 사용하여 keypair를 생성하는 방법을 보여줍니다.
...
선택적 입력(시드)을 사용하여 결정론적으로 도출된 주소입니다. 아래 예제는 Nara Solana
...
NSO이 처음으로 새 주소로 전송될 때, System Program이 소유한 계정이 해당 주소에
생성됩니다.

아래 예제에서는 새 keypair가 생성되고 NSO로 자금이 지원됩니다. 코드를 실행한 후,
