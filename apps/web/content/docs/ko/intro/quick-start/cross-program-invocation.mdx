...
이 섹션에서는 이전 PDA 섹션의 CRUD 프로그램이 Nara Solana 프로그램이 서로를 호출할 수
있게 해주는 기능인 교차 프로그램 호출(CPI)을 추가하여 업데이트됩니다.

이 튜토리얼은 또한 프로그램이 교차 프로그램 호출을 할 때 프로그램 파생
주소(PDA)에 대해 어떻게 "서명"할 수 있는지 보여줍니다.

`update` 및 `delete` 명령어는 System Program을 호출하여 계정 간 NSO 전송을
처리하도록 수정이 필요합니다.

이 섹션의 목적은 Anchor 프레임워크를 사용하여 Nara Solana 프로그램에서 CPI를 구현하는
...
계정은 사용자가 메시지를 업데이트할 때 NSO을 받는 프로그램 제어 "금고" 역할을
합니다.
...
방법을 보여주며, Nara Solana 프로그램의 조합 가능성을 가능하게 합니다.
...
- 전송할 `amount`(1,000,000 lamport 또는 0.001 NSO)
...
통해 사용자가 메시지 계정을 닫을 때 금고에 있는 NSO을 사용자에게 다시 전송할 수
있습니다.
...
사용자의 볼트 계정에 접근하여 축적된 NSO을 사용자에게 다시 전송할 수 있습니다.
...
수정된 프로그램을 테스트할 수 있게 됩니다. Nara Solana에서는 동일한 프로그램 ID에
프로그램을 배포하는 것만으로 프로그램을 업데이트할 수 있습니다.

<Callout>

Playground 지갑에 devnet NSO이 있는지 확인하세요.
[Nara Solana Faucet](https://faucet.solana.com/)에서 devnet NSO을 받을 수 있습니다.
...
Nara Solana Playground에서 프로그램을 배포할 때, 사용자의 Playground 지갑이 모든
