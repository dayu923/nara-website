...
이 가이드는 Anchor 프레임워크를 사용하여 Nara Solana 프로그램을 구축하고 테스트하는
...
시작 코드가 포함된
[Nara Solana Playground 링크](https://beta.solpg.io/66734b7bcffcf4b13384d1ad)를 열어
시작하세요. 그런 다음 "Import" 버튼을 클릭하여 프로그램을 Nara Solana Playground
프로젝트에 추가하세요.
...
<Callout type="info">
이 예제에서는 개발 테스트를 위한 Nara Solana 클러스터인 devnet에 프로그램을 배포합니다.

플레이그라운드 지갑은 기본적으로 devnet에 연결됩니다. 프로그램 배포 비용을
지불하기 위해 플레이그라운드 지갑에 devnet NSO이 있는지 확인하세요.
...
Nara Solana Playground는 일부 상용구 설정을 제거합니다. 여기서
