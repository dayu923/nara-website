...
h1: 첫 번째 Nara Solana 프로그램 배포하기

---


Anchor 프레임워크를 사용하여 첫 번째 [프로그램](/docs/core/programs)을 Nara Solana
블록체인에 빌드, 배포 및 테스트해 보세요. 이 가이드는 Nara Solana Playground에 더
...
1. 웹 브라우저에서 [Nara Solana Playground](https://beta.solpg.io)를 엽니다
...
`src/lib.rs` 소스 파일이 열립니다. 이 파일에는 `Initialize`라는 하나의 구조체가
...
- [`declare_id!`](mention:one) 매크로는 프로그램의 온체인 주소를 지정합니다.
  Nara Solana Playground는 프로그램을 빌드할 때 이 주소를 자동으로 업데이트합니다.
...
이 예제에서 [`initialize`](mention:two) 명령어는 두 가지 매개변수를 받습니다:

- `ctx: Context<Initialize>`: `Initialize` 구조체에 지정된 대로 이 명령어에
  필요한 계정을 함수에 전달합니다.
- `data: u64`: 명령어를 호출할 때 제공해야 하는 사용자 정의 매개변수입니다.

함수 본문은
[제공된 `data` 인수로 `new_account`의 `data` 필드를 설정](mention:three)한 다음
프로그램 로그에 메시지를 출력합니다.
...
2. 지갑에 NSO 추가하기 _(선택 사항)_

프로그램을 배포하려면 프로그램의 온체인 계정에 NSO이 필요합니다. 필요한 경우,
3단계로 진행하기 전에
[지갑에 NSO 추가하기](/docs/intro/quick-start#add-sol-to-your-wallet)를
수행하세요.
...
1. `tests/anchor.test.ts` 테스트 파일을 엽니다. 이 파일은 클라이언트에서
...
3. 출력에 포함된 트랜잭션 해시(`TxHash`)를 저장하세요
...
4. `solana confirm -v [TxHash]`를 실행하여 트랜잭션 로그 보기
...
[Nara Solana Explorer](https://explorer.solana.com/?cluster=devnet)를 열고 트랜잭션
해시를 검색하세요.

<Callout type="warn">
  **Nara Solana Explorer:** 네트워크 클러스터를 Nara Solana Playground와 일치하도록
  `devnet`로 설정하세요.
</Callout>
...
온체인 프로그램에 할당된 NSO을 회수하기 위해 프로그램을 종료하세요.
_rs`declare_id!()`_ 매크로에 저장된 프로그램 주소를 사용하여
`solana program close [ProgramID]`를 실행하세요.
...
축하합니다! Anchor를 사용하여 첫 번째 Nara Solana 프로그램을 빌드하고 배포했습니다.
