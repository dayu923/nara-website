...
title: 권한 설정
description: 민트 또는 토큰 계정에 대한 권한을 설정하는 방법을 알아보세요.
...
- **민트 권한**: 새 토큰 생성을 제어합니다. 모든 토큰 계정에 토큰을 발행할 수
  있습니다. 고정 공급량 토큰을 만들기 위해 초기 공급량이 생성된 후 종종
  취소됩니다.
- **동결 권한**: 토큰 계정을 동결하고 해제하는 기능을 제어합니다. 토큰 계정이
  토큰을 전송하지 못하도록 방지할 수 있습니다. 사용자에게 토큰이 동결되지
  않는다는 보장을 위해 종종 취소됩니다.

### Token Account 권한

- **계정 소유자**: 토큰 계정에 대한 완전한 제어 권한을 가집니다. 토큰 전송,
  소각, 위임자 승인 및 잔액이 0일 때 계정을 닫을 수 있습니다.
- **종료 권한**: 잔액이 0일 때 토큰 계정을 닫을 수 있습니다. 기본적으로 이는
  계정 소유자이지만 다른 계정에 위임할 수 있습니다.
...
명령은 민트와 토큰 계정의 권한을 변경하거나 취소합니다. 현재 권한을 가진 사람만
자신의 권한을 새 주소로 이전하거나 권한을 null로 설정하여 영구적으로 취소할 수
있습니다. 한번 취소되면 권한은 복원될 수 없습니다.

<Callout type="info">
  [토큰
  프로그램](https://github.com/solana-program/token/blob/a7c488ca39ed4cd71a87950ed854929816e9099f/program/src/instruction.rs#L153)과
  [토큰 확장
  프로그램](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/instruction.rs#L195)은
  동일한 기능을 구현하기 위해 유사한 구현을 공유합니다.
</Callout>
