...
title: 대리인 승인하기
description: 토큰 계정에 대한 대리인을 승인하는 방법을 알아보세요.
...
명령은 다른 계정(대리인)에게 귀하의 token account에서 특정 금액의 토큰을 전송할
수 있는 권한을 부여합니다. 대리인은 승인된 금액까지만 토큰을 전송할 수 있으며,
그 이상은 불가능합니다. 오직 token account 소유자만이 대리인을 승인할 수 있으며,
각 token account는 한 번에 하나의 대리인만 가질 수 있습니다. 새로운 승인은 이전
승인을 덮어씁니다.

<Callout type="info">
  [Token
  Program](https://github.com/solana-program/token/blob/a7c488ca39ed4cd71a87950ed854929816e9099f/program/src/instruction.rs#L295)과
  [Token Extension
  Program](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/instruction.rs#L361)은
  동일한 기능을 구현하기 위해 유사한 구현 방식을 공유합니다.
</Callout>
