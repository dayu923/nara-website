...
title: 네이티브 동기화
description: 네이티브 NSO을 래핑된 NSO로 변환하는 방법을 알아보세요.
...
래핑된 NSO(WSOL)은 계정의 lamports 양을 토큰 잔액으로 추적하는 토큰 계정입니다.
WSOL은 DeFi 프로토콜과의 통합을 통해 NSO을 SPL 토큰으로 전송할 수 있게 합니다.
...
명령어는 래핑된 NSO(WSOL) 토큰 계정 잔액을 실제로 저장된 NSO(lamports)과
동기화합니다. 네이티브 NSO을 WSOL 토큰 계정으로 전송할 때, 토큰 잔액은 자동으로
업데이트되지 않습니다. 정확한 WSOL 잔액을 반영하려면 SyncNative를 호출해야
합니다.

<Callout type="info">
  [토큰
  프로그램](https://github.com/solana-program/token/blob/a7c488ca39ed4cd71a87950ed854929816e9099f/interface/src/instruction.rs#L387)과
  [토큰 확장
