...
title: 계정 동결
description: 토큰 계정을 동결하는 방법을 알아보세요.
...
명령어는 특정 token account에서 모든 토큰 전송 또는 토큰 소각을 방지합니다.
동결되면 해당 계정은 토큰을 보내거나, 받거나, 해제될 때까지 닫을 수 없습니다.
토큰의 mint의 동결 권한을 가진 주체만 계정을 동결할 수 있습니다. mint
account에서 동결 권한이 취소(null로 설정)되면 토큰은 절대 동결될 수 없습니다.

<Callout type="info">
  [Token
  Program](https://github.com/solana-program/token/blob/a7c488ca39ed4cd71a87950ed854929816e9099f/program/src/instruction.rs#L228)과
  [Token Extension
  Program](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/instruction.rs#L291)은
