...
title: 토큰 전송하기
description:
  한 토큰 계정에서 다른 토큰 계정으로 비공개적으로 토큰을 전송하는 방법을
  알아보세요.
...
## 한 token account에서 다른 token account로 기밀하게 토큰을 전송하는 방법

한 token account에서 다른 token account로 기밀하게 토큰을 전송하려면, 송신자와
수신자 모두 _confidential_transfer_state_ 상태로 구성되고 기밀 전송이 승인된
token account를 가지고 있어야 합니다. 송신자의 token account는 또한 전송할 수
있는 기밀 잔액을 보유하고 있어야 합니다.

토큰을 기밀하게 전송하려면:

1. 클라이언트 측에서
   [세 가지 증명](https://github.com/solana-program/token-2022/blob/d073cb89dbcd430387b5c6fb4b7157911351e4a3/confidential-transfer/proof-generation/src/transfer.rs#L35)을
   생성합니다:
   ...
   **등가 증명
   ...
   전송 후 새로운 가용 잔액 암호문이 해당
   [Pedersen 커밋먼트](https://en.wikipedia.org/wiki/Commitment_scheme)와
   일치하는지 확인하여, 소스 계정의 새로운 가용 잔액이 *현재 잔액 - 전송
   금액*으로 올바르게 계산되었는지 검증합니다.
   ...
   **암호문 유효성 증명
   ...
   전송 금액 암호문이 세 당사자(소스, 대상, 감사자) 모두에 대해 올바르게
   생성되었는지 확인하여, 전송 금액이 각 당사자의 공개 키로 올바르게
   암호화되었는지 검증합니다.
   ...
   **범위 증명
   ...
   새로운 가용 잔액과 전송 금액(상위/하위 비트로 분할됨)이 모두 음수가 아니며
   지정된 범위 내에 있는지 확인합니다.

2. 각 증명에 대해:
   - ZK ElGamal 증명 프로그램을 호출하여 증명 데이터를 검증합니다.
   - 다른 명령어에서 사용할 증명별 메타데이터를 증명 "컨텍스트 상태" 계정에
     저장합니다.
...
3. [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604)
   명령어를 호출하고, 증명 컨텍스트 상태 계정들을 제공합니다.

4. 증명 컨텍스트 상태 계정들을 닫아 생성에 사용된 NSO을 회수합니다.

다음 다이어그램은 송신자의 token account에서 수신자의 token account로 토큰을
전송하는 과정을 보여줍니다.
...
한 token account에서 다른 token account로 비공개적으로 토큰을 전송하려면 다음을
수행해야 합니다:

- 클라이언트 측에서 동등성 증명, 암호문 유효성 증명 및 범위 증명 생성
- Zk ElGamal 증명 프로그램을 호출하여 증명을 확인하고 "컨텍스트 상태" 계정
  초기화
- [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604)
  명령어를 호출하여 세 가지 증명 계정 제공
- 세 가지 증명 계정을 닫아 rent 회수

`spl_token_client` 크레이트는 다음 메서드를 제공합니다:

- `confidential_transfer_create_context_state_account` 메서드: 증명 계정을
  생성합니다.
- `confidential_transfer_transfer` 메서드: `Transfer` 명령어를 호출합니다.
- `confidential_transfer_close_context_state_account` 메서드: 증명 계정을
  닫습니다.

### 예제 코드

다음 예제는 token account 간에 비공개적으로 토큰을 전송하는 방법을 보여줍니다.

예제를 실행하려면 다음 명령어를 사용하여 메인넷에서 복제된 Token Extensions
Program으로 로컬 validator를 시작하세요. 로컬 validator를 시작하려면 Nara Solana
CLI가 설치되어 있어야 합니다.
...
<Callout type="info">
  작성 시점에서는 기본 로컬 validator에서 비공개 전송이 활성화되어 있지
