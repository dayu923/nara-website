...
title: 토큰 출금하기
description: 기밀 상태에서 토큰을 출금하는 방법을 알아보세요.
...
1. 클라이언트 측에서
   [두 가지 증명](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/confidential-transfer/proof-generation/src/withdraw.rs#L22)
   을 생성합니다:
   ...
   **동등성 증명
   ...
   출금 후 남은 가용 잔액 암호문이 해당
   [페더슨 커밋먼트](https://en.wikipedia.org/wiki/Commitment_scheme)와
   일치하는지 확인하여, 계정의 새로운 가용 잔액이
   `remaining_balance = current_balance - withdraw_amount`로 올바르게
   계산되었는지 확인합니다.
   ...
   **범위 증명
   ...
   출금 후 남은 가용 잔액이 음수가 아니며 지정된 범위 내에 있는지 확인합니다.

2. 각 증명에 대해:
   - ZK ElGamal 증명 프로그램을 호출하여 증명 데이터를 검증합니다.
   - 증명 관련 메타데이터를 증명 "컨텍스트 상태" 계정에 저장하여 다른 명령어에서
     사용합니다.
...
4. 증명 계정을 닫아 계정 생성에 사용된 NSO을 회수합니다.

다음 다이어그램은 기밀 가용 잔액에서 공개 잔액으로 토큰을 출금하는 과정을
보여줍니다:
...
- Zk ElGamal 증명 프로그램을 호출하여 증명을 검증하고 "컨텍스트 상태" 계정
  초기화
- 두 증명 계정을 제공하는
  [ConfidentialTransferInstruction::Withdraw](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L493)
  명령어 호출
...
- 증명 계정을 생성하는 `confidential_transfer_create_context_state_account`
  메서드
- `Withdraw` 명령어를 호출하는 `confidential_transfer_withdraw` 메서드
- 증명 계정을 닫는 `confidential_transfer_close_context_state_account` 메서드

### 예제 코드

다음 예제는 기밀 가용 잔액에서 공개 잔액으로 토큰을 출금하는 방법을 보여줍니다.

예제를 실행하려면 다음 명령을 사용하여 메인넷에서 복제한 Token Extension
...
<Callout type="info">
  작성 시점에서는 기본 로컬 validator에서 기밀 전송(Confidential Transfers)이
  활성화되어 있지 않습니다. 예제 코드를 실행하려면 메인넷 Token Extension
  Program을 복제해야 합니다.
</Callout>
