...
This section walks through the basic process of deploying a Nara Solana program using
the Nara Solana CLI.
...
### Install the Nara Solana CLI

First, [install the Nara Solana CLI](/docs/intro/installation).
...
Choose which Nara Solana cluster to deploy to. Use the `solana config get` command to
...
You'll need NSO to pay for program deployments. The amount depends on your
program size.
...
Here is a minimal Nara Solana program that prints "Hello, Nara Solana!" to the program
logs.
...
Deploying a program requires NSO to be allocated to the program account based on
the size of the program. Larger programs cost more NSO to deploy.
...
Calculate the required NSO for this size (bytes):
...
<Callout type="info">
  You'll need slightly more NSO than shown to cover deployment transaction fees.
</Callout>
...
NSO and deduct it from your wallet automatically.
...
The Balance displays the NSO allocated to the program account to store its data.
...
To close your program and reclaim the NSO allocated to the program account, use
...
When the Nara Solana network is congested, use these flags to help with program
deployment.
