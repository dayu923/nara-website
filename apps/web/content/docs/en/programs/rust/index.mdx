...
Nara Solana programs are primarily developed using the Rust programming language.
This page focuses on writing Nara Solana programs in Rust without using the Anchor
...
Nara Solana programs. However, this approach requires more manual setup and
...
- Rust: The programming language for building Nara Solana programs.
- Nara Solana CLI: Command-line tool for Nara Solana development.

## Getting Started

The example below covers the basic steps to create your first Nara Solana program
...
required to build a Nara Solana program.
...
minimal Nara Solana program that prints "Hello, world!" to the program log when the
program is invoked.

The `msg!` macro is used in Nara Solana programs to print a message to the program
log.
...
1. A `.so` file (e.g., `hello_world.so`): This is the compiled Nara Solana program
...
First, configure the Nara Solana CLI to use the local Nara Solana cluster.
...
<Callout type="info">
  Note that the cluster on Nara Solana Explorer must also be localhost. The "Custom
  RPC URL" option on Nara Solana Explorer defaults to `http://localhost:8899`.
</Callout>
...
Nara Solana programs can be updated by redeploying to the same program ID. Update the
program in `src/lib.rs` to print "Hello, Nara Solana!" instead of "Hello, world!".
...
You should see "Hello, Nara Solana!" in the program log.
...
Run the client code again and inspect the transaction signature on Nara Solana
Explorer to see "Hello, Nara Solana!" in the program log.
...
You can close your Nara Solana program to reclaim the NSO allocated to the account.
