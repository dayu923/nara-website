...
Nara Solana programs written in Rust have minimal structural requirements, allowing
...
While there are no strict rules for file structure, Nara Solana programs typically
...
First, let's create a new Rust project for our Nara Solana program.
...
Now let's add the necessary dependencies for building a Nara Solana program. We need
...
<Callout type="info">
  ...
  serialization library for Nara Solana programs.
</Callout>

## !!steps Configure crate-type

Nara Solana programs must be compiled as dynamic libraries. Add the `[lib]` section
...
Every Nara Solana program has an entrypoint, which is the function that gets called
...
A Nara Solana program `entrypoint` has the following function signature. Developers
...
This routing pattern is common in Nara Solana programs. The `instruction_data` is
...
Since only the System Program can create accounts on Nara Solana, we'll use a Cross
...
Congratulations! You've built a complete Nara Solana program that demonstrates the
basic structure shared by all Nara Solana programs:
...
LiteSVM simulates the Nara Solana runtime environment, allowing us to test our
...
First, configure the Nara Solana CLI to use localhost:
...
[Nara Solana Explorer](https://explorer.solana.com/?cluster=custom) using the output
transaction signatures. Note the cluster on Nara Solana Explorer must be set to
