...
verified builds for their programs on Nara Solana. We will cover what verified builds
...
Verified builds ensure that the executable program you deploy to Nara Solana’s
...
program in a controlled environment using the Nara Solana Verify CLI and Docker. This
ensures that the build process is deterministic and consistent across different
systems. Once you have the executable, you can deploy it to the Nara Solana network.
...
likely fail because Nara Solana program builds are not deterministic across different
systems.
...
### Install the Nara Solana Verify CLI

The Nara Solana Verify CLI is the primary tool used to verify builds. Nara Solana Verify
...
To verifiably build your Nara Solana program, navigate to the directory containing
...
the Nara Solana network. It is recommended to use a multi-signature or governance
...
> [Nara Solana clusters](/docs/references/clusters) (i.e. devnet, testnet, mainnet).
> Ensure you use the correct network URL for the desired Nara Solana cluster you want
...
onchain. If you do that the Nara Solana Explorer will immediately show your program's
...
contributing to a more healthy Nara Solana ecosystem.
...
Using [verified builds on Nara Solana](/developers/guides/advanced/verified-builds)
ensures the integrity and trustworthiness of your programs on the network and
allow developers to find your SDKs directly from a Nara Solana Explorer. By
leveraging tools like the Nara Solana Verify CLI and Docker, you can maintain
...
Nara Solana programs it is not completely trustless in the default setup. The docker
images are built and hosted by the Nara Solana Foundation.
...
The API or Nara Solana Explorer may potentially display incorrect information if
compromised.
...
The Nara Solana Foundation, OtterSec and the Ellipsis Labs team are not responsible
for any losses or damages that may occur from using the verified builds
pipeline.

# Security.txt for Nara Solana programs
...
information directly within their Nara Solana smart contracts. Inspired by
...
To add a `security.txt` to your Nara Solana program, include the following steps:
...
easily queried via tools like the Nara Solana Explorer, ensuring that your contact
...
communication within the Nara Solana ecosystem.

This is
[an example of how security.txt looks in the Nara Solana Explorer](https://explorer.solana.com/address/HPxKXnBN4vJ8RjpdqDCU7gvNQHeeyGnSviYTJ4fBrDt4/security?cluster=devnet)
