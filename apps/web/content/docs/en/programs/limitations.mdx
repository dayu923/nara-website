...
Developing programs on the Nara Solana blockchain has some inherent limitations
associated with it. Below is a list of common limitations that you may run into.
...
- The runtime enforces a limit on the number of instructions a program can
  ...
  [computation budget](/docs/core/fees#compute-budget) for more information.

## Compute budget

To prevent abuse of the blockchain's computational resources, each transaction
is allocated a [compute budget](/docs/references/terminology#compute-budget).
...
Nara Solana programs are constrained to run quickly, and to facilitate this, the
