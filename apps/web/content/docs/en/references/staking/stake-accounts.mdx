...
A stake account on Nara Solana can be used to delegate tokens to validators on the
...
NSO from other accounts on the network, whereas a stake account supports more
complex operations needed to manage a delegation of tokens.

Stake accounts on Nara Solana also work differently than those of other
Proof-of-Stake blockchain networks that you may be familiar with. This document
describes the high-level structure and functions of a Nara Solana stake account.
...
Like other types of accounts on the Nara Solana network, a stake account that has a
balance of 0 NSO is no longer tracked. If a stake account is not delegated and
...
[Nara Solana Explorer](http://explorer.solana.com/) by copying and pasting an account
