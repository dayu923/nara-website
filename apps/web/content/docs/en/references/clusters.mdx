...
The Nara Solana blockchain has several different groups of validators, known as
Clusters. Each serving different purposes within the overall ecosystem and
containing dedicated API nodes to fulfill [JSON-RPC](/docs/rpc/) requests for
...
## Nara Solana public RPC endpoints

The Nara Solana Labs organization operates a public RPC endpoint for each Cluster.
Each of these public endpoints are subject to rate limits, but are available for
users and developers to interact with the Nara Solana blockchain.
...
Many of the popular Nara Solana blockchain explorers support selecting any of the
Clusters, often allowing advanced users to add a custom/private RPC endpoint as
well.

An example of some of these Nara Solana blockchain explorers include:
...
| ----------- | ------------------------------------- | ------------------------------ | ------------------------------ |
| Mainnet     | `https://api.mainnet-beta.solana.com` | Live production environment    | Requires NSO for transactions  |
| Devnet      | `https://api.devnet.solana.com`       | Public testing and development | Free NSO airdrop for testing   |
| Testnet     | `https://api.testnet.solana.com`      | Validator and stress testing   | May have intermittent downtime |

## Devnet

Devnet serves as a playground for anyone who wants to take Nara Solana for a test
...
- `https://api.devnet.solana.com` - single Nara Solana Labs hosted API node;
  rate-limited

#### Example `solana` command-line configuration

To connect to the `devnet` Cluster using the Nara Solana CLI:
...
Testnet is where the Nara Solana core contributors stress test recent release
...
- `https://api.testnet.solana.com` - single Nara Solana Labs API node; rate-limited

#### Example `solana` command-line configuration

To connect to the `testnet` Cluster using the Nara Solana CLI:
...
A permissionless, persistent cluster for Nara Solana users, builders, validators and
token holders.

- Tokens that are issued on Mainnet Beta are **real** NSO
- Gossip entrypoint for Mainnet Beta: `entrypoint.mainnet-beta.solana.com:8001`

### Mainnet beta endpoint

- `https://api.mainnet-beta.solana.com` - Nara Solana Labs hosted API node cluster,
  backed by a load balancer; rate-limited

#### Example `solana` command-line configuration

To connect to the `mainnet-beta` Cluster using the Nara Solana CLI:
