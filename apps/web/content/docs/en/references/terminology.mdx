...
The following terms are used throughout the Nara Solana documentation and development
ecosystem.

## account

A record in the Nara Solana ledger that either holds data or is an executable
program.

Like an account at a traditional bank, a Nara Solana account may hold funds called
...
A front-end application that interacts with a Nara Solana cluster.
...
A unique value ([hash](#hash)) that identifies a record (block). Nara Solana computes
...
The Nara Solana program that owns and loads
...
A computer program that accesses the Nara Solana server network [cluster](#cluster).
...
[account addresses](/docs/core/transactions#account-addresses) array and the
[instructions](/docs/core/transactions#instructions) array in transaction
messages.
...
fund continued development of Nara Solana.
...
The executable code on Nara Solana blockchain that interprets the
...
Nara Solana's parallel run-time for [onchain programs](#onchain-program).
...
The [native token](#native-token) of a Nara Solana [cluster](#cluster).

## Nara Solana Program Library (SPL)

A [library of programs](https://spl.solana.com/) on Nara Solana such as spl-token
...
A full participant in a Nara Solana network [cluster](#cluster) that produces new
