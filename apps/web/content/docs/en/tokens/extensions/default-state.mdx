...
title: Default State
description: Learn how to enable the DefaultAccountStateExtension.

---


## How to enable the DefaultAccountStateExtension
...
sets a default state for all new token accounts created for a mint. When
enabled, this extension automatically initializes every new token account with a
specified state, such as frozen. This allows token creators to enforce specific
behaviors, like requiring all new accounts to be frozen by default until
explicitly unfrozen.
