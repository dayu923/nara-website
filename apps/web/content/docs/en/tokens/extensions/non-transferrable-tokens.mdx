...
description: Learn how to enable the InitializeNonTransferableMint.

---


## How to enable the InitializeNonTransferableMint extension
...
prevents tokens from being transferred between token accounts after minting.
Once minted, the tokens remain permanently in the token account. However, the
token account owner can still burn tokens and close the token account when the
