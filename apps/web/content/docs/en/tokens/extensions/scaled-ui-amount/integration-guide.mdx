...
# Supporting Scaled UI Amount Extension on Nara Solana
...
- We expect scaled amounts to be used most frequently as this aligns with how
  ...
  splits.
- Multiplier updates may include a timestamp that's in the past. We recommend
  using the block timestamp for historical data.
- Note that the active multiplier may be the "multiplier" or the "newMultiplier"
  depending on the current timestamp and when the new multiplier is set to be
  active.
