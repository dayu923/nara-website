...
instruction permanently closes a token account and transfers all remaining NSO
(rent) to a specified destination account. The token account balance must be
zero before closing. Only the token account owner or designated close authority
