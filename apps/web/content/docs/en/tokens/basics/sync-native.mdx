...
title: Sync Native
description: Learn how to convert native NSO to wrapped NSO.
...
Wrapped NSO (WSOL) is a token account that tracks the amount of lamports in the
account as the token balance. WSOL enables integration with DeFi protocols to
transfer NSO as an SPL token.
...
instruction synchronizes a wrapped NSO (WSOL) token account balance with the
actual NSO (lamports) stored in it. When you transfer native NSO to a WSOL token
