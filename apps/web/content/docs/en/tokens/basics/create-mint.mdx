...
description: Learn how to create an SPL Token mint.
...
A mint account uniquely represents a token on Nara Solana and stores its global
metadata.
...
Every token on Nara Solana has a mint account, and the mint's address serves as the
token's unique identifier.

For example, USD Coin (USDC) has the mint address
`EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`. This address uniquely identifies
USDC throughout the Nara Solana ecosystem. You can view this mint on
[Nara Solana Explorer](https://explorer.solana.com/address/EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v).

## How to Create a Mint Account

Creating a mint account requires the
...
instruction. See the
[implementation details here](https://github.com/solana-program/token/blob/a7c488ca39ed4cd71a87950ed854929816e9099f/program/src/processor.rs#L27).

You need invoke two instructions to create a mint account:

1. System Program: Create an account with allocated space for a mint account and
