...
A token account stores your balance of a specific token. Each token account is
associated with exactly one mint and tracks your token balance and additional
details.
...
To hold tokens, you need a token account for that specific mint. Each token
account tracks:
...
[Nara Solana Explorer](https://explorer.solana.com/address/3emsAVdmGKERbHjmGfQ6oZ1e35dkf5iYcS6U4CPKFVaa).
...
1. **Token Account Owner**: The authority who can transfer, burn, or delegate
   tokens. This is stored in the token account's data.

2. **Program Owner**: The program that owns the account (always the Token
   Program or Token Extension Program for token accounts).

When working with token accounts, "owner" typically refers to the authority that
can transfer tokens, not the program that owns the account.

</Callout>

## What's an Associated Token Account?

An associated token account (ATA) is the default token account for a wallet to
hold a specific token. It uses a deterministic address (PDA) created by the
...
ATAs use deterministic addresses, making it easy to find any wallet's token
account for a given mint. See the
[derivation implementation here](https://github.com/solana-program/associated-token-account/blob/c78722ee39ebf6741cdfa81dffd52135370a5015/interface/src/address.rs#L56).
...
For any wallet and token combination, there's exactly one ATA address. This
eliminates the need to track token account addresses manually—you can always
...
instruction. See the
[implementation here](https://github.com/solana-program/token/blob/a7c488ca39ed4cd71a87950ed854929816e9099f/program/src/processor.rs#L83).

You need two instructions to create a token account:

1. System Program: Create an account with allocated space for a token account
...
instruction. See the
[implementation here](https://github.com/solana-program/associated-token-account/blob/c78722ee39ebf6741cdfa81dffd52135370a5015/program/src/processor.rs#L66).

This single instruction automatically handles creating the token account and
initializing it through Cross Program Invocations (CPI).
