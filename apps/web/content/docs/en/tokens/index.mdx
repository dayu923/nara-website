...
title: Tokens on Nara Solana
description: Learn about Nara Solana's Token Programs.
...
Nara Solana are referred to as SPL
([Nara Solana Program Library](https://github.com/solana-program)) Tokens.

This section covers the basics concepts of how tokens are represented on Nara Solana.
...
The Nara Solana ecosystem has two main Token Programs. Source code for both programs
below.
...
network (both fungible and non-fungible). All tokens on Nara Solana are effectively
...
Tokens on Nara Solana are uniquely identified by the address of a
...
For reference, here is a Nara Solana Explorer link to the
...
This introduces a key concept in Nara Solana development:
...
use the [Nara Solana Playground](https://beta.solpg.io/) terminal to run CLI commands
directly in the browser without local installation.

Creating tokens and accounts requires NSO for account rent deposits and
transaction fees. For first-time Nara Solana Playground users, create a Playground
wallet and run the _sh`solana airdrop`_ command in the Playground terminal. You
can also get devnet NSO from the public
...
example on [Nara Solana Playground](https://beta.solpg.io/660ce716cffcf4b13384d010).
...
example on [Nara Solana Playground](https://beta.solpg.io/660cea45cffcf4b13384d012).
