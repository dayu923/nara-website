...
Read data from the Nara Solana network by fetching different accounts. This section
will help you understand the structure of Nara Solana
[accounts](/docs/core/accounts). Each Nara Solana account has a unique
...
used to hold NSO and sign transactions. When NSO is sent to a new address for
the first time, a system account is automatically created.

The example below generates a [new keypair](mention:keypair),
[requests NSO](mention:airdrop) to fund the new public key address, and
...
The `lamports` field contains the account's NSO balance, in
...
address uniquely identifies a token on the Nara Solana network.
...
the program that owns the account. Most Nara Solana programs provide client libraries
...
data on the Nara Solana Explorer.
