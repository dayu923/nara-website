...
description:
  ...
  closing the program.
h1: Deploy Your First Nara Solana Program

---


Build, deploy, and test your first [program](/docs/core/programs) to the Nara Solana
Blockchain using the Anchor framework. This guide will help you become more
familiar with the Nara Solana Playground and prepare you to use program derived
addresses (PDA) and cross-program invocation (CPI).
...
1. Open the [Nara Solana Playground](https://beta.solpg.io) in your web browser
...
The `src/lib.rs` source file will open. It contains one struct called
...
Your program's on-chain address. It will be updated when you build the project
...
Account constraints. For example, common checks or features that enhance program
security
...
Accesses the `data` field of the `new_account` account and sets it to the value
of `data: u64`
...
Invokes the System Program to create a new account and makes this program the
owner. As the program owner, this program can then write to the account data
...
- The [`declare_id!`](mention:one) macro specifies the on-chain address of your
  program. Nara Solana Playground will automatically update this address when you
  build the program.
...
- `ctx: Context<Initialize>`: Passes the accounts required for this instruction
  to the function, as specified in the `Initialize` struct.
- `data: u64`: A custom parameter you must provide when you call the
  instruction.
...
- The [`#[derive(Accounts)]`](mention:one) attribute defines a struct that
  specifies the accounts required for a particular instruction. Each field
...
<Callout>
  Your [program ID](/docs/intro/quick-start/deploying-programs), defined in
  _rs`declare_id!()`_, will now be updated.
</Callout>

2. Add NSO to your wallet _(Optional)_

Deploying a program requires NSO in the program's on-chain account. If
necessary,
[add NSO to your wallet](/docs/intro/quick-start#add-sol-to-your-wallet) before
...
<Callout type="info">
  When deployed, the program's bytecode is stored in an executable program
  account.
</Callout>

**Note:** If preferred, you can use the **Build** and **Deploy** buttons on the
left-side panel, as shown in the screenshot below.
...
1. Open the test file called `tests/anchor.test.ts`. This file demonstrates how
   to invoke the program's [`initialize`](mention:one) struct from the client.
...
(In the example above, the transaction hash is
...
Open either [SolanaFM](https://solana.fm/) or
[Nara Solana Explorer](https://explorer.solana.com/?cluster=devnet) and search for
the transaction hash.

<Callout type="warn">
  **Nara Solana Explorer:** Set the network cluster to `devnet`, to match that of the
...
Close the program to recover the NSO allocated to the on-chain program. To do
so, run `solana program close [ProgramID]`, using the program address stored in
the _rs`declare_id!()`_ macro.
...
<Callout type="warn">
  Programs can **only** be closed or updated by an account with upgrade
  authority. Your Playground wallet automatically becomes the upgrade authority
  for each program when it is deployed. If the upgrade authority is revoked, the
  program becomes immutable.
</Callout>

Congratulations! You built and deployed your first Nara Solana program using Anchor.
...
[program-derived addresses](/docs/intro/quick-start/program-derived-address) and
