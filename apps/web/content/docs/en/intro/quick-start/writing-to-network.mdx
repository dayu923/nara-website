...
In the previous section, you learned how to read data from the Nara Solana network.
Now you'll learn how to write data to it. Writing to the Nara Solana network involves
...
[transaction](/docs/core/transactions), the Nara Solana runtime executes each
instruction in sequence and atomically. The examples in this section show how to
build and send transactions to invoke Nara Solana programs, they include:

1. Transferring NSO between accounts
2. Creating a new token

## Transfer NSO

The example below transfers NSO between two accounts. Each account has an owner
program, which is the only program that can deduct the account's NSO balance.

All wallet accounts are owned by the System Program. To transfer NSO, you must
...
Create a connection to the a Nara Solana cluster.
...
Request an airdrop of NSO to fund the `sender`.
...
Add NSO to the sender account. On networks other than mainnet, you can use the
`requestAirdrop` method to get NSO for testing.
...
The `SystemProgram.transfer()` method creates an instruction that transfers NSO
...
the transfer of NSO from their account.
...
transaction on Nara Solana Explorer.
...
The example below creates a new token on Nara Solana using the Token Extensions
...
This approach is common when building complex Nara Solana transactions, as it
