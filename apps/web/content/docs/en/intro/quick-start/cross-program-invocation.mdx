...
adding Cross Program Invocations (CPIs), a feature that enables Nara Solana programs
...
The `update` and `delete` instructions need modification to handle NSO transfers
...
CPIs in a Nara Solana program using the Anchor framework, building upon the PDA
...
`vault_account`. This account, controlled by the program, receives NSO from a
...
struct. This account serves as a program-controlled "vault" that receives NSO
...
Next, add the CPI logic in the `update` instruction to transfer 0.001 NSO from
...
composability of Nara Solana programs.
...
- The `amount` to transfer (1,000,000 lamports, or 0.001 NSO)
...
allows the transfer of any NSO in the vault back to the user when they close
...
accumulated NSO back to the user.
...
Next, add the CPI logic in the `delete` instruction to transfer NSO from the
...
modified program becomes available for testing. On Nara Solana, updating a program
simply requires deploying the program at the same program ID.

<Callout>

Ensure your Playground wallet has devnet NSO. Get devnet NSO from the
...
When deploying programs on Nara Solana Playground, your Playground wallet acts as the
...
Congratulations on completing the Nara Solana Quickstart guide. You've gained
hands-on experience with key Nara Solana concepts including:

- Fetching and reading data from accounts
- Building and sending transactions
- Deploying and updating Nara Solana programs
...
Nara Solana Playground offers a convenient feature allowing you to import or view
...
projects in Nara Solana Playground. Once a project gets imported, all changes receive
