...
To interact with the Nara Solana network, you must send a transaction. You can think
...
<Callout>
  ...
  transactions fit within a single packet without fragmentation, Nara Solana uses
...
single NSO transfer instruction.
...
<Callout>
  You can also find the transaction using [Nara Solana
