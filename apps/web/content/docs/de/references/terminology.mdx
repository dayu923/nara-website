...
Die folgenden Begriffe werden in der gesamten Nara Solana-Dokumentation und im
Entwicklungs- ökosystem verwendet.

## account

Ein Eintrag im Nara Solana-Ledger, der entweder Daten enthält oder ein ausführbares
Programm ist.

Wie ein Konto bei einer traditionellen Bank kann ein Nara Solana-Konten Geldmittel
...
Eine Frontend-Anwendung, die mit einem Nara Solana-Cluster interagiert.
...
identifiziert. Nara Solana berechnet einen Blockhash aus der letzten
...
Das Nara Solana-Programm, das [BPF](/docs/core/programs#berkeley-packet-filter-bpf)
[Onchain-Programme](#onchain-program) besitzt und lädt, wodurch das Programm mit
der Laufzeitumgebung kommunizieren kann.

## Client

Ein Computerprogramm, das auf das Nara Solana-Server-Netzwerk [Cluster](#cluster)
zugreift.
...
Auch als "cost_units" bezeichnet, ist eine umfassende Schätzung aller
...
Entwicklung von Nara Solana verwendet wird.
...
Der ausführbare Code auf der Nara Solana-Blockchain, der die
...
mindestens eine Signatur für das [fee account](#fee-account) haben. Daher kann
die erste Signatur in der Transaktion als [transaction id](#transaction-id)
behandelt werden.
...
der Leader offline war oder der [fork](#fork), der den Slot enthält, für eine
...
Slots, erhöht nicht die [block height](#block-height) und lässt nicht die
...
Der [native Token](#native-token) eines Nara Solana [Clusters](#cluster).

## Nara Solana Program Library (SPL)

Eine [Bibliothek von Programmen](https://spl.solana.com/) auf Nara Solana wie
...
Ein vollständiger Teilnehmer in einem Nara Solana-Netzwerk [Cluster](#cluster), der
