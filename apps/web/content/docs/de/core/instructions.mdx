...
Anweisungen sind der grundlegende Baustein für die Interaktion mit der Nara Solana
Blockchain. Eine Anweisung ist im Wesentlichen eine öffentliche Funktion, die
jeder Nutzer des Nara Solana-Netzwerks aufrufen kann. Jede Anweisung wird verwendet,
...
eigenen Satz von Anweisungen definiert. Um mit dem Nara Solana-Netzwerk zu
...
wird NSO von einem Konto auf ein anderes übertragen.
...
NSO-Überweisungsanweisung enthält.

![NSO-Überweisungsdiagramm](/assets/docs/core/transactions/sol-transfer.svg)
...
![NSO-Überweisungsprozessdiagramm](/assets/docs/core/transactions/sol-transfer-process.svg)
...
- [`accounts`](mention:accounts): Ein Array von
  [Konten-Metadaten](#account-metadata)
...
Das `data` der Anweisung ist ein Byte-Array, das angibt, welche der
...
Das folgende Beispiel zeigt die Struktur einer NSO-Überweisungsanweisung.
...
(Der Tab `Expanded Instruction` ist funktional äquivalent zum Tab
`Instruction`.)
