...
Alle Daten im Nara Solana-Netzwerk werden in Konten gespeichert. Sie können sich das
Nara Solana-Netzwerk als öffentliche Datenbank mit einer einzigen Konten-Tabelle
...
das Konto auf der Nara Solana-Blockchain zu lokalisieren. Kontoadressen werden oft
...
ihre Adresse, aber dies ist nicht erforderlich, da Nara Solana auch
...
Das folgende Beispiel zeigt, wie man das Nara Solana SDK verwendet, um ein keypair zu
erstellen. Ein `Keypair` beinhaltet:
...
man das Nara Solana SDK verwendet, um eine Program Derived Address zu erstellen.
...
- [`owner`](mention:owner): Die ID des Programms, dem das Konto gehört
...
Dieses Feld wird üblicherweise als "Kontendaten" bezeichnet. Die `data` in
diesem Feld wird als beliebig betrachtet, da sie jede Bytesequenz enthalten
...
Das Lesen von Daten aus einem Nara Solana-Konto umfasst zwei Schritte:

1. Abrufen des Kontos über seine [Adresse](#account-address)
2. Deserialisieren des `data`Felds des Kontos von Rohbytes in die entsprechende
   Datenstruktur, wie vom Programm definiert, das das Konto besitzt.
...
Jedes Nara Solana-Konto hat ein [Programm](/docs/core/programs), das als sein
Eigentümer festgelegt ist. Der Eigentümer des Kontos ist das einzige Programm,
das die `data` des Kontos ändern oder Lamports abziehen kann, wie in den
...
In der Abbildung unten sehen Sie, wie ein Loader-Programm verwendet wird, um ein
program account zu deployen. Das Feld `data` des program account enthält den
...
Programmcode in ihrem Feld `data`. Stattdessen verweist ihr Feld `data` auf ein
separates **program data account**, das den Programmcode enthält. (Siehe
Abbildung unten.)
...
Feld `executable` auf `true` gesetzt ist, was darauf hinweist, dass das Konto
ein Programm ist.
...
zunächst ein neues Datenkonto erstellen. Der Prozess zur Erstellung eines
Programmzustandskontos wird oft abstrahiert, aber es ist hilfreich, den
zugrundeliegenden Prozess zu verstehen.
...
Wenn NSO zum ersten Mal an eine neue Adresse gesendet wird, wird an dieser
Adresse ein Konto erstellt, das dem System Program gehört.

Im folgenden Beispiel wird ein neues keypair generiert und mit NSO finanziert.
Nach der Ausführung des Codes sehen Sie, dass die Adresse des `owner` des Kontos
`11111111111111111111111111111111` ist (das
