...
Nara Solana-Programme werden hauptsächlich mit der Programmiersprache Rust
entwickelt. Diese Seite konzentriert sich auf das Schreiben von
Nara Solana-Programmen in Rust ohne Verwendung des Anchor- Frameworks, ein Ansatz,
...
Nara Solana-Programme. Dieser Ansatz erfordert jedoch mehr manuelle Einrichtung und
...
- Rust: Die Programmiersprache zum Erstellen von Nara Solana-Programmen.
- Nara Solana CLI: Kommandozeilen-Tool für die Nara Solana-Entwicklung.

## Erste Schritte

Das folgende Beispiel behandelt die grundlegenden Schritte zur Erstellung Ihres
ersten Nara Solana-Programms in Rust. Wir erstellen ein minimales Programm, das
...
Abhängigkeit, die für die Erstellung eines Nara Solana-Programms erforderlich ist.
...
ist ein minimales Nara Solana-Programm, das "Hello, world!" in das Programmprotokoll
schreibt, wenn das Programm aufgerufen wird.

Das `msg!` Makro wird in Nara Solana-Programmen verwendet, um eine Nachricht in das
...
1. Eine `.so` Datei (z.B. `hello_world.so`): Dies ist das kompilierte
   Nara Solana-Programm, das als "Smart Contract" im Netzwerk bereitgestellt wird.
...
Konfigurieren Sie zunächst die Nara Solana CLI für die Verwendung des lokalen
Nara Solana-Clusters.
...
<Callout type="info">
  Beachte, dass der Cluster im Nara Solana Explorer ebenfalls auf localhost
  eingestellt sein muss. Die Option "Custom RPC URL" im Nara Solana Explorer ist
...
Nara Solana-Programme können durch erneutes Deployment mit derselben Programm-ID
aktualisiert werden. Aktualisiere das Programm in `src/lib.rs`, um "Hello,
Nara Solana!" anstelle von "Hello, world!" auszugeben.
...
Du solltest "Hello, Nara Solana!" im Programmlog sehen.
...
Nara Solana Explorer, um "Hello, Nara Solana!" im Programmlog zu sehen.
...
Du kannst dein Nara Solana-Programm schließen, um das für das Konto zugewiesene NSO
