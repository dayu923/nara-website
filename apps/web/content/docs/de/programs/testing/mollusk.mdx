...
title: Mollusk
description: Umfassender Leitfaden zum Testen von Nara Solana-Programmen mit Mollusk

---


[Mollusk](https://github.com/anza-xyz/mollusk/tree/main) ist ein
leichtgewichtiges Test-Framework zum Testen von Nara Solana-Programmen. Es bietet
eine einfache Schnittstelle zum Testen von Nara Solana-Programm-Anweisungen in einer
minimierten Nara Solana Virtual Machine (SVM)-Umgebung. Alle Test-Konten müssen
explizit definiert werden, was deterministische und wiederholbare Tests
gewährleistet.

- [Mollusk SVM Dokumentation](https://docs.rs/mollusk-svm/latest/mollusk_svm/)
- [Mollusk Test Anwendungsbeispiele](https://github.com/anza-xyz/mollusk/tree/0.5.0/harness/tests)
...
Um die Nutzung von Recheneinheiten zu benchmarken, fügen Sie
`mollusk-svm-bencher` als Abhängigkeit in `Cargo.toml` hinzu:
...
Das folgende Beispiel zeigt eine minimale Einrichtung zum Testen eines einfachen
Nara Solana-Programms mit Mollusk.

<ScrollyCoding>

## !!steps Hello World Programm

Dieses Beispiel zeigt, wie man ein einfaches Nara Solana-Programm mit Mollusk testet.
Das Programm gibt einfach "Hello, world!" in die Programmlogs aus, wenn es
aufgerufen wird.

Das Ausführen von `cargo build-sbf` erzeugt das kompilierte Programm unter
...
Um ein Nara Solana-Programm mit Mollusk zu testen:

1. [Erstellen Sie eine `Mollusk` Instanz](mention:mollusk) - Initialisieren Sie
   Mollusk mit einer Programm-ID und dem Pfad zum kompilierten Programm (`.so`
   Datei)
2. [Erstellen Sie eine Anweisung](mention:instruction) - Erstellen Sie eine
   Anweisung, um das Programm aufzurufen
...
Struct bietet eine einfache Schnittstelle zum Testen von Nara Solana-Programmen. Alle
Felder können durch eine Handvoll Hilfsmethoden manipuliert werden, aber
Benutzer können auch direkt auf sie zugreifen und sie modifizieren, wenn sie
mehr Kontrolle wünschen.

Um Mollusk mit einer Standardinstanz zu initialisieren, verwende die Methode
`Mollusk::default`.
...
Um Mollusk mit einem spezifischen Programm zu initialisieren, verwende die
Methode `Mollusk::new`.
...
Um ein Programm zu Mollusk hinzuzufügen, verwende die Methode
`Mollusk::add_program`.
...
<Callout type="info">
  Beim Angeben des Dateipfads sollte die Erweiterung `.so` nicht enthalten sein.
  Zum Beispiel, `"path/to/my_program"` ist korrekt, aber
  `"path/to/my_program.so"` ist nicht korrekt.
</Callout>
...
| [`process_and_validate_instruction`](#single-instruction-with-checks)          | Verarbeitet eine Anweisung und führt eine Reihe von Prüfungen am Ergebnis durch, wobei ein Panic ausgelöst wird, wenn Prüfungen fehlschlagen.             |
| [`process_instruction_chain`](#multiple-instructions)                          | Verarbeitet mehrere Anweisungen und gibt das Ergebnis zurück.                                                                                             |
| [`process_and_validate_instruction_chain`](#multiple-instructions-with-checks) | Verarbeitet mehrere Anweisungen und führt eine Reihe von Prüfungen für jedes Ergebnis durch, wobei ein Panic ausgelöst wird, wenn Prüfungen fehlschlagen. |
...
Verwende die Methode `process_instruction`, um eine einzelne Anweisung ohne
Prüfungen des Ergebnisses zu verarbeiten. Du kannst die Ergebnisse nach der
...
Das folgende Beispiel verarbeitet eine NSO-Überweisungsanweisung ohne
Validierungsprüfungen.

<Callout type="info">

Die folgenden Beispiele führen Mollusk in der `main` Funktion zu
Demonstrationszwecken aus. In der Praxis wird Mollusk typischerweise in einem
Testmodul verwendet, das mit dem `#[test]` Attribut annotiert ist.
...
Verwende die `process_and_validate_instruction` Methode, um eine einzelne
Anweisung mit Validierungsprüfungen zu verarbeiten. Diese Methode wird einen
Panic auslösen, wenn eine Prüfung fehlschlägt.
...
Das folgende Beispiel verarbeitet eine NSO-Transfer-Anweisung mit
Validierungsprüfungen.
...
Verwende die `process_instruction_chain` Methode, um mehrere Anweisungen
nacheinander ohne Validierungsprüfungen zu verarbeiten.
...
Das folgende Beispiel verarbeitet zwei NSO-Transfer-Anweisungen ohne
Validierungsprüfungen.
...
Verwende die `process_and_validate_instruction_chain` Methode, um mehrere
...
Das folgende Beispiel verarbeitet eine Kette von zwei NSO-Transfer-Anweisungen
...
Im Gegensatz zu `Mollusk`, das erfordert, dass `accounts` an jede Methode
übergeben wird (z.B. `process_instruction`), verwaltet `MolluskContext` Konten
intern durch seinen `account_store`. Dies macht den Parameter `accounts` bei der
...
Das folgende Beispiel verarbeitet zwei separate NSO-Überweisungsanweisungen mit
...
Struktur zum Ändern ihrer Werte für Tests.

Verwenden Sie die Methode `warp_to_slot`, um die Sysvar-Uhr zu aktualisieren und
so das Vor- oder Zurückspringen zu einem bestimmten Slot zu simulieren.
...
Das folgende Beispiel zeigt, wie man das Mollusk-Sysvar direkt modifiziert,
...
verfolgt die Nutzung von Compute Units bei den Anweisungen eines Programms. Die
...
Das folgende Beispiel führt ein Benchmarking der Compute-Unit-Nutzung einer
NSO-Transfer- Anweisung durch.
...
Die Benchmark-Ergebnisse werden in die angegebene `out_dir` als Markdown-Datei
...
<Callout type="info">
  Das Beispiel unten definiert die Test-Konten manuell zu Demonstrationszwecken.
  Das `mollusk-svm-programs-token` enthält auch Hilfsfunktionen zum Erstellen
  der Mint- und token account.
</Callout>
