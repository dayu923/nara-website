...
Nara Solana-Programms mit der Nara Solana CLI.
...
### Installation der Nara Solana CLI

Zuerst [installiere die Nara Solana CLI](/docs/intro/installation).
...
Wählen Sie, auf welchem Nara Solana-Cluster Sie deployen möchten. Verwenden Sie den
Befehl `solana config get`, um Ihre aktuelle Konfiguration zu überprüfen:
...
Sie benötigen NSO, um für Programm-Deployments zu bezahlen. Die Menge hängt von
...
Hier ist ein minimales Nara Solana-Programm, das "Hello, Nara Solana!" in die Programm-
Logs ausgibt.
...
Das Deployen eines Programms erfordert NSO, das basierend auf der Größe des
Programms dem Programmkonto zugewiesen wird. Größere Programme kosten mehr NSO
für das Deployment.
...
Berechnen Sie das erforderliche NSO für diese Größe (Bytes):
...
<Callout type="info">
  Sie benötigen etwas mehr NSO als angezeigt, um die Transaktionsgebühren für
...
CLI berechnet automatisch die erforderlichen NSO und zieht sie von Ihrer Wallet
ab.
...
oder das Programm schließen, um das zugewiesene NSO des Programms
zurückzufordern.
...
Das Balance zeigt das NSO an, das dem Programmkonto zugewiesen wurde, um seine
Daten zu speichern.
...
`solana program set-upgrade-authority` mit der Option `--final`, um die
...
Um Ihr Programm zu schließen und das dem Programmkonto zugewiesene NSO
...
Wenn das Nara Solana-Netzwerk überlastet ist, verwenden Sie diese Flags, um bei dem
...
- **`--with-compute-unit-price`**: Legt priority fee in Mikro-Lamports (0,000001
  NSO) pro Compute-Einheit fest. Prüfen Sie die
  [Helius Priority Fee API](https://docs.helius.dev/guides/priority-fee-api) für
  aktuelle Raten. Eine priority fee ist eine zusätzliche Gebühr, die an den
