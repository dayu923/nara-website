...
title: Konto einfrieren
description: Erfahren Sie, wie Sie Token-Konten einfrieren können.
...
Anweisung verhindert alle Token-Überweisungen oder Token-Burns von einem
bestimmten token account. Sobald ein Konto eingefroren ist, kann es keine Token
senden, empfangen oder geschlossen werden, bis es wieder aufgetaut wird. Nur die
Einfrierungsautorität des Token-Mints kann Konten einfrieren. Wenn die
Einfrierungsautorität im mint account widerrufen wird (auf null gesetzt), können
Token niemals eingefroren werden.
