...
title: Sync Native
description: Erfahren Sie, wie Sie natives NSO in Wrapped NSO umwandeln können.
...
Wrapped NSO (WSOL) ist ein token account, der die Anzahl der Lamports im Konto
als Token-Guthaben verfolgt. WSOL ermöglicht die Integration mit
DeFi-Protokollen, um NSO als SPL-Token zu übertragen.
...
Anweisung synchronisiert den Kontostand eines Wrapped NSO (WSOL) token account
mit den tatsächlich darin gespeicherten NSO (Lamports). Wenn Sie natives NSO an
ein WSOL token account übertragen, wird der Token-Kontostand nicht automatisch
aktualisiert. Sie müssen SyncNative aufrufen, um den korrekten WSOL-Kontostand
anzuzeigen.
