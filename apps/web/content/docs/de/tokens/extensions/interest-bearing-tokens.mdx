...
title: Zinstragende Token
description:
  Erfahren Sie, wie Sie die InterestBearingMintExtension aktivieren können.

---


## Wie man die InterestBearingExtension aktiviert
...
ermöglicht es, einen Zinssatz direkt auf einem mint account festzulegen. Die
Zinsen werden kontinuierlich basierend auf dem Netzwerk-Zeitstempel verzinst,
aber der tatsächliche Token- Bestand, der in jedem token account gespeichert
ist, bleibt unverändert. Der Gesamtwert (Token- Bestand plus aufgelaufene
Zinsen) ist nur eine Berechnung. Mit dieser Erweiterung werden keine neuen Token
geminted.
