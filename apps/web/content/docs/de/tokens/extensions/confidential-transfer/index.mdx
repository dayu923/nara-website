...
title: Vertraulicher Transfer
description:
  Erfahren Sie mehr über die Vertraulicher-Transfer-Erweiterung und wie Sie sie
  verwenden können, um optionale Funktionen zu Token-Mints und Konten
  hinzuzufügen.
...
<Callout type="warn">
  ...
  Erweiterung für vertrauliche Überweisungen derzeit nicht verfügbar ist.
  Während die Konzepte weiterhin gültig sind, werden die Code- Beispiele nicht
  funktionieren.
</Callout>

## Was sind vertrauliche Überweisungen?

Vertrauliche Überweisungen ermöglichen es Ihnen, Token zwischen Token-Konten zu
übertragen, ohne den Überweisungsbetrag preiszugeben. Dies ist nützlich für
datenschutzfördernde Transaktionen. Nur die Überweisungsbeträge und
Token-Guthaben sind privat. Die Adressen der Token-Konten bleiben öffentlich.

- [Protokollübersicht](https://www.solana-program.com/docs/confidential-balances/overview) -
  Details zum zugrundeliegenden kryptografischen Protokoll
...
- [Confidential Balances Cookbook](https://github.com/solana-developers/Confidential-Balances-Sample) -
  Code-Snippets zur Verwendung der Confidential Transfer-Erweiterung

### Wie funktioniert es?

Die Confidential Transfer-Erweiterung fügt
...
zum Token Extensions Program hinzu, die es Ihnen ermöglichen, Token zwischen
Konten zu übertragen, ohne den Überweisungsbetrag preiszugeben.
...
Der grundlegende Ablauf vertraulicher Token-Überweisungen ist wie folgt:

1. Erstellen Sie ein Mint-Account mit der Confidential Transfer-Erweiterung.
2. Erstellen Sie Token-Konten mit Confidential Transfer-Erweiterung für den
   Sender und Empfänger.
3. Prägen Sie Token auf das Konto des Senders.
...
5. **Anwenden** des ausstehenden Guthabens des Senders auf das **vertrauliche
   verfügbare Guthaben**.
6. Vertrauliche **Überweisung** von Token vom Token-Konto des Senders zum
   Token-Konto des Empfängers.
7. **Anwenden** des ausstehenden Guthabens des Empfängers auf das **vertrauliche
   verfügbare Guthaben**.
8. **Abheben** des vertraulichen verfügbaren Guthabens des Empfängers zum
   **öffentlichen Guthaben**.

Weitere Details zu den Schritten im vertraulichen Überweisungsablauf finden Sie
...
<Cards>
  ...
  >
    Wie man ein mint account mit der Confidential Transfer-Erweiterung erstellt
  </Card>
  <Card
    title="Token Account erstellen"
    href="/docs/tokens/extensions/confidential-transfer/create-token-account"
  >
    Wie man ein token account mit der Confidential Transfer-Erweiterung
    konfiguriert
  </Card>
  <Card
    title="Tokens einzahlen"
    href="/docs/tokens/extensions/confidential-transfer/deposit-tokens"
  >
    Wie man Tokens auf das vertrauliche ausstehende Guthaben einzahlt
  </Card>
  <Card
    title="Ausstehenden Saldo anwenden"
    href="/docs/tokens/extensions/confidential-transfer/apply-pending-balance"
  >
    Wie man ausstehenden Saldo auf das verfügbare vertrauliche Guthaben anwendet
  </Card>
  <Card
    title="Tokens abheben"
    href="/docs/tokens/extensions/confidential-transfer/withdraw-tokens"
  >
    Wie man Tokens vom vertraulichen verfügbaren Guthaben abhebt
  </Card>
  <Card
    title="Tokens übertragen"
    href="/docs/tokens/extensions/confidential-transfer/transfer-tokens"
  >
    Wie man vertraulich Tokens zwischen token accounts überträgt
  </Card>
</Cards>

Das folgende Diagramm zeigt eine detaillierte Sequenz des grundlegenden Ablaufs
für vertrauliche Token-Übertragungen:
...
Das folgende Rust-Beispiel zeigt die Schritte für vertrauliche
Token-Übertragungen entsprechend dem vorhergehenden Diagramm. Die Schritte sind
zur besseren Übersichtlichkeit in Hilfsfunktionen unterteilt.
...
müssen die Nara Solana CLI installiert haben, um den lokalen Validator zu verwenden.
...
<Callout type="info">
  Zum Zeitpunkt der Erstellung sind die Confidential Transfers im
  Standard-Lokalvalidator nicht aktiviert. Sie müssen das Token Extensions
...
## Confidential Transfer Anweisungen

Die vollständige Liste der Confidential Transfer-Erweiterung
[Anweisungen](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/instruction.rs#L29)
ist wie folgt:

| Anweisung                           | Beschreibung                                                                                                                                                                                      |
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | Richtet mint account für vertrauliche Übertragungen ein. Diese Anweisung muss in derselben Transaktion wie die _rs`TokenInstruction::InitializeMint`_ Anweisung enthalten sein.                   |
| _rs`UpdateMint`_                    | Aktualisiert die Einstellungen für vertrauliche Übertragungen für ein Mint.                                                                                                                       |
| _rs`ConfigureAccount`_              | Richtet ein token account für vertrauliche Übertragungen ein.                                                                                                                                     |
| _rs`ApproveAccount`_                | Genehmigt ein token account für vertrauliche Übertragungen, wenn das Mint eine Genehmigung für neue token accounts erfordert.                                                                     |
| _rs`EmptyAccount`_                  | Leert die ausstehenden und verfügbaren vertraulichen Guthaben, um das Schließen eines token accounts zu ermöglichen.                                                                              |
| _rs`Deposit`_                       | Konvertiert öffentliches Token-Guthaben in ausstehenden vertraulichen Saldo.                                                                                                                      |
| _rs`Withdraw`_                      | Konvertiert verfügbares vertrauliches Guthaben zurück in öffentliches Guthaben.                                                                                                                   |
| _rs`Transfer`_                      | Überträgt Tokens vertraulich zwischen token accounts.                                                                                                                                             |
| _rs`ApplyPendingBalance`_           | Konvertiert ausstehenden Saldo in verfügbaren Saldo nach Einzahlungen oder Überweisungen.                                                                                                         |
| _rs`EnableConfidentialCredits`_     | Ermöglicht einem token account, vertrauliche Token-Übertragungen zu empfangen.                                                                                                                    |
| _rs`DisableConfidentialCredits`_    | Blockiert eingehende vertrauliche Übertragungen, während öffentliche Übertragungen weiterhin möglich sind.                                                                                        |
| _rs`EnableNonConfidentialCredits`_  | Ermöglicht einem token account, öffentliche Token-Übertragungen zu empfangen.                                                                                                                     |
| _rs`DisableNonConfidentialCredits`_ | Blockiert reguläre Übertragungen, um das Konto nur für vertrauliche Übertragungen empfänglich zu machen.                                                                                          |
| _rs`TransferWithFee`_               | Überträgt Tokens vertraulich zwischen token accounts mit einer Gebühr.                                                                                                                            |
| _rs`ConfigureAccountWithRegistry`_  | Alternative Methode zur Konfiguration von token accounts für vertrauliche Übertragungen unter Verwendung eines _rs`ElGamalRegistry`_ Kontos anstelle eines _rs`VerifyPubkeyValidity`_ Nachweises. |
