...
description:
  Erfahren Sie, wie Sie ausstehende Guthaben anwenden, um Mittel verfügbar zu
  machen.

---


## Wie man ausstehenden Saldo auf verfügbaren Saldo anwendet

Bevor Token vertraulich übertragen werden können, muss das öffentliche
Token-Guthaben in ein vertrauliches Guthaben umgewandelt werden. Diese
Umwandlung erfolgt in zwei Stufen:

1. **Vertraulicher ausstehender Saldo**: Zunächst werden Token vom öffentlichen
   Guthaben auf ein "ausstehendes" vertrauliches Guthaben "eingezahlt".
2. **Vertraulicher verfügbarer Saldo**: Der ausstehende Saldo wird dann auf den
   verfügbaren Saldo "angewendet", wodurch die Token für vertrauliche
   Überweisungen verfügbar werden.

Dieser Abschnitt erklärt die zweite Stufe: das Anwenden des ausstehenden Saldos
auf den verfügbaren Saldo.

Wenn Token vom öffentlichen Guthaben "eingezahlt" werden oder wenn Token
vertraulich von einem Token-Konto auf ein anderes übertragen werden, werden die
Token zunächst dem vertraulichen ausstehenden Saldo hinzugefügt. Bevor Token für
vertrauliche Überweisungen verwendet werden können, muss der ausstehende Saldo
auf den verfügbaren Saldo "angewendet" werden.
...
Das folgende Diagramm zeigt die Schritte, die beim Anwenden des ausstehenden
Saldos auf den verfügbaren Saldo beteiligt sind:
...
Um einen ausstehenden Saldo in einen verfügbaren Saldo umzuwandeln, rufen Sie
die
...
Das `spl_token_client` Crate bietet eine
`confidential_transfer_apply_pending_balance` Methode, die eine Transaktion mit
der `ApplyPendingBalance` Anweisung erstellt und sendet, wie im Beispiel unten
gezeigt.

### Beispielcode

Das folgende Beispiel zeigt, wie man den vertraulichen ausstehenden Saldo auf
den vertraulichen verfügbaren Saldo anwendet.
...
<Callout type="info">
  Zum Zeitpunkt der Erstellung sind die vertraulichen Überweisungen auf dem
  Standard-lokalen Validator nicht aktiviert. Sie müssen das Token Extensions
  Program des Mainnets klonen, um den Beispielcode auszuführen.
</Callout>
