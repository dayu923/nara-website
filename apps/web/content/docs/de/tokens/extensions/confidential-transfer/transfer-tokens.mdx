...
description:
  Erfahren Sie, wie Sie Token vertraulich von einem Token-Konto auf ein anderes
  übertragen können.
...
<Callout type="warn">
  ...
  vertrauliche Übertragungserweiterung derzeit nicht verfügbar ist. Während die
  Konzepte weiterhin gültig sind, werden die Codebeispiele nicht funktionieren.
</Callout>

## Wie man vertraulich Token von einem Token-Konto auf ein anderes überträgt

Um Token vertraulich von einem Token-Konto auf ein anderes zu übertragen, müssen
sowohl Sender als auch Empfänger Token-Konten haben, die mit dem
_rs`ConfidentialTransferAccount`_ Status konfiguriert und für vertrauliche
Übertragungen freigegeben sind. Das Token-Konto des Senders muss außerdem über
ein verfügbares vertrauliches Guthaben verfügen, von dem aus übertragen werden
kann.

Um Token vertraulich zu übertragen:

1. Erstellen Sie
   [drei Beweise](https://github.com/solana-program/token-2022/blob/d073cb89dbcd430387b5c6fb4b7157911351e4a3/confidential-transfer/proof-generation/src/transfer.rs#L35)
   clientseitig:
   ...
   **Gleichheitsbeweis
   ...
   Überprüft, dass der neue verfügbare Guthaben-Chiffretext nach der Übertragung
   mit seinem entsprechenden
   [Pedersen-Commitment](https://en.wikipedia.org/wiki/Commitment_scheme)
   übereinstimmt und stellt sicher, dass das neue verfügbare Guthaben des
   Quellkontos korrekt als `new_balance = current_balance - transfer_amount`
   berechnet wird.

   **Chiffretext-Gültigkeitsbeweis
   ...
   Überprüft, dass die Überweisungsbetrags-Chiffretexte für alle drei Parteien
   (Quelle, Ziel und Prüfer) ordnungsgemäß generiert werden und stellt sicher,
   dass der Überweisungsbetrag korrekt unter dem öffentlichen Schlüssel jeder
   Partei verschlüsselt ist.
   ...
   **Bereichsbeweis
   ...
   Überprüft, dass das neue verfügbare Guthaben und der Überweisungsbetrag
   (aufgeteilt in niedrige/hohe Bits) alle nicht-negativ und innerhalb eines
   bestimmten Bereichs sind.

2. Für jeden Beweis:
   - Rufen Sie das ZK ElGamal-Beweisprogramm auf, um die Beweisdaten zu
     verifizieren.
   - Speichern Sie die beweisspezifischen Metadaten in einem
     Beweis-"Kontext-Status"-Konto zur Verwendung in anderen Anweisungen.
...
3. Rufen Sie die
   ...
   Anweisung auf und stellen Sie die Beweis-Kontext-Status-Konten bereit.

4. Schließen Sie die Beweis-Kontext-Status-Konten, um das für ihre Erstellung
   verwendete NSO zurückzugewinnen.

Das folgende Diagramm zeigt die Schritte, die bei der Übertragung von Tokens vom
Token-Konto eines Senders zum Token-Konto eines Empfängers erforderlich sind.
...
Um Tokens vertraulich von einem Token-Konto auf ein anderes zu übertragen,
müssen Sie:

- Einen Gleichheitsbeweis, Chiffretext-Gültigkeitsbeweis und Bereichsbeweis
  clientseitig generieren
- Das Zk ElGamal-Beweisprogramm aufrufen, um die Beweise zu verifizieren und die
  "Kontext-Status"-Konten zu initialisieren
- Die
  [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604)
  Anweisung aufrufen und dabei die drei Beweis-Konten bereitstellen
- Die drei Beweis-Konten schließen, um die Rent zurückzugewinnen.

Das `spl_token_client` Crate stellt die folgenden Methoden bereit:

- `confidential_transfer_create_context_state_account` Methode, die ein
  Beweis-Konto erstellt.
- `confidential_transfer_transfer` Methode, die die `Transfer` Anweisung
  aufruft.
- `confidential_transfer_close_context_state_account` Methode, die ein
  Beweis-Konto schließt.

### Beispielcode

Das folgende Beispiel zeigt, wie man vertraulich Tokens zwischen Token-Konten
überträgt.
...
<Callout type="info">
  Zum Zeitpunkt der Erstellung sind die vertraulichen Übertragungen im
