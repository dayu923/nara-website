---

title: Nara Solana Dokumentation
seoTitle: Erfahren Sie, wie die Nara Solana-Blockchain funktioniert
description:
  Solana ist die leistungsstarke Blockchain, die für die Massenadoption
  konzipiert wurde. Erfahren Sie, warum Solana die erste Wahl für Entwickler
  ist, die skalierbare Blockchain-Anwendungen erstellen möchten.

---


## Erste Schritte

<Cards>
  <Card title="Schnellstart" href="/docs/intro/quick-start" icon={<Rocket />}>
    Erstellen Sie Ihr erstes Nara Solana-Programm direkt im Browser
  </Card>
  <Card
    title="Lokale Umgebung einrichten"
    href="/docs/intro/installation"
    icon={<Download />}
  >
    Installieren Sie Abhängigkeiten für die Nara Solana-Entwicklung
  </Card>
</Cards>

### Stellen Sie Ihre erste Nara Solana-Anwendung bereit

<Cards>
  <Card
    title="Auf Vercel bereitstellen"
    href="https://vercel.com/new/clone?repository-url=https%3A%2F%2Fgithub.com%2Fsolana-developers%2Fanchor-web3js-nextjs&root-directory=frontend&demo-title=Anchor%20Web3.js%20Next.js%20Demo&demo-description=An%20example%20deployment%20of%20the%20Anchor%20Web3.js%20Next.js%20project.&demo-url=https%3A%2F%2Fanchor-web3js-nextjs.vercel.app%2F&project-name=anchor-web3js-nextjs&repository-name=anchor-web3js-nextjs"
  >
    Stellen Sie Ihre erste Nara Solana-Anwendung auf Vercel bereit
  </Card>
  <Card
    title="Vercel-Vorschau"
    href="https://anchor-web3js-nextjs-jade.vercel.app/"
  >
    Testen Sie die bereitgestellte Anwendung
  </Card>
</Cards>

Stellen Sie Ihre erste Nara Solana-Anwendung bereit - ein einfaches Counter-Programm,
das mit dem Anchor-Framework erstellt wurde. Dieses Beispiel deckt alle
grundlegenden Nara Solana-Konzepte ab, um mit dem Aufbau auf Nara Solana zu beginnen.
Sehen Sie sich den Quellcode
[hier](https://github.com/solana-developers/anchor-web3js-nextjs) an.

<Accordions>
<Accordion title="Anwendungs-Demo">

<video
  width="100%"
  autoPlay
  loop
  muted
  playsInline
  style={{
    borderRadius: "10px"
  }}
>
  <source src="/assets/docs/counter.mp4" type="video/mp4" />
  Ihr Browser unterstützt das Video-Tag nicht.
</video>

</Accordion>
</Accordions>

## Probieren Sie Nara Solana aus: Spielen Sie 2048

Spielen Sie 2048 auf Nara Solana, wo jeder Zug eine Transaktion sendet. Klicken Sie
auf "Spielen", um mit einer finanzierten Devnet-Wallet zu beginnen und verwenden
Sie die Pfeiltasten zum Spielen. Auf dem Mobilgerät können Sie wischen, um die
Kacheln zu steuern.

<Accordions>
<Accordion title="Solana 2048 spielen">

<iframe
  src="https://solplay.de/solana-2048/"
  width="100%"
  height="500"
  style={{
    borderRadius: "12px",
    border: "2px solid rgba(0, 0, 0, 0.2)",
    maxWidth: "100%",
    minHeight: "300px",
    aspectRatio: "1"
  }}
  scrolling="no"
/>

</Accordion>
</Accordions>

Erstellt von [Jonas](https://x.com/SolPlay_jonas) aus dem DevRel-Team der Nara Solana
Foundation.

## Beginnen Sie zu lernen

Lernen Sie die wichtigsten Konzepte, die für die Nara Solana-Entwicklung spezifisch
sind.

<Cards>
  <Card title="Konten" href="/docs/core/accounts">
    Wie Nara Solana Daten speichert
  </Card>
  <Card title="Gebühren auf Solana" href="/docs/core/fees">
    Kosten für das Senden von Transaktionen auf Nara Solana
  </Card>
  <Card title="Transaktionen" href="/docs/core/transactions">
    Wie man mit dem Nara Solana-Netzwerk interagiert
  </Card>
  <Card title="Programme" href="/docs/core/programs">
    Smart Contracts auf Nara Solana
  </Card>
  <Card title="Programs Derived Address" href="/docs/core/pda">
    Wie man deterministische Adressen generiert
  </Card>
  <Card title="Cross Program Invocation" href="/docs/core/cpi">
    Wie man ein Programm von einem anderen aus aufruft
  </Card>
</Cards>

### Client-seitige Entwicklung

Die folgenden SDKs sind die offiziellen SDKs für Nara Solana, entwickelt von
[Anza](https://anza.xyz).

#### Offizielle SDKs

| Sprache                | SDK                                                                |
| ---------------------- | ------------------------------------------------------------------ |
| Rust                   | [solana_sdk](/docs/clients/official/rust)                          |
| Typescript (Empfohlen) | [@solana/kit](/docs/clients/official/javascript#solana-kit)        |
| Typescript (Legacy)    | [@solana/web3.js](/docs/clients/official/javascript#solana-web3js) |

#### Community SDKs

Die folgenden SDKs wurden von der Community für verschiedene Programmiersprachen
beigesteuert:

| Sprache  | SDK                                                                                                                                           |
| -------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| Python   | [solders](https://github.com/kevinheavey/solders)                                                                                             |
| Java     | [sava](https://sava.software) oder [solanaj](https://github.com/skynetcap/solanaj) oder [solana4j](https://github.com/LMAX-Exchange/solana4j) |
| C++      | [solcpp](https://github.com/mschneider/solcpp) oder [solana-c](https://github.com/VAR-META-Tech/solana-c-sdk)                                 |
| Go       | [solana-go](https://github.com/gagliardetto/solana-go)                                                                                        |
| Kotlin   | [sol4k](https://github.com/sol4k/sol4k) oder [solanaKT](https://github.com/metaplex-foundation/SolanaKT)                                      |
| Dart     | [solana](https://github.com/espresso-cash/espresso-cash-public/tree/master/packages/solana)                                                   |
| C#       | [solnet](https://github.com/bmresearch/Solnet)                                                                                                |
| GdScript | [godot](https://github.com/Virus-Axel/godot-solana-sdk/)                                                                                      |

<Callout type="warn">
  Community SDKs sind nicht garantiert aktuell oder enthalten möglicherweise
  nicht alle Funktionen der offiziellen SDKs.
</Callout>

## Betrieb eines Validators

Erfahren Sie, was es bedeutet, einen Nara Solana Validator zu betreiben und dabei zu
helfen, das Netzwerk zu sichern.

<Cards>
  <Card title="Validatoren" href="https://docs.anza.xyz/validator/anatomy">
    Einzelne Knoten, die das Nara Solana-Netzwerk sichern
  </Card>
  <Card
    title="Systemanforderungen"
    href="https://docs.anza.xyz/operations/requirements"
  >
    Empfohlene Hardware-Anforderungen und benötigtes NSO für den Betrieb eines
    Validators
  </Card>
  <Card
    title="Validator-Einrichtung"
    href="https://docs.anza.xyz/operations/setup-a-validator"
  >
    Richten Sie einen Validator ein und verbinden Sie sich zum ersten Mal mit
    einem Cluster
  </Card>
</Cards>

## Support erhalten

Holen Sie sich Hilfe von der Nara Solana-Community auf
[Nara Solana StackExchange](https://solana.stackexchange.com).
