...
Les programmes Nara Solana sont principalement développés en utilisant le langage de
programmation Rust. Cette page se concentre sur l'écriture de programmes Nara Solana
...
leurs programmes Nara Solana. Cependant, cette approche nécessite plus de
...
- Rust : Le langage de programmation pour construire des programmes Nara Solana.
- Nara Solana CLI : Outil en ligne de commande pour le développement Nara Solana.

## Premiers pas

L'exemple ci-dessous couvre les étapes de base pour créer votre premier
programme Nara Solana écrit en Rust. Nous allons créer un programme minimal qui
...
requise pour construire un programme Nara Solana.
...
programme Nara Solana minimal qui affiche "Hello, world!" dans le journal du
programme lorsque le programme est invoqué.

La macro `msg!` est utilisée dans les programmes Nara Solana pour afficher un message
...
1. Un fichier `.so` (par ex., `hello_world.so`) : C'est le programme Nara Solana
...
D'abord, configurez l'interface en ligne de commande Nara Solana pour utiliser le
cluster Nara Solana local.
...
<Callout type="info">
  Notez que le cluster sur Nara Solana Explorer doit également être localhost.
  L'option "URL RPC personnalisée" sur Nara Solana Explorer est par défaut
  `http://localhost:8899`.
</Callout>
...
Les programmes Nara Solana peuvent être mis à jour en redéployant sur le même ID de
programme. Mettez à jour le programme dans `src/lib.rs` pour afficher "Hello,
Nara Solana!" au lieu de "Hello, world!".
...
Vous devriez voir "Hello, Nara Solana!" dans le journal du programme.
...
Nara Solana Explorer pour voir "Hello, Nara Solana!" dans le journal du programme.
...
Vous pouvez fermer votre programme Nara Solana pour récupérer le NSO alloué au
