...
Les programmes Nara Solana écrits en Rust ont des exigences structurelles minimales,
...
programmes Nara Solana suivent généralement un modèle commun :
...
[Programme Token](https://github.com/solana-program/token/tree/main/program/src).

## Exemple de programme

Pour démontrer comment construire un programme Rust natif avec plusieurs
instructions, nous allons examiner un programme de compteur simple qui
...
Tout d'abord, créons un nouveau projet Rust pour notre programme Nara Solana.
...
Nara Solana. Nous avons besoin de `solana-program` pour le SDK principal et `borsh`
...
<Callout type="info">
  Il n'y a pas d'obligation d'utiliser Borsh. Cependant, c'est une bibliothèque
  de sérialisation couramment utilisée pour les programmes Nara Solana.
</Callout>

## !!steps Configurer crate-type

Les programmes Nara Solana doivent être compilés comme des bibliothèques dynamiques.
Ajoutez la section `[lib]` pour configurer la façon dont Cargo compile le
programme.
...
Chaque programme Nara Solana possède un point d'entrée, qui est la fonction appelée
lorsque le programme est invoqué. Commençons par ajouter les importations dont
nous aurons besoin pour le programme et configurer le point d'entrée.
...
Un `entrypoint` de programme Nara Solana a la signature de fonction suivante. Les
...
## !!steps Définir l'énumération d'instructions
...
Maintenant, nous devons désérialiser le `instruction_data` (octets bruts) en
l'une de nos variantes d'énumération `CounterInstruction`. La méthode Borsh
`try_from_slice` gère automatiquement cette conversion.
...
Maintenant, mettons à jour la fonction principale `process_instruction` pour
acheminer les instructions vers leurs fonctions gestionnaires appropriées.

Ce modèle d'acheminement est courant dans les programmes Nara Solana. Le
`instruction_data` est désérialisé en une variante d'une énumération
...
Implémentons le gestionnaire pour créer et initialiser un nouveau compte
compteur. Comme seul le System Program peut créer des comptes sur Nara Solana, nous
utiliserons une Cross Program Invocation (CPI), essentiellement en appelant un
autre programme depuis notre programme.
...
<Callout type="warn">
  Cette instruction est uniquement à des fins de démonstration. Elle n'inclut
  pas les vérifications de sécurité et de validation nécessaires pour les
  programmes en production.
</Callout>
...
Maintenant, implémentons le gestionnaire qui incrémente un compteur existant.
Cette instruction :
...
## !!steps Programme complet

Félicitations ! Vous avez construit un programme Nara Solana complet qui démontre la
structure de base partagée par tous les programmes Nara Solana :

- **Point d'entrée** : Définit où l'exécution du programme commence et achemine
...
- **Gestion des instructions** : Définit les instructions et leurs fonctions de
  gestion associées
- **Gestion d'état** : Définit les structures de données des comptes et gère
  leur état dans les comptes appartenant au programme
- **Cross Program Invocation (CPI)** : Appelle le System Program pour créer de
  nouveaux comptes appartenant au programme

La prochaine étape consiste à tester le programme pour s'assurer que tout
...
Maintenant, testons notre programme de compteur. Nous utiliserons
[LiteSVM](https://github.com/LiteSVM/litesvm), un framework de test qui nous
permet de tester des programmes sans les déployer sur un cluster.
...
`litesvm` pour les tests et `solana-sdk`.
...
## !!steps Créer un module de test

Maintenant, ajoutons un module de test à notre programme. Nous commencerons par
la structure de base et les importations.
...
LiteSVM simule l'environnement d'exécution Nara Solana, nous permettant de tester
...
<Callout type="warn">
  Vous devez exécuter `cargo build-sbf` avant d'exécuter les tests pour générer
...
Maintenant, testons l'instruction d'incrémentation pour nous assurer qu'elle met
...
Exécutez les tests avec la commande suivante. Le drapeau `--nocapture` affiche
la sortie du test.
...
## !!steps Implémenter le code du client
...
Ajoutez le code du client à `examples/client.rs` et remplacez le `program_id`
...
D'abord, configurez la CLI Nara Solana pour utiliser localhost :
...
Avec le validator en cours d'exécution, déployez votre programme sur le cluster
local :
...
`solana program show` avec l'ID de votre programme :
...
transactions sur [Nara Solana Explorer](https://explorer.solana.com/?cluster=custom)
en utilisant les signatures de transaction de sortie. Notez que le cluster sur
Nara Solana Explorer doit être défini sur "Custom RPC URL", qui est par défaut
`http://localhost:8899` sur lequel le `solana-test-validator` s'exécute.
