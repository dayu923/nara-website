...
title: Mollusk
description:
  Guide complet pour tester les programmes Solana en utilisant Mollusk

---


[Mollusk](https://github.com/anza-xyz/mollusk/tree/main) est un harnais de test
léger pour tester les programmes Nara Solana. Il fournit une interface simple pour
tester les instructions des programmes Nara Solana dans un environnement minifié de
la Machine Virtuelle Nara Solana (SVM). Tous les comptes de test doivent être
explicitement définis, assurant des tests déterministes et reproductibles.
...
Nara Solana basique en utilisant Mollusk.

<ScrollyCoding>

## !!steps Programme Hello World

Cet exemple démontre comment tester un programme Nara Solana basique en utilisant
Mollusk. Le programme affiche simplement "Hello, world!" dans les journaux du
programme lorsqu'il est invoqué.

L'exécution de `cargo build-sbf` génère le programme compilé à
...
Pour tester un programme Nara Solana avec Mollusk :
...
3. [Traiter et valider](mention:process) - Traiter l'instruction en utilisant
   Mollusk et valider le résultat
...
fournit une interface simple pour tester les programmes Nara Solana. Tous les champs
...
| Méthode                                                                                | Description                                                                                                                                  |
| -------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| [`process_instruction`](#instruction-unique)                                           | Traite une instruction et renvoie le résultat.                                                                                               |
| [`process_and_validate_instruction`](#instruction-unique-avec-vérifications)           | Traite une instruction et effectue une série de vérifications sur le résultat, provoquant une panique si une vérification échoue.            |
| [`process_instruction_chain`](#instructions-multiples)                                 | Traite plusieurs instructions et renvoie le résultat.                                                                                        |
| [`process_and_validate_instruction_chain`](#instructions-multiples-avec-vérifications) | Traite plusieurs instructions et effectue une série de vérifications sur chaque résultat, provoquant une panique si une vérification échoue. |
...
L'exemple suivant traite une instruction de transfert de NSO sans vérifications
de validation.
...
Utilisez la méthode `process_and_validate_instruction` pour traiter une
...
L'exemple suivant traite une instruction de transfert de NSO avec des
...
L'exemple suivant traite deux instructions de transfert de NSO sans
vérifications de validation.
...
L'exemple suivant traite une chaîne de deux instructions de transfert de NSO
...
[méthodes d'assistance](https://docs.rs/mollusk-svm-result/0.5.0/mollusk_svm_result/check/struct.Check.html)
...
est un wrapper autour de `Mollusk` qui maintient l'état du compte à travers
plusieurs appels d'instruction via son `account_store`. Les méthodes de
...
(par ex. `process_instruction`), `MolluskContext` gère les comptes en interne
via son `account_store`. Cela élimine le besoin du paramètre `accounts` lors du
traitement des instructions.
...
L'exemple suivant traite deux instructions de transfert NSO distinctes avec un
...
accédant au champ `sysvars` pour changer les paramètres de rent. Vous pouvez
...
L'exemple suivant évalue l'utilisation des unités de calcul d'une instruction de
transfert de NSO.
...
Les résultats de l'évaluation sont écrits dans le `out_dir` spécifié sous forme
...
<Callout type="info">
  ...
  démonstration. Le module `mollusk-svm-programs-token` inclut également des
