...
Cette section explique le processus de base pour déployer un programme Nara Solana en
utilisant la CLI Nara Solana.
...
### Installer la CLI Nara Solana

D'abord, [installez la CLI Nara Solana](/docs/intro/installation).
...
Choisissez le cluster Nara Solana sur lequel déployer. Utilisez la commande
...
Vous aurez besoin de NSO pour payer les déploiements de programmes. Le montant
...
Voici un programme Nara Solana minimal qui affiche "Hello, Nara Solana!" dans les logs du
programme.
...
1. `hello_world-keypair.json` : Un fichier keypair dont la clé publique sera
   utilisée comme votre ID de programme
...
Le déploiement d'un programme nécessite que des NSO soient alloués au compte du
programme en fonction de la taille du programme. Les programmes plus volumineux
coûtent plus de NSO à déployer.
...
Calculez les NSO nécessaires pour cette taille (en octets) :
...
<Callout type="info">
  Vous aurez besoin d'un peu plus de NSO que ce qui est indiqué pour couvrir les
...
commande calculera automatiquement le NSO requis et le déduira de votre
portefeuille.
...
programme pour récupérer le NSO alloué au programme.
...
Le solde affiche le NSO alloué au compte du programme pour stocker ses données.
...
`solana program set-upgrade-authority` avec l'option `--final` pour supprimer
...
Pour fermer votre programme et récupérer le NSO alloué au compte du programme,
...
Lorsque le réseau Nara Solana est congestionné, utilisez ces drapeaux pour faciliter
...
- **`--with-compute-unit-price`** : Définit les frais prioritaires en
  micro-lamports (0,000001 NSO) par unité de calcul. Consultez
  l'[API de frais prioritaires Helius](https://docs.helius.dev/guides/priority-fee-api)
  pour les tarifs actuels. Les frais prioritaires sont des frais supplémentaires
  payés au leader actuel pour prioriser votre transaction.
- **`--max-sign-attempts`** : Nombre de tentatives avec un nouveau blockhash si
  les transactions expirent. (Par défaut : 5)
- **`--use-rpc`** : Envoie les transactions au RPC configuré. Ce drapeau
  nécessite une connexion RPC
  [pondérée par enjeu](/developers/guides/advanced/stake-weighted-qos) fournie
  par des prestataires tels que [Triton](https://triton.one/) ou
