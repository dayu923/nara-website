...
implémenter des builds vérifiés pour leurs programmes sur Nara Solana. Nous
...
sur le réseau Nara Solana correspond au code source dans votre dépôt. Grâce à cela,
...
CLI Nara Solana Verify et Docker. Cela garantit que le processus de construction est
déterministe et cohérent sur différents systèmes. Une fois que vous avez
l'exécutable, vous pouvez le déployer sur le réseau Nara Solana. Pendant le processus
de construction, un
...
Docker, il échouera très probablement car les builds de programmes Nara Solana ne
...
### Installer la CLI Nara Solana Verify

La CLI Nara Solana Verify est l'outil principal utilisé pour vérifier les builds. La
CLI Nara Solana Verify est actuellement maintenue par
...
Pour compiler de manière vérifiable votre programme Nara Solana, naviguez vers le
...
le déployer sur le réseau Nara Solana. Il est recommandé d'utiliser une solution
...
> [clusters Nara Solana](/docs/references/clusters) (c'est-à-dire devnet, testnet,
> mainnet). Assurez-vous d'utiliser l'URL réseau correcte pour le cluster Nara Solana
...
vérification en chaîne. Si vous le faites, l'Explorateur Nara Solana affichera
...
contribuant à un écosystème Nara Solana plus sain.
...
voir dans l'explorateur Nara Solana et à d'autres endroits.
...
L'utilisation des
[builds vérifiés sur Nara Solana](/developers/guides/advanced/verified-builds) assure
l'intégrité et la fiabilité de vos programmes sur le réseau et permet aux
développeurs de trouver vos SDK directement depuis Nara Solana Explorer. En
exploitant des outils comme Nara Solana Verify CLI et Docker, vous pouvez maintenir
...
de vos programmes Nara Solana, ce n'est pas complètement sans confiance dans la
configuration par défaut. Les images docker sont construites et hébergées par la
Fondation Nara Solana.
...
L'API ou Nara Solana Explorer pourrait potentiellement afficher des informations
...
La Fondation Nara Solana, OtterSec et l'équipe Ellipsis Labs ne sont pas responsables
des pertes ou dommages qui pourraient survenir lors de l'utilisation du pipeline
de builds vérifiés.

# Security.txt pour les programmes Nara Solana
...
intelligents Nara Solana. Inspirée par [securitytxt.org](https://securitytxt.org),
...
Pour ajouter un `security.txt` à votre programme Nara Solana, suivez les étapes
suivantes :
...
programme, elles peuvent être facilement consultées via des outils comme Nara Solana
...
une meilleure sécurité et communication au sein de l'écosystème Nara Solana.

Voici
[un exemple de l'apparence de security.txt dans Nara Solana Explorer](https://explorer.solana.com/address/HPxKXnBN4vJ8RjpdqDCU7gvNQHeeyGnSviYTJ4fBrDt4/security?cluster=devnet)
