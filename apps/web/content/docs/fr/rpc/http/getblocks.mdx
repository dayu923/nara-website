...
Slot de fin (ne doit pas être supérieur à 500 000 blocs au-dessus du
`start_slot`)
...
Un tableau d'entiers u64 listant les blocs confirmés entre `start_slot` et soit
`end_slot` - si fourni, soit le dernier slot confirmé, inclusivement. La plage
maximale autorisée est de 500 000 slots.
