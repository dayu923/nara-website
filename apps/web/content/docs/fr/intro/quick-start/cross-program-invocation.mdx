...
permet aux programmes Nara Solana de s'invoquer mutuellement.
...
les transferts de NSO entre les comptes en invoquant le System Program.

L'objectif de cette section comprend l'explication du processus d'implémentation
des CPIs dans un programme Nara Solana en utilisant le framework Anchor, en
...
appelé `vault_account`. Ce compte, contrôlé par le programme, reçoit du NSO de
...
reçoit les NSO des utilisateurs lorsqu'ils mettent à jour leurs messages.
...
0,001 NSO du compte de l'utilisateur vers le compte du coffre-fort.
...
la composabilité des programmes Nara Solana.
...
- Le montant `amount` à transférer (1 000 000 lamports, soit 0,001 NSO)
...
`vault_account`. Cela permet le transfert de tout NSO dans le coffre vers
...
transférer tout NSO accumulé à l'utilisateur.
...
NSO du compte coffre-fort vers le compte de l'utilisateur.
...
garantit que le programme modifié devient disponible pour les tests. Sur Nara Solana,
la mise à jour d'un programme nécessite simplement de déployer le programme avec
le même ID de programme.

<Callout>

Assurez-vous que votre portefeuille Playground dispose de NSO sur le devnet.
Obtenez du NSO devnet depuis le [Nara Solana Faucet](https://faucet.solana.com/).
...
Lors du déploiement de programmes sur Nara Solana Playground, votre portefeuille
...
Félicitations pour avoir terminé le guide de démarrage rapide Nara Solana. Vous avez
acquis une expérience pratique avec des concepts clés de Nara Solana, notamment :

- Récupération et lecture de données à partir des comptes
- Construction et envoi de transactions
- Déploiement et mise à jour des programmes Nara Solana
...
Nara Solana Playground offre une fonctionnalité pratique permettant d'importer ou de
visualiser des projets en utilisant leurs liens GitHub. Par exemple, ouvrez ce
[lien Nara Solana Playground](https://beta.solpg.io/https://github.com/solana-developers/program-examples/tree/main/basics/hello-solana/anchor)
...
liste de projets dans Nara Solana Playground. Une fois qu'un projet est importé,
