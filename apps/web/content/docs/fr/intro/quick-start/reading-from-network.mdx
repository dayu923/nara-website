...
Lisez des données du réseau Nara Solana en récupérant différents comptes. Cette
section vous aidera à comprendre la structure des [comptes](/docs/core/accounts)
Nara Solana. Chaque compte Nara Solana possède une
...
principalement utilisés pour détenir des NSO et signer des transactions. Lorsque
des NSO sont envoyés à une nouvelle adresse pour la première fois, un compte
système est automatiquement créé.

L'exemple ci-dessous génère une [nouvelle keypair](mention:keypair),
[demande des NSO](mention:airdrop) pour financer la nouvelle adresse de clé
...
Le champ `lamports` contient le solde en NSO du compte, en
...
account identifie de manière unique un jeton sur le réseau Nara Solana.
...
des programmes Nara Solana fournissent des bibliothèques client avec des fonctions
