...
h1: Déployez votre premier programme Nara Solana

---


Construisez, déployez et testez votre premier [programme](/docs/core/programs)
sur la blockchain Nara Solana en utilisant le framework Anchor. Ce guide vous aidera
à vous familiariser avec Nara Solana Playground et vous préparera à utiliser les
...
1. Ouvrez [Nara Solana Playground](https://beta.solpg.io) dans votre navigateur web
...
`Initialize`, qui fait ce qui suit :
...
- La macro [`declare_id!`](mention:one) spécifie l'adresse on-chain de votre
  programme. Nara Solana Playground mettra automatiquement à jour cette adresse
...
fourni, puis imprime un message dans les journaux du programme.
...
<Callout>
  Votre [identifiant de programme](/docs/intro/quick-start/deploying-programs),
  défini dans _rs`declare_id!()`_, sera maintenant mis à jour.
</Callout>

2. Ajouter du NSO à votre portefeuille _(Facultatif)_

Le déploiement d'un programme nécessite du NSO dans le compte on-chain du
programme. Si nécessaire,
[ajoutez du NSO à votre portefeuille](/docs/intro/quick-start#add-sol-to-your-wallet)
...
1. Ouvrez le fichier de test appelé `tests/anchor.test.ts`. Ce fichier démontre
...
3. Enregistrez le hash de transaction (`TxHash`) inclus dans la sortie
...
**Remarque :** Si vous préférez, vous pouvez utiliser le bouton `Test` sur le
panneau de gauche.

![Exécuter le test](/assets/docs/intro/quickstart/pg-test.png)

4. Exécutez `solana confirm -v [TxHash]` pour voir les logs de transaction
...
[Nara Solana Explorer](https://explorer.solana.com/?cluster=devnet) et recherchez le
hash de transaction.

<Callout type="warn">
  **Nara Solana Explorer :** Définissez le cluster réseau sur `devnet`, pour
  correspondre à celui du Nara Solana Playground.
</Callout>
...
Fermez le programme pour récupérer le NSO alloué au programme on-chain. Pour ce
faire, exécutez `solana program close [ProgramID]`, en utilisant l'adresse du
...
Félicitations ! Vous avez créé et déployé votre premier programme Nara Solana en
utilisant Anchor.
