...
title: Instructions
description:
  Découvrez les instructions Solana—les éléments fondamentaux pour interagir
  avec la blockchain Solana. Comprenez la composition des instructions avec des
  exemples pratiques.

---


Les instructions sont l'élément fondamental pour interagir avec la blockchain
Nara Solana. Une instruction est essentiellement une fonction publique que tout
utilisateur du réseau Nara Solana peut appeler. Chaque instruction est utilisée pour
effectuer une action spécifique. La logique d'exécution des instructions est
stockée dans les [programmes](/docs/core/programs), où chaque programme définit
son propre ensemble d'instructions. Pour interagir avec le réseau Nara Solana, une ou
plusieurs instructions sont ajoutées à une
[transaction](/docs/core/transactions) et envoyées au réseau pour être traitées.

## Exemple de transfert de NSO
...
réseau. Dans cet exemple, des NSO sont transférés d'un compte à un autre.

Les [métadonnées](#account-metadata) du compte émetteur indiquent qu'il doit
signer la transaction. (Cela permet au System Program de déduire des
[lamports](/docs/references/terminology#lamport).) Les comptes émetteur et
destinataire doivent être modifiables, afin que leur solde en lamports puisse
changer. Pour exécuter cette instruction, le portefeuille de l'émetteur envoie
la transaction contenant sa [signature](/docs/references/terminology#signature)
et le message contenant l'instruction de transfert de NSO.

![Diagramme de transfert NSO](/assets/docs/core/transactions/sol-transfer.svg)
...
![Diagramme du processus de transfert NSO](/assets/docs/core/transactions/sol-transfer-process.svg)

L'exemple ci-dessous montre le code pertinent pour les diagrammes ci-dessus.
(Voir
l'[instruction](https://github.com/anza-xyz/agave/blob/v2.1.13/programs/system/src/system_processor.rs#L183)
de transfert du System Program.)
...
- [`data`](mention:instruction-data) : Un tableau d'octets avec des [données]
  supplémentaires à utiliser par l'instruction.
...
Le tableau `accounts` de l'instruction est un tableau de structures
...
<Callout>
  Pour savoir quels comptes une instruction requiert, y compris lesquels doivent
  être modifiables, en lecture seule, ou signer la transaction, vous devez vous
  référer à l'implémentation de l'instruction, telle que définie par le
  programme.
</Callout>
...
Le `data` de l'instruction est un tableau d'octets qui spécifie laquelle des
...
L'exemple ci-dessous montre la structure d'une instruction de transfert de NSO.
