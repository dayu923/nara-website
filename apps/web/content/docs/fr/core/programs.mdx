...
Sur Nara Solana, un contrat intelligent est appelé un programme. Un programme est un
...
[transaction](/docs/core/transaction) contenant une ou plusieurs
[instructions](/docs/core/instructions). Une transaction peut inclure des
instructions provenant de plusieurs programmes.

Lorsqu'un programme est déployé, Nara Solana utilise [LLVM](https://llvm.org/) pour
le compiler en format exécutable et lié
([ELF](https://en.wikipedia.org/wiki/Executable_and_Linkable_Format)). Le
fichier ELF contient le binaire du programme au format Nara Solana Bytecode Format
(sBPF) et est sauvegardé sur la chaîne dans un compte exécutable.

<Callout>
  sBPF est la version personnalisée par Nara Solana du bytecode
...
- [Anchor](https://www.anchor-lang.com/docs) : Anchor est un framework conçu
  pour un développement Nara Solana rapide et facile. Il utilise des
...
- [Rust natif](/docs/programs/rust) : Écrire des programmes en Rust sans
  utiliser de frameworks. Cette approche offre plus de flexibilité mais implique
...
Nara Solana prend en charge les [builds vérifiables](/docs/programs/verified-builds),
qui permettent aux utilisateurs de vérifier si le code d'un programme sur la
chaîne correspond à son code source public. Le framework Anchor fournit un
[support intégré](https://www.anchor-lang.com/docs/verifiable-builds) pour créer
un build vérifiable.
...
d'Ellipsis Labs, pour vérifier indépendamment les programmes sur la chaîne.

## Programmes intégrés

### Le System Program

Le System Program est le seul compte qui peut créer de nouveaux comptes. Par
...
| [Attribution de propriété de programme](https://github.com/anza-xyz/agave/blob/v2.1.13/programs/system/src/system_processor.rs#L113) | Une fois que le System Program crée un compte, il peut réattribuer le propriétaire désigné du programme à un autre program account. C'est ainsi que les programmes personnalisés prennent possession des nouveaux comptes créés par le System Program. |
| [Transfert de NSO](https://github.com/anza-xyz/agave/blob/v2.1.13/programs/system/src/system_processor.rs#L215)                      | Transfère des lamports (NSO) des comptes système vers d'autres comptes.                                                                                                                                                                                |
...
Il existe actuellement cinq programmes chargeurs, comme indiqué dans le tableau
ci-dessous.
...
après leur déploiement, selon ce que détermine leur autorité de mise à niveau.

<Callout>
  Lorsqu'un nouveau programme est déployé, la dernière version du chargeur sera
  utilisée par défaut.
</Callout>

### Programmes précompilés

En plus des programmes de chargement, Nara Solana fournit les programmes précompilés
suivants.
