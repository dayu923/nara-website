...
description: Apprenez comment minter de nouvelles unités d'un token.

---


## Comment minter des tokens

Le minting crée de nouvelles unités d'un token en utilisant l'instruction
...
- Seule l'autorité de mint peut créer de nouveaux tokens
- Un token account de destination doit exister pour recevoir les tokens mintés
