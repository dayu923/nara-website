...
title: Sync Native
description: Apprenez comment convertir du NSO natif en NSO encapsulé.

---


## Synchroniser le NSO natif

Le NSO encapsulé (WSOL) est un compte de token qui suit le montant de lamports
dans le compte comme solde de token. Le WSOL permet l'intégration avec les
protocoles DeFi pour transférer du NSO comme un token SPL.
...
synchronise le solde d'un compte de token NSO encapsulé (WSOL) avec les NSO
(lamports) réellement stockés. Lorsque vous transférez du NSO natif vers un
compte de token WSOL, le solde du token ne se met pas à jour automatiquement.
Vous devez appeler SyncNative pour refléter le solde WSOL correct.
