...
description:
  Apprenez comment transférer des jetons de manière privée d'un compte de jetons
  à un autre.
...
## Comment transférer confidentiellement des tokens d'un token account à un autre

Pour transférer confidentiellement des tokens d'un token account à un autre,
l'expéditeur et le destinataire doivent avoir des token accounts configurés avec
l'état _rs`ConfidentialTransferAccount`_ et approuvés pour les transferts
confidentiels. Le token account de l'expéditeur doit également disposer d'un
solde confidentiel disponible à transférer.

Pour transférer des tokens confidentiellement :

1. Créer
   [trois preuves](https://github.com/solana-program/token-2022/blob/d073cb89dbcd430387b5c6fb4b7157911351e4a3/confidential-transfer/proof-generation/src/transfer.rs#L35)
   côté client :
   ...
   **Preuve d'égalité
   ...
   : Vérifie que le nouveau texte chiffré du solde disponible après le transfert
   correspond à son
   [engagement de Pedersen](https://en.wikipedia.org/wiki/Commitment_scheme)
   correspondant, garantissant que le nouveau solde disponible du compte source
   est correctement calculé comme
   `new_balance = current_balance - transfer_amount`.

   **Preuve de validité du texte chiffré
   ...
   : Vérifie que les textes chiffrés du montant du transfert sont correctement
   générés pour les trois parties (source, destination et auditeur),
   garantissant que le montant du transfert est correctement chiffré sous la clé
   publique de chaque partie.
   ...
   **Preuve d'intervalle
   ...
   : Vérifie que le nouveau solde disponible et le montant du transfert (divisés
   en bits bas/hauts) sont tous non négatifs et dans une plage spécifiée.

2. Pour chaque preuve :
   - Invoquer le programme de preuve ZK ElGamal pour vérifier les données de
     preuve.
   - Stocker les métadonnées spécifiques à la preuve dans un compte "context
     state" de preuve à utiliser dans d'autres instructions.

3. Invoquer l'instruction
   ...
   en fournissant les comptes d'état de contexte de preuve.

4. Fermer les comptes d'état de contexte de preuve pour récupérer le NSO utilisé
   pour les créer.

Le diagramme suivant montre les étapes impliquées dans le transfert de jetons du
compte de jetons d'un expéditeur vers le compte de jetons d'un destinataire.
...
Pour transférer confidentiellement des jetons d'un compte de jetons à un autre,
vous devez :

- Générer une preuve d'égalité, une preuve de validité du texte chiffré et une
  preuve d'intervalle côté client
...
- Invoquer l'instruction
  [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604)
  en fournissant les trois comptes de preuve
...
- Méthode `confidential_transfer_create_context_state_account` qui crée un
  compte de preuve
- Méthode `confidential_transfer_transfer` qui invoque l'instruction `Transfer`
- Méthode `confidential_transfer_close_context_state_account` qui ferme un
  compte de preuve

### Exemple de code

L'exemple suivant montre comment transférer confidentiellement des jetons entre
des comptes de jetons.
...
avoir installé le CLI Nara Solana pour démarrer le validateur local.
