...
description: Apprenez comment déposer des tokens dans un état confidentiel.
...
<Callout type="warn">
  ...
  Bien que les concepts restent valides, les exemples de code ne fonctionneront
  pas.
</Callout>

## Comment déposer des tokens dans un solde confidentiel en attente

Avant que les tokens puissent être transférés de manière confidentielle, le
solde public des tokens doit être converti en solde confidentiel. Cette
conversion se déroule en deux étapes :

1. **Solde confidentiel en attente** : Initialement, les tokens sont "déposés"
   depuis le solde public vers un solde confidentiel "en attente".
2. **Solde confidentiel disponible** : Le solde en attente est ensuite
   "appliqué" au solde disponible, rendant les tokens disponibles pour les
   transferts confidentiels.

Cette section explique la première étape : déposer le solde public des tokens
vers le solde confidentiel en attente.

Le diagramme suivant montre les étapes impliquées dans le dépôt de tokens depuis
le solde public vers le solde confidentiel en attente :
...
Pour convertir un solde public en solde confidentiel en attente, invoquez
l'instruction
...
Le
[montant maximum](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/mod.rs#L21)
par instruction de dépôt est limité à 2^48.

Le crate `spl_token_client` fournit une méthode `confidential_transfer_deposit`
qui construit et envoie une transaction avec l'instruction `Deposit`, comme
démontré dans l'exemple ci-dessous.

### Exemple de code

L'exemple suivant démontre comment déposer un solde public de tokens vers un
solde confidentiel en attente.
...
<Callout type="info">
  ...
  sur le validator local par défaut. Vous devez cloner le programme Token
  Extension du réseau principal pour exécuter le code d'exemple.
</Callout>
