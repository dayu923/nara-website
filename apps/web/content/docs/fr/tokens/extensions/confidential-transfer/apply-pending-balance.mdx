...
description:
  Apprenez comment appliquer un solde en attente pour rendre les fonds
  disponibles.
...
Avant que les tokens puissent être transférés confidentiellement, le solde
public des tokens doit être converti en solde confidentiel. Cette conversion se
...
Cette section explique la deuxième étape : l'application du solde en attente au
solde disponible.

Lorsque des tokens sont "déposés" depuis le solde public ou lorsque des tokens
sont transférés confidentiellement d'un token account à un autre, les tokens
sont initialement ajoutés au solde confidentiel en attente. Avant que les tokens
puissent être utilisés pour des transferts confidentiels, le solde en attente
doit être "appliqué" au solde disponible.
...
Le diagramme suivant montre les étapes impliquées dans l'application du solde en
attente au solde disponible :
...
Pour convertir un solde en attente en solde disponible, invoquez l'instruction
...
Le crate `spl_token_client` fournit une méthode
`confidential_transfer_apply_pending_balance` qui construit et envoie une
transaction avec l'instruction `ApplyPendingBalance`, comme démontré dans
l'exemple ci-dessous.

### Exemple de code

L'exemple suivant démontre comment appliquer le solde confidentiel en attente au
solde confidentiel disponible.
...
avoir installé la CLI Nara Solana pour démarrer le validateur local.
...
<Callout type="info">
  ...
  sur le validateur local par défaut. Vous devez cloner le programme Token
  Extensions du réseau principal pour exécuter le code d'exemple.
</Callout>
