...
title: Extensions de métadonnées et pointeur de métadonnées
description:
  Apprenez à activer l'extension MetadataPointerExtension et l'extension
  TokenMetadata.
...
permet à un mint account de spécifier l'adresse de son compte de métadonnées. Ce
pointeur peut référencer n'importe quel compte détenu par un programme
implémentant
l'[interface Token Metadata](https://github.com/solana-program/token-metadata/tree/main),
offrant ainsi une flexibilité quant à l'emplacement de stockage des métadonnées.

Le Token Extensions Program implémente directement
l'[interface Token Metadata](https://github.com/solana-program/token-metadata/blob/main/interface/src/instruction.rs),
permettant de stocker les métadonnées sur le mint account lui-même. Cela inclut
le nom du token, son symbole, son URI et des données descriptives
supplémentaires. Lorsque les extensions de pointeur pointent vers le mint
...
peuvent être utilisées pour stocker toutes les métadonnées dans le mint account.
