...
description: Apprenez comment activer l'InterestBearingMintExtension.
...
le solde réel des jetons stocké dans chaque token account reste inchangé. La
valeur totale (solde de jetons plus les intérêts accumulés) n'est qu'un calcul.
Aucun nouveau jeton n'est créé avec cette extension.
