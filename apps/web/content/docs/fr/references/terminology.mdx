...
développement de Nara Solana.

## compte

Un enregistrement dans le registre Nara Solana qui contient soit des données, soit un
programme exécutable.

Comme un compte dans une banque traditionnelle, un compte Nara Solana peut contenir
...
Une application frontale qui interagit avec un cluster Nara Solana.
...
Une valeur unique ([hash](#hash)) qui identifie un enregistrement (bloc). Nara Solana
...
Le programme Nara Solana qui possède et charge les
...
Un programme informatique qui accède au réseau de serveurs Nara Solana
[cluster](#cluster).
...
- La longueur du tableau (encodée comme
...
[adresses de compte](/docs/core/transactions#account-addresses) et le tableau
d'[instructions](/docs/core/transactions#instructions) dans les messages de
transaction.
...
de la blockchain. Les unités de calcul sont utilisées pour mesurer les
ressources informatiques utilisées pour traiter une transaction.

## temps de confirmation

La durée d'horloge entre le moment où un [leader](#leader) crée une
[entrée de tick](#tick) et crée un [bloc confirmé](#confirmed-block).
...
Un certain nombre d'[epochs](#epoch) après que le [stake](#stake) a été
désactivé pendant qu'il devient progressivement disponible pour le retrait.
Pendant cette période, le stake est considéré comme "en cours de désactivation".
...
Également appelées "cost model", est une estimation complète de toutes les
ressources nécessaires pour traiter une transaction, mesurée en
...
d'informations, voir [appel entre programmes](/docs/core/cpi).
...
Un service hors chaîne qui agit comme gardien de la clé privée d'un utilisateur.
Il sert généralement à valider et signer les transactions.

## entrée

Une entrée sur le [registre](#ledger) soit un [tick](#tick) soit une
[entrée de transaction](#transactions-entry).

## identifiant d'entrée

Un [hachage](#hash) résistant à la préimage du contenu final d'une entrée, qui
...
d'inclusion de la transaction dans le registre. C'est le premier compte dans la
...
Une empreinte digitale d'une séquence d'octets.

## inflation

Une augmentation de l'offre de jetons au fil du temps utilisée pour financer les
récompenses de validation et pour financer le développement continu de Nara Solana.
...
Un [jeton natif](#native-token) fractionné avec la valeur de 0,000000001
[sol](#sol).
...
[cluster](#cluster) valide. Il effectue plus de vérifications du registre qu'un
...
[formatage des messages dans les transactions](/docs/core/transactions#message-header)
ici.
...
Le code exécutable sur la blockchain Nara Solana qui interprète les
...
La clé privée d'un [keypair](#keypair).
...
d'Histoire peut être vérifiée en moins de temps qu'il n'en a fallu pour la
produire.
...
Les transactions doivent demander le minimum d'unités de calcul nécessaires à
l'exécution pour minimiser les frais.

## clé publique (pubkey)

La clé publique d'un [keypair](#keypair).
...
sont stockés de façon permanente sur la chaîne jusqu'à ce que le compte soit
fermé. Il n'est pas possible de créer un compte qui tombe en dessous du seuil
d'exemption de rent.

## root

Un [block](#block) ou [slot](#slot) qui a atteint le maximum de
[lockout](#lockout) sur un [validator](#validator). Le root est le bloc le plus
élevé qui est un ancêtre de toutes les branches actives sur un validator. Tous
les blocs ancêtres d'un root sont également transitivement un root. Les blocs
qui ne sont ni un ancêtre ni un descendant du root sont exclus de la
considération pour le consensus et peuvent être écartés.

## runtime

Le composant d'un [validator](#validator) responsable de l'exécution du
[programme](#program).

## Sealevel

L'environnement d'exécution parallèle de Nara Solana pour les
[programmes onchain](#onchain-program).

## shred

Une fraction d'un [block](#block) ; la plus petite unité envoyée entre
[validators](#validator).

## signature

Une signature ed25519 de 64 octets composée de R (32 octets) et S (32 octets).
Avec l'exigence que R soit un point d'Edwards empaqueté qui n'est pas d'ordre
petit et que S soit un scalaire dans la plage de `0 <= S < L`. Cette exigence
garantit l'absence de malléabilité de signature. Chaque transaction doit avoir
au moins une signature pour le [compte de frais](#fee-account). Ainsi, la
première signature dans une transaction peut être traitée comme
...
l'échantillon est petite, ainsi que pour les validators avec un faible nombre de
slots de leader, mais peut également être utile pour identifier les mauvaises
configurations de nœuds.

## skipped slot

Un [slot](#slot) passé qui n'a pas produit de [block](#block), parce que le
...
slot ignoré n'apparaîtra pas comme un ancêtre pour les blocs aux slots suivants,
n'incrémentera pas la [hauteur de bloc](#block-height), ni n'expirera le plus
ancien `recent_blockhash`.

La détermination qu'un slot a été sauté ne peut être faite que lorsqu'il devient
plus ancien que le dernier slot [enraciné](#root) (donc non-sauté).
...
égal selon la [PoH](#proof-of-history-poh).
...
Le [jeton natif](#native-token) d'un [cluster](#cluster) Nara Solana.

## Bibliothèque de programmes Nara Solana (SPL)

Une [bibliothèque de programmes](https://spl.solana.com/) sur Nara Solana comme
...
SWQoS permet un
[traitement préférentiel pour les transactions provenant de validateurs stakés](/developers/guides/advanced/stake-weighted-qos).
...
Une [entrée](#entry) du registre qui estime la durée de l'horloge murale.
...
mêmes fonctionnalités que le [Token Program](#token-program), mais est livré
avec des extensions telles que les transferts confidentiels, la logique de
transfert personnalisée, les métadonnées étendues, et bien plus encore.

## token mint

Un compte qui peut produire (ou 'minter') des tokens. Différents tokens sont
distingués par leurs adresses uniques de token mint.
...
pour transférer, geler et minter des tokens.
...
Un participant complet dans un [cluster](#cluster) du réseau Nara Solana qui produit
...
de son exécution, laquelle peut ensuite être vérifiée en moins de temps qu'il
n'en a fallu pour la produire.
...
pendant qu'il devient progressivement effectif. Durant cette période, l'enjeu
