...
uniemożliwia wszystkie transfery tokenów lub spalanie tokenów z określonego
konta tokenów. Po zamrożeniu konto nie może wysyłać tokenów, odbierać tokenów
ani być zamknięte, dopóki nie zostanie odmrożone. Tylko autorytet zamrażania
tokenu w mint account może zamrażać konta. Jeśli autorytet zamrażania zostanie
cofnięty (ustawiony na null) na mint account, tokeny nigdy nie będą mogły być
zamrożone.
