...
title: Ustawianie autorytetu
description: Dowiedz się, jak ustawić autorytet dla mintów lub kont tokenów.

---


## Zrozumienie autorytetów tokenów

### Autorytety konta mint

- **Autorytet mint**: Kontroluje tworzenie nowych tokenów. Może mintować tokeny
  na dowolne konto tokenów. Często jest odwoływany po utworzeniu początkowej
  podaży, aby stworzyć token o stałej podaży.
- **Autorytet zamrażania**: Kontroluje możliwość zamrażania i rozmrażania kont
  tokenów. Może uniemożliwić dowolnemu kontu tokenów transfer tokenów. Często
  jest odwoływany, aby zagwarantować użytkownikom, że ich tokeny nie mogą zostać
  zamrożone.

### Autorytety konta tokenów

- **Właściciel konta**: Ma pełną kontrolę nad kontem tokenów. Może transferować
  tokeny, spalać tokeny, zatwierdzać delegatów i zamykać konto, gdy saldo wynosi
  zero.
- **Autorytet zamknięcia**: Może zamknąć konto tokenów, gdy jego saldo wynosi
  zero. Domyślnie jest to właściciel konta, ale może zostać przekazany innemu
  kontu.

## Jak ustawić autorytet
...
zmiienia lub odwołuje autorytety na mintach i kontach tokenów. Tylko obecny
autorytet może przenieść swoje uprawnienia na nowy adres lub odwołać je trwale,
ustawiając autorytet na null. Po odwołaniu autorytet nie może zostać
przywrócony.

<Callout type="info">
  [Program tokenów
  ](https://github.com/solana-program/token/blob/a7c488ca39ed4cd71a87950ed854929816e9099f/program/src/instruction.rs#L153)
  oraz [Program rozszerzeń tokenów
  ](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/instruction.rs#L195)
