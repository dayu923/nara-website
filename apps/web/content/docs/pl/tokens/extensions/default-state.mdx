...
title: Domyślny stan
description: Dowiedz się, jak włączyć DefaultAccountStateExtension.

---


## Jak włączyć DefaultAccountStateExtension
...
ustawia domyślny stan dla wszystkich nowych kont tokenów utworzonych dla danego
mintu. Po włączeniu, to rozszerzenie automatycznie inicjalizuje każde nowe konto
tokena z określonym stanem, na przykład zamrożonym. Pozwala to twórcom tokenów
wymuszać określone zachowania, takie jak wymaganie, aby wszystkie nowe konta
były domyślnie zamrożone, dopóki nie zostaną wyraźnie odmrożone.
