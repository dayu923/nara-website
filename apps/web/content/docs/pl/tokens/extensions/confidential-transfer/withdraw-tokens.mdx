...
title: Wypłata tokenów
description: Dowiedz się, jak wypłacić tokeny ze stanu poufnego.
...
1. Utwórz
   [dwa dowody](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/confidential-transfer/proof-generation/src/withdraw.rs#L22)
   po stronie klienta:
   ...
   **Dowód równości
   ...
   Weryfikuje, że szyfrogram pozostałego dostępnego salda po wypłacie odpowiada
   jego
   [zobowiązaniu Pedersena](https://en.wikipedia.org/wiki/Commitment_scheme),
   zapewniając, że nowe dostępne saldo konta jest poprawnie obliczone jako
   ...
   **Dowód zakresu
   ...
   Weryfikuje, że pozostałe dostępne saldo po wypłacie jest nieujemne i mieści
...
2. Dla każdego dowodu:
   ...
   - Przechowaj metadane specyficzne dla dowodu w koncie "stanu kontekstu
     dowodu" do użycia w innych instrukcjach.
...
3. Wywołaj instrukcję
   ...
   podając dwa konta dowodowe.

4. Zamknij konta dowodowe, aby odzyskać NSO użyte do ich utworzenia.

Poniższy diagram pokazuje kroki związane z wypłatą tokenów z poufnego dostępnego
salda na saldo publiczne:
...
- Wywołać program dowodowy ZK ElGamal, aby zweryfikować dowody i zainicjalizować
...
Crate `spl_token_client` zapewnia następujące metody:

- Metoda `confidential_transfer_create_context_state_account`, która tworzy
  konto dowodowe.
- Metoda `confidential_transfer_withdraw`, która wywołuje instrukcję `Withdraw`.
- Metoda `confidential_transfer_close_context_state_account`, która zamyka konto
  dowodowe.

### Przykładowy kod

Poniższy przykład demonstruje, jak wypłacić tokeny z poufnego dostępnego salda
na saldo publiczne.

Aby uruchomić przykład, rozpocznij lokalne działanie validatora z Token
Extensions Program sklonowanym z mainnet za pomocą następującego polecenia.
Musisz mieć zainstalowane Nara Solana CLI, aby uruchomić lokalny validator.
...
<Callout type="info">
  W momencie pisania tego tekstu, Confidential Transfers nie są włączone w
  domyślnym lokalnym validatorze. Musisz sklonować Token Extensions Program z
  mainnet, aby uruchomić kod przykładu.
</Callout>
