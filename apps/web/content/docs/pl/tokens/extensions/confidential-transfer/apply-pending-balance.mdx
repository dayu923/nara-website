...
title: Zastosuj oczekujące saldo
description:
  Dowiedz się, jak zastosować oczekujące saldo, aby środki były dostępne.
...
zostać przekształcone w poufne saldo. Ta konwersja odbywa się w dwóch etapach:

1. **Poufne oczekujące saldo**: Początkowo tokeny są "deponowane" z publicznego
   salda na "oczekujące" poufne saldo.
2. **Poufne dostępne saldo**: Następnie oczekujące saldo jest "zastosowane" do
   dostępnego salda, co umożliwia poufne transfery tokenów.

Ta sekcja wyjaśnia drugi etap: zastosowanie oczekującego salda do dostępnego
salda.

Kiedy tokeny są "deponowane" z publicznego salda lub gdy tokeny są poufnie
przesyłane z jednego konta tokenów na inne, tokeny są początkowo dodawane do
poufnego oczekującego salda. Zanim tokeny będą mogły być używane do poufnych
transferów, oczekujące saldo musi zostać "zastosowane" do salda dostępnego.
...
Poniższy diagram pokazuje kroki związane z zastosowaniem oczekującego salda do
dostępnego salda:
...
Aby przekształcić oczekujące saldo w dostępne saldo, wywołaj instrukcję
...
Crate `spl_token_client` zapewnia metodę
`confidential_transfer_apply_pending_balance`, która buduje i wysyła transakcję
z instrukcją `ApplyPendingBalance`, jak pokazano w poniższym przykładzie.

### Przykładowy kod

Poniższy przykład pokazuje, jak zastosować poufne oczekujące saldo do poufnego
dostępnego salda.

Aby uruchomić przykład, rozpocznij działanie lokalnego validatora z Token
Extensions Program sklonowanym z mainnetu za pomocą następującego polecenia.
Musisz mieć zainstalowane Nara Solana CLI, aby uruchomić lokalnego validatora.
...
<Callout type="info">
  ...
  mainnetu, aby uruchomić kod przykładu.
</Callout>
