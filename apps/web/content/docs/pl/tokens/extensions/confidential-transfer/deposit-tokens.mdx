...
title: Wpłacanie tokenów
description: Dowiedz się, jak wpłacać tokeny do stanu poufnego.
...
<Callout type="warn">
  ...
  przechodzi audyt bezpieczeństwa. Oznacza to, że rozszerzenie poufnych
  transferów jest obecnie niedostępne. Chociaż koncepcje są nadal aktualne,
  przykłady kodu nie będą działać.
</Callout>

## Jak wpłacić tokeny do poufnego salda oczekującego

Zanim tokeny będą mogły być przesyłane poufnie, publiczne saldo tokenów musi
zostać przekształcone w saldo poufne. Proces ten odbywa się w dwóch etapach:

1. **Poufne saldo oczekujące**: Na początku tokeny są "wpłacane" z publicznego
   salda na "oczekujące" poufne saldo.
2. **Poufne saldo dostępne**: Następnie saldo oczekujące jest "zastosowane" do
   salda dostępnego, co umożliwia poufne transfery tokenów.

Ta sekcja wyjaśnia pierwszy etap: wpłacanie publicznego salda tokenów na poufne
saldo oczekujące.

Poniższy diagram przedstawia kroki związane z wpłacaniem tokenów z publicznego
salda na poufne saldo oczekujące:
...
Aby przekształcić publiczne saldo w poufne saldo oczekujące, należy wywołać
instrukcję
...
[Maksymalna kwota](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/mod.rs#L21)
na jedną instrukcję wpłaty jest ograniczona do 2^48.

Crate `spl_token_client` udostępnia metodę `confidential_transfer_deposit`,
która buduje i wysyła transakcję z instrukcją `Deposit`, jak pokazano w
poniższym przykładzie.

### Przykładowy kod

Poniższy przykład demonstruje, jak wpłacić publiczne saldo tokenów na poufne
saldo oczekujące.

Aby uruchomić przykład, uruchom lokalny walidator z Token Extension Program
sklonowanym z mainnetu za pomocą następującego polecenia. Aby uruchomić lokalny
walidator, musisz mieć zainstalowane Nara Solana CLI.
...
<Callout type="info">
  ...
  lokalnym validatorze. Musisz sklonować główną sieć Token Extensions Program,
