...
description:
  Dowiedz się, jak prywatnie przesyłać tokeny z jednego konta tokenów na inne.
...
<Callout type="warn">
  ...
  transferów jest obecnie niedostępne. Chociaż koncepcje pozostają ważne,
  przykłady kodu nie będą działać.
</Callout>

## Jak poufnie przesyłać tokeny z jednego konta tokenów na inne

Aby poufnie przesłać tokeny z jednego konta tokenów na inne, zarówno nadawca,
jak i odbiorca muszą mieć konta tokenów skonfigurowane ze stanem
_rs`ConfidentialTransferAccount`_ i zatwierdzone do poufnych transferów. Konto
tokenów nadawcy musi również mieć dostępne poufne saldo do przesłania.

Aby poufnie przesłać tokeny:

1. Utwórz
   [trzy dowody](https://github.com/solana-program/token-2022/blob/d073cb89dbcd430387b5c6fb4b7157911351e4a3/confidential-transfer/proof-generation/src/transfer.rs#L35)
   po stronie klienta:
   ...
   **Dowód równości
   ...
   Weryfikuje, że nowe zaszyfrowane dostępne saldo po transferze odpowiada jego
   odpowiedniemu
   [zobowiązaniu Pedersena](https://en.wikipedia.org/wiki/Commitment_scheme),
   zapewniając, że nowe dostępne saldo konta źródłowego jest poprawnie obliczone
   jako `new_balance = current_balance - transfer_amount`.

   **Dowód ważności szyfrogramu
   ...
   Weryfikuje, że szyfrogramy kwoty transferu zostały poprawnie wygenerowane dla
   wszystkich trzech stron (źródła, celu i audytora), zapewniając, że kwota
   transferu jest poprawnie zaszyfrowana pod każdym kluczem publicznym strony.
   ...
   **Dowód zakresu
   ...
   Weryfikuje, że nowe dostępne saldo i kwota transferu (podzielone na bity
   niskie/wysokie) są wszystkie nieujemne i mieszczą się w określonym zakresie.

2. Dla każdego dowodu:
   - Wywołaj program dowodowy ZK ElGamal, aby zweryfikować dane dowodowe.
   - Przechowaj metadane specyficzne dla dowodu w koncie "stanu kontekstu"
     dowodu, aby użyć ich w innych instrukcjach.

3. Wywołaj instrukcję
   ...
   dostarczając konta stanu kontekstu dowodów.

4. Zamknij konta stanu kontekstu dowodu, aby odzyskać NSO użyte do ich
   utworzenia.

Poniższy diagram przedstawia kroki związane z transferem tokenów z konta tokenów
nadawcy na konto tokenów odbiorcy.
...
Aby poufnie przenieść tokeny z jednego konta tokenów na drugie, musisz:

- Wygenerować dowód równości, dowód ważności szyfrogramu i dowód zakresu po
  stronie klienta
- Wywołać program dowodowy Zk ElGamal, aby zweryfikować dowody i zainicjować
  konta "stanu kontekstu"
- Wywołać instrukcję
  [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604)
  dostarczając trzy konta dowodowe.
- Zamknąć trzy konta dowodowe, aby odzyskać rent.

Pakiet `spl_token_client` zapewnia następujące metody:

- Metoda `confidential_transfer_create_context_state_account`, która tworzy
  konto dowodowe.
- Metoda `confidential_transfer_transfer`, która wywołuje instrukcję `Transfer`.
- Metoda `confidential_transfer_close_context_state_account`, która zamyka konto
  dowodowe.

### Przykładowy kod

Poniższy przykład demonstruje, jak poufnie przenosić tokeny między kontami
tokenów.

Aby uruchomić przykład, uruchom lokalny walidator z Token Extension Program
sklonowanym z mainnet za pomocą następującego polecenia. Musisz mieć
zainstalowane Nara Solana CLI, aby uruchomić lokalny walidator.
...
<Callout type="info">
  W momencie pisania tego tekstu poufne transfery nie są włączone na domyślnym
  lokalnym walidatorze. Musisz sklonować Token Extension Program z mainnet, aby
