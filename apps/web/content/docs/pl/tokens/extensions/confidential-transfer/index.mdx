...
title: Poufne transfery
description:
  Dowiedz się więcej o rozszerzeniu Poufny Transfer i jak go używać, aby dodać
  opcjonalne funkcje do tokenów i kont.
...
Poufne transfery umożliwiają przesyłanie tokenów między kontami tokenów bez
ujawniania kwoty transferu. Jest to przydatne w przypadku transakcji
zachowujących prywatność. Tylko kwoty transferów i salda tokenów są poufne.
Adresy kont tokenów pozostają publiczne.

- [Przegląd protokołu](https://www.solana-program.com/docs/confidential-balances/overview) -
  Szczegóły dotyczące podstawowego protokołu kryptograficznego
...
- [Książka kucharska Poufnych Salda](https://github.com/solana-developers/Confidential-Balances-Sample) -
  Fragmenty kodu pokazujące, jak używać rozszerzenia Poufny Transfer

### Jak to działa?

Rozszerzenie Poufny Transfer dodaje
...
do programu Token Extension, które pozwalają na przesyłanie tokenów między
...
Podstawowy przepływ poufnych transferów tokenów wygląda następująco:

1. Utwórz konto mint z rozszerzeniem poufnego transferu.
2. Utwórz konta tokenów z rozszerzeniem poufnego transferu dla nadawcy i
   odbiorcy.
3. Wydaj tokeny na konto nadawcy.
4. **Wpłać** publiczne saldo nadawcy na **poufne saldo oczekujące**.
5. **Zastosuj** saldo oczekujące nadawcy do **poufnego dostępnego salda**.
6. Poufnie **przenieś** tokeny z konta tokenów nadawcy na konto tokenów
   odbiorcy.
7. **Zastosuj** saldo oczekujące odbiorcy do **poufnego dostępnego salda**.
8. **Wypłać** poufne dostępne saldo odbiorcy na **publiczne saldo**.

Więcej szczegółów na temat kroków w przepływie poufnego transferu znajdziesz na
odpowiednich stronach:

<Cards>
  <Card
    title="Utwórz konto mint"
    href="/docs/tokens/extensions/confidential-transfer/create-mint"
  >
    Jak utworzyć konto mint z rozszerzeniem Poufny Transfer
  </Card>
  <Card
    title="Utwórz konto tokenów"
    href="/docs/tokens/extensions/confidential-transfer/create-token-account"
  >
    Jak skonfigurować konto tokenów z rozszerzeniem Poufny Transfer
  </Card>
  <Card
    title="Wpłać tokeny"
  ...
  >
    Jak zastosować saldo oczekujące do dostępnego poufnego salda
  </Card>
  <Card
    title="Wypłać tokeny"
...
Poniższy przykład w języku Rust pokazuje kroki dla poufnych transferów tokenów
odpowiadające wcześniejszemu diagramowi. Kroki zostały podzielone na funkcje
pomocnicze dla przejrzystości.

Aby uruchomić przykład, uruchom lokalny validator z Token Extension Program
sklonowanym z mainnet za pomocą następującego polecenia. Musisz mieć
zainstalowane Nara Solana CLI, aby użyć lokalnego validatora.
...
<Callout type="info">
  W momencie pisania tego tekstu, poufne transfery nie są włączone w domyślnym
  lokalnym validatorze. Musisz sklonować Token Extension Program z mainnet, aby
  uruchomić przykładowy kod.
</Callout>
...
Pełna lista instrukcji rozszerzenia poufnych transferów
[instrukcje](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/instruction.rs#L29)
wygląda następująco:

| Instrukcja                          | Opis                                                                                                                                                        |
| ----------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | Konfiguruje konto mint dla poufnych transferów. Ta instrukcja musi być zawarta w tej samej transakcji co instrukcja _rs`TokenInstruction::InitializeMint`_. |
| _rs`UpdateMint`_                    | Aktualizuje ustawienia poufnych transferów dla mint.                                                                                                        |
| _rs`ConfigureAccount`_              | Konfiguruje konto tokena dla poufnych transferów.                                                                                                           |
| _rs`ApproveAccount`_                | Zatwierdza konto tokena dla poufnych transferów, jeśli mint wymaga zatwierdzenia dla nowych kont tokenów.                                                   |
| _rs`EmptyAccount`_                  | Opróżnia oczekujące i dostępne poufne salda, aby umożliwić zamknięcie konta tokena.                                                                         |
| _rs`Deposit`_                       | Konwertuje publiczne saldo tokena na oczekujące poufne saldo.                                                                                               |
| _rs`Withdraw`_                      | Konwertuje dostępne poufne saldo z powrotem na saldo publiczne.                                                                                             |
| _rs`Transfer`_                      | Przenosi tokeny między kontami tokenów w sposób poufny.                                                                                                     |
| _rs`ApplyPendingBalance`_           | Konwertuje oczekujące saldo na dostępne saldo po depozytach lub transferach.                                                                                |
| _rs`EnableConfidentialCredits`_     | Pozwala kontu tokena na odbieranie poufnych transferów tokenów.                                                                                             |
| _rs`DisableConfidentialCredits`_    | Blokuje przychodzące poufne transfery, jednocześnie umożliwiając transfery publiczne.                                                                       |
| _rs`EnableNonConfidentialCredits`_  | Pozwala kontu tokena na odbieranie publicznych transferów tokenów.                                                                                          |
| _rs`DisableNonConfidentialCredits`_ | Blokuje regularne transfery, aby konto mogło odbierać tylko poufne transfery.                                                                               |
| _rs`TransferWithFee`_               | Przenosi tokeny między kontami tokenów w sposób poufny z opłatą.                                                                                            |
| _rs`ConfigureAccountWithRegistry`_  | Alternatywny sposób konfiguracji kont tokenów dla poufnych transferów przy użyciu konta _rs`ElGamalRegistry`_ zamiast dowodu _rs`VerifyPubkeyValidity`_.    |

| Instrukcja                          | Opis                                                                                                                                                     |
| ----------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | Ustawia konto mint dla poufnych transferów. Ta instrukcja musi być zawarta w tej samej transakcji co instrukcja _rs`TokenInstruction::InitializeMint`_.  |
| _rs`UpdateMint`_                    | Aktualizuje ustawienia poufnych transferów dla mint.                                                                                                     |
| _rs`ConfigureAccount`_              | Ustawia konto tokena dla poufnych transferów.                                                                                                            |
| _rs`ApproveAccount`_                | Zatwierdza konto tokena dla poufnych transferów, jeśli mint wymaga zatwierdzenia dla nowych kont tokenów.                                                |
| _rs`EmptyAccount`_                  | Opróżnia oczekujące i dostępne poufne salda, aby umożliwić zamknięcie konta tokena.                                                                      |
| _rs`Deposit`_                       | Konwertuje publiczne saldo tokena na oczekujące poufne saldo.                                                                                            |
| _rs`Withdraw`_                      | Konwertuje dostępne poufne saldo z powrotem na saldo publiczne.                                                                                          |
| _rs`Transfer`_                      | Przenosi tokeny między kontami tokenów w sposób poufny.                                                                                                  |
| _rs`ApplyPendingBalance`_           | Konwertuje oczekujące saldo na dostępne saldo po wpłatach lub transferach.                                                                               |
| _rs`EnableConfidentialCredits`_     | Umożliwia kontu tokena odbieranie poufnych transferów tokenów.                                                                                           |
| _rs`DisableConfidentialCredits`_    | Blokuje przychodzące poufne transfery, jednocześnie umożliwiając transfery publiczne.                                                                    |
| _rs`EnableNonConfidentialCredits`_  | Umożliwia kontu tokena odbieranie publicznych transferów tokenów.                                                                                        |
| _rs`DisableNonConfidentialCredits`_ | Blokuje regularne transfery, aby konto mogło odbierać tylko poufne transfery.                                                                            |
| _rs`TransferWithFee`_               | Przenosi tokeny między kontami tokenów w sposób poufny z opłatą.                                                                                         |
| _rs`ConfigureAccountWithRegistry`_  | Alternatywny sposób konfiguracji kont tokenów dla poufnych transferów przy użyciu konta _rs`ElGamalRegistry`_ zamiast dowodu _rs`VerifyPubkeyValidity`_. |
