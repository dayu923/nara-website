...
Programy Nara Solana są głównie tworzone w języku programowania Rust. Ta strona
koncentruje się na pisaniu programów Nara Solana w Rust bez użycia frameworka Anchor,
...
programami Nara Solana. Jednak podejście to wymaga więcej ręcznej konfiguracji i
...
- Rust: Język programowania do tworzenia programów Nara Solana.
- Nara Solana CLI: Narzędzie wiersza poleceń do rozwoju Nara Solana.

## Pierwsze kroki

Poniższy przykład obejmuje podstawowe kroki tworzenia pierwszego programu Nara Solana
...
do zbudowania programu Nara Solana.
...
program Nara Solana, który wypisuje "Hello, world!" do dziennika programu, gdy
program zostanie wywołany.

Makro `msg!` jest używane w programach Nara Solana do wypisywania wiadomości do
dziennika programu.
...
1. Plik `.so` (np. `hello_world.so`): Jest to skompilowany program Nara Solana, który
...
Najpierw skonfiguruj Nara Solana CLI, aby używać lokalnego klastra Nara Solana.
...
<Callout type="info">
  Pamiętaj, że klaster na Nara Solana Explorer musi również być localhost. Opcja
  "Custom RPC URL" na Nara Solana Explorer domyślnie wskazuje na
  `http://localhost:8899`.
</Callout>
...
Programy Nara Solana można aktualizować, ponownie wdrażając je pod tym samym ID
programu. Zaktualizuj program w `src/lib.rs`, aby wyświetlał "Hello, Nara Solana!"
zamiast "Hello, world!".
...
Powinieneś zobaczyć "Hello, Nara Solana!" w logu programu.
...
Uruchom ponownie kod klienta i sprawdź podpis transakcji w Nara Solana Explorer, aby
zobaczyć "Hello, Nara Solana!" w logu programu.
...
Możesz zamknąć swój program Nara Solana, aby odzyskać NSO przypisane do konta.
