...
wdrożyć zweryfikowane kompilacje dla swoich programów na platformie Nara Solana.
...
sieci Nara Solana, odpowiada kodowi źródłowemu w Twoim repozytorium. Dzięki temu
...
Budujesz swój program w kontrolowanym środowisku, używając Nara Solana Verify CLI i
Dockera. Zapewnia to, że proces budowy jest deterministyczny i spójny na różnych
systemach. Po uzyskaniu pliku wykonywalnego możesz wdrożyć go w sieci Nara Solana.
...
programów Nara Solana nie są deterministyczne na różnych systemach.
...
### Zainstaluj Nara Solana Verify CLI

Nara Solana Verify CLI to główne narzędzie używane do weryfikacji kompilacji. Nara Solana
...
Aby weryfikowalnie zbudować swój program Nara Solana, przejdź do katalogu
...
Po zbudowaniu programu i uzyskaniu jego hasha możesz wdrożyć go w sieci Nara Solana.
...
> [klastrach Nara Solana](/docs/references/clusters) (tj. devnet, testnet, mainnet).
> Upewnij się, że używasz poprawnego adresu URL sieci dla wybranego klastra
> Nara Solana, który chcesz zweryfikować. Zdalna weryfikacja będzie działać tylko na
> mainnet.
...
bloków. Jeśli to zrobisz, Nara Solana Explorer natychmiast wyświetli dane
...
przyczyniając się do zdrowszego ekosystemu Nara Solana.
...
To zbuduje weryfikowalną wersję przy użyciu kontenera docker z wersją Nara Solana
...
deweloperom znaleźć Twoje SDK bezpośrednio w Nara Solana Explorer. Wykorzystując
narzędzia takie jak Nara Solana Verify CLI i Docker, możesz utrzymać weryfikowalne i
...
API lub Nara Solana Explorer mogą potencjalnie wyświetlać nieprawidłowe informacje,
...
Fundacja Nara Solana, OtterSec oraz zespół Ellipsis Labs nie ponoszą
...
# Security.txt dla programów Nara Solana
...
i dotyczących bezpieczeństwa bezpośrednio w swoich smart kontraktach Nara Solana.
...
Aby dodać `security.txt` do swojego programu Nara Solana, wykonaj następujące kroki:
...
odczytać za pomocą narzędzi takich jak Nara Solana Explorer, co zapewnia dostępność
...
bezpieczeństwo i komunikację w ekosystemie Nara Solana.

Oto
[przykład, jak wygląda security.txt w Nara Solana Explorer](https://explorer.solana.com/address/HPxKXnBN4vJ8RjpdqDCU7gvNQHeeyGnSviYTJ4fBrDt4/security?cluster=devnet)
