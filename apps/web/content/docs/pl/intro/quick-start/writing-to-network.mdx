...
W poprzedniej sekcji nauczyłeś się, jak odczytywać dane z sieci Nara Solana. Teraz
nauczysz się, jak zapisywać dane do niej. Zapisywanie do sieci Nara Solana polega na
...
[transakcję](/docs/core/transactions), środowisko wykonawcze Nara Solana wykonuje
każdą instrukcję sekwencyjnie i atomowo. Przykłady w tej sekcji pokazują, jak
budować i wysyłać transakcje w celu wywołania programów Nara Solana, obejmują one:

1. Przelewanie NSO między kontami
2. Tworzenie nowego tokena

## Przelew NSO

Poniższy przykład pokazuje, jak przelać NSO między dwoma kontami. Każde konto ma
program właściciela, który jest jedynym programem mogącym obciążyć saldo NSO
konta.

Wszystkie konta portfela są własnością System Program. Aby przelać NSO, musisz
wywołać instrukcję
...
Utwórz połączenie z klastrem Nara Solana.
...
Poproś o airdrop NSO, aby zasilić `sender`.
...
Dodaj NSO do konta nadawcy. Na sieciach innych niż mainnet możesz użyć metody
`requestAirdrop`, aby uzyskać NSO do testowania.
...
Metoda `SystemProgram.transfer()` tworzy instrukcję, która przesyła NSO z konta
...
wyszukania transakcji w Nara Solana Explorer.
...
[Token Extensions Program](mention:programId)
...
To podejście jest powszechne przy budowaniu złożonych transakcji Nara Solana,
...
Podpis transakcji zwrócony może być użyty do sprawdzenia transakcji w Nara Solana
Explorer.
