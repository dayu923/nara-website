...
Odczytuj dane z sieci Nara Solana, pobierając różne konta. Ta sekcja pomoże Ci
zrozumieć strukturę kont Nara Solana [kont](/docs/core/accounts). Każde konto Nara Solana
...
używane do przechowywania NSO i podpisywania transakcji. Gdy NSO jest wysyłany
...
[żąda NSO](mention:airdrop) w celu zasilenia nowego adresu publicznego oraz
...
Pole `lamports` zawiera saldo NSO konta, w
...
jednoznacznie identyfikuje token w sieci Nara Solana.
...
przez program, który jest właścicielem konta. Większość programów Nara Solana
