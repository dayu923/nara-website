...
h1: Αναπτύξτε το πρώτο σας πρόγραμμα Nara Solana

---


Δημιουργήστε, αναπτύξτε και δοκιμάστε το πρώτο σας
[πρόγραμμα](/docs/core/programs) στο Blockchain της Nara Solana χρησιμοποιώντας το
πλαίσιο Anchor. Αυτός ο οδηγός θα σας βοηθήσει να εξοικειωθείτε περισσότερο με
το Nara Solana Playground και να προετοιμαστείτε για τη χρήση Program Derived Address
...
1. Ανοίξτε το [Nara Solana Playground](https://beta.solpg.io) στο πρόγραμμα
   περιήγησής σας
...
Το αρχείο πηγαίου κώδικα `src/lib.rs` θα ανοίξει. Περιέχει μια δομή που
...
Προσπελαύνει το πεδίο `data` του λογαριασμού `new_account` και το ορίζει στην
τιμή του `data: u64`
...
- Η μακροεντολή [`declare_id!`](mention:one) καθορίζει τη διεύθυνση on-chain του
  προγράμματός σας. Το Nara Solana Playground θα ενημερώσει αυτόματα αυτή τη
  διεύθυνση όταν δημιουργήσετε το πρόγραμμα.
...
- Το χαρακτηριστικό [`#[derive(Accounts)]`](mention:one) ορίζει μια δομή που
  καθορίζει τους λογαριασμούς που απαιτούνται για μια συγκεκριμένη εντολή. Κάθε
  πεδίο αντιπροσωπεύει έναν ξεχωριστό λογαριασμό.

  Το Anchor χειρίζεται αυτόματα τους συνήθεις ελέγχους ασφαλείας που σχετίζονται
  με την επικύρωση λογαριασμών μέσω τύπων πεδίων (π.χ. `Signer<'info>`) και
...
- Το χαρακτηριστικό [`#[account]`](mention:one) ορίζει μια δομή που
  αντιπροσωπεύει τον τύπο δεδομένων για λογαριασμούς που δημιουργεί και κατέχει
  αυτό το πρόγραμμα.
...
<Callout>
  Το [program ID](/docs/intro/quick-start/deploying-programs) σας, που ορίζεται
  στο _rs`declare_id!()`_, θα ενημερωθεί τώρα.
</Callout>

2. Προσθήκη NSO στο πορτοφόλι σας _(Προαιρετικό)_

Η ανάπτυξη ενός προγράμματος απαιτεί NSO στον on-chain λογαριασμό του
προγράμματος. Εάν είναι απαραίτητο,
[προσθέστε NSO στο πορτοφόλι σας](/docs/intro/quick-start#add-sol-to-your-wallet)
...
1. Ανοίξτε το αρχείο δοκιμής με όνομα `tests/anchor.test.ts`. Αυτό το αρχείο
   δείχνει πώς να καλέσετε τη δομή [`initialize`](mention:one) του προγράμματος
   από τον πελάτη.
...
3. Αποθηκεύστε το hash της συναλλαγής (`TxHash`) που περιλαμβάνεται στην έξοδο
...
4. Εκτελέστε `solana confirm -v [TxHash]` για να δείτε τα αρχεία καταγραφής της
   συναλλαγής
...
[Nara Solana Explorer](https://explorer.solana.com/?cluster=devnet) και αναζητήστε το
hash της συναλλαγής.

<Callout type="warn">
  **Nara Solana Explorer:** Ορίστε το δίκτυο σε `devnet`, ώστε να ταιριάζει με αυτό
  του Nara Solana Playground.
</Callout>
...
Κλείστε το πρόγραμμα για να ανακτήσετε το NSO που διατέθηκε στο πρόγραμμα
...
Συγχαρητήρια! Δημιουργήσατε και αναπτύξατε το πρώτο σας πρόγραμμα Nara Solana
