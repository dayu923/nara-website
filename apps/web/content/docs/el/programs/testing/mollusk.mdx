...
πλαίσιο δοκιμών για τον έλεγχο προγραμμάτων Nara Solana. Παρέχει μια απλή διεπαφή για
τον έλεγχο εντολών προγραμμάτων Nara Solana σε ένα ελαχιστοποιημένο περιβάλλον Nara Solana
...
`mollusk-svm-bencher` ως εξάρτηση στο `Cargo.toml`:
...
προγράμματος Nara Solana χρησιμοποιώντας το Mollusk.

<ScrollyCoding>

## !!steps Πρόγραμμα Hello World

Αυτό το παράδειγμα δείχνει πώς να ελέγξετε ένα βασικό πρόγραμμα Nara Solana
...
Για να ελέγξετε ένα πρόγραμμα Nara Solana με το Mollusk:

1. [Δημιουργήστε ένα στιγμιότυπο `Mollusk`](mention:mollusk) - Αρχικοποιήστε το
...
παρέχει μια απλή διεπαφή για τη δοκιμή προγραμμάτων Nara Solana. Όλα τα πεδία μπορούν
...
| [`process_and_validate_instruction`](#single-instruction-with-checks)          | Επεξεργάζεται μια εντολή και εκτελεί μια σειρά ελέγχων στο αποτέλεσμα, προκαλώντας πανικό αν αποτύχει κάποιος έλεγχος.            |
| [`process_instruction_chain`](#multiple-instructions)                          | Επεξεργάζεται πολλαπλές εντολές και επιστρέφει το αποτέλεσμα.                                                                     |
| [`process_and_validate_instruction_chain`](#multiple-instructions-with-checks) | Επεξεργάζεται πολλαπλές εντολές και εκτελεί μια σειρά ελέγχων σε κάθε αποτέλεσμα, προκαλώντας πανικό αν αποτύχει κάποιος έλεγχος. |
...
Χρησιμοποιήστε τη μέθοδο `process_instruction` για να επεξεργαστείτε μια μονή
...
Το ακόλουθο παράδειγμα επεξεργάζεται μια εντολή μεταφοράς NSO χωρίς ελέγχους
επικύρωσης.

<Callout type="info">

Τα παρακάτω παραδείγματα εκτελούν το Mollusk στη συνάρτηση `main` για λόγους
...
Χρησιμοποιήστε τη μέθοδο `process_and_validate_instruction` για την επεξεργασία
...
Το ακόλουθο παράδειγμα επεξεργάζεται μια εντολή μεταφοράς NSO με ελέγχους
επικύρωσης.
...
Χρησιμοποιήστε τη μέθοδο `process_instruction_chain` για την επεξεργασία
...
Το ακόλουθο παράδειγμα επεξεργάζεται δύο εντολές μεταφοράς NSO χωρίς ελέγχους
επικύρωσης.
...
Χρησιμοποιήστε τη μέθοδο `process_and_validate_instruction_chain` για την
...
Το ακόλουθο παράδειγμα επεξεργάζεται μια αλυσίδα δύο εντολών μεταφοράς NSO με
...
είναι ένα περίβλημα γύρω από το `Mollusk` που διατηρεί την κατάσταση του
λογαριασμού σε πολλαπλές κλήσεις εντολών μέσω του `account_store`. Οι μέθοδοι
για την επεξεργασία εντολών είναι πανομοιότυπες με το `Mollusk`.

Σε αντίθεση με το `Mollusk`, που απαιτεί την παράδοση του `accounts` σε κάθε
μέθοδο (π.χ. `process_instruction`), το `MolluskContext` διαχειρίζεται τους
...
Το ακόλουθο παράδειγμα επεξεργάζεται δύο ξεχωριστές εντολές μεταφοράς NSO με
...
Χρησιμοποιήστε τη μέθοδο `warp_to_slot` για να ενημερώσετε το ρολόι sysvar ώστε
...
απευθείας προσπελαύνοντας το πεδίο `sysvars` για να αλλάξετε τις παραμέτρους του
...
`out_dir` ως αρχείο markdown με όνομα `compute_units.md`.
...
<Callout type="info">
  ...
  επίδειξης. Το `mollusk-svm-programs-token` περιλαμβάνει επίσης βοηθητικές
