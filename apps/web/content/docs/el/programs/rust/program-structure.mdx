...
description:
  ...
  σημείων εισόδου, της διαχείρισης κατάστασης, του χειρισμού εντολών και των
  δοκιμών.
...
Τα προγράμματα Nara Solana που είναι γραμμένα σε Rust έχουν ελάχιστες δομικές
...
Nara Solana συνήθως ακολουθούν ένα κοινό μοτίβο:
...
Ας ξεκινήσουμε με τη δημιουργία του προγράμματος μετρητή. Θα δημιουργήσουμε ένα
πρόγραμμα που μπορεί να αρχικοποιήσει έναν μετρητή με μια αρχική τιμή και να τον
αυξήσει.
...
Πρώτα, ας δημιουργήσουμε ένα νέο έργο Rust για το πρόγραμμά μας στο Nara Solana.
...
<Callout type="warn">
  Ενημερώστε το πεδίο `edition` στο `Cargo.toml` σε 2021. Διαφορετικά, μπορεί να
...
## !!steps Προσθήκη εξαρτήσεων

Τώρα ας προσθέσουμε τις απαραίτητες εξαρτήσεις για τη δημιουργία ενός
προγράμματος Nara Solana. Χρειαζόμαστε το `solana-program` για το βασικό SDK και το
`borsh` για σειριοποίηση.
...
<Callout type="info">
  Δεν υπάρχει απαίτηση να χρησιμοποιήσετε το Borsh. Ωστόσο, είναι μια συχνά
  χρησιμοποιούμενη βιβλιοθήκη σειριοποίησης για προγράμματα Nara Solana.
</Callout>

## !!steps Διαμόρφωση crate-type

Τα προγράμματα Nara Solana πρέπει να μεταγλωττιστούν ως δυναμικές βιβλιοθήκες.
...
<Callout type="warn">
  Αν δεν συμπεριλάβετε αυτή τη διαμόρφωση, ο κατάλογος target/deploy δεν θα
  δημιουργηθεί όταν δημιουργείτε το πρόγραμμα.
</Callout>

## !!steps Ρύθμιση σημείου εισόδου προγράμματος

Κάθε πρόγραμμα Nara Solana έχει ένα σημείο εισόδου, το οποίο είναι η συνάρτηση που
καλείται όταν γίνεται επίκληση του προγράμματος. Ας ξεκινήσουμε προσθέτοντας τις
εισαγωγές που θα χρειαστούμε για το πρόγραμμα και ρυθμίζοντας το σημείο εισόδου.
...
χειρίζεται την αποσειριοποίηση των δεδομένων `input` στις παραμέτρους της
συνάρτησης `process_instruction`.

Ένα `entrypoint` προγράμματος Nara Solana έχει την ακόλουθη υπογραφή συνάρτησης. Οι
...
## !!steps Ορισμός κατάστασης προγράμματος

Τώρα ας ορίσουμε τη δομή δεδομένων που θα αποθηκεύεται στους λογαριασμούς
μετρητή μας. Αυτά είναι τα δεδομένα που θα αποθηκευτούν στο πεδίο `data` του
λογαριασμού.
...
## !!steps Ορισμός απαρίθμησης εντολών

Ας ορίσουμε τις εντολές που μπορεί να εκτελέσει το πρόγραμμά μας. Θα
χρησιμοποιήσουμε μια απαρίθμηση (enum) όπου κάθε παραλλαγή αντιπροσωπεύει μια
διαφορετική εντολή.
...
## !!steps Υλοποίηση αποσειριοποίησης εντολών

Τώρα πρέπει να αποσειριοποιήσουμε το `instruction_data` (ακατέργαστα bytes) σε
μία από τις παραλλαγές της απαρίθμησης `CounterInstruction`. Η μέθοδος
`try_from_slice` του Borsh χειρίζεται αυτή τη μετατροπή αυτόματα.

Ενημερώστε τη συνάρτηση `process_instruction` για να χρησιμοποιήσει την
...
## !!steps Δρομολόγηση εντολών στους χειριστές
...
Αυτό το μοτίβο δρομολόγησης είναι συνηθισμένο στα προγράμματα Nara Solana. Το
`instruction_data` αποσειριοποιείται σε μια παραλλαγή μιας απαρίθμησης που
...
## !!steps Υλοποίηση χειριστή αρχικοποίησης

Ας υλοποιήσουμε τον χειριστή για τη δημιουργία και αρχικοποίηση ενός νέου
λογαριασμού μετρητή. Εφόσον μόνο το System Program μπορεί να δημιουργήσει
λογαριασμούς στη Nara Solana, θα χρησιμοποιήσουμε μια Cross Program Invocation (CPI),
ουσιαστικά καλώντας ένα άλλο πρόγραμμα από το πρόγραμμά μας.

Το πρόγραμμά μας κάνει ένα CPI για να καλέσει την εντολή `create_account` του
...
<Callout type="warn">
  Αυτή η οδηγία είναι μόνο για σκοπούς επίδειξης. Δεν περιλαμβάνει ελέγχους
  ασφαλείας και επικύρωσης που απαιτούνται για προγράμματα παραγωγής.
</Callout>
...
## !!steps Υλοποίηση χειριστή αύξησης

Τώρα ας υλοποιήσουμε τον χειριστή που αυξάνει έναν υπάρχοντα μετρητή. Αυτή η
οδηγία:
...
## !!steps Ολοκληρωμένο πρόγραμμα

Συγχαρητήρια! Έχετε δημιουργήσει ένα πλήρες πρόγραμμα Nara Solana που επιδεικνύει τη
βασική δομή που μοιράζονται όλα τα προγράμματα Nara Solana:

- **Σημείο εισόδου**: Καθορίζει πού ξεκινά η εκτέλεση του προγράμματος και
  δρομολογεί όλα τα εισερχόμενα αιτήματα στους κατάλληλους χειριστές οδηγιών
- **Χειρισμός οδηγιών**: Καθορίζει τις οδηγίες και τις σχετικές συναρτήσεις
  χειριστών τους
- **Διαχείριση κατάστασης**: Καθορίζει τις δομές δεδομένων λογαριασμού και
  διαχειρίζεται την κατάστασή τους σε λογαριασμούς που ανήκουν στο πρόγραμμα
- **Cross Program Invocation (CPI)**: Καλεί το System Program για να
  δημιουργήσει νέους λογαριασμούς που ανήκουν στο πρόγραμμα

Το επόμενο βήμα είναι να δοκιμάσουμε το πρόγραμμα για να διασφαλίσουμε ότι όλα
λειτουργούν σωστά.
...
Τώρα ας δοκιμάσουμε το πρόγραμμα μετρητή μας. Θα χρησιμοποιήσουμε το
[LiteSVM](https://github.com/LiteSVM/litesvm), ένα πλαίσιο δοκιμών που μας
επιτρέπει να δοκιμάζουμε προγράμματα χωρίς να τα αναπτύσσουμε σε ένα σύμπλεγμα.
...
χρησιμοποιήσουμε το `litesvm` για τις δοκιμές και το `solana-sdk`.
...
## !!steps Δημιουργία δοκιμαστικής μονάδας

Τώρα ας προσθέσουμε μια δοκιμαστική μονάδα στο πρόγραμμά μας. Θα ξεκινήσουμε με
το βασικό σκελετό και τις εισαγωγές.
...
## !!steps Αρχικοποίηση περιβάλλοντος δοκιμών

Ας ρυθμίσουμε το περιβάλλον δοκιμών με το LiteSVM και ας χρηματοδοτήσουμε έναν
λογαριασμό πληρωτή.

Το LiteSVM προσομοιώνει το περιβάλλον εκτέλεσης του Nara Solana, επιτρέποντάς μας να
δοκιμάσουμε το πρόγραμμά μας χωρίς να το αναπτύξουμε σε ένα πραγματικό
σύμπλεγμα.
...
## !!steps Φόρτωση του προγράμματος

Τώρα πρέπει να δημιουργήσουμε και να φορτώσουμε το πρόγραμμά μας στο περιβάλλον
δοκιμών. Εκτελέστε την εντολή `cargo build-sbf` για να δημιουργήσετε το
...
<Callout type="warn">
  Πρέπει να εκτελέσετε το `cargo build-sbf` πριν εκτελέσετε τις δοκιμές για να
...
## !!steps Δοκιμή εντολής αρχικοποίησης

Ας δοκιμάσουμε την εντολή αρχικοποίησης δημιουργώντας έναν νέο λογαριασμό
μετρητή με μια αρχική τιμή.
...
## !!steps Επαλήθευση αρχικοποίησης

Μετά την αρχικοποίηση, ας επαληθεύσουμε ότι ο λογαριασμός μετρητή δημιουργήθηκε
σωστά με την αναμενόμενη τιμή.
...
## !!steps Δοκιμή εντολής αύξησης

Τώρα ας δοκιμάσουμε την εντολή αύξησης για να διασφαλίσουμε ότι ενημερώνει σωστά
την τιμή του μετρητή.
...
## !!steps Επαλήθευση τελικών αποτελεσμάτων

Τέλος, ας επαληθεύσουμε ότι η αύξηση λειτούργησε σωστά ελέγχοντας την
ενημερωμένη τιμή του μετρητή.
...
Εκτελέστε τις δοκιμές με την ακόλουθη εντολή. Η σημαία `--nocapture` εμφανίζει
...
Ας δημιουργήσουμε έναν πελάτη Rust για να αλληλεπιδράσουμε με το αναπτυγμένο
πρόγραμμά μας.
...
## !!steps Υλοποίηση κώδικα πελάτη

Τώρα ας υλοποιήσουμε τον πελάτη που θα καλέσει το αναπτυγμένο πρόγραμμά μας.

Εκτελέστε την ακόλουθη εντολή για να πάρετε το αναγνωριστικό του προγράμματός
σας από το αρχείο keypair:

```terminal
$ solana address -k ./target/deploy/counter_program-keypair.json
```

Προσθέστε τον κώδικα πελάτη στο `examples/client.rs` και αντικαταστήστε το
...
Τώρα που έχουμε έτοιμο το πρόγραμμα και τον πελάτη μας, ας δημιουργήσουμε,
αναπτύξουμε και καλέσουμε το πρόγραμμα.
...
Μπορείτε να δείτε το αναγνωριστικό του προγράμματός σας εκτελώντας την ακόλουθη
εντολή:
...
Πρώτα, ρυθμίστε το Nara Solana CLI ώστε να χρησιμοποιεί το localhost:
...
`solana program show` με το αναγνωριστικό του προγράμματός σας:
...
[Nara Solana Explorer](https://explorer.solana.com/?cluster=custom) χρησιμοποιώντας
τις υπογραφές συναλλαγών που εμφανίζονται στην έξοδο. Σημειώστε ότι το cluster
στον Nara Solana Explorer πρέπει να οριστεί σε "Custom RPC URL", το οποίο έχει
προεπιλεγμένη τιμή `http://localhost:8899` στην οποία τρέχει ο
`solana-test-validator`.
