...
blockchain του Nara Solana. Μια οδηγία είναι ουσιαστικά μια δημόσια συνάρτηση που
οποιοσδήποτε χρησιμοποιεί το δίκτυο Nara Solana μπορεί να καλέσει. Κάθε οδηγία
...
το δίκτυο Nara Solana, μία ή περισσότερες οδηγίες προστίθενται σε μια
[συναλλαγή](/docs/core/transactions) και αποστέλλονται στο δίκτυο για
επεξεργασία.

## Παράδειγμα μεταφοράς NSO
...
παράδειγμα, το NSO μεταφέρεται από έναν λογαριασμό σε έναν άλλο.
...
περιέχει την οδηγία μεταφοράς NSO.

![Διάγραμμα μεταφοράς NSO](/assets/docs/core/transactions/sol-transfer.svg)
...
![Διάγραμμα διαδικασίας μεταφοράς NSO](/assets/docs/core/transactions/sol-transfer-process.svg)
...
- [`program_id`](mention:program-id): Το [ID](#program-id) του προγράμματος που
  καλείται.
- [`accounts`](mention:accounts): Ένας πίνακας
  [μεταδεδομένων λογαριασμού](#account-metadata)
- [`data`](mention:instruction-data): Ένας πίνακας byte με πρόσθετα [δεδομένα]
  που θα χρησιμοποιηθούν από την εντολή.
...
Ο πίνακας `accounts` της εντολής είναι ένας πίνακας από δομές
...
![Μια συναλλαγή με μία εντολή. Η εντολή περιέχει δύο δομές `AccountMeta` στον πίνακα `accounts`.](/assets/docs/core/transactions/accountmeta.svg)

Τα μεταδεδομένα λογαριασμού περιλαμβάνουν τις ακόλουθες πληροφορίες:

- [pubkey](mention:pubkey): Η διεύθυνση δημόσιου κλειδιού του λογαριασμού
...
Το `data` της εντολής είναι ένας πίνακας byte που καθορίζει ποια από τις εντολές
του προγράμματος θα κληθεί. Περιλαμβάνει επίσης τυχόν ορίσματα που απαιτούνται
από την εντολή.

## Παράδειγμα δημιουργίας εντολής

Το παρακάτω παράδειγμα δείχνει τη δομή μιας εντολής μεταφοράς NSO.
...
ίδια τρία απαραίτητα κομμάτια πληροφοριών: [`program_id`](mention:program-id),
