...
description:
  Μάθετε πώς να μεταφέρετε tokens ιδιωτικά από έναν λογαριασμό token σε άλλον.
...
<Callout type="warn">
  ...
  εμπιστευτικών μεταφορών δεν είναι προς το παρόν διαθέσιμη. Ενώ οι έννοιες
  εξακολουθούν να ισχύουν, τα παραδείγματα κώδικα δεν θα εκτελεστούν.
</Callout>

## Πώς να μεταφέρετε εμπιστευτικά tokens από έναν token account σε άλλον

Για να μεταφέρετε εμπιστευτικά tokens από έναν token account σε άλλον, τόσο ο
αποστολέας όσο και ο παραλήπτης πρέπει να έχουν token accounts διαμορφωμένους με
την κατάσταση _rs`ConfidentialTransferAccount`_ και εγκεκριμένους για
εμπιστευτικές μεταφορές. Ο token account του αποστολέα πρέπει επίσης να έχει
διαθέσιμο εμπιστευτικό υπόλοιπο για μεταφορά.

Για να μεταφέρετε tokens εμπιστευτικά:

1. Δημιουργήστε
   [τρεις αποδείξεις](https://github.com/solana-program/token-2022/blob/d073cb89dbcd430387b5c6fb4b7157911351e4a3/confidential-transfer/proof-generation/src/transfer.rs#L35)
   ...
   **Απόδειξη Ισότητας
   ...
   Επαληθεύει ότι το νέο διαθέσιμο κρυπτογραφημένο υπόλοιπο μετά τη μεταφορά
   ταιριάζει με την αντίστοιχη
   [δέσμευση Pedersen](https://en.wikipedia.org/wiki/Commitment_scheme),
   διασφαλίζοντας ότι το νέο διαθέσιμο υπόλοιπο του λογαριασμού προέλευσης
   υπολογίζεται σωστά ως `new_balance = current_balance - transfer_amount`.

   **Απόδειξη Εγκυρότητας Κρυπτογραφημένου Κειμένου
   ...
   Επαληθεύει ότι τα κρυπτογραφημένα κείμενα του ποσού μεταφοράς δημιουργούνται
   σωστά για όλα τα τρία μέρη (προέλευση, προορισμός και ελεγκτής),
   διασφαλίζοντας ότι το ποσό μεταφοράς είναι σωστά κρυπτογραφημένο με το
   ...
   **Απόδειξη Εύρους
   ...
   Επαληθεύει ότι το νέο διαθέσιμο υπόλοιπο και το ποσό μεταφοράς (χωρισμένο σε
   χαμηλά/υψηλά bits) είναι όλα μη αρνητικά και εντός συγκεκριμένου εύρους.

2. Για κάθε απόδειξη:
   - Καλέστε το πρόγραμμα απόδειξης ZK ElGamal για να επαληθεύσετε τα δεδομένα
     απόδειξης.
   - Αποθηκεύστε τα συγκεκριμένα μεταδεδομένα απόδειξης σε έναν λογαριασμό
     "context state" απόδειξης για χρήση σε άλλες εντολές.

3. Καλέστε την εντολή
   ...
   παρέχοντας τους λογαριασμούς context state απόδειξης.

4. Κλείστε τους λογαριασμούς context state απόδειξης για να ανακτήσετε το NSO
   που χρησιμοποιήθηκε για τη δημιουργία τους.

Το παρακάτω διάγραμμα δείχνει τα βήματα που εμπλέκονται στη μεταφορά tokens από
έναν λογαριασμό token του αποστολέα σε έναν λογαριασμό token του παραλήπτη.
...
Για να μεταφέρετε εμπιστευτικά tokens από έναν λογαριασμό token σε άλλον, πρέπει
να:

- Δημιουργήσετε μια απόδειξη ισότητας, απόδειξη εγκυρότητας κρυπτογραφημένου
  κειμένου και απόδειξη εύρους από την πλευρά του πελάτη
- Καλέσετε το πρόγραμμα απόδειξης Zk ElGamal για να επαληθεύσετε τις αποδείξεις
  και να αρχικοποιήσετε τους λογαριασμούς "κατάστασης περιεχομένου"
- Καλέσετε την οδηγία
  [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604)
  παρέχοντας τους τρεις λογαριασμούς απόδειξης
- Κλείσετε τους τρεις λογαριασμούς απόδειξης για να ανακτήσετε το rent

Το πακέτο `spl_token_client` παρέχει τις ακόλουθες μεθόδους:

- Μέθοδος `confidential_transfer_create_context_state_account` που δημιουργεί
  έναν λογαριασμό απόδειξης
- Μέθοδος `confidential_transfer_transfer` που καλεί την οδηγία `Transfer`
- Μέθοδος `confidential_transfer_close_context_state_account` που κλείνει έναν
  λογαριασμό απόδειξης

### Παράδειγμα κώδικα

Το παρακάτω παράδειγμα δείχνει πώς να μεταφέρετε εμπιστευτικά tokens μεταξύ
λογαριασμών token.
...
εντολή. Πρέπει να έχετε εγκατεστημένο το Nara Solana CLI για να ξεκινήσετε τον τοπικό
validator.
...
<Callout type="info">
  Κατά τη στιγμή της συγγραφής, οι εμπιστευτικές μεταφορές δεν είναι
