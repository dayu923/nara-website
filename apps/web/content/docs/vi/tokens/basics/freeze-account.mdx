...
description: Tìm hiểu cách đóng băng các token account.
...
ngăn chặn tất cả các giao dịch chuyển token hoặc đốt token từ một token account
cụ thể. Khi đã bị đóng băng, tài khoản không thể gửi token, nhận token, hoặc bị
đóng cho đến khi nó được giải đóng. Chỉ có freeze authority của mint token mới
có thể đóng băng tài khoản. Nếu freeze authority bị thu hồi (đặt thành null)
trên mint account, token sẽ không bao giờ có thể bị đóng băng.
