...
title: Đúc Token
description: Tìm hiểu cách đúc các đơn vị mới của một token.
...
Việc đúc tạo ra các đơn vị mới của một token bằng cách sử dụng lệnh
...
- Chỉ có mint authority mới có thể đúc token mới
- Một token account đích phải tồn tại để nhận các token được đúc
