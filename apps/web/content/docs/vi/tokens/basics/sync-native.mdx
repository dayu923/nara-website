...
title: Đồng bộ Native
description: Tìm hiểu cách chuyển đổi NSO gốc sang wrapped NSO.
...
Wrapped NSO (WSOL) là một token account theo dõi số lượng lamports trong tài
khoản dưới dạng số dư token. WSOL cho phép tích hợp với các giao thức DeFi để
chuyển NSO dưới dạng token SPL.
...
đồng bộ hóa số dư token account wrapped NSO (WSOL) với số lượng NSO thực tế
(lamports) được lưu trữ trong đó. Khi bạn chuyển NSO gốc vào token account WSOL,
số dư token không tự động cập nhật. Bạn phải gọi SyncNative để hiển thị chính
xác số dư WSOL.
