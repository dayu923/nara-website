...
title: Đốt token
description: Tìm hiểu cách đốt token.
...
xóa vĩnh viễn token bằng cách giảm số dư trong token account. Token đã đốt sẽ bị
loại bỏ khỏi lưu thông và làm giảm `supply` được theo dõi trên mint account. Chỉ
chủ sở hữu token account mới có thể đốt token từ tài khoản của họ.
...
<Callout type="info">
  ...
  có cách triển khai tương tự để đạt được cùng một chức năng.
</Callout>
