...
description: Tìm hiểu cách kích hoạt InitializeNonTransferableMint.

---


## Cách kích hoạt tiện ích mở rộng InitializeNonTransferableMint
...
ngăn chặn việc chuyển token giữa các token account sau khi đúc. Sau khi được
đúc, token sẽ vĩnh viễn nằm trong token account. Tuy nhiên, chủ sở hữu token
account vẫn có thể đốt token và đóng token account khi số dư về không.
