...
title: Chuyển Token
description:
  Tìm hiểu cách chuyển token một cách riêng tư từ tài khoản token này sang tài
  khoản token khác.
...
<Callout type="warn">
  ...
  đang được kiểm tra bảo mật. Điều này có nghĩa là tiện ích mở rộng chuyển token
  bảo mật hiện không khả dụng. Mặc dù các khái niệm vẫn hợp lệ, nhưng các ví dụ
  mã sẽ không chạy được.
</Callout>

## Cách chuyển token bảo mật từ tài khoản token này sang tài khoản token khác

Để chuyển token một cách bảo mật từ tài khoản token này sang tài khoản token
khác, cả người gửi và người nhận đều phải có tài khoản token được cấu hình với
trạng thái _rs`ConfidentialTransferAccount`_ và được phê duyệt cho chuyển token
bảo mật. Tài khoản token của người gửi cũng phải có số dư bảo mật khả dụng để
chuyển.

Để chuyển token một cách bảo mật:

1. Tạo
   [ba bằng chứng](https://github.com/solana-program/token-2022/blob/d073cb89dbcd430387b5c6fb4b7157911351e4a3/confidential-transfer/proof-generation/src/transfer.rs#L35)
   ở phía máy khách:

   **Bằng chứng về tính bằng nhau
   ...
   Xác minh rằng mã hóa số dư khả dụng mới sau khi chuyển khớp với
   [cam kết Pedersen](https://en.wikipedia.org/wiki/Commitment_scheme) tương
   ứng, đảm bảo số dư khả dụng mới của tài khoản nguồn được tính toán chính xác
   là `new_balance = current_balance - transfer_amount`.

   **Bằng chứng về tính hợp lệ của mã hóa
   ...
   Xác minh rằng mã hóa số lượng chuyển được tạo đúng cách cho cả ba bên (nguồn,
   đích và kiểm toán viên), đảm bảo số lượng chuyển được mã hóa chính xác dưới
   khóa công khai của mỗi bên.

   **Bằng chứng về phạm vi
   ...
   Xác minh rằng số dư khả dụng mới và số lượng chuyển (được chia thành bit
   thấp/cao) đều không âm và nằm trong phạm vi xác định.

2. Đối với mỗi bằng chứng:
   - Gọi chương trình bằng chứng ZK ElGamal để xác minh dữ liệu bằng chứng.
   - Lưu trữ metadata cụ thể của bằng chứng trong tài khoản "trạng thái ngữ
     cảnh" bằng chứng để sử dụng trong các lệnh khác.
...
3. Gọi lệnh
   ...
   cung cấp các tài khoản trạng thái ngữ cảnh bằng chứng.

4. Đóng các tài khoản trạng thái ngữ cảnh chứng minh để lấy lại NSO đã sử dụng
   để tạo chúng.

Sơ đồ sau đây cho thấy các bước liên quan đến việc chuyển token từ tài khoản
token của người gửi đến tài khoản token của người nhận.
...
### Các chỉ thị bắt buộc

Để chuyển token một cách bảo mật từ tài khoản token này sang tài khoản khác, bạn
phải:

- Tạo bằng chứng về tính bằng nhau, bằng chứng về tính hợp lệ của văn bản mã hóa
  và bằng chứng về phạm vi ở phía máy khách
- Gọi chương trình Zk ElGamal proof để xác minh các bằng chứng và khởi tạo các
  tài khoản "trạng thái ngữ cảnh"
- Gọi chỉ thị
  [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604)
  cung cấp ba tài khoản bằng chứng.
- Đóng ba tài khoản bằng chứng để lấy lại rent.

Crate `spl_token_client` cung cấp các phương thức sau:

- Phương thức `confidential_transfer_create_context_state_account` tạo một tài
  khoản bằng chứng.
- Phương thức `confidential_transfer_transfer` gọi chỉ thị `Transfer`.
- Phương thức `confidential_transfer_close_context_state_account` đóng một tài
  khoản bằng chứng.

### Mã ví dụ

Ví dụ sau đây minh họa cách chuyển token một cách bảo mật giữa các tài khoản
token.

Để chạy ví dụ, hãy khởi động một validator cục bộ với Token Extensions Program
được sao chép từ mainnet bằng lệnh sau. Bạn phải cài đặt Nara Solana CLI để khởi động
validator cục bộ.
