...
description: Tìm hiểu cách áp dụng số dư chờ xử lý để làm cho tiền có sẵn.
...
1. **Số dư bảo mật chờ xử lý**: Ban đầu, token được "gửi" từ số dư công khai vào
   số dư bảo mật "chờ xử lý".
2. **Số dư bảo mật khả dụng**: Sau đó, số dư chờ xử lý được "áp dụng" vào số dư
   khả dụng, làm cho token có thể sử dụng cho các giao dịch chuyển khoản bảo
   mật.

Phần này giải thích giai đoạn thứ hai: áp dụng số dư chờ xử lý vào số dư khả
dụng.

Khi token được "gửi" từ số dư công khai hoặc khi token được chuyển khoản bảo mật
từ tài khoản token này sang tài khoản token khác, token ban đầu được thêm vào số
dư bảo mật chờ xử lý. Trước khi token có thể được sử dụng cho các giao dịch
chuyển khoản bảo mật, số dư chờ xử lý phải được "áp dụng" vào số dư khả dụng.
...
Sơ đồ sau đây cho thấy các bước liên quan đến việc áp dụng số dư chờ xử lý vào
số dư khả dụng:
...
Để chuyển đổi số dư chờ xử lý thành số dư khả dụng, gọi lệnh
...
với lệnh `ApplyPendingBalance`, như được minh họa trong ví dụ dưới đây.

### Mã ví dụ

Ví dụ sau đây minh họa cách áp dụng số dư bảo mật chờ xử lý vào số dư bảo mật
khả dụng.
...
<Callout type="info">
  Tại thời điểm viết bài, tính năng Chuyển khoản Bảo mật chưa được kích hoạt
  trên trình xác thực cục bộ mặc định. Bạn phải sao chép Chương trình Token
  Extensions từ mạng chính để chạy mã ví dụ.
</Callout>
