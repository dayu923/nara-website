...
description:
  Tìm hiểu về tiện ích mở rộng Chuyển khoản bảo mật và cách sử dụng nó để thêm
  tính năng tùy chọn vào mint và tài khoản token.
...
Chuyển khoản bảo mật cho phép bạn chuyển token giữa các tài khoản token mà không
tiết lộ số lượng chuyển khoản. Điều này hữu ích cho các giao dịch bảo vệ quyền
riêng tư. Chỉ có số lượng chuyển khoản và số dư token là riêng tư. Các địa chỉ
tài khoản token vẫn công khai.

- [Tổng quan về giao thức](https://www.solana-program.com/docs/confidential-balances/overview) -
  Chi tiết về giao thức mã hóa cơ bản
- [Hướng dẫn nhanh](https://www.solana-program.com/docs/confidential-balances#setup) -
  Thiết lập và các lệnh CLI cơ bản
- [Sổ tay số dư bảo mật](https://github.com/solana-developers/Confidential-Balances-Sample) -
  Các đoạn mã về cách sử dụng tiện ích mở rộng Chuyển khoản bảo mật

### Nó hoạt động như thế nào?

Tiện ích mở rộng Chuyển khoản bảo mật bổ sung
[các hướng dẫn](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/instruction.rs#L29)
vào Token Extensions Program cho phép bạn chuyển token giữa các tài khoản mà
không tiết lộ số lượng chuyển khoản.
...
2. Tạo các tài khoản token với tiện ích mở rộng chuyển khoản bảo mật cho người
   gửi và người nhận.
3. Mint token vào tài khoản người gửi.
4. **Gửi** số dư công khai của người gửi vào **số dư đang chờ xử lý bảo mật**.
5. **Áp dụng** số dư đang chờ xử lý của người gửi vào **số dư khả dụng bảo
   mật**.
6. **Chuyển khoản** bảo mật token từ tài khoản token người gửi đến tài khoản
   token người nhận.
7. **Áp dụng** số dư đang chờ xử lý của người nhận vào **số dư khả dụng bảo
   mật**.
8. **Rút** số dư khả dụng bảo mật của người nhận vào **số dư công khai**.
...
<Cards>
  ...
  >
    Cách tạo mint account với tiện ích mở rộng Chuyển khoản bảo mật
  </Card>
  <Card
    title="Tạo Tài khoản Token"
    href="/docs/tokens/extensions/confidential-transfer/create-token-account"
  >
    Cách cấu hình tài khoản token với tiện ích mở rộng Chuyển khoản bảo mật
  </Card>
  <Card
    title="Gửi Token"
    href="/docs/tokens/extensions/confidential-transfer/deposit-tokens"
  >
    Cách gửi token vào số dư đang chờ xử lý bảo mật
  </Card>
  <Card
    title="Áp dụng Số dư đang chờ xử lý"
    href="/docs/tokens/extensions/confidential-transfer/apply-pending-balance"
  >
    Cách áp dụng số dư đang chờ xử lý vào số dư khả dụng bảo mật
  </Card>
  <Card
    title="Rút Token"
  ...
  <Card
    title="Chuyển khoản Token"
    href="/docs/tokens/extensions/confidential-transfer/transfer-tokens"
  >
    Cách chuyển khoản token bảo mật giữa các tài khoản token
  </Card>
</Cards>

Sơ đồ dưới đây cho thấy trình tự chi tiết của luồng cơ bản cho các giao dịch
chuyển token bảo mật:
...
Ví dụ Rust dưới đây cho thấy các bước để chuyển token bảo mật tương ứng với sơ
đồ trước đó. Các bước được tách thành các hàm trợ giúp để dễ hiểu hơn.

Để chạy ví dụ, hãy khởi động một validator cục bộ với Token Extensions Program
được sao chép từ mainnet bằng lệnh sau. Bạn phải cài đặt Nara Solana CLI để sử dụng
validator cục bộ.
...
| Lệnh                                | Mô tả                                                                                                                                                                     |
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | Thiết lập tài khoản mint cho các giao dịch chuyển bảo mật. Lệnh này phải được bao gồm trong cùng một giao dịch với lệnh _rs`TokenInstruction::InitializeMint`_.           |
| _rs`UpdateMint`_                    | Cập nhật cài đặt chuyển token bảo mật cho một mint.                                                                                                                       |
| _rs`ConfigureAccount`_              | Thiết lập một token account cho các giao dịch chuyển bảo mật.                                                                                                             |
| _rs`ApproveAccount`_                | Phê duyệt một token account cho các giao dịch chuyển bảo mật nếu mint yêu cầu phê duyệt cho các token account mới.                                                        |
| _rs`EmptyAccount`_                  | Làm trống số dư bảo mật đang chờ xử lý và khả dụng để cho phép đóng token account.                                                                                        |
...
| _rs`ApplyPendingBalance`_           | Chuyển đổi số dư đang chờ xử lý thành số dư khả dụng sau khi gửi tiền hoặc chuyển khoản.                                                                                  |
| _rs`EnableConfidentialCredits`_     | Cho phép một token account nhận các giao dịch chuyển token bảo mật.                                                                                                       |
| _rs`DisableConfidentialCredits`_    | Chặn các giao dịch chuyển bảo mật đến nhưng vẫn cho phép các giao dịch chuyển công khai.                                                                                  |
| _rs`EnableNonConfidentialCredits`_  | Cho phép một token account nhận các giao dịch chuyển token công khai.                                                                                                     |
| _rs`DisableNonConfidentialCredits`_ | Chặn các giao dịch chuyển thông thường để tài khoản chỉ nhận các giao dịch chuyển bảo mật.                                                                                |
| _rs`TransferWithFee`_               | Chuyển token giữa các token account một cách bảo mật với phí.                                                                                                             |
| _rs`ConfigureAccountWithRegistry`_  | Cách thay thế để cấu hình token account cho các giao dịch chuyển bảo mật bằng cách sử dụng tài khoản _rs`ElGamalRegistry`_ thay vì bằng chứng _rs`VerifyPubkeyValidity`_. |

| Chỉ thị                             | Mô tả                                                                                                                                                                          |
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| _rs`InitializeMint`_                | Thiết lập mint account cho các giao dịch chuyển tiền bảo mật. Chỉ thị này phải được đưa vào cùng giao dịch với chỉ thị _rs`TokenInstruction::InitializeMint`_.                 |
| _rs`UpdateMint`_                    | Cập nhật cài đặt chuyển tiền bảo mật cho một mint.                                                                                                                             |
| _rs`ConfigureAccount`_              | Thiết lập token account cho các giao dịch chuyển tiền bảo mật.                                                                                                                 |
| _rs`ApproveAccount`_                | Phê duyệt token account cho các giao dịch chuyển tiền bảo mật nếu mint yêu cầu phê duyệt cho các token account mới.                                                            |
| _rs`EmptyAccount`_                  | Làm trống số dư bảo mật đang chờ xử lý và khả dụng để cho phép đóng token account.                                                                                             |
| _rs`Deposit`_                       | Chuyển đổi số dư token công khai thành số dư bảo mật đang chờ xử lý.                                                                                                           |
| _rs`Withdraw`_                      | Chuyển đổi số dư bảo mật khả dụng trở lại thành số dư công khai.                                                                                                               |
| _rs`Transfer`_                      | Chuyển token giữa các token account một cách bảo mật.                                                                                                                          |
| _rs`ApplyPendingBalance`_           | Chuyển đổi số dư đang chờ xử lý thành số dư khả dụng sau khi gửi tiền hoặc chuyển khoản.                                                                                       |
| _rs`EnableConfidentialCredits`_     | Cho phép token account nhận các giao dịch chuyển token bảo mật.                                                                                                                |
| _rs`DisableConfidentialCredits`_    | Chặn các giao dịch chuyển tiền bảo mật đến nhưng vẫn cho phép chuyển tiền công khai.                                                                                           |
| _rs`EnableNonConfidentialCredits`_  | Cho phép token account nhận các giao dịch chuyển token công khai.                                                                                                              |
| _rs`DisableNonConfidentialCredits`_ | Chặn các giao dịch chuyển khoản thông thường để tài khoản chỉ nhận được các giao dịch chuyển tiền bảo mật.                                                                     |
| _rs`TransferWithFee`_               | Chuyển token giữa các token account một cách bảo mật với phí.                                                                                                                  |
| _rs`ConfigureAccountWithRegistry`_  | Cách thay thế để cấu hình token account cho các giao dịch chuyển tiền bảo mật bằng cách sử dụng tài khoản _rs`ElGamalRegistry`_ thay vì bằng chứng _rs`VerifyPubkeyValidity`_. |
