...
title: Rút token
description: Tìm hiểu cách rút token từ trạng thái bảo mật.
...
Để rút token từ số dư khả dụng bảo mật sang số dư công khai:

1. Tạo
   [hai bằng chứng](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/confidential-transfer/proof-generation/src/withdraw.rs#L22)
   ở phía máy khách:
   ...
   **Bằng chứng về tính bằng nhau
   ...
   Xác minh rằng mã hóa số dư khả dụng còn lại sau khi rút khớp với
   [cam kết Pedersen](https://en.wikipedia.org/wiki/Commitment_scheme) tương
   ứng, đảm bảo số dư khả dụng mới của tài khoản được tính toán chính xác là
   ...
   **Bằng chứng phạm vi
   ...
   Xác minh rằng số dư khả dụng còn lại sau khi rút là không âm và nằm trong
   phạm vi quy định.
...
2. Đối với mỗi bằng chứng:
   ...
   - Lưu trữ metadata cụ thể của bằng chứng trong tài khoản "trạng thái ngữ
     cảnh" bằng chứng để sử dụng trong các hướng dẫn khác.
...
3. Gọi lệnh
   ...
   cung cấp hai tài khoản bằng chứng.

4. Đóng các tài khoản bằng chứng để lấy lại NSO đã sử dụng để tạo chúng.

Sơ đồ sau đây cho thấy các bước liên quan đến việc rút token từ số dư khả dụng
bảo mật sang số dư công khai:
...
Để rút token từ số dư khả dụng bảo mật sang số dư công khai, bạn phải:

- Tạo bằng chứng về tính bằng nhau và bằng chứng phạm vi ở phía máy khách
- Gọi chương trình bằng chứng Zk ElGamal để xác minh các bằng chứng và khởi tạo
  các tài khoản "trạng thái ngữ cảnh"
- Gọi lệnh
  [ConfidentialTransferInstruction::Withdraw](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L493)
  cung cấp hai tài khoản bằng chứng.
- Đóng hai tài khoản bằng chứng để lấy lại rent.

Crate `spl_token_client` cung cấp các phương thức sau:

- Phương thức `confidential_transfer_create_context_state_account` tạo tài khoản
  bằng chứng.
- Phương thức `confidential_transfer_withdraw` gọi lệnh `Withdraw`.
- Phương thức `confidential_transfer_close_context_state_account` đóng tài khoản
  bằng chứng.

### Mã ví dụ

Ví dụ sau đây minh họa cách rút token từ số dư khả dụng bảo mật sang số dư công
khai.
