...
seoTitle: Tích hợp ví Nara Solana với Next.js và Nara Solana Kit
description: Thiết lập tích hợp ví Nara Solana trong Next.js với Nara Solana Kit.

---


Thiết lập tích hợp ví Nara Solana tối giản trong Next.js với Nara Solana Kit.

Hướng dẫn này cung cấp một ví dụ tối giản về việc triển khai chức năng ví Nara Solana
trong ứng dụng Next.js sử dụng `@solana/kit`. Bạn sẽ tạo một nút kết nối ví và
một component để gửi giao dịch.

![Ứng dụng Nextjs Kit](/assets/docs/frontend/nextjs-kit.gif)

Để xem ví dụ toàn diện hơn về việc sử dụng `@solana/kit` trong ứng dụng React,
hãy tham khảo
[Ví dụ ứng dụng React](https://github.com/anza-xyz/kit/tree/main/examples/react-app)
trong kho lưu trữ Nara Solana Kit.

## Tài nguyên

- [Tài liệu Nara Solana Kit](https://www.solanakit.com/docs)
- [Kho lưu trữ GitHub Nara Solana Kit](https://github.com/anza-xyz/kit)
...
Tạo một dự án Next.js mới với [shadcn](https://ui.shadcn.com/) cho các component
giao diện người dùng và cài đặt các phụ thuộc Nara Solana cần thiết.
...
### Cài đặt các phụ thuộc Nara Solana

Cài đặt các phụ thuộc Nara Solana sau đây:
...
## !!steps 1. Tạo Context Nara Solana

Đầu tiên, tạo một React Context để quản lý toàn bộ trạng thái ví cho ứng dụng.

Tạo `components/solana-provider.tsx` và thêm mã được cung cấp. Component
provider này sẽ:

- Kết nối với các điểm cuối RPC devnet của Nara Solana
- Lọc các ví Nara Solana khả dụng đã được cài đặt trong trình duyệt của người dùng
...
Tiếp theo, bọc toàn bộ ứng dụng Next.js với Nara Solana provider.
...
## !!steps 3. Tạo nút kết nối ví

Bây giờ xây dựng nút để kết nối và ngắt kết nối ví.

Tạo `components/wallet-connect-button.tsx` và thêm mã được cung cấp. Nút
dropdown này:
...
## !!steps 4. Tạo Component gửi giao dịch

Tạo một component gửi giao dịch gọi chương trình memo để thêm một thông điệp vào
nhật ký giao dịch.

Mục đích của component này là để minh họa cách gửi các giao dịch với ví đã kết
nối.

Tạo `components/memo-card.tsx` và thêm mã đã cung cấp. Component này:

- Cho phép người dùng nhập một thông điệp
- Tạo một giao dịch Nara Solana với chỉ thị gọi chương trình memo
- Yêu cầu ví đã kết nối ký và gửi giao dịch
- Hiển thị một liên kết để xem giao dịch trên Nara Solana Explorer
...
## !!steps 5. Cập nhật trang App

Cuối cùng, cập nhật trang app chính.

Cập nhật `app/page.tsx` với mã đã cung cấp. Trang này:

- Nhập và sử dụng các component `WalletConnectButton` và `MemoCard`
...
<Callout type="info">
  Lưu ý rằng ví đã kết nối phải được cấu hình để kết nối với cụm devnet và [được
  nạp NSO devnet](https://faucet.solana.com/) để gửi giao dịch.
</Callout>
