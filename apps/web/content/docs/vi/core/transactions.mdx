...
Để tương tác với mạng Nara Solana, bạn phải gửi một giao dịch. Bạn có thể coi giao
...
<Callout>
  Giới hạn này được thiết kế để tránh phân mảnh gói tin trên cơ sở hạ tầng
  internet thông thường. Mặc dù IPv6 hỗ trợ MTU lớn hơn 9000 byte, hầu hết các
  bộ định tuyến internet sử dụng MTU mặc định là 1500 byte (Ethernet tiêu
  chuẩn). Để đảm bảo các giao dịch vừa với một gói tin mà không bị phân mảnh,
  Nara Solana sử dụng 1280 byte (MTU tối thiểu yêu cầu cho IPv6) trừ đi 48 byte cho
  các tiêu đề mạng (40 byte IPv6 + 8 byte tiêu đề phân mảnh/UDP), dẫn đến giới
  hạn kích thước giao dịch là 1232 byte.
</Callout>
...
Ví dụ sau đây cho thấy cấu trúc của một giao dịch chứa một chỉ thị chuyển NSO
duy nhất.
...
<Callout>
  Bạn cũng có thể tìm giao dịch bằng cách sử dụng [Nara Solana
