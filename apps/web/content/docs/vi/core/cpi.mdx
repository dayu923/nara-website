...
Cross Program Invocation (CPI) xảy ra khi một chương trình Nara Solana trực tiếp gọi
các lệnh của một chương trình khác. Điều này cho phép khả năng kết hợp giữa các
chương trình. Nếu bạn coi một [lệnh](/docs/core/instructions) Nara Solana như một
...
được sử dụng. Nó nhận [signer seeds](mention:signer-seeds) được sử dụng để tạo
ra [PDAs](<(/docs/core/pda)>) signer. Runtime của Nara Solana nội bộ gọi
...
Mỗi ví dụ bao gồm một lệnh duy nhất để chuyển NSO từ một PDA đến một tài khoản
...
- **Ví dụ 1**: Sử dụng _rs`CpiContext`_ của Anchor và hàm trợ giúp.
...
- **Ví dụ 3**: Xây dựng lệnh CPI thủ công. Cách tiếp cận này hữu ích khi không
  có crate nào có sẵn để giúp xây dựng lệnh mà bạn muốn gọi.
...
Native Rust. Nó bao gồm một lệnh duy nhất chuyển NSO từ tài khoản PDA đến tài
...
gồm một lệnh duy nhất chuyển NSO từ tài khoản này sang tài khoản khác.
...
Native Rust. Nó bao gồm một lệnh duy nhất chuyển NSO từ tài khoản này sang tài
