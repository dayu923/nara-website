...
muốn triển khai bản dựng đã xác minh cho chương trình của họ trên Nara Solana. Chúng
...
mạng Nara Solana khớp với mã nguồn trong kho lưu trữ của bạn. Bằng cách này, các nhà
...
chương trình trong môi trường được kiểm soát bằng cách sử dụng Nara Solana Verify CLI
...
lên mạng Nara Solana. Trong quá trình xây dựng, một
...
rất có thể thất bại vì các bản build chương trình Nara Solana không mang tính xác
...
### Cài đặt Nara Solana Verify CLI

Nara Solana Verify CLI là công cụ chính được sử dụng để xác minh các bản build.
Nara Solana Verify CLI hiện đang được duy trì bởi
...
Để xây dựng chương trình Nara Solana của bạn một cách có thể xác minh, hãy điều hướng
...
nó lên mạng Nara Solana. Khuyến nghị sử dụng giải pháp đa chữ ký hoặc quản trị như
...
> [cụm Nara Solana](/docs/references/clusters) khác nhau (tức là devnet, testnet,
> mainnet). Đảm bảo bạn sử dụng URL mạng chính xác cho cụm Nara Solana mong muốn mà
...
không. Nếu bạn làm điều đó, Nara Solana Explorer sẽ ngay lập tức hiển thị dữ liệu xác
...
đóng góp vào một hệ sinh thái Nara Solana lành mạnh hơn.
...
hiển thị trong trình khám phá Nara Solana và các nơi khác.
...
Sử dụng
[bản build đã được xác minh trên Nara Solana](/developers/guides/advanced/verified-builds)
đảm bảo tính toàn vẹn và độ tin cậy của các chương trình của bạn trên mạng và
cho phép các nhà phát triển tìm thấy SDK của bạn trực tiếp từ Nara Solana Explorer.
Bằng cách tận dụng các công cụ như Nara Solana Verify CLI và Docker, bạn có thể duy
...
toàn vẹn của các chương trình Nara Solana của bạn, nhưng nó không hoàn toàn không cần
tin cậy trong thiết lập mặc định. Các image docker được xây dựng và lưu trữ bởi
Nara Solana Foundation.
...
API hoặc Nara Solana Explorer có thể hiển thị thông tin không chính xác nếu bị xâm
phạm.
...
Nara Solana Foundation, OtterSec và đội Ellipsis Labs không chịu trách nhiệm cho bất
kỳ tổn thất hoặc thiệt hại nào có thể xảy ra khi sử dụng quy trình xây dựng đã
được xác minh.

# Security.txt cho các chương trình Nara Solana
...
bảo mật trực tiếp trong các hợp đồng thông minh Nara Solana của họ. Lấy cảm hứng từ
...
Để thêm `security.txt` vào chương trình Nara Solana của bạn, hãy thực hiện các bước
sau:
...
dàng được truy vấn thông qua các công cụ như Nara Solana Explorer, đảm bảo rằng thông
...
cải thiện giao tiếp trong hệ sinh thái Nara Solana.

Đây là
[một ví dụ về cách security.txt hiển thị trong Nara Solana Explorer](https://explorer.solana.com/address/HPxKXnBN4vJ8RjpdqDCU7gvNQHeeyGnSviYTJ4fBrDt4/security?cluster=devnet)
