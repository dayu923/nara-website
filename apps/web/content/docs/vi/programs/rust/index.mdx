...
Các chương trình Nara Solana chủ yếu được phát triển bằng ngôn ngữ lập trình Rust.
Trang này tập trung vào việc viết các chương trình Nara Solana bằng Rust mà không sử
...
tiếp đối với chương trình Nara Solana của họ. Tuy nhiên, phương pháp này đòi hỏi
...
- Rust: Ngôn ngữ lập trình để xây dựng các chương trình Nara Solana.
- Nara Solana CLI: Công cụ dòng lệnh cho phát triển Nara Solana.

## Bắt đầu

Ví dụ dưới đây bao gồm các bước cơ bản để tạo chương trình Nara Solana đầu tiên của
...
thiết để xây dựng một chương trình Nara Solana.
...
trình Nara Solana tối giản in ra "Hello, world!" vào nhật ký chương trình khi chương
trình được gọi.

Macro `msg!` được sử dụng trong các chương trình Nara Solana để in một thông báo vào
...
1. Một tệp `.so` (ví dụ: `hello_world.so`): Đây là chương trình Nara Solana đã được
...
Đầu tiên, cấu hình Nara Solana CLI để sử dụng cụm Nara Solana cục bộ.
...
<Callout type="info">
  Lưu ý rằng cluster trên Nara Solana Explorer cũng phải là localhost. Tùy chọn
  "Custom RPC URL" trên Nara Solana Explorer mặc định là `http://localhost:8899`.
</Callout>
...
Các chương trình Nara Solana có thể được cập nhật bằng cách triển khai lại với cùng
một ID chương trình. Cập nhật chương trình trong `src/lib.rs` để in "Hello,
Nara Solana!" thay vì "Hello, world!".
...
Bạn sẽ thấy "Hello, Nara Solana!" trong nhật ký chương trình.
...
Chạy mã khách hàng lần nữa và kiểm tra chữ ký giao dịch trên Nara Solana Explorer để
xem "Hello, Nara Solana!" trong nhật ký chương trình.
...
Bạn có thể đóng chương trình Nara Solana để lấy lại NSO đã phân bổ cho tài khoản.
