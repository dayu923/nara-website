...
Các chương trình Nara Solana được viết bằng Rust có yêu cầu cấu trúc tối thiểu, cho
...
Mặc dù không có quy tắc nghiêm ngặt cho cấu trúc tệp, các chương trình Nara Solana
...
Ví dụ, xem
[Chương trình Token](https://github.com/solana-program/token/tree/main/program/src).
...
Hãy bắt đầu bằng việc xây dựng chương trình bộ đếm. Chúng ta sẽ tạo một chương
trình có thể khởi tạo bộ đếm với giá trị bắt đầu và tăng nó lên.

<ScrollyCoding>

## !!steps Tạo một chương trình mới

Đầu tiên, hãy tạo một dự án Rust mới cho chương trình Nara Solana của chúng ta.
...
Bạn sẽ thấy các tệp mặc định `src/lib.rs` và `Cargo.toml`.
...
Bây giờ hãy thêm các phụ thuộc cần thiết để xây dựng chương trình Nara Solana. Chúng
ta cần `solana-program` cho SDK cốt lõi và `borsh` cho quá trình tuần tự hóa.
...
<Callout type="info">
  Không bắt buộc phải sử dụng Borsh. Tuy nhiên, đây là thư viện tuần tự hóa được
  sử dụng phổ biến cho các chương trình Nara Solana.
</Callout>

## !!steps Cấu hình crate-type

Các chương trình Nara Solana phải được biên dịch dưới dạng thư viện động. Thêm phần
`[lib]` để cấu hình cách Cargo xây dựng chương trình.
...
<Callout type="warn">
  Nếu bạn không bao gồm cấu hình này, thư mục target/deploy sẽ không được tạo ra
  khi bạn xây dựng chương trình.
</Callout>

## !!steps Thiết lập điểm vào chương trình

Mỗi chương trình Nara Solana đều có một điểm vào, đó là hàm được gọi khi chương trình
được kích hoạt. Hãy bắt đầu bằng việc thêm các import cần thiết cho chương trình
và thiết lập điểm vào.
...
xử lý việc giải tuần tự hóa dữ liệu `input` thành các tham số của hàm
`process_instruction`.

Một `entrypoint` của chương trình Nara Solana có chữ ký hàm sau đây. Các nhà phát
...
## !!steps Định nghĩa enum instruction
...
## !!steps Triển khai giải mã instruction

Bây giờ chúng ta cần giải mã `instruction_data` (dữ liệu thô) thành một trong
các biến thể enum `CounterInstruction` của chúng ta. Phương thức Borsh
`try_from_slice` xử lý việc chuyển đổi này một cách tự động.
...
## !!steps Định tuyến các instruction đến các trình xử lý

Bây giờ hãy cập nhật hàm chính `process_instruction` để định tuyến các lệnh đến
các hàm xử lý thích hợp của chúng.

Mẫu định tuyến này phổ biến trong các chương trình Nara Solana. `instruction_data`
...
có System Program mới có thể tạo tài khoản trên Nara Solana, chúng ta sẽ sử dụng
Cross Program Invocation (CPI), về cơ bản là gọi một chương trình khác từ chương
trình của chúng ta.

Chương trình của chúng ta thực hiện một CPI để gọi lệnh `create_account` của
...
<Callout type="warn">
  Hướng dẫn này chỉ nhằm mục đích minh họa. Nó không bao gồm các kiểm tra bảo
  mật và xác thực cần thiết cho các chương trình sản phẩm thực tế.
</Callout>
...
## !!steps Triển khai trình xử lý tăng giá trị

Bây giờ hãy triển khai trình xử lý để tăng giá trị của một bộ đếm hiện có. Chỉ
thị này:
...
Chúc mừng! Bạn đã xây dựng một chương trình Nara Solana hoàn chỉnh thể hiện cấu trúc
cơ bản được chia sẻ bởi tất cả các chương trình Nara Solana:

- **Entrypoint**: Xác định nơi bắt đầu thực thi chương trình và định tuyến tất
  cả các yêu cầu đến đến các trình xử lý chỉ thị thích hợp
...
- **Quản lý trạng thái**: Xác định cấu trúc dữ liệu tài khoản và quản lý trạng
  thái của chúng trong các tài khoản thuộc sở hữu của chương trình
- **Cross Program Invocation (CPI)**: Gọi System Program để tạo các tài khoản
  mới thuộc sở hữu của chương trình
...
Bây giờ hãy kiểm tra chương trình đếm của chúng ta. Chúng ta sẽ sử dụng
[LiteSVM](https://github.com/LiteSVM/litesvm), một framework kiểm tra cho phép
chúng ta kiểm tra các chương trình mà không cần triển khai lên cluster.

<ScrollyCoding>

## !!steps Thêm các phụ thuộc cho kiểm tra

Đầu tiên, hãy thêm các phụ thuộc cần thiết cho việc kiểm tra. Chúng ta sẽ sử
dụng `litesvm` cho việc kiểm tra và `solana-sdk`.
...
bắt đầu với cấu trúc cơ bản và các phần import.
...
Hãy thiết lập môi trường kiểm thử với LiteSVM và cấp vốn cho tài khoản thanh
toán.

LiteSVM mô phỏng môi trường runtime của Nara Solana, cho phép chúng ta kiểm thử
chương trình mà không cần triển khai lên một cụm thực.
...
lệnh `cargo build-sbf` để xây dựng chương trình. Điều này sẽ tạo ra tệp
...
<Callout type="warn">
  Bạn phải chạy `cargo build-sbf` trước khi chạy kiểm thử để tạo ra tệp `.so`.
...
## !!steps Kiểm thử lệnh khởi tạo

Hãy kiểm thử lệnh khởi tạo bằng cách tạo một tài khoản bộ đếm mới với giá trị
bắt đầu.
...
## !!steps Xác minh quá trình khởi tạo

Sau khi khởi tạo, hãy xác minh tài khoản bộ đếm đã được tạo đúng cách với giá
trị mong đợi.
...
Cuối cùng, hãy xác minh rằng lệnh tăng đã hoạt động đúng bằng cách kiểm tra giá
trị bộ đếm đã được cập nhật.
...
Chạy các bài kiểm tra với lệnh sau. Cờ `--nocapture` sẽ in ra kết quả của bài
kiểm tra.
...
Đầu tiên, cấu hình Nara Solana CLI để sử dụng localhost:
...
Bạn có thể xác minh việc triển khai bằng lệnh `solana program show` với ID
...
[Nara Solana Explorer](https://explorer.solana.com/?cluster=custom) bằng cách sử dụng
chữ ký giao dịch đầu ra. Lưu ý rằng cluster trên Nara Solana Explorer phải được đặt
