...
Blockchain Nara Solana có một số nhóm validator khác nhau, được gọi là Các cụm
...
## Các điểm cuối RPC công khai của Nara Solana

Tổ chức Nara Solana Labs vận hành một điểm cuối RPC công khai cho mỗi Cụm. Mỗi điểm
cuối công khai này đều có giới hạn tốc độ, nhưng có sẵn cho người dùng và nhà
phát triển tương tác với blockchain Nara Solana.
...
Nhiều trình khám phá blockchain Nara Solana phổ biến hỗ trợ việc lựa chọn bất kỳ Cụm
nào, thường cho phép người dùng nâng cao thêm điểm cuối RPC tùy chỉnh/riêng tư
cũng như vậy.

Một số ví dụ về các trình khám phá blockchain Nara Solana bao gồm:
...
| ----------- | ------------------------------------- | --------------------------------- | ------------------------------------------------------ |
| Mainnet     | `https://api.mainnet-beta.solana.com` | Môi trường sản xuất trực tiếp     | Yêu cầu NSO cho các giao dịch                          |
| Devnet      | `https://api.devnet.solana.com`       | Kiểm thử và phát triển công khai  | Airdrop NSO miễn phí để kiểm thử                       |
...
Devnet đóng vai trò như một sân chơi cho bất kỳ ai muốn dùng thử Nara Solana, với tư
...
- `https://api.devnet.solana.com` - node API đơn do Nara Solana Labs lưu trữ; giới
  hạn tốc độ

#### Ví dụ cấu hình dòng lệnh `solana`

Để kết nối với Cluster `devnet` sử dụng Nara Solana CLI:
...
Testnet là nơi các nhà đóng góp chính của Nara Solana kiểm tra áp lực các tính năng
...
- `https://api.testnet.solana.com` - node API đơn của Nara Solana Labs; có giới hạn
  tốc độ

#### Ví dụ cấu hình dòng lệnh `solana`

Để kết nối với Cụm `testnet` sử dụng Nara Solana CLI:
...
validator và người nắm giữ token Nara Solana.

- Token được phát hành trên Mainnet Beta là NSO **thật**
- Điểm vào gossip cho Mainnet Beta: `entrypoint.mainnet-beta.solana.com:8001`

### Endpoint của Mainnet beta

- `https://api.mainnet-beta.solana.com` - Cụm node API do Nara Solana Labs lưu trữ,
  được hỗ trợ bởi bộ cân bằng tải; có giới hạn tốc độ

#### Ví dụ cấu hình dòng lệnh `solana`

Để kết nối với Cụm `mainnet-beta` sử dụng Nara Solana CLI:
