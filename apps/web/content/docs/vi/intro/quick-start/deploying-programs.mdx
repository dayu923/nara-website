...
h1: Triển khai chương trình Nara Solana đầu tiên của bạn

---


Xây dựng, triển khai và kiểm thử [program](/docs/core/programs) đầu tiên của bạn
lên Blockchain Nara Solana bằng framework Anchor. Hướng dẫn này sẽ giúp bạn làm quen
với Nara Solana Playground và chuẩn bị cho bạn sử dụng Program Derived Address (PDA)
...
1. Mở [Nara Solana Playground](https://beta.solpg.io) trong trình duyệt web của bạn
...
Truy cập vào trường `data` của tài khoản `new_account` và đặt giá trị của nó
thành `data: u64`

### !msg

Macro _rs`msg!`_ ghi một thông báo vào nhật ký chương trình của giao dịch
...
- Macro [`declare_id!`](mention:one) chỉ định địa chỉ on-chain của chương trình
  của bạn. Nara Solana Playground sẽ tự động cập nhật địa chỉ này khi bạn xây dựng
  chương trình.
...
- `ctx: Context<Initialize>`: Truyền các tài khoản cần thiết cho chỉ thị này vào
  hàm, như được chỉ định trong struct `Initialize`.
- `data: u64`: Một tham số tùy chỉnh mà bạn phải cung cấp khi gọi chỉ thị.

Phần thân hàm [đặt trường `data` của `new_account`](mention:three) thành tham số
`data` được cung cấp, sau đó in một thông báo vào nhật ký chương trình.
...
- Thuộc tính [`#[derive(Accounts)]`](mention:one) định nghĩa một struct chỉ định
  các tài khoản cần thiết cho một hướng dẫn cụ thể. Mỗi trường đại diện cho một
  tài khoản riêng biệt.

  Anchor tự động xử lý các kiểm tra bảo mật phổ biến liên quan đến xác thực tài
  khoản thông qua các kiểu trường (ví dụ: `Signer<'info>`) và các ràng buộc (ví
  dụ: `#[account(mut)]`).
...
<Callout>
  ...
  định nghĩa trong _rs`declare_id!()`_, sẽ được cập nhật.
</Callout>

2. Thêm NSO vào ví của bạn _(Tùy chọn)_

Việc triển khai chương trình đòi hỏi NSO trong tài khoản on-chain của chương
trình. Nếu cần thiết,
[thêm NSO vào ví của bạn](/docs/intro/quick-start#add-sol-to-your-wallet) trước
khi tiếp tục bước 3.
...
1. Mở tệp kiểm thử có tên `tests/anchor.test.ts`. Tệp này minh họa cách gọi
   struct [`initialize`](mention:one) của chương trình từ phía client.
...
3. Lưu lại mã băm giao dịch (`TxHash`) có trong kết quả đầu ra
...
**Lưu ý:** Nếu muốn, bạn có thể sử dụng nút `Test` trên bảng điều khiển bên
trái.

![Chạy kiểm thử](/assets/docs/intro/quickstart/pg-test.png)

4. Chạy `solana confirm -v [TxHash]` để xem nhật ký giao dịch
...
[Nara Solana Explorer](https://explorer.solana.com/?cluster=devnet) và tìm kiếm mã
băm giao dịch.

<Callout type="warn">
  **Nara Solana Explorer:** Đặt cụm mạng thành `devnet`, để khớp với cài đặt của
...
Đóng chương trình để thu hồi NSO đã cấp phát cho chương trình trên chuỗi. Để
thực hiện, chạy `solana program close [ProgramID]`, sử dụng địa chỉ chương trình
...
Chúc mừng! Bạn đã xây dựng và triển khai chương trình Nara Solana đầu tiên của mình
bằng Anchor.
