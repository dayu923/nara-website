...
Trong phần trước, bạn đã học cách đọc dữ liệu từ mạng Nara Solana. Bây giờ bạn sẽ học
cách ghi dữ liệu vào mạng. Việc ghi dữ liệu vào mạng Nara Solana liên quan đến việc
...
[giao dịch](/docs/core/transactions), thời gian chạy Nara Solana thực thi từng chỉ
thị theo trình tự và một cách nguyên tử. Các ví dụ trong phần này cho thấy cách
xây dựng và gửi giao dịch để gọi các chương trình Nara Solana, bao gồm:

1. Chuyển NSO giữa các tài khoản
2. Tạo một token mới

## Chuyển NSO

Ví dụ dưới đây chuyển NSO giữa hai tài khoản. Mỗi tài khoản có một chương trình
sở hữu, đây là chương trình duy nhất có thể trừ số dư NSO của tài khoản.

Tất cả các tài khoản ví đều thuộc sở hữu của System Program. Để chuyển NSO, bạn
phải gọi chỉ thị
...
Tạo kết nối đến một cụm Nara Solana.
...
Yêu cầu airdrop NSO để cấp vốn cho `sender`.
...
Thêm NSO vào tài khoản người gửi. Trên các mạng khác ngoài mainnet, bạn có thể
sử dụng phương thức `requestAirdrop` để nhận NSO cho việc kiểm thử.
...
Phương thức `SystemProgram.transfer()` tạo một chỉ thị chuyển NSO từ tài khoản
...
chuyển NSO từ tài khoản của họ.
...
dịch trên Nara Solana Explorer.
...
Ví dụ dưới đây tạo một token mới trên Nara Solana sử dụng Token Extensions Program.
...
Cách tiếp cận này phổ biến khi xây dựng các giao dịch Nara Solana phức tạp, vì nó đảm
