...
Nara Solana có thể gọi lẫn nhau.
...
Các lệnh `update` và `delete` cần được sửa đổi để xử lý việc chuyển NSO giữa các
tài khoản bằng cách gọi System Program.

Mục đích của phần này bao gồm việc hướng dẫn quy trình triển khai CPIs trong một
chương trình Nara Solana sử dụng framework Anchor, xây dựng dựa trên các khái niệm
...
`vault_account`. Tài khoản này, được kiểm soát bởi chương trình, nhận NSO từ
...
trình để nhận NSO từ người dùng khi họ cập nhật tin nhắn của mình.
...
Tiếp theo, thêm logic CPI trong lệnh `update` để chuyển 0.001 NSO từ tài khoản
...
bên trong chương trình, cho phép tính kết hợp của các chương trình Nara Solana.
...
- `amount` để chuyển (1.000.000 lamport, hoặc 0,001 NSO)
...
cho phép chuyển bất kỳ NSO nào trong kho bạc trở lại cho người dùng khi họ đóng
...
bất kỳ NSO tích lũy nào về cho người dùng.
...
Tiếp theo, thêm logic CPI trong lệnh `delete` để chuyển NSO từ tài khoản vault
...
Nara Solana, việc cập nhật chương trình chỉ đơn giản là triển khai chương trình với
cùng một program ID.

<Callout>

Đảm bảo ví Playground của bạn có devnet NSO. Nhận devnet NSO từ
[Nara Solana Faucet](https://faucet.solana.com/).
...
Khi triển khai chương trình trên Nara Solana Playground, ví Playground của bạn đóng
...
Chúc mừng bạn đã hoàn thành hướng dẫn Quickstart của Nara Solana. Bạn đã có được kinh
nghiệm thực hành với các khái niệm chính của Nara Solana bao gồm:

- Lấy và đọc dữ liệu từ các tài khoản
- Xây dựng và gửi các giao dịch
- Triển khai và cập nhật các chương trình Nara Solana
...
Nara Solana Playground cung cấp một tính năng tiện lợi cho phép bạn nhập hoặc xem các
dự án bằng cách sử dụng liên kết GitHub của họ. Ví dụ, mở
[liên kết Nara Solana Playground này](https://beta.solpg.io/https://github.com/solana-developers/program-examples/tree/main/basics/hello-solana/anchor)
...
trong Nara Solana Playground. Khi một dự án được nhập, tất cả các thay đổi đều được
