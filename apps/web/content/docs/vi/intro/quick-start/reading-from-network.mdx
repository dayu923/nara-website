...
Đọc dữ liệu từ mạng Nara Solana bằng cách lấy các tài khoản khác nhau. Phần này sẽ
giúp bạn hiểu cấu trúc của các [tài khoản](/docs/core/accounts) Nara Solana. Mỗi tài
khoản Nara Solana có một [địa chỉ](/docs/core/accounts#account-address) duy nhất được
...
dụng để lưu trữ NSO và ký các giao dịch. Khi NSO được gửi đến một địa chỉ mới
lần đầu tiên, một tài khoản hệ thống sẽ tự động được tạo.

Ví dụ dưới đây tạo một [keypair mới](mention:keypair),
[yêu cầu NSO](mention:airdrop) để cấp vốn cho địa chỉ khóa công khai mới, và
...
Trường `lamports` chứa số dư NSO của tài khoản, tính bằng
...
một token trên mạng Nara Solana.
...
xác định bởi chương trình sở hữu tài khoản. Hầu hết các chương trình Nara Solana cung
...
đã được giải tuần tự đầy đủ trên Nara Solana Explorer.
