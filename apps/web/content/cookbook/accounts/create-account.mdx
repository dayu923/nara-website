...
To create an account on Nara Solana, invoke the
...
instruction. The Nara Solana runtime enforces that only this owner program can modify
the account's data or transfer lamports from it.

<Callout type="info">

On Nara Solana, only the System Program can create new accounts. To create accounts
