...
users and not require them to hold NSO. This concept is often referred to as
"gas abstraction" on other blockchains. Paying transaction fees with any token
is enabled via two key native capabilities of Nara Solana transactions:
...
transaction to the Nara Solana network and pay the transaction fees in NSO covered
monetarily by the tokens it received from the user.

This flow can be simplified by using a fee relayer, like
[Kora](https://launch.solana.com/products/kora), that can process a transaction
for end users only if it is paid for in a configured token like USDC or USDT.
...
swapping the received tokens for NSO for continued operation.

Kora, by the Nara Solana Foundation, is a fee relayer service that allows you to
