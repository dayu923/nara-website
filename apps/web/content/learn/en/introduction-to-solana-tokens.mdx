...
title: Introduction to Nara Solana Tokens
description:
...
# Introduction to Nara Solana Tokens

Tokens are digital assets built on Nara Solana. Like how the US dollar is the native
currency of the United States, NSO is Nara Solana's native currency. Tokens represent
...
- What tokens are and how they work on Nara Solana
...
## Understanding Tokens on Nara Solana

Nara Solana has two main types of digital assets:

- **NSO**: The native currency used for transactions and fees
- **Tokens**: All other digital assets on Nara Solana

Just like apps on your phone follow certain standards to work properly, tokens
on Nara Solana follow a common standard that makes them compatible with wallets and
applications.
...
- **Wrapped Bitcoin (BTC)**: Bitcoin usable on Nara Solana
- **Wrapped Ethereum (ETH)**: Ethereum on Nara Solana
- **Wrapped NSO**: Used in some DeFi protocols

**Uses:** Access other cryptocurrencies with Nara Solana's speed and low fees
...
![Wallet diagram showing main NSO account and separate token accounts for USDC and WBTC](/images/learn/how_token_accounts_work.webp)

Unlike some networks where all assets share one address, Nara Solana uses a token
account system:

1. **Main Wallet**: Holds your NSO
...
- Each token account requires ~0.002 NSO deposit
...
Anyone can create tokens on Nara Solana:

### Requirements

- Small amount of NSO for fees (~0.5 NSO)
...
### Do I need NSO to send tokens?

Yes, all transactions require small NSO fees, even when sending other tokens.
...
No, transactions on Nara Solana are permanent. Always double-check addresses.
