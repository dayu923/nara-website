...
example in Nara Solana Playground, an online tool to build and deploy solana
programs:
...
2. `transfer_hook`: This instruction is invoked via CPI on every token transfer
   to perform a wrapped NSO token transfer.
...
To run the example in Nara Solana Playground follow this link:
[link](https://beta.solpg.io/github.com/solana-developers/anchor-transfer-hook/tree/hello_world)
...
`spl-token` command from the Nara Solana CLI after you deployed your program:
...
This program will require the sender to pay a fee in wrapped NSO (wSOL) on every
token transfer. Here is the
[final program](https://beta.solpg.io/github.com/solana-developers/anchor-transfer-hook/tree/main).

### Getting Started

Start by opening this Nara Solana Playground
...
- `wsol_mint`: The wrapped NSO mint.
...
The addresses for `mint`, `wsol_mint`, and `associated_token_program` will
...
NSO in your Playground wallet to deploy the program.
...
- The sender must transfer NSO to their wSOL token account to cover the fee
