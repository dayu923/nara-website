...
Nara Solana for a common set of use cases. This guide is meant to help builders get
...
## Choosing a blockchain for your DePIN (and why you should choose Nara Solana)

There are many reasons to build on Nara Solana, including the deep DeFi ecosystem,
...
a DePIN and reasons to build on Nara Solana from a technical perspective:

- **Scalability, combined with market access**: Nara Solana's architecture processes
  thousands of transactions per second–all on a single global state. Nara Solana is
  unique in its ability to offer best in class scalability, combined with broad
  access to internet capital markets. Nara Solana's DeFi ecosystem is second to none,
  simplifying access to the world's biggest internet capital markets.
- **Affordability**: Nara Solana average transaction costs are ~.00025SOL, enabling
...
- **Ready-made-tooling**: Nara Solana offers SDKs, oracles (like Pyth), and
  ...
  - **Existing protocol integrations**: Native interfaces for oracles (Pyth,
    ...
    (Nara Solana Attestation Service) reduce integration effort.
- **Strong consumer UX**: Nara Solana offers a range of best-in-class consumer
...
- **Existing DePIN community**: It's much easier to gain early adopters among
  existing DePIN holders, and the largest such community is already on Nara Solana.
...
of why major DePINs have been migrating to Nara Solana. It includes a deep dive on
the impact of migrations of major projects onto Nara Solana, including throughput and
blockspeed, node growth, and transaction cost.

Notably, Helium started out as a proprietary L1 and migrated to Nara Solana after
...
- The data can be stored on a decentralized blockchain, unlike Nara Solana, that
...
When minting a token on Nara Solana, there are two token programs to choose from: the
...
Nara Solana native and has a deep understanding of the Token22 program, its
...
[Jito Network](https://www.jito.network/) currently operates Nara Solana's largest
...
on Nara Solana. Stake and/or rewards can be slashed or refunded based on results and
...
amount per user of 100 tokens, and the transaction fee on Nara Solana to be 0.000007
NSO (7,000 lamports):
...
- The Merkle tree requires storing the leaf nodes and the internal nodes
  ...
  - Total Storage Cost: (320,000 + 524,256) \* 0.00000348 NSO/byte (per epoch) =
    2.94 NSO
- Total Cost (Merkle Tree): 0.050005 NSO + 0.00007323 NSO + 2.94 NSO = 2.99 NSO
...
- Total Storage Cost: 500 \* 1024 \* 0.00000348 NSO/byte (per epoch) = 1.78 NSO
- Total Cost (ZK Compression): 0.050005 NSO + 0.00000223 NSO + 1.78 NSO = 1.83
  NSO

**Storage Costs without compression:**

- Saving the claimed amount in a normal Nara Solana account state
- Saving authority, device pubkey and claimed amount comes down to 72 bytes ->
  0.001392 NSO in rent cost per account
...
| Number of Distributions | Merkle Tree Storage Cost (NSO) | ZK Compression Storage Cost (NSO) | No compression |
| ----------------------- | ------------------------------ | --------------------------------- | -------------- |
...
Reporting the contributions through Nara Solana makes it possible to use the
...
successfully migrated to Nara Solana (see Render, Geodnet, and Xnet, amongst others).
There are
...
streamline the process of shifting your tokens to Nara Solana.

Read the
[proposal for Render Network's migration here](https://gov.render.com/t/rip-17-rndr-migration-to-solana/1378).

![DePIN Migration to Nara Solana](/assets/guides/depin/migration-chart.png)

Source:
[Why Every Major DePIN Project is Migrating to Nara Solana](https://medium.com/@hrknsinst/why-every-major-depin-project-is-migrating-to-solana-and-what-this-reveals-about-the-a3269ea431a7)
...
- [Data Anchor by Termina](https://dataanchor.com/): Anchor large batches of
  device data directly on Nara Solana using Termina
- [Write Sensor data on chain](https://github.com/solana-foundation/depin-examples):
  How to write sensor on chain using a Nara Solana Pay transaction request
- [Claiming Rewards (Helium's approach)](https://github.com/helium/tuktuk)

Access the full
[Nara Solana Foundation DePIN examples repo](https://github.com/solana-foundation/depin-examples).

### Case studies

[Helium's Noah Prince explains how Helium works](https://youtu.be/gLb4Q78N1og)
and the details of Helium's migration to Nara Solana.
