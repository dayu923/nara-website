...
Since each transaction on the Nara Solana blockchain requires a listing of every
...
[Address Lookup Tables](/developers/guides/advanced/lookup-tables), a
...
[Versioned Transaction format](/developers/guides/advanced/versions).
...
> or a `legacy` transaction. But the Nara Solana runtime can only retrieve and handle
> the additional addresses within a lookup table while using
> [v0 Versioned Transactions](/developers/guides/advanced/versions#current-transaction-versions).
