...
title: Add Nara Solana to Your Exchange
description:
...
This guide describes how to add Nara Solana's native token NSO to your cryptocurrency
exchange.
...
- to have a self-administered gateway to the Nara Solana mainnet-beta cluster to get
...
Nara Solana nodes demand relatively high computing power to handle our fast blocks
and high TPS. For specific requirements, please see
[hardware recommendations](https://docs.anza.xyz/operations/requirements).

To run an api node:

1. [Install the Nara Solana command-line tool suite](/docs/intro/installation)
...
[shreds](/docs/references/terminology#shred) your node retains on disk. If you
do not include this parameter, the validator will keep the entire ledger until
it runs out of disk space. The default value attempts to keep the ledger disk
...
about selecting a custom limit value is
...
Discord, or Twilio. For details, run `solana-watchtower --help`.
...
> [best practices for Nara Solana Watchtower](https://docs.anza.xyz/operations/best-practices/monitoring#solana-watchtower)
> here in the docs.
...
traffic from all other Nara Solana validators. While this is the most efficient mode
of operation, and is strongly recommended, it is possible to restrict the
validator to only require inbound traffic from one other Nara Solana validator.
...
Nara Solana accounts do not require any on-chain initialization; once they contain
some NSO, they exist. To set up a deposit account for your exchange, simply
generate a Nara Solana keypair using any of our
[wallet tools](https://docs.anza.xyz/cli/wallets).

We recommend using a unique deposit account for each of your users.

Nara Solana accounts must be made rent-exempt by containing 2-years worth of
[rent](/docs/core/fees#rent) in NSO. In order to find the minimum rent-exempt
...
greater security. If so, you will need to move NSO to hot accounts using our
[offline methods](https://docs.anza.xyz/cli/examples/offline-signing).

## Listening for Deposits

When a user wants to deposit NSO into your exchange, instruct them to send a
...
[lamports](/docs/references/terminology#lamport), indexed to the `accountKeys`
list. For example, if the deposit address of interest is
`G1wZ113tiUHdSpQEBcid8n1x8BAvcWZoZgxPKxgE5B7o`, this transaction represents a
transfer of 1040000000 - 1030000000 = 10,000,000 lamports = 0.01 NSO
...
To accommodate a user's request to withdraw NSO, you must generate a Nara Solana
transfer transaction, and send it to the api node to be forwarded to your
cluster.

### Synchronous

Sending a synchronous transfer to the Nara Solana cluster allows you to easily ensure
that a transfer is successful and finalized by the cluster.

Nara Solana's command-line tool offers a simple command, `solana transfer`, to
...
The [Nara Solana Javascript SDK](https://github.com/solana-labs/solana-web3.js)
...
[confirmed blocks](/docs/references/terminology#confirmed-block) have elapsed
since the transaction was processed. If `confirmations: null`, it is
[finalized](/docs/references/terminology#finality).
...
Nara Solana addresses a 32-byte array, encoded with the bitcoin base58 alphabet. This
...
This check is insufficient on its own as Nara Solana addresses are not checksummed,
...
The address of a normal account in Nara Solana is a Base58-encoded string of a
...
Every deposit and withdrawal of NSO must be greater or equal to the minimum
rent-exempt balance for the account at the wallet address (a basic NSO account
holding no data), currently: 0.000890880 NSO
...
transactions with higher economic value. Valid Transactions on Nara Solana may be
delayed or dropped if Prioritization Fees are not implemented properly.

[Prioritization Fees](/docs/references/terminology#prioritization-fee) are
additional fees that can be added on top of the
...
Nara Solana make use of priority fees to avoid disruption.
...
amounts of NSO) prepended to transactions to make them economically compelling
...
The `units` value provided will replace the Nara Solana runtime's default compute
budget value.

<Callout type="warn" title="Set the lowest CU required for the transaction">
...
different Nara Solana cluster, like devnet. For example, a
...
token creation and exchange on the Nara Solana blockchain.

The SPL Token workflow is similar to that of native NSO tokens, but there are a
...
1. SPL Token accounts must remain [rent-exempt](/docs/core/fees#rent-exempt) for
   ...
   native NSO tokens be deposited at account creation. For SPL Token accounts,
   this amount is 0.00203928 NSO (2,039,280 lamports).
...
recommended that the addresses for these accounts be derived from NSO deposit
wallets using the
...
be scanned for successful transactions including the user's and exchange's token
account addresses.

<Callout type="warn">

The `preTokenBalances` and `postTokenBalances` fields from the transaction's
metadata must be used to determine the effective balance change. These fields
include the token mint, token account owner (wallet address), and balances of
the token accounts before and after the transaction.

</Callout>

<Callout>

If a token account is created as part of a transaction (such as when receiving
tokens for the first time), it won't appear in the `preTokenBalances` array
since it didn't exist before the transaction. In this scenario, you should treat
the initial balance as zero when calculating deposit amounts. The newly created
account will only appear in the `postTokenBalances` array with its final balance
after the transaction completes.

</Callout>

#### Example 1: Single Token Transfer

The transaction detail below shows an example of a transaction including a
single token transfer instruction.

The transaction transfers 100 base units of the token (not adjusted for mint
decimal) and includes the following accounts:

- Sender (owner): `4fvXFPXSL9i7VbiRzoizuW4bhn1dMvRgQzQ6VevssYxw`
- Sender Token Account: `6zhjktfYBRUp7fgXLoWU7GFFrCuZa9iQkYTQzDzuvsAS`
- Recipient Token Account: `G5nNekUhhWFqJAiCMpKHootZ5Bfa7MXwuQ5vvKcvuxKM`
- Token Extension Program ID: `TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb`

Note that the recipient (owner) account and mint account are not required in a
token transfer instruction. For reference, they are listed here as the addresses
are included in the parsed transaction metadata.

- Recipient (owner): `8fjS2shNWY8xniiEMLNk1Aek4MAu8Qp2LCXJckVwTD4n`
...
Program logs from when the transaction was processed.

In this case, logs indicate the transaction invokes the transfer instruction on
the Token Program.

### !postBalances

The lamport balance of all accounts involved in the transaction after it was
processed.

### !postTokenBalances

The token balance of all token accounts involved in the transaction after it was
processed.

### !preBalances

The lamport balance of all accounts involved in the transaction before it was
processed.

### !preTokenBalances

The token balance of all token accounts involved in the transaction before it
was processed.

### !rewards

The rewards of all accounts involved in the transaction before it was processed.

### !accountKeys

List of account addresses required for the instruction in the transaction.

### !instructions

List of instructions in the transaction. In this case, there is only one
instruction, a token transfer instruction.

### !recentBlockhash

The blockhash of the last block processed when the transaction was sent.

### !senderPostTokenBalance

The sender token account has a final balance of 0 tokens.

### !recipientPostTokenBalance

The recipient token account has a final balance of 100 base units of the token.

The mint has 2 decimals, so the recipient token account has an final balance of
1 token.

### !senderPreTokenBalance

The sender token account had an initial balance of 100 base units of the token.

The mint has 2 decimals, so the sender token account had an initial balance of 1
token.

### !recipientPreTokenBalance

The recipient token account had an initial balance of 0 tokens.

### !fromOwnerPreTokenBalance

The is the owner of the token account sending tokens before the transaction is
processed.

### !fromOwnerPostTokenBalance

The is the owner of the token account sending tokens after the transaction is
processed.

### !toOwnerPreTokenBalance

The is the owner of the token account receiving tokens before the transaction is
processed.

### !toOwnerPostTokenBalance

The is the owner of the token account receiving tokens after the transaction is
processed.

### !signatures

The signatures included in the transaction. In this case, there is only one
signature (from the sender).

The first signature on a transaction is from the transaction fee payer. It is
also used to identify the transaction and is commonly referred to as the
...
The transaction detail below shows an example of a transaction where the
recipient token account is created in the same transaction as a token transfer.
...
The `owner` field in the `postTokenBalances` differs from the
`preTokenBalances`. This indicates that the token account ownership has changed.

### !preTokenBalances

The `owner` field in the `preTokenBalances` is the original owner of the token
account.
...
- If the `owner` field of the `preTokenBalances` and `postTokenBalances` fields
  remains the same, calculate the difference between the `amount` fields.
- If the token account ownership changes (different `owner` field between
  `preTokenBalances` and `postTokenBalances`), and the new owner in
  `postTokenBalance` matches your exchange's expected `owner` address, then
  consider the entire balance shown in the `amount` field of `postTokenBalances`
  as the deposited amount.
...
Check whether the `owner` field matches between the `preTokenBalances` and
`postTokenBalances` to determine if the token account ownership has changed
during the transaction.

### !amount

Calculate token balance changes by using the `amount` field in
`preTokenBalances` and `postTokenBalances` for each token account.

In case there are multiple transfers within the same transaction, this
difference represents the net tokens transferred for each respective token
account.
...
The withdrawal address a user provides must be that of their NSO wallet.
...
account data. If the address has no NSO balance, user confirmation should be
...
SPL Token accounts, funding the withdrawal account will require 0.00203928 NSO
(2,039,280 lamports).
...
wrapped/synthetic token creation and exchange on the Nara Solana blockchain.
...
required deposit amount of NSO for the new account.

Because of extensions, however, accounts may be larger than 165 bytes, so they
may require more than 0.00203928 NSO to fund.
...
Since the Nara Solana runtime requires all accounts to be explicitly passed to a
...
Be sure to test your complete workflow on Nara Solana devnet and testnet
[clusters](/docs/references/clusters) before moving to production on
...
devnet or testnet NSO for development and testing.
