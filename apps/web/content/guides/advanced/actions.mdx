...
[Nara Solana Actions](#actions) are specification-compliant APIs that return
transactions on the Nara Solana blockchain to be previewed, signed, and sent across a
...
things you can do throughout the Nara Solana ecosystem right into your environment,
allowing you to perform blockchain transactions without needing to navigate away
to a different app or webpage.

[Blockchain links](#blinks) – or blinks – turn any Nara Solana Action into a
...
To quickly get started with creating custom Nara Solana Actions:
...
- install the
  [Nara Solana Actions SDK](https://www.npmjs.com/package/@solana/actions) in your
  application
...
> [how to build a Nara Solana Action](https://www.youtube.com/watch?v=kCht01Ycif0)
...
> that performs a native NSO transfer here and several other example Actions in
> [this repo](https://github.com/solana-developers/solana-actions/tree/main/examples).

When deploying your custom Nara Solana Actions to production:
...
The Nara Solana Actions specification uses a set of standard APIs to deliver signable
...
> You can think of Actions as an API endpoint that will return metadata and
> something for a user to sign (either a transaction or an authentication
...
![Nara Solana Actions Execution and Lifecycle](/assets/docs/action-execution-and-lifecycle.png)
...
> Actions. Application developers may choose which Actions they want to support
...
While Nara Solana Actions and blinks are a permissionless protocol/specification,
...
As a public good for the Nara Solana ecosystem, [Dialect](https://dialect.to)
maintains a public registry — together with the help of Nara Solana Foundation and
...
The Nara Solana Actions specification consists of key sections that are part of a
request/response interaction flow:

- Nara Solana Action [URL scheme](#url-scheme) providing an Action URL
...
A Nara Solana Action URL describes an interactive request for a signable Nara Solana
...
<Callout type="warn" title="Cross-Origin headers for actions.json">
...
- `label` - The value must be a UTF-8 string that will be rendered on a button
  ...
  example, "Mint NFT", "Vote Yes", or "Stake 1 NSO".
...
NSO: a button labeled "Stake 1 NSO", another button labeled "Stake 5 NSO", and a
...
Nara Solana Actions can be "chained" together in a successive series. After an
...
instruct clients on what website URLs support Nara Solana Actions and provide a
mapping that can be used to perform [GET requests](#get-request) to an Actions
API server.

<Callout type="warn" title="Cross-Origin headers are required">
...
[`getSignaturesForAddress`](/docs/rpc/http/getsignaturesforaddress), the results
`memo` field will return each memo instruction's message as a single string with
...
Because Nara Solana validators index transactions by the account keys, the
[`getSignaturesForAddress`](/docs/rpc/http/getsignaturesforaddress) RPC method
can be used locate all transactions including the `identity` account.
