...
functionality in the Nara Solana runtime, including
[Address Lookup Tables](/developers/guides/advanced/lookup-tables).
...
The Nara Solana runtime supports two transaction versions:
...
`@solana/web3.js` library, to send perform a NSO transfer between two accounts.

#### Notes:

- `payer` is a valid `Keypair` wallet, funded with NSO
...
transaction. In this example below, we are creating a simple NSO transfer
instruction:
...
- using
  [Versioned Transactions for Address Lookup Tables](/developers/guides/advanced/lookup-tables#how-to-create-an-address-lookup-table)
- view an
