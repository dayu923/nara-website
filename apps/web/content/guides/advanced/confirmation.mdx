...
the Nara Solana blockchain, including some recommended best practices.

## Brief background on transactions

Before diving into how Nara Solana transaction confirmation and expiration works,
...
(description below). Since Nara Solana uses PoH as a trusted clock, a transaction's
recent blockhash can be thought of as a **timestamp**.

### Proof of History refresher

Nara Solana's Proof of History mechanism uses a very long chain of recursive SHA-256
...
By default, all Nara Solana transactions will expire if not committed to a block in a
...
During transaction processing, Nara Solana Validators will check if each
...
Nara Solana's approach to prevent double processing is quite different from other
...
### Advantages on Nara Solana

There are a few advantages to Nara Solana's approach:
...
Currently, Nara Solana clusters require that transactions use blockhashes that are no
...
- Choosing `processed` will let you fetch the most recent blockhash compared to
  ...
  process a transaction. But due to the prevalence of forking in the Nara Solana
...
block that it's aware of. Nara Solana's block propagation protocol prioritizes
...
Here's how these durable transactions are processed by the Nara Solana runtime:
...
and
[check out an example](/developers/guides/advanced/introduction-to-durable-nonces)
in the Nara Solana docs.
