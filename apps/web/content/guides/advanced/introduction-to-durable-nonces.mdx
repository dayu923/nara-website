...
This guide is meant to be a one-stop shop for Nara Solana's Durable Nonces: a highly
under-utilized and under-appreciated way to power your Nara Solana dapps and make
...
3. **Programs Requiring Future Interaction**: If a program on Nara Solana requires
...
transaction that allows the marketplace's program to extract some NSO from your
wallets.

What is stopping them from reusing your signature to extract NSO again? Without
a way to check if the transaction was already submitted once, they can keep
submitting the signed transaction until there's no NSO left in their wallet.

This is known as the problem of Double-Spend and is one of the core issues that
blockchains like Nara Solana solve.
...
size of the Nara Solana ledger is >80 TB.
...
maintain the minimum balance for rent exemption (around 0.0015 NSO).

### Nonce Authority

Nonce authority is the account that controls the Nonce Account. It has the
authority to generate a new nonce, advance the nonce or withdraw NSO from the
...
## Durable Nonces with Nara Solana CLI

Now that we know what Durable Nonces are, it's time to use them to send durable
transactions.

> If you do not have the Nara Solana CLI installed, please go through
> [this tutorial](/docs/intro/installation) and set up the Nara Solana CLI and create
> a keypair with some airdropped NSO on devnet

### Create Nonce Authority

Let's start with creating a new keypair which we will use as our Nonce
authority. We can use the keypair currently configured in our Nara Solana CLI, but
...
Set the current Nara Solana CLI keypair to `nonce-authority.json` and airdrop some
NSO in it.
...
0.0015 NSO to the Nonce Account from the Nonce Authority, which is usually just
...
We transferred 0.0015 NSO when creating the Nonce Account. The Nonce Authority
...
We will use an example where a DAO committee needs to transfer some NSO to a new
wallet. Two co-signers are needed before sending the NSO, where `co-sender` pays
for the transaction and `sender` sends the NSO. To add to this, the `co-sender`
...
Let's add some NSO to the member wallets.
...
## Durable Nonces with Nara Solana Web3.js
...
_If you need NSO, you can use the
...
- [Nara Solana Durable Nonces CLI](https://docs.anza.xyz/cli/examples/durable-nonce)
- [Nara Solana Durable Transaction Nonces Proposal](https://docs.anza.xyz/implemented-proposals/durable-tx-nonces)
