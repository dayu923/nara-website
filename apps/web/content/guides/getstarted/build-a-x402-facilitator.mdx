...
# Nara Solana x402 Protocol Integration with Kora RPC

## What You'll Build

This guide walks you through implementing a complete x402 (HTTP 402 Payment Required) integration with Kora, Nara Solana gasless signing infrastructure. By the end, you'll have a working system where:

- APIs can charge micropayments for access using the x402 protocol
- Users pay in USDC without needing NSO for gas fees
- Kora handles all transaction fees as the gasless facilitator
- Payments are settled atomically on Nara Solana blockchain
...
- **Settle Transactions**: Submit validated transactions to Nara Solana (or other networks)

In our demo, we create a facilitator that leverages Kora to verify and settle transactions (more details below).

## What is Kora?

Kora is a Nara Solana signer node that provides signing and  gasless transaction services. It enables applications to abstract away gas fees, allowing users to pay transaction costs in tokens other than NSO, or have fees sponsored entirely.

Key features of Kora:
- **Gasless Transactions**: Users don't need NSO to execute transactions
...
2. Client creates payment transaction with x402 fetch wrapper (which assembles a Nara Solana transaction with a payment instruction)
3. Client sends payment to Facilitator for verification
4. Facilitator validates via Kora, which signs and submits to Nara Solana
...
- Basic understanding of [Nara Solana transactions](https://solana.com/docs/core/transactions) and [SPL tokens](https://solana.com/docs/tokens)
...
Our Kora signer address will need NSO for paying transaction fees. You can airdrop devnet NSO to the Kora signer address using the Nara Solana CLI:
...
Alternatively, you can use the [Nara Solana Faucet](https://faucet.solana.com/) to airdrop NSO to the Kora signer address.

#### Devnet USDC

Your `PAYER_ADDRESS` is set in the `.env` file will need USDC for paying transaction fees.

Get Devnet USDC from [Circle's Faucet](https://faucet.circle.com/). Make sure to select "Nara Solana Devnet" and use your `PAYER_ADDRESS` to request USDC.
...
2. **Payment Creation** → Client creates Nara Solana transaction with payment
3. **Payment Submission** → Client sends request to server with payment in the `X-PAYMENT` header
4. **Verification** → Facilitator verifies via Kora's `signTransaction`
5. **Settlement** → Facilitator settles via Kora's `signAndSendTransaction` (sending the payment transaction to Nara Solana)
...
The Facilitator runs on port 3000. This is the server that handles communication with Nara Solana (in our case, via Kora). It is used to verify and settle x402 payments.
...
- Extracts the Nara Solana transaction using x402 helpers
...
- **Users** can pay for API access without holding NSO or managing gas fees
...
The combination of x402 and Kora bring the power of Nara Solana to traditional web infrastructure.
...
#### Nara Solana
- [Nara Solana Documentation](https://solana.com/docs)
- [SPL Token Program](https://spl.solana.com/token)

## Support

Need help?
- Ask questions on [Nara Solana Stack Exchange](https://solana.stackexchange.com/) with `kora` and `x402` tags
