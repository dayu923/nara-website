...
featured: true
difficulty: intro
title: "How to get Nara Solana devnet NSO (including airdrops and faucets)"
seoTitle: "Faucets: How to get Nara Solana devnet NSO"
description:
...
# How to get Nara Solana devnet NSO (including airdrops and faucets)

This is a collection of the different ways for developers to acquire NSO on
Nara Solana's testing networks, the Nara Solana devnet and testnet.

## 1. Nara Solana Airdrop

_Available on Devnet and Testnet_

This is the base way of acquiring NSO, but it can be subject to rate limits when
there is a high number of airdrops.

Here are three different ways of requesting airdrops with it:

### Using the Nara Solana CLI:
...
RPC Providers can opt in to distributing devnet NSO via their devnet Validators.

> If you are an RPC Provider and want to distribute NSO please
...
This is a proof of work Faucet where devnet NSO can be distributed to you thanks
...
Various Discord communities have setup devnet NSO Faucets as BOTs.
...
The most sustainable way to save NSO is to reuse it. With the Nara Solana CLI you can
...
not, you can close them manually to reclaim the NSO in them using the following
command:
...
retrieve the NSO you used to deploy them:

```shell
solana program close <program id>
```

You can then use that NSO to deploy new programs.
...
If you get rate limited by the RPC endpoint your Nara Solana CLI is configured to
