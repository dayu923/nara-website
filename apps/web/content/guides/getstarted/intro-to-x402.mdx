...
difficulty: beginner
title: "How to get started with x402 on Nara Solana"
description:
...
returning a protected response. On Nara Solana, this is commonly implemented by
asking the client to submit a small transfer, then the server verifies this
on-chain and serves the content.
...
server and client and list all the available 402 SDKs with the current Nara Solana
support described.
...
validation logic with a few lines of code. The facilitator abstracts the
blockchain integration details from the server and client which makes it easier
to implement.
...
**Nara Solana support**: The protocol itself is chain-agnostic; on Nara Solana it supports
all SPL tokens. Nara Solana support is available or in development for most 402 SDKs.

Further down is a list of the
[available 402 SDKs](#sdks-and-their-solana-support) with their current Nara Solana
support.
...
view or Spotify paying for each song instead of paying for subscriptions. Here
are a few possible ideas. But the whole spectrum of possibilities is open to
imagination:

**AI & Agent Commerce:**

- **AI Agent API Access**: Pay per LLM inference, image generation, or AI model
  API call (See [ACK example](#ack))
- **MCP Server Monetization**: Charge for Model Context Protocol tools, data
  sources, and specialized agent capabilities (See [MCPay.tech](#mcpaytech))
- **Agent-to-Agent Payments**: Enable autonomous agents to transact with each
  other for services and data (See [a2a-x402 example](#a2a-x402-google))
...
- **High-Resolution Images**: Unlock full-resolution downloads after payment
  (see [ACK example](#ack)) or
  [x402 coinbase example](https://github.com/coinbase/x402/tree/main/typescript/packages/x402/src/paywall)
...
The key advantage of x402 on Nara Solana is **low transaction costs** (fractions of a
cent) making true micro payments viable, plus **instant settlement** enabling
real-time access control.

## SDKs and their Nara Solana support

This is an evolving list and will be updated as more SDKs are released or Nara Solana
support is added.

| SDK / Project                           | Nara Solana support           | Notes                                                      | Docs / URL                                                    |
| --------------------------------------- | ------------------------ | ---------------------------------------------------------- | ------------------------------------------------------------- |
| [Corbits](#corbits)                     | Yes                      | Convenient SDK for 402 on Nara Solana                           | [Docs](https://corbits.dev/)                                  |
...
| [A2A x402 (Google)](#a2a-x402-google)   | In development           | Agent-to-Agent payments using google ai                    | [GitHub](https://github.com/google-agentic-commerce/a2a-x402) |
| [Nexus (Thirdweb)](#nexus-thirdweb)     | In development           | x402 wrapper around API keys                               | [Nexus](https://nexus.thirdweb.com/)                          |
| [x402scan](#x402scan)                   | N/A (Explorer)           | x402 ecosystem explorer (not an SDK)                       | [x402scan.com](https://x402scan.com/)                         |
...
Nara Solana-first SDK to implement x402 flows quickly on Nara Solana. See the docs:
https://corbits.dev/

Example that lets you pay for Nara Solana RPC requests.
...
[Coinbase's reference implementation](https://github.com/coinbase/x402) of the
x402 protocol provides TypeScript libraries and examples for both client and
server flows. The repository includes end-to-end tests covering 6 different SVM
(Nara Solana Virtual Machine) scenarios. The implementation covers payment
verification, receipt generation, and error handling.
...
- Test suite with Nara Solana transaction examples
...
![ACK Flow Diagram](/assets/guides/x402/ack-flow-diagram.png)
...
Nara Solana-first x402 facilitator with a live echo merchant to test and refund
payments. PayAI is at the moment taking over all transaction fees. Site:
https://payai.network/

### A2A x402 (Google)

Agent-to-agent 402 initiative exploring standardized payment-required flows.
Nara Solana support is currently in progress and a working chat example can be found
[here](https://github.com/Woody4618/a2a-x402/tree/solana)

### Crossmint

Crossmint is an all-in-one platform for companies and agents to integrate crypto
rails — including wallets, onramps, stablecoin orchestration, and more. Nara Solana
x402 support is currently in progress and is supposed to be finished by
30.10.2025. Site: https://www.crossmint.com/

### x402scan

Explorer for the x402 ecosystem that provides comprehensive stats, project
listings, and analytics for x402 implementations. Track transaction
volumes, discover active merchants, and monitor the growth of payment-required
endpoints across different networks. Site: https://x402scan.com/
...
**Nara Solana-specific alternative:** On Nara Solana, you could implement a variant where
the client submits the transaction directly to the network with a memo
instruction (rather than sending it to the server), then sends just the
transaction signature to the server for verification. This solves the
connection-loss problem—if the client disconnects after payment but before
receiving content, they can retry with the same signature since the payment is
already confirmed on-chain. However, this approach deviates from the x402.org
standard flow (which expects the server to broadcast the transaction), so we're
using the standard approach in this example.
