...
description: Learn how to use tokens in Nara Solana games with an on-chain tutorial
tags:
  - games
  - anchor
  - program
...
Tokens on Nara Solana can serve various purposes, such as in-game rewards,
...
check out the
[Token Vault example](https://beta.solpg.io/tutorials/spl-token-vault) in Nara Solana
Playground.

## Overview

In this tutorial, we will build a game using Anchor to introduce the basics of
interacting with the [Token Program](/docs/core/tokens) on Nara Solana. The game
...
[Nara Solana Playground](https://beta.solpg.io/) and create a new Anchor project. If
you're new to Nara Solana Playground, you'll also need to create a Playground Wallet.
...
`ADMIN_PUBKEY` with your Nara Solana Playground wallet's public key.
...
using the Nara Solana Playground. Your final program should look like this:
...
in Nara Solana Playground, remove the placeholder code, and add the code snippets
...
Finally, run the client by clicking the “Run” button in Nara Solana Playground. You
can copy the Token Mint address printed to the console and verify on Nara Solana
