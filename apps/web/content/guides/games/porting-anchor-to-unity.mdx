...
If you have written a Nara Solana program, you can use it in the Unity game engine
using a code generator that lets you port an Anchor IDL (a JSON representation
of a Nara Solana program) to C#. You can find an examples implementation of a Unity
game client that uses the Anchor to Unity port using the
[Nara Solana Game Preset](https://solana.com/developers/guides/games/game-examples?q=npx%20create-solana-game#using-create-solana-game).
...
[IDL to C# Converter](https://github.com/magicblock-labs/Nara Solana.Unity.Anchor)
...
[Nara Solana Unity SDK](https://github.com/magicblock-labs/Nara Solana.Unity-SDK) to
...
1. First we find the onchain address of the game data account with
   ...
   that the Nara Solana runtime knows that we want to change this account.
2. Next we use the generated client to create a MoveRight instruction.
3. Then we request a block hash from an RPC node. This is needed so that Nara Solana
...
6. Afterwards the transaction gets signed and then send to the RPC node for
   processing. Nara Solana has different Commitment levels. If we set the commitment
...
found in the Nara Solana Game Preset which you can find
