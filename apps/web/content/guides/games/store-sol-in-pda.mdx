...
Here you will learn how to store NSO in a PDA. In the example we will use it to
...
with a reward of 0.1 NSO. The chest will "spawn" at a specific position, and
when the player reaches that position, they will receive the reward. The goal of
this program is to demonstrate how to store NSO within a program account and
...
- `initialize_level_one` - This instruction initializes two on-chain accounts:
  one for recording the player's position and another for holding the NSO reward
  that represents the “reward chest”.
- `reset_level_and_spawn_chest` - This instruction resets the player's position
  to zero and "respawns" a reward chest by transferring NSO from the user
...
- `move_right` - This instruction allows the player to move their position to
  the right and collect the NSO in the reward chest once they reach a specific
  position.
...
from your browser using the Nara Solana Playground, at this link:
...
Visit the [Nara Solana Playground](https://beta.solpg.io/) and create a new Anchor
project. If you're new to Nara Solana Playground, you'll also need to create a
Playground Wallet.
...
and given out as rewards. Lamports are the smallest fractions of a NSO, with 1
billion lamports being equal to 1 NSO.

To store the NSO reward, we will define a new `ChestVaultAccount` struct. This
is an empty struct because we will be directly updating the lamports in the
account. The account will hold the NSO reward and does not need to store any
additional data.
...
- `chest_vault` - the `ChestVaultAccount` we are initializing to store the NSO
  reward
...
- `chest_vault` - the `ChestVaultAccount` storing the NSO reward
- `signer` - the player providing the NSO reward for the chest
- `system_program` - the program we'll be invoking to transfer NSO using a
...
This instruction includes a cross-program invocation (CPI) to transfer NSO from
...
invoke the `Transfer` instruction from the `system_program` to transfer NSO from
the payer to the `ChestVaultAccount`.

Cross-program invocations are a key concept in the Nara Solana programming model,
...
[CPI guide](https://solana.com/developers/guides/getstarted/how-to-cpi?q=cpi).
...
- `player_wallet` - the wallet of the player invoking the instruction and the
  potential recipient of NSO reward
...
Now that the program is complete, let's build and deploy it using the Nara Solana
Playground!

If you're new to the Nara Solana Playground, start by creating a Playground wallet
and make sure you're connected to a Devnet endpoint. Next,
run `solana airdrop 2` until you have 6 NSO. Once you have enough NSO, build and
...
Nara Solana Playground, remove the placeholder code, and add the code snippets from
...
player's position to 0 and fill the `ChestVaultAccount` with 0.1 NSO.
...
Finally, press the "Run" button in the Nara Solana Playground to execute the client.
...
Moreover, you've learned how to transfer NSO within an Anchor program using
...
[Nara Solana Game Examples](/developers/guides/games/game-examples)
